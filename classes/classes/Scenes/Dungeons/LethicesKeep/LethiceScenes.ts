	...
	export class LethiceScenes extends BaseContent
	{
		...
		public  LethiceScenes() {}
		
		private  exportSaveData(): void {
		var  currFileName: string = undefined;
		var  sharedObject:SharedObject = undefined;
		...
		private  killHerPrompt(): void {
			if (player.weaponVerb == "slash" || player.weaponVerb == "cleave" || player.weaponVerb == "keen cut" || player.weaponVerb == "stab") {
				...
				addButton(0, "Snap Her Neck", killHer, 0, undefined, undefined, "Snap her neck and make it quick.");
				addButton(1, "Behead Her", killHer, 1, undefined, undefined, "Off with her head!\n\n(Warning: Contains gore!)");
			}
			else {
				killHer();
			}
		...
		private  killHer(method: number = 0): void
		{
			clearOutput();
			if (method == 0) {
		...
		private  postEndingChoices(): void {
			clearOutput();
		...
		public  takeLethiciteArmor(): void {
		...
		private  putBackLethiciteArmor(): void {
