	...
	export class KihaFollower extends NPCAwareContent implements TimeAwareInterface
	{
		...
		public  kihaSprite(nude: boolean = false): void {
			if (!pregnancy.isPregnant) {
...
public  canKihaGetPregnant(): boolean {
...
public  kihaKnockUpAttempt(): void {
...
public  totalKihaChildren(): number {
...
var  campo = undefined;
var  leave = camp.returnToCampUseOneHour;
	if (output) {
		clearOutput();
		kihaSprite();
...
//[Leave]
public  leaveKihaAfterCorruptionBitch(): void {
	clearOutput();
...
private  kihaDestroyGladesToggle(): void {
	clearOutput();
...
private  guardMyCampKiha(): void {
	...
	addButton(0,"Next",warmLoverKihaIntro);
		...
		private  giveKihaUndergarmentsPrompt(): void {
			clearOutput();
			credits.modContent = true;
		...
		private  giveKihaUndergarments(type: number): void {
			clearOutput();
			credits.modContent = true;
		...
		private  kihaPregUpdate(): boolean
		{
		...
		public  kihaGivesBirth(): void {
			//Determines how many eggs!
		var  eggCounter: number = (rand(5) + 1) * 2;
		...
		var  oldTotal: number = totalKihaChildren();
			//Check out the little dragons.
		var  maleCount: number = 0;
		var  femaleCount: number = 0;
		var  hermCount: number = 0;
		var  genderChooser: number;
		var  childList: any[] = [];
			for (var i: number = 0; i < eggCounter; i++) {
		...
		private  kihaBreastfeedingTime(): void {
		...
		public  kihaTellsChildrenStory(): void {
		...
		private  kihaTrainsHerKids(): void {
		var  availableGenders: any[] = [];
		var  selectedGender: number = 0;
		...
		private  kihaChildGraduationTime(): void {
