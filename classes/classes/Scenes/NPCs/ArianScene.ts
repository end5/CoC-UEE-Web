...
private  imbueTalisman(): void {
	...
	*/
	...
	if (player.hasItem(consumables.W__BOOK, 1) && player.hasItem(consumables.B__BOOK, 1))
		addButton(0, "Dispelling", arianSpellPlace, "Dispelling Spell", undefined, undefined, "Cancels all magical and alchemical effects currently affecting the PC, beneficial or not. (Does not cancel the effects of demon's lust aura.)");
	else
	...
	if (player.hasItem(consumables.WETCLTH, 2) && player.hasItem(consumables.VITAL_T, 2))
		addButton(1, "Healing", arianSpellPlace, "Healing Spell", undefined, undefined, "Heals the user, no chance for failure. More powerful than standard healing spell.");
	else
	...
	if (player.hasItem(consumables.GOB_ALE, 2) && player.hasItem(consumables.S_GOSSR))
		addButton(2, "Immolation", arianSpellPlace, "Immolation Spell", undefined, undefined, "Sets your opponent on fire, dealing damage over time.");
	else
	...
	if (player.hasItem(consumables.L_DRAFT, 2) && player.hasItem(consumables.F_DRAFT, 1))
		addButton(3, "Lust Reduc", arianSpellPlace, "Lust Reduction Spell", undefined, undefined, "Reduces the user's current lust, no chance for failure.", "Lust Reduction");
	else
	...
	if (player.hasItem(useables.B_CHITN, 2) && player.hasItem(useables.T_SSILK))
		addButton(4, "Shielding", arianSpellPlace, "Shielding Spell", undefined, undefined, "Increases defense for the duration of the battle.");
	else
...
private  arianSpellPlace(spell: any): void {
	spriteSelect(SpriteDb.s_arian);
	clearOutput();
