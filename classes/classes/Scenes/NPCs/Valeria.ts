	...
	export class Valeria extends NPCAwareContent implements TimeAwareInterface {
		...
		public static  GOOSTUFFED_EFFECT_MIN_DURATION: number = 10;
		public static  GOOSTUFFED_EFFECT_MAX_DURATION_DELTA: number = 300;
		public static  GOOSTUFFED_PREGNANCY_DURATION: number = int.MAX_VALUE;
		...
		public  valeriaFluidsEnabled(): boolean {
		...
		public  valeriaSparIntensity(): number {
		var  amount: number = Math.max(0, flags[kFLAGS.VALERIA_SPAR_VICTORIES]);
...
public  feedValeria(amnt: number): void {
...
public  pcWinsValeriaSparDefeat(offCamp: boolean = false): void {
	clearOutput();
	credits.authorText = "Savin";
	spriteSelect(SpriteDb.s_valeria);
	if (!offCamp) {
...
public  gooFlation(clearText: boolean = true): void {
	spriteSelect(SpriteDb.s_valeria);
	if (clearText) {
		clearOutput();
		credits.authorText = "Savin";
...
public  valeriaSexDominated(offCamp: boolean = false): void {
	spriteSelect(SpriteDb.s_valeria);
	clearOutput();
	credits.authorText = "Savin";
...
public  isabellaAndValeriaSpar(): void {
	clearOutput();
	credits.authorText = "Savin";
