	...
	export class IsabellaScene extends NPCAwareContent implements TimeAwareInterface 
	{
		...
		public  isabellaOffspringData: any[] = [];
		
		public static  OFFSPRING_HUMAN_BOYS: number = 1;
		public static  OFFSPRING_HUMAN_GIRLS: number = 2;
		public static  OFFSPRING_HUMAN_HERMS: number = 3;
		public static  OFFSPRING_COWGIRLS: number = 4;
		public static  OFFSPRING_COWFUTAS: number = 5;
		
		public static  ISABELLA_PREGNANCY_LAST_STAGE: number = 240;
		...
		public  totalIsabellaChildren(): number {
			return (isabellaOffspringData.length / 2);
		}
		
		public  getIsabellaChildType(type: number): number {
			//1: Human boys
			//2: Human girls
			//3: Human herms
			//4: Cowgirls
			//5: Cowfutas
		var  count: number = 0;
			for (var i: number = 0; i < isabellaOffspringData.length; i += 2) {
				if (isabellaOffspringData[i] == type) count++;
			}
			return count;
		}
		
		public  isabellaSparIntensity(): number {
		var  amount: number = Math.max(0, flags[kFLAGS.ISABELLA_SPAR_VICTORIES]);
		...
		public  preFollowerMenus(oralEnabled: boolean = true): void {
		...
		var  x: number = player.cockThatFits(38);
			...
			else if (player.cockThatFits(38) != -1) outputText("swallow your " + player.cockDescript(x) + " whole, plying her smooth, spit-covered tongue over every inch of it");
