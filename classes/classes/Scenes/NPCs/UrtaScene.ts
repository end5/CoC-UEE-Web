	...
	export class UrtaScene extends NPCAwareContent implements TimeAwareInterface {
	...
	if (flags[kFLAGS.URTA_PC_LOVE_COUNTER] == 1)
	{
		...
		if (flags[kFLAGS.URTA_DRINK_FREQUENCY] == -1 || !urtaDrunk()) {
			...
			if (player.hasCock() && !player.isTaur()) {
				addButton(3, "Vixen & Cream", vixenAndCreamPart1, false, undefined, undefined, "Try something special! \n\nNOTE: This is very long! Don't select this if you have ADHD.");
			} else {
...
private  takeUrtasCumHomeWithYou(): void {
	urtaSprite();
	urtaLove(0.5);
	flags[kFLAGS.URTA_COMFORTABLE_WITH_OWN_BODY]++;
	clearOutput();
...
var  discussRomance = (urtaJustFriends() ? undefined : urtaDiscussesEdrynRomance);
	menu();
...
private  urtaDiscussionTeaseAfterRomance(): void {
	...
	if (!urtaLove()) {
		...
		addButton(2, "Eat Out", eatUrtaOutNomNomPussy).hint("Get a taste of Urta's vagina! (And optionally, drink a glass of her cum.)");
		if (player.hasCock() && !player.isTaur()) addButton(3, "Vixen & Cream", vixenAndCreamPart1, true, undefined, undefined, "Try something special! \n\nNOTE: This is very long! Don't select this if you have ADHD.");
...
private  urtaTakesPCOnWalkies(): void {
	...
	private  vixenAndCreamPart1(teased: boolean = false): void {
		clearOutput();
	...
	private  vixenAndCreamPart2(): void {
		clearOutput();
	...
	private  vixenAndCreamPart3(): void {
		clearOutput();
	...
	private  vixenAndCreamPart4(): void {
		clearOutput();
	...
	private  vixenAndCreamPartFinal(): void {
		clearOutput();
		...
		for (var i: number = 0; i < 3; i++) { //Attempt 3x
			knockUpUrtaChance();
		}
	 ...
	 */
	private  getPlayerCockNoun(index: number): string 
	{
