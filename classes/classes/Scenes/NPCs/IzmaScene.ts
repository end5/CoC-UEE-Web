...
export class IzmaScene extends NPCAwareContent implements TimeAwareInterface, Encounter {
...
//Required prequisites for encounter: Encounter a normal shark 
  ...
  //creates Izmacounter and sets value to 1 if it doesn't exist)
		...
		public  izmaelScene:IzmaelScene = new IzmaelScene();
		
		public  IzmaScene(pregnancyProgression:PregnancyProgression, output:GuiOutput)
		{
...
public  totalIzmaChildren(): number {
...
private  sharkBookMenus(free: boolean = false): void {
...
private  buyBookCombatManual(): void {
	clearOutput();
	spriteSelect(SpriteDb.s_izma);
...
private  confirmBuyCombatManual(): void {
	clearOutput();
...
private  buyBookEtiquetteGuide(): void {
	clearOutput();
	spriteSelect(SpriteDb.s_izma);
...
private  confirmBuyEtiquetteGuide(): void {
	clearOutput();
...
public  izmaFollowerMenu(): void {
	...
	addButton(7, flags[kFLAGS.IZMA_PREGNANCY_ENABLED] == 1 ? "NoKidsPlease" : "Have Kids?", childToggle, undefined, undefined, undefined, flags[kFLAGS.IZMA_PREGNANCY_ENABLED] == 1 ? "Tell Izma that she should start taking the herbal contraceptives." : "Tell Izma to stop taking contraceptives to allow for reproduction.", flags[kFLAGS.IZMA_PREGNANCY_ENABLED] == 1 ? "No Kids Please" : "Have Kids?");
...
private  removeIzmasPenis(forced: boolean = false): void {
	clearOutput();
var  gemCost: number = 0;
	if (forced) {
...
//Click Latexy Submenu In Izma's Menu
private  izmaValeriaSubmenu(): void {
	clearOutput();
...
//[No Feeding]
private  izmaValeriaToggle(): void {
	clearOutput();
