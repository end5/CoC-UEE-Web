	...
	export class IsabellaFollowerScene extends NPCAwareContent 
	{
		...
		public  isabellaKnockUpAttempt(): void {
		...
		protected  campIzzySexMenu(): void {
			spriteSelect(SpriteDb.s_isabella);
			menu();
			if (player.hasCock()) {
		...
		private  isabellaAccentUncoaching(): void {
			clearOutput();
		...
		var  x: number = player.smallestCockIndex();
			...
			outputText("You nod, catching a whiff of her musky pussy's aroma without meaning to.  It's not like you could avoid it, with her body mere inches in front of your " + player.faceDescript() + ", shaking so perfectly and so closely that an occasional droplet catches on your nose.  Your hands clamp around the chair with an iron grip as you fight not to rise and rape her tender, teasing asshole on the spot.  " + player.SMultiCockDesc() + " is trickling and dribbling, bouncing with your heartbeats as your " + player.legs() + " shiver and shake with denied lusts.  You NEED to do something... to fuck, thrust, or just fap... anything - your " + player.multiCockDescriptLight() + " can't take it anymore!\n\n");
		...
		private  getIzzyBurped(): void {
			...
			var  changed: boolean = false;
				...
				if (player.hasCock() && player.longestCockLength() < 18) {
					outputText("  A pressure builds in your crotch, at first subtle, then incredibly obnoxious to the point where you're forced to undo the lower portion of your " + player.armorName + " to seek some sort of relief.  Your shock is total as you stare at your former " + player.cockDescript(0) + ", now measuring at least 18 inches long – a condition that Isabella has already noticed and is clearly formulating a gripe about.");
		...
		public  isabellaTalksAboutPotentialPregnancy(): void {
		...
		private  toggleIsabellaContraceptives(): void {
			clearOutput();
			credits.modContent = true;
		...
		private  isabellaCowifyPrompt(): void {
			clearOutput();
			credits.modContent = true;
		...
		private  isabellaTakesOviElixir(): void {
			clearOutput();
			credits.modContent = true;
		...
		private  babyMF(male: string, female: string, gender: number): string {
			if (gender == 1) return male;
			else return female;
		}

		//Isabella Gives Birth!
		public  isabellaGivesBirth(): void {
			credits.modContent = true;
			spriteSelect(SpriteDb.s_isabella);
		var  babyGender: number = 0;
			//Decide the gender (If Isabella is a cowmother, human offsprings will always be male. The rest is cowgirls.)
		var  babyGenderChooser: number = rand(100);
		...
		var  offspringType: number = babyGender;
