...
import { Yamata } from "./Yamata";
import { kGAMECLASS } from "../../../GlobalFlags/kGAMECLASS";

export class AikoScene extends BaseContent implements Encounter, TimeAwareInterface {
    ...
    // public static const AIKO_TIMES_MET: number 			= 2353	//1=first, 2=second 3=third, 4+=fourth
    // public static const AIKO_CORRUPTION: number 			= 2354	//corruption score /100
    // public static const AIKO_CORRUPTION_ACTIVE: number	= 2355	//0=off, 1=on (corruption can be switched on by preference when you meet her and toggled from e4+)
    // public static const AIKO_AFFECTION: number 			= 2356	//affection score  /100
    // public static const AIKO_SEXED: number 				= 2357	//times sexed Aiko
    // public static const AIKO_BALL_RETURNED: number 		= 2358	//1=returned ball 2=didnt return ball (lock out Aiko encounters)
    // public static const AIKO_FIRST_CHOICE: number			= 2359	//1=riches, 2=power, 3=sex, 4=nothing, 5=fight, 6=touchFluffTail
    // public static const AIKO_FIRST_SEX_COOLDOWN: number	= 2360	//cooldown period after first time sex prank
    // public static const AIKO_HAS_FOUGHT: number			= 2361	//has fought Aiko about ball
    // public static const AIKO_FIGHT_WON: number			= 2362	//1=won, 2=lost fight about ball
    // public static const AIKO_HOT_BLOOD: number			= 2363	//once off special encounter
    // public static const AIKO_APOLOGY_SINCERE: number 		= 2364	//1=sincere, 2=trick apologize for fighting
    // public static const AIKO_RAPE: number					= 2365	//1=raped Aiko in scene 2
    // public static const AIKO_TALK_CULTURE: number 		= 2366	//1=first time 2+=repeated times
    // public static const AIKO_TALK_AIKO: number			= 2367	//1=first time 2+=repeated times
    // public static const AIKO_TALK_ARCHERY: number			= 2368	//1=first time 2+=repeated times
    // public static const AIKO_BOSS_INTRO: number			= 2369	//1=Yamata scene introduction at lvl 24+ and Aiko 100 affection
    // public static const AIKO_BOSS_COMPLETE: number		= 2370	//1=Yamata is dead 2 Yamata is released
    // public static const AIKO_BOSS_OUTRO: number			= 2371	//
    // public static const YAMATA_MASOCHIST: number			= 2372	//counter increases to 100, if not defeated
    // public static const AIKO_SPAR_VICTORIES: number		= 2308;	//counter for sparring

    private talkAndSex: boolean = false;

    public encounterChance(): number { return 0.35; }
    public encounterName(): string { return "aiko"; }
    public execEncounter(): void { this.encounterAiko(); }

    // Encounter Aiko
    public encounterAiko(): void {
        CoC.timeAwareClassAdd(this);

        this.clearOutput();
        this.outputText(this.images.showImage("aiko-intro"));
        this.spriteSelect(SpriteDb.s_aiko);
        this.talkAndSex = false;
        if (this.flags[kFLAGS.AIKO_TIMES_MET] == 0) {
            this.flags[kFLAGS.AIKO_AFFECTION] = 50;
        }

        // Count meetings
        this.flags[kFLAGS.AIKO_TIMES_MET]++;

        this.outputText(this.images.showImage("Aiko-intro"));
        // First encounter with Aiko
        if (this.flags[kFLAGS.AIKO_TIMES_MET] == 1) {
            ...
            if (this.player.kitsuneScore() == 0) {
                this.outputText("<i>“I am Aiko, guardian of these woods. You’re trespassing in kitsune territory. Didn’t you see the sign?”</i>\n\n"
                    ...
                    + (this.player.cor > 20 ? "and tell her as much with a slight scowl," : "") + " but you nod your head " + (this.player.cor <= 20 ? "politely" : "")
            ...
            else if (this.player.tail.type == Tail.FOX && this.player.tail.venom == 9) {
                this.outputText("<i>“I am Aiko, guardian of these woo...”</i> she suddenly stops and wides her eyes, noting the swishy wheel of nine tails behind you. Gritting her teeth, she immediately falls on her knees, biting her lower lip much to your confusion. <i>“Please accept my deepest apologies! I... I didn't think you were nine-tails, my " + this.player.mf("lord", "lady") + "! I'm so sorry!”</i>\n\n"
                    ...
                    + "\n\n<i>“" + this.player.mf("Lord", "Lady") + " [name]... no... [name]...It can be a bit sudden.. but can I ask you my ball back, please? You know how important is for us, probably you have one of your own, too.”</i> She seems much more relaxed after you told her to not worry about formalities.\n\n"
                    + "<i>“Even if I'm strong, I don't know if I can match the power of a High " + this.player.mf("Priest", "Priestess") + " of our god... but if I have to, I can get pretty rough... So tell me, [name], do I have to get rough?”</i>");
            } else {
                this.outputText("<i>“I am Aiko, guardian of these woods. I see that you are one of my kind, but you are not from my village. So, are other clans still unaffected by the corruption?”</i>\n\n"
            ...
            // [Yes][No]
            this.menu();
            this.addButton(0, "Yes", this.aikoAggroGreeting).hint("I want to keep this ball!");
            this.addButton(1, "No", this.aikoPeaceGreeeting).hint("Don't want to fight this cute fox-girl.");
        }
        // Once off silly mode encounter
        else if (this.flags[kFLAGS.AIKO_HOT_BLOOD] == 0 && this.flags[kFLAGS.AIKO_TIMES_MET] > 3 && this.silly() && (this.flags[kFLAGS.AIKO_CORRUPTION] < 50 || this.flags[kFLAGS.AIKO_CORRUPTION_ACTIVE] == 0) && rand(15) == 0) {
            this.outputText("As you are walking a familiar-looking weed-choked trail in the deeper regions of the forest, you suddenly stop where you are, raising a hand to your ear. It’s faint, but you can hear the sounds of battle echoing through the trees.\n\n"
            ...
            this.flags[kFLAGS.AIKO_HOT_BLOOD]++;
            this.doNext(this.camp.returnToCampUseOneHour);
        }
        // Second encounter with Aiko
        else if (this.flags[kFLAGS.AIKO_TIMES_MET] == 2) {
            this.outputText("You have a vague sense of déjà vu as you explore deeper into the forest. You can’t place it, but you feel like you’ve been here before… even stranger still is the feeling that you’re being watched. Attempting to shrug it off, you keep on the trail, but the back of your neck continues to tingle with a nagging suspicion, and you begin to succumb to paranoia, looking over your shoulder nervously.\n\n"
                ...
                + (this.player.dogScore() >= 4 ? "Your canine chase instinct overrides your previous sentiments of paranoia, and you dash headlong through" : "You hesitate at first, but then decide to give chase, dashing into") + " the bushes after it.\n\n"
                ...
                + (this.flags[kFLAGS.AIKO_AFFECTION] >= 55 ? "fondness" : (this.flags[kFLAGS.AIKO_AFFECTION] > 50 ? "familiarity" : "contempt"))
                ...
                + "<i>“I’m surprised you managed to find this place again,”</i> she says once she has completed her short ritual, " + (this.flags[kFLAGS.AIKO_HAS_FOUGHT] > 0 ? "smiling" : "glaring") + " at you.");

            if (this.flags[kFLAGS.AIKO_AFFECTION] >= 55) {
                this.outputText("<i>“Pleasantly surprised,”</i> she adds, her expression turning soft and affectionate. "
                    + (this.flags[kFLAGS.AIKO_FIRST_CHOICE] == 6 ? "<i>“I’ve actually been hoping you’d find your way back here…”</i>" : "")
                    + "\n\nShe starts to make her way toward you, swaying gently as she walks. <i>“I wanted to say thank you again for returning my Ball…" + (this.flags[kFLAGS.AIKO_FIRST_CHOICE] == 6 ? " and especially… for the other things you did,”</i> she says with a slight blush" : "”</i> she says") + ". <i>“Most people probably wouldn’t have been that nice.”</i>\n\n"
                ...
                this.menu();
                this.addButton(0, "Talk", this.aikoTalkE2).hint("Talk with the foxy girl.");
                this.addButton(1, "Sex", this.aikoSexE2).hint("Have some fun times with foxy Aiko!");
                this.addButton(4, "Fight", this.aikoFight).hint("Attack that fox bitch!");
                this.addButton(14, "Leave", this.leave).hint("Leave the clearing");
            }
            else if ((this.flags[kFLAGS.AIKO_AFFECTION] > 50)) {
                this.outputText("<i>“So, did you enjoy your ‘present’?”</i> she says with a snicker, barely suppressing a full-blown grin. You give her a flat look, mustering up your best deadpan, and reply that you’ve had better.\n\n"
                    + "<i>“Oh, don’t be so serious,”</i> she retorts, sticking out her tongue at you and smirking. <i>“Just a little harmless fun, is all.”</i>\n\n"
                    + "At your expense, you are quick to remind her, crossing your arms indignantly. " + (this.flags[kFLAGS.AIKO_FIRST_CHOICE] == 3 ? "You have to admit, the sex was good—you just could have gone without the undesired side-effects." : "") + " You would like to pursue the subject of her trickery further, but Aiko quickly deflects the discussion before you get a chance to speak up.\n\n"
                ...
                this.outputText("<i>“You’ve got some nerve showing your face around here,”</i> she says coldly, her bow appearing from thin air. <i>“I hope you weren’t counting on " + (this.flags[kFLAGS.AIKO_FIGHT_WON] == 1 ? "another free pass " : "a free pass this time") + "; I’ve hidden my Ball somewhere else now, so there’s no chance of you taking it hostage again.”</i>\n\n"
                    + "[if (player.isCorruptEnough(50))There she goes about that stupid ball again.]" + (this.flags[kFLAGS.AIKO_FIGHT_WON] ? "You tell her that she seems pretty high and mighty for someone who lost so handily the last time." + (this.silly() ? "Maybe, you suggest, she should check herself, before—and you quote—she wrecks herself." : "") : "She seems pretty confident that she can take you again, after her previous victory.") + " You can practically feel the daggers emanating from her eyes as she stares you down unflinchingly, looking like she’s almost itching for you to give her a reason to release her grip on the arrow pointed at you.\n\n"
                    + (this.flags[kFLAGS.AIKO_FIGHT_WON] ? "<i>“It’s your move.”</i>" : "<i>“So, are you going to leave peacefully, or do we do it the hard way again?”</i>"));

                // [Apologize][Sex][Fight][Leave]
                this.menu();
                this.addButton(0, "Apologize", this.aikoApologize1).hint("Apologize to Aiko for fighting her.");
                this.addButton(1, "Sex", this.aikoSexE2).hint("Have some fun times with foxy Aiko!");
                this.addButton(2, "Fight", this.aikoFight).hint("Attack that fox bitch!");
                this.addButton(3, "Leave", this.leave).hint("Leave the clearing");
            }

        }
        // Third encounter with Aiko
        else if (this.flags[kFLAGS.AIKO_TIMES_MET] == 3) {
            this.outputText("You walk out into the familiar clearing where the Ancient Tree stands, and you spot the kitsune priestess Aiko practicing with her bow. She hasn’t noticed your approach yet, drawing a new arrow from her quiver and sending a shot arcing through the air. You can see that her targets appear to be several squares of parchment tacked to the tree trunks, and with a squint you recognize that the closest one appears to bear a crude drawing of " + (this.flags[kFLAGS.AIKO_AFFECTION] >= 50 ? "a minotaur" : "you!") + "\n\n"
                + "Her arrows thud into the targets one after another, maintaining a beautifully dense grouping." + (!this.player.hasStatusEffect(StatusEffects.Kelt) ? "She’s quite good!" : (this.player.statusEffectv1(StatusEffects.Kelt) < 75 ? "She’s quite good, though perhaps not as good as Kelt. Still, you might stand to learn a thing or two from her, and she’s probably better company than the arrogant centaur." : "She’s quite good, but not as good as Kelt or yourself. She might make a decent practice partner, but you probably wouldn’t learn anything new from her.")) + "\n\n"
                + "Finally, you clear your throat, and she whips around, the arrow she was about to fire now trained on you instead! " + (this.flags[kFLAGS.AIKO_AFFECTION] >= 50 ? "She relaxes when she recognizes you, letting the draw on her bow out slowly and lowering the weapon away from your chest." : "She snarls with disgust when she sees it’s you, keeping the weapon trained on you the entire time.")
                + "\n\n");

            if (this.flags[kFLAGS.AIKO_AFFECTION] >= 50) {
                this.outputText("<i>“Oh, [name]! I was just practicing… Sorry about the uh, arrow,”</i> she says, tapping the shaft of the arrow up against her bow lightly. <i>“You startled me, is all.”</i>\n\n"
                    + "You tell her it’s fine, and you understand—one never can be too careful.\n\n");

                if (this.flags[kFLAGS.AIKO_RAPE] < 1) {
                    this.outputText("She smiles, dismissing her bow and walking toward you happily. " + (this.flags[kFLAGS.AIKO_AFFECTION] >= 65 ? "With a big grin, she wraps her arms around your shoulders and gives you a kiss on the cheek, then backs up to talk to you." : "") + "\n\n<i>“What brings you back out here again?”</i>");

                    // [Talk][Sex][Fight][Leave]
                    this.menu();
                    this.addButton(0, "Talk", this.aikoTalkE3).hint("Talk with the foxy girl.");
                    this.addButton(1, "Sex", this.aikoSex).hint("Have some fun times with foxy Aiko!");
                    this.addButton(3, "Spar", this.aikoSpar).hint("Exchange some friendly blows.");
                    this.addButton(4, "Fight", this.aikoFight).hint("Attack that fox bitch!").disableIf(this.player.isPureEnough(40), "You are not corrupt enough to just attack her out of the blue.");
                    this.addButton(14, "Leave", this.leave).hint("Leave the clearing");
                } else {
                    this.outputText("She nods, but seems to shrink back from you uneasily. You notice she hasn’t dismissed her bow yet, even though it isn’t pointed at you anymore.\n\n"
                    ...
                    this.addButton(0, "Talk", this.aikoTalkE3).hint("Talk with the foxy girl.");
                    this.addButton(1, "Sex", this.aikoSex).hint("Have some fun times with foxy Aiko!");
                    this.addButton(4, "Fight", this.aikoFight).hint("Attack that fox bitch!").disableIf(this.player.isPureEnough(60), "You are not corrupt enough to just attack her out of the blue.");
                    this.addButton(14, "Leave", this.leave).hint("Leave the clearing");
                }
            } else {
                this.outputText("<i>“You just won’t take a hint, will you?!”</i> she growls, narrowing her eyes.\n\n"
        ...
        else if (this.flags[kFLAGS.AIKO_BOSS_COMPLETE] > 0 && this.flags[kFLAGS.AIKO_BOSS_OUTRO] == 0) {
            this.outputText("As you make your way into the bioluminescent deepwoods, you remember you had a discussion to do with Aiko, she was " + (this.flags[kFLAGS.AIKO_BOSS_COMPLETE] == 1 ? "really glad the nightmare was over" : "really angry you let her sister go") + ".\n\n"
                + "You move across the bushes and you notice a familiar presence. You lean your back on a tree as you see a blue flame moving towards you. Immediately after that, the flame explodes and Aiko materializes in front of you. " + (this.flags[kFLAGS.AIKO_BOSS_COMPLETE] == 1 ? "She's very happy to see you" : "She looks bittersweet.") + "\n\n"
            ...
            this.flags[kFLAGS.AIKO_BOSS_OUTRO] = 1;
            this.doNext(this.camp.returnToCampUseOneHour);
        }
        else if (this.flags[kFLAGS.AIKO_BOSS_COMPLETE] == 0 && this.player.level >= 25 && this.flags[kFLAGS.AIKO_AFFECTION] > 90 && this.flags[kFLAGS.AIKO_TIMES_MET] > 3) {
            if (this.flags[kFLAGS.AIKO_BOSS_INTRO] > 0) {
                if (this.flags[kFLAGS.AIKO_CORRUPTION] < 50 || this.flags[kFLAGS.AIKO_CORRUPTION_ACTIVE] == 0) {
                    ...
                    // [Go Now][Prepare]
                    this.menu();
                    this.addButton(0, "Talk", this.yamataTalk).hint("Ask Aiko for more info on Yamata.");
                    this.addButton(1, "Go Now", this.yamataStart).hint("Go with Aiko to take on Yamata and stop her madness!");
                    this.addButton(2, "Prepare", this.yamataPrepare).hint("Go home and prepare, this will be a very tough fight!");
                } else {
            ...
            else if (this.flags[kFLAGS.AIKO_CORRUPTION] < 50 || this.flags[kFLAGS.AIKO_CORRUPTION_ACTIVE] == 0) {
                ...
                this.flags[kFLAGS.AIKO_BOSS_INTRO]++;
                // [Go Now][Prepare]
                this.menu();
                this.addButton(0, "Go Now", this.yamataStart).hint("Go with Aiko to take on Yamata and stop her madness!");
                this.addButton(1, "Prepare", this.yamataPrepare).hint("Go home and prepare, this will be a very tough fight!");
            } else {
            ...
            this.outputText("You enter the clearing where the Ancient Tree stands, calling out for Aiko.\n\n");
            let temp: number;
            if (this.flags[kFLAGS.AIKO_CORRUPTION] < 50 || this.flags[kFLAGS.AIKO_CORRUPTION_ACTIVE] == 0) {
                if (this.flags[kFLAGS.AIKO_AFFECTION] >= 50) {
                    temp = (this.flags[kFLAGS.AIKO_AFFECTION] >= 50 ? rand(3) : rand(2) + 1);
                    if (temp == 0) // 33% chance if affection > 50
                        this.outputText("<i>“DYNAMIC ENTRY!”</i>\n\n"
                            + "Before you can react, you feel Aiko’s thighs suddenly slam down on your shoulders as she seems to drop out of nowhere. " + (this.player.str > 60 ? "The impact startles you, but you maintain your balance and help her down to the ground." : (this.player.str >= 30 ? "The impact knocks the wind out of you, but thankfully Aiko is pretty light, and you manage to get her to the ground safely." : "You buckle under the force of the impact, and wind up on your face with Aiko sitting on your back. <i>“Heheh, oops. Sorry!”</i> she says as she helps you up.")) + "\n\n");
                    if (temp == 1) // 33% chance if affection > 50 else 50%
                        this.outputText("After a minute or two, the cute kitsune priestess emerges from the forest, her bow slung over her shoulder. She has a few patches of dirt on her forehead, and it looks like she’s been out hunting small game. Pleased to see you, she goes to freshen up and returns a few minutes later, looking pristine as always.\n\n");
                    if (temp == 2) // 33% chance if affection > 50 else 50%
                        this.outputText("The branches of the trees rustle a little, and suddenly she swings down, hanging from her knees off one of the lower boughs. Grinning widely, she releases her grip and gracefully flips to the ground… only to fail to stick the landing and slip onto her plump derriere with a yelp! She picks herself up and brushes herself off, trying to pretend like nothing happened, but you can’t help but chuckle a little.\n\n");

                    if (this.flags[kFLAGS.AIKO_AFFECTION] >= 75)
                        this.outputText("She gives you a quick hug and a peck on the cheek in greeting. ");
                    this.outputText((this.flags[kFLAGS.AIKO_CORRUPTION] <= 20 || this.flags[kFLAGS.AIKO_CORRUPTION_ACTIVE] == 0 ? "She then walks over to the great tree and casually leans against it, her arms behind her back as she smiles at you cutely. She’s every bit as peppy and effervescent as always." : (this.flags[kFLAGS.AIKO_CORRUPTION] <= 40 ? "She then walks up to the tree, seeming a bit less energetic than usual. The old spark in her eyes has gone dull, but in its place there is a glimmer of something… else. You wonder if you’re starting to rub off on her a little." : "She has an odd sway in her step, and something about her just seems… off. You’re beginning to suspect that hanging around you has started to seriously affect her personality."))
                    ...
                    // [Talk][Sex][Both][Spar][Fight]--[Leave]
                    this.menu();
                    this.addButton(0, "Talk", this.aikoTalk).hint("Talk with the foxy girl.");
                    this.addButton(1, "Sex", this.aikoSex).hint("Have some fun times with foxy Aiko!");
                    this.addButton(2, "Both", this.aikoTalkAndSex).hint("Have a chat and then some fun times with foxy Aiko after!");
                    this.addButton(3, "Spar", this.aikoSpar).hint("Exchange some friendly blows.");
                    this.addButton(4, "Fight", this.aikoFight).hint("Attack that fox bitch!").disableIf(this.player.isPureEnough(75), "You are not corrupt enough to just attack her out of the blue.");
                    this.addButton(9, "Corruption " + (this.flags[kFLAGS.AIKO_CORRUPTION_ACTIVE] == 0 ? "off" : "on"), this.toggleCorruption).hint(this.flags[kFLAGS.AIKO_CORRUPTION_ACTIVE] == 0 ? "Aiko's corruption is disabled" : "Aiko's corruption is enabled");
                    this.addButton(14, "Leave", this.leave).hint("Leave the clearing");
                } else {
                    this.outputText("You have to duck when an arrow goes whizzing by your head, sticking into a tree behind you with a thud. Aiko marches out of the underbrush and pulls out another arrow, walking up to stand by the Ancient Tree. " + ((this.flags[kFLAGS.AIKO_CORRUPTION] <= 20 || this.flags[kFLAGS.AIKO_CORRUPTION_ACTIVE] == 0) ? "You can see that same spark of life and pride in her eyes as always." : (this.flags[kFLAGS.AIKO_CORRUPTION] <= 40 ? "The spark of pride and life is gone from her eyes, replaced with something colder. You have your suspicions that she has you to thank for that." : "She seems to have an unsettling sway in her gait, like she’s just a little off balance. There is a look in her eye that suggests her mind might be going slightly unhinged as well.")) + " Ethereal blue flames dance around the clearing, casting everything in a pale, ghostly light.\n\n"
                        + "<i>“That was a warning shot... What do you want?”</i>");

                    // [Talk][Sex][Talk&Sex][Fight]--[Corruption]--[Leave]
                    this.menu();
                    this.addButton(0, "Talk", this.aikoTalk).hint("Talk with the foxy girl.");
                    this.addButton(1, "Submit", this.submitToAiko).hint("Have some fun times with foxy Aiko!\nThough in her current state who knows what she'll get up to!");
                    this.addButton(2, "Talk&Sex", this.aikoTalkAndSex).hint("Have a chat and then some fun times with foxy Aiko after!");
                    this.addButton(3, "Fight4Rape", this.aikoFight).hint("She likes it when you try to rape her!");
                    this.addButton(9, "Corruption " + (this.flags[kFLAGS.AIKO_CORRUPTION_ACTIVE] == 0 ? "off" : "on"), this.toggleCorruption).hint(this.flags[kFLAGS.AIKO_CORRUPTION_ACTIVE] == 0 ? "Aiko's corruption is disabled" : "Aiko's corruption is enabled");
                    this.addButton(14, "Leave", this.leave).hint("Leave the clearing");
                }
            }
    ...
    public timeChange(): boolean {
        if (this.player.hasStatusEffect(StatusEffects.AikoLustPrank)) {
            this.player.addStatusValue(StatusEffects.AikoLustPrank, 1, -1);
            if (this.player.statusEffectv1(StatusEffects.AikoLustPrank) <= 0) {
                this.player.removeStatusEffect(StatusEffects.AikoLustPrank);
                this.dynStats("sen", -15);
                this.outputText("The constant flush on your skin finally fades away as Aiko’s magic wears off. At last, you can finally move around freely again without getting turned on at the slightest touch!");
            }
        }
        return false;
    }
    ...
    private leave(): void {
        if (this.player.hasStatusEffect(StatusEffects.Spar)) {
            this.player.removeStatusEffect(StatusEffects.Spar);
            this.combat.cleanupAfterCombat();
        }
        if (this.player.hasStatusEffect(StatusEffects.DomFight)) {
            this.player.removeStatusEffect(StatusEffects.DomFight);
            this.combat.cleanupAfterCombat();
        }
        this.doNext(this.camp.returnToCampUseOneHour);
    }
    ...
    private initScene: string = "Peace";
    private initialCorruptionSetting(): void {
        this.clearOutput();
        this.outputText("Aiko is a character with optional corruption scenes. Her corruption is disabled by default but can be enabled now or late in her encounters.\n\n"
            + "Do you wish to enable corruption for Aiko?\n\n");
        this.menu();
        this.addButton(0, "Enable corruption", this.aikoEnableCor).hint("Aiko's corruption will progress as normal and scenes may differ from pure.");
        this.addButton(1, "Disable corruption", this.aikoDisableCor).hint("Aiko's corruption will not progress and all pure scenes will be be used.");
    }

    private aikoEnableCor(): void {
        this.flags[kFLAGS.AIKO_CORRUPTION_ACTIVE] = 1;
        this.outputText("Aiko's corruption has been enabled and will progress as normal with certain interactions.");
        if (this.initScene == "Aggro")
            this.doNext(this.aikoAggroGreeting);
        else
            this.doNext(this.aikoPeaceGreeeting);
    }

    private aikoDisableCor(): void {
        this.flags[kFLAGS.AIKO_CORRUPTION_ACTIVE] = 0;
        this.outputText("Aiko's corruption has been disabled and will not progress regardless of interactions.");
        if (this.initScene == "Aggro")
            this.doNext(this.aikoAggroGreeting);
        else
            this.doNext(this.aikoPeaceGreeeting);
    ...
    private toggleCorruption(): void {
        this.flags[kFLAGS.AIKO_CORRUPTION_ACTIVE] ^= 1;
        this.flags[kFLAGS.AIKO_TIMES_MET]--;
        this.doNext(this.encounterAiko);
    }

    /////////////////////////////////////////////////////////////////////
    /////////////////////////// ONCE OFF SCENES///////////////////////////
    private aikoAggroGreeting(): void {
        this.clearOutput();
        this.spriteSelect(SpriteDb.s_aiko);
        this.outputText("You ask her if that was a threat, stowing the ball and readying your [weapon]. You’re certain that she’s trying to trick you somehow, and you won’t stand for it!\n\n"
        ...
        this.initScene = "Aggro";
        // [Riches][Power][Sex][Nothing][Fight][TouchFluffTail]
        this.menu();
        this.addButton(0, "Riches", this.aikoE1Riches).hint("GIVE ME ALL THE GOLD!!!");
        this.addButton(1, "Power", this.aikoE1Power).hint("I can actually have more power?!!!");
        this.addButton(2, "Sex", this.aikoE1SexPart1).hint("That sexy proposition sounded very enticing...");
        this.addButton(3, "Nothing", this.aikoE1Nothing).hint("Give it back, it's hers' after all.");
        this.addButton(4, "Fight", this.aikoFight).hint("This ball is mine!");
        this.addButton(5, "Fluffy Tail", this.aikoTouchFluffTail).hint("Those tails look so fluffy... GOTTA TOUCH THEM!!!");
        this.addButton(9, "Corruption", this.initialCorruptionSetting).hint("Activate Aiko's Corruption");
    }

    private aikoPeaceGreeeting(): void {
        this.clearOutput();
        this.spriteSelect(SpriteDb.s_aiko);
        this.outputText("You wonder if you should take that as a threat for a moment, until Aiko lets out a snorting laugh. <i>“Nah, I’m just kidding. You seem legit. But seriously, gimme back my Ball.”</i>\n\n"
        ...
        this.addButton(2, "Sex", this.aikoE1SexPart1).hint("That sexy proposition sounded very enticing...");
        this.addButton(3, "Nothing", this.aikoE1Nothing).hint("Give it back, it's hers' after all.");
        this.addButton(4, "Fluffy Tail", this.aikoTouchFluffTail).hint("Those tails look so fluffy... GOTTA TOUCH THEM!!!");
        this.addButton(9, "Corruption", this.initialCorruptionSetting).hint("Activate Aiko's Corruption");
    }

    private aikoE1Riches(): void {
        this.clearOutput();
        this.spriteSelect(SpriteDb.s_aiko);
        this.outputText("You consider her offer for a moment, and then tell her that you could always use a few extra gems.\n\n"
        ...
        if (this.player.hasKeyItem("Camp - Chest") < 0) {
            ...
            this.outputText("Well, maybe you could use the chest for storage… \n\n<b>You now have " + AikoScene.num2Text(this.inventory.itemStorageDirectGet().length) + " storage item slots at camp.</b>");
        } else {
            this.outputText("Before your eyes, the chest transforms into a white piece of paper with a cartoonish face drawn on it, blowing you the raspberry.");
        }
        this.flags[kFLAGS.AIKO_BALL_RETURNED] = 1;
        this.flags[kFLAGS.AIKO_FIRST_CHOICE] = 1;
        this.flags[kFLAGS.AIKO_AFFECTION]++;
        this.doNext(this.camp.returnToCampUseOneHour);
    }

    private aikoE1Power(): void {
        this.clearOutput();
        this.spriteSelect(SpriteDb.s_aiko);
        this.outputText("After a little thought, you tell her that a little more power could go a long way toward helping you on your quest.\n\n"
            + "<i>“Teehee… Alright " + this.player.mf("Mr.", "Mrs.") + " Hero, your wish is my command,”</i> Aiko teases, grinning mischievously. She thinks for a minute and then appears to be struck with inspiration, her face lighting up as she runs off, shouting to you, <i>”Stay right here! Don’t move!”</i>\n\n"
        ...
        this.flags[kFLAGS.AIKO_BALL_RETURNED] = 1;
        this.flags[kFLAGS.AIKO_FIRST_CHOICE] = 2;
        this.flags[kFLAGS.AIKO_AFFECTION]++;
        this.inventory.takeItem(this.weapons.PIPE, this.camp.returnToCampUseOneHour);
    }

    private aikoE1SexPart1(): void {
        this.clearOutput();
        this.spriteSelect(SpriteDb.s_aiko);
        this.outputText("You eye her lithe body up and down for a moment, licking your lips involuntarily as she slides her hands up and down her supple curves. A heat emanating from your loins tells you that your body has already made up your mind for you. You are about to give voice to your desires when Aiko saunters up to you, placing a finger to your lips and whispering, <i>“Shh… naughty " + this.player.mf("boy", "girl") + "… I can see you undressing me with your eyes. I know what you want.”</i>\n\n"
            + "Before you can say anything, she drops to her knees in front of you, pulling your [armor] aside to expose your groin.\n\n");

        this.flags[kFLAGS.AIKO_BALL_RETURNED] = 1;
        this.flags[kFLAGS.AIKO_FIRST_CHOICE] = 3;
        if (this.player.hasCock() && this.player.hasVagina()) {
            this.outputText("She examines your loins for a moment and then looks up at you, saying, <i>“So, how would you prefer me to pleasure you, hm?”</i>");

            // [Male][Female]
            this.menu();
            this.addButton(0, "Male", this.aikoSexBJ).hint("Have her pleasure your dick with her mouth.");
            this.addButton(1, "Female", this.aikoSexCunni).hint("Let her explore the soft folds of your pussy with her tongue.");
        }
        else if (this.player.hasCock()) {
            this.doNext(this.aikoSexBJ);
        }
        else if (this.player.hasVagina()) {
            this.doNext(this.aikoSexCunni);
        } else {
            this.outputText("Seeing that you don't have any of the usual equipment she seems quite stumped. <i>“Uhmm I don't exactly know how you want me to please you if you don't have the right stuff...”</i>");
            this.doNext(this.aikoE1SexPart2);
        }
    }

    private aikoE1SexPart2(): void {
        this.clearOutput();
        this.spriteSelect(SpriteDb.s_aiko);
        this.outputText("<i>“So… I don’t mean to be pushy, but…”</i>"
        ...
        this.flags[kFLAGS.AIKO_BALL_RETURNED] = 1;
        this.flags[kFLAGS.AIKO_AFFECTION] += 2;
        this.doNext(this.applyAikoLustPrankEffect);
    }

    private applyAikoLustPrankEffect(): void {
        this.player.createStatusEffect(StatusEffects.AikoLustPrank, rand(48) + 24, 0, 0, 0);
        this.dynStats("sen", 35);
        this.doNext(this.camp.returnToCampUseOneHour);
    }

    private aikoE1Nothing(): void {
        this.clearOutput();
        this.spriteSelect(SpriteDb.s_aiko);
        this.outputText("You don’t know if it’s because you’re feeling charitable, or because of some nagging suspicion, but after some consideration, you decide to return her ball without asking for anything in return.\n\n"
        ...
        this.player.dynStats("lus", 15, "cor", -5);
        const temp: number = rand(4);
        if (temp == 0) {
            this.outputText("You feel like you could arm-wrestle a minotaur, your muscles rippling with new reserves of strength!");
            this.player.dynStats("str", rand(4) + 1);
        }
        if (temp == 1) {
            this.outputText("You feel like you could brush off a blow from even the strongest demon, your body feeling more resilient!");
            this.player.dynStats("tou", rand(4) + 1);
        }
        if (temp == 2) {
            this.outputText("You feel like you could outrun a centaur, as you fill with newfound energy!");
            this.player.dynStats("spe", rand(4) + 1);
        }
        if (temp == 3) {
            this.outputText("You feel like you could memorize an entire library’s worth of books, your mind becoming noticeably sharper!");
            this.player.dynStats("int", rand(4) + 1);
        }
        this.flags[kFLAGS.AIKO_BALL_RETURNED] = 1;
        this.flags[kFLAGS.AIKO_FIRST_CHOICE] = 4;
        this.flags[kFLAGS.AIKO_AFFECTION] += 5;
        this.doNext(this.camp.returnToCampUseOneHour);
    }

    private aikoTouchFluffTail(): void {
        this.clearOutput();
        this.spriteSelect(SpriteDb.s_aiko);
        this.outputText("<i>“Wh-what?!”</i> Aiko shouts, a deep crimson blush on her cheeks.\n\n"
        ...
        this.flags[kFLAGS.AIKO_BALL_RETURNED] = 1;
        this.flags[kFLAGS.AIKO_FIRST_CHOICE] = 6;
        this.flags[kFLAGS.AIKO_AFFECTION] += 15;
        this.player.dynStats("lus", 10 + rand(11), "cor", -5);
        this.doNext(this.camp.returnToCampUseOneHour);
    }

    public aikoLosesIntro(): void {
        this.clearOutput();
        this.spriteSelect(SpriteDb.s_aiko);
        ...
        this.flags[kFLAGS.AIKO_FIGHT_WON] = 1;
        this.doNext(this.aikoRapeSex);
    }

    public aikoWinsIntro(): void {
        this.clearOutput();
        this.spriteSelect(SpriteDb.s_aiko);
        this.outputText("You slump to the ground, [if (player.lust >= player.maxLust())crippled with desire|thoroughly thrashed], looking up at the fiery-eyed fox-woman.\n\n");

        if (this.flags[kFLAGS.AIKO_TIMES_MET] == 1) {
            if (this.player.kitsuneScore() > 0) {
                if (this.player.tail.type == Tail.FOX && this.player.tail.venom == 9) {
                    this.outputText("<i>“... C-can it be? I... I defeated a nine-tails?”</i> With dismay painted across her face, she carefully makes her bow disappear in a puff of smoke. Surrounding herself with an array of fireballs, she approaches you and slowly reaches for your pouch, retrieving her white ball.\n\n"
                    ...
                    this.outputText("<i>“I didn't want to do this, but you forced me”</i> she states while lowering her bow, allowing it to disappear in a puff of smoke. Keeping her guard up, she approaches you surrounding herself with fireballs. Cautiously she leans down and snatches the white ball from your pouch.\n\n"
                        + "<i>“This ball is just too important for me to leave it in the hands of a stranger.”</i>\n\n"
                        + "She carefully tucks the ball into her cleavage, stepping back and keeping the fireballs pointed toward you at arm’s length. Holding her first two fingers up near her face, she whispers an incantation, and as the last syllables leave her lips, your vision begins to swim. Just before you black out, you can hear Aiko’s voice calling out as if from miles away.\n\n"
                        + "<i>“I'm sorry... you were kinda charming...”</i>");
                }
            } else {
                this.outputText("<i>“Just remember, you brought this on yourself,”</i> she says as she lowers her bow, allowing it to disappear in a puff of smoke. She keeps her eyes trained on you as she walks over, cautiously preparing a fireball in one hand, and reaches down to retrieve the white ball from your pouch.\n\n"
            ...
            this.outputText("Aiko stands over you for a moment, murmuring an incantation under her breath, and you watch her point her first two fingers at your forehead just before the world goes dark.");
        }
        this.flags[kFLAGS.AIKO_BALL_RETURNED] = 1;
        this.combat.cleanupAfterCombat();
    }

    private aikoRequestsBallAfterLose(): void {
        this.clearOutput();
        this.spriteSelect(SpriteDb.s_aiko);
        this.outputText("You turn away from the collapsed kitsune, ready to leave, but stop when you feel her hand clutching your [armor] desperately.\n\n"
        ...
        // [Give][Don't Give]
        this.menu();
        this.addButton(0, "Give", this.aikoLoseGiveBall).hint("Give her the damn ball already...");
        this.addButton(1, "Don't Give", this.aikoLoseRefuseBall).hint("She doesn't deserve to have it.");
    }

    private aikoLoseGiveBall(): void {
        this.clearOutput();
        this.spriteSelect(SpriteDb.s_aiko);
        this.outputText("You look down at her pleading expression, [if(player.isCorruptEnough(50)) and can’t help but get annoyed by the pitiful girl’s persistence|and can’t help feel a twinge of remorse for the pitiful girl]. Hesitantly, you reach into your pouch, pulling the ball out, and a small glimmer of hope shines in Aiko’s eyes at the sight of it.\n\n"
    ...
    private aikoLoseRefuseBall(): void {
        ...
        this.flags[kFLAGS.AIKO_BALL_RETURNED] = 2;
        this.player.dynStats("cor", 5);
        this.combat.cleanupAfterCombat();
    }

    /////////////////////////////////////////////////////////////////////
    //////////////////////// ENCOUNTER TWO////////////////////////////////
    private aikoTalkE2(): void {
        this.clearOutput();
        this.spriteSelect(SpriteDb.s_aiko);
        this.outputText("You smile and tell her you’d just like to talk. She smiles back, suppressing a slight look of disappointment, and nods, saying, <i>“Oh, really? Well, I guess I could use the company.”</i>\n\n"
        ...
        this.flags[kFLAGS.AIKO_AFFECTION] += 3;
        if (this.flags[kFLAGS.AIKO_CORRUPTION_ACTIVE] == 1) this.flags[kFLAGS.AIKO_CORRUPTION] -= 5;
        if (this.flags[kFLAGS.AIKO_CORRUPTION] < 0) this.flags[kFLAGS.AIKO_CORRUPTION] = 0;
        this.doNext(this.camp.returnToCampUseOneHour);
    }

    private aikoSexE2(): void {
        this.clearOutput();
        this.spriteSelect(SpriteDb.s_aiko);
        this.outputText("You smile and ask if she’d be interested in a little <i>“roll in the grass.”</i>\n\n");
        if (this.flags[kFLAGS.AIKO_AFFECTION] >= 55) {
            this.outputText("She smiles a bit in return, giggling at your forwardness.\n\n"
            ...
            this.outputText("She gives you a big grin and says <i>“Straight to the point, huh? Well, at least you’re a " + this.player.mf("man", "woman") + " who knows what " + this.player.mf("he", "she") + " wants.”</i>\n\n"
            ...
            this.outputText("<i>“You’ve got to be kidding me. After what you did?!”</i>\n\n"
                + "She sneers at you, baring her sharp animalistic teeth angrily. <i>“Seriously, what are you, some kind of psycho? Get out of here, you unbelievable " + this.player.mf("bastard", "bitch") + "!”</i>\n\n"
            ...
            // [Force][Leave]
            this.menu();
            this.addButton(0, "Force", this.aikoFight).hint("You have needs! Force her.");
            this.addButton(14, "Leave", this.leave).hint("Don't force the subject. You screwed up, don't take it out on her.");
        }
    }

    private aikoApologize1(): void {
        this.clearOutput();
        this.spriteSelect(SpriteDb.s_aiko);
        this.outputText("You put your hands up slowly, trying to look as non-threatning as possible. You’re just here to talk, you explain.\n\n"
            ...
            + "You tell her that you want to apologize for what you did. You’ve had a little time to reflect on it and you feel terrible about " + (this.flags[kFLAGS.AIKO_RAPE] <= 0 ? "attacking her" : "your brutal rape") + ". There’s no way mere words could make up for your shameful actions, so you understand if she can’t forgive you; you just needed to tell her you’re sorry.\n\n"
        ...
        // [Sincere][Trick Her]
        this.menu();
        this.addButton(0, "Sincere", this.aikoApologySincere).hint("You truly are sorry for your actions.");
        this.addButton(1, "Trick Her", this.aikoApologyTrick).hint("Nope, just another trick to get close!").disableIf(!this.player.isCorruptEnough(33), "<b>You are not corrupt enough for this deceit</b>");
    }

    private aikoApologySincere(): void {
        this.clearOutput();
        this.spriteSelect(SpriteDb.s_aiko);
        this.outputText("You smile as she offers her hand, still visibly a little apprehensive, but willing to give you the benefit of the doubt. Grasping her hand firmly, you give it a light shake, and the two of you both relax.\n\n"
    ...
    private aikoApologyTrick(): void {
        this.clearOutput();
        this.spriteSelect(SpriteDb.s_aiko);
        this.outputText("You smile as she offers her hand, still visibly a little apprehensive, but willing to give you the benefit of the doubt. Big mistake on her part. Your smile turns to an evil grin as you roughly grab her hand and yank her off balance, wrestling her to the ground.\n\n");
        this.player.dynStats("cor", 5);
        this.flags[kFLAGS.AIKO_AFFECTION] -= 8;
        if (this.flags[kFLAGS.AIKO_CORRUPTION_ACTIVE] == 1) this.flags[kFLAGS.AIKO_CORRUPTION] += 10;

        if (this.player.str > 35) {
            this.outputText("She screams, struggling in vain against your superior physical strength as you beat her into submission, ripping her clothes off to claim your prize.\n\n"
                + "<i>“Y-you fucking " + this.player.mf("bastard", "cunt") + "!”</i> she is in tears now, her face covered with dirt and scratches from the way you tumbled around with her. <i>“I knew I shouldn’t have trusted you!”</i>\n\n"
            ...
            this.outputText("She yells out and plants a flaming slap on your face that sends you reeling, scrabbling to her feet quickly and putting some distance between herself and you.\n\n"
            ...
            this.player.createOrFindStatusEffect(StatusEffects.DomFight);
            this.startCombat(new Aiko());
        }
    }

    private aikoTalkE3(): void {
        this.clearOutput();
        this.spriteSelect(SpriteDb.s_aiko);
        const bowSkill: number = this.player.statusEffectv1(StatusEffects.Kelt);
        const hasBow: boolean = (this.player.hasKeyItem("Bow") >= 0 || this.player.hasKeyItem("Kelt's Bow") >= 0);
        if (this.flags[kFLAGS.AIKO_AFFECTION] >= 50) {
            this.outputText("You tell her you just want to talk today. " + (this.flags[kFLAGS.AIKO_HAS_FOUGHT] > 0 ? "Aiko breathes a big sigh of relief and nods, smiling." : "") + "\n\n"
                + "<i>“Sure, I’d love to talk. I was starting to get a little lonely anyway,”</i> she says, taking a seat on the grass.\n\n"
                + "You tell her that you noticed she’s pretty handy with a bow" + (this.flags[kFLAGS.AIKO_HAS_FOUGHT] > 0 ? ", though you add that you already had some personal experience with her archery skills previously" : "") + ".\n\n"
                ...
                + "<i>“Oh, yeah, sure. They’re all super top-secret though—even I don’t know where most of them are. We keep to ourselves mainly, though the village chiefs sometimes get together for really important stuff. I… actually I probably shouldn’t be talking about this with an outsider, " + (this.player.tail.type == Tail.FOX && this.player.tail.venom == 9 ? "even if " + this.player.mf("he", "she") + " is a nine-tails... " : "") + " the Elders tend to get mad easily about this stuff, unfortunately.”</i>\n\n"
            ...
            if (hasBow) {
                this.outputText("You pull out your bow and ask if she’d be up for a little friendly competition, grinning widely. She seems pretty excited about it, and promptly goes to prepare the course.\n\n"
                    + "Over the next hour, the two of you take turns shooting at the targets. ");
                if (bowSkill < 33) {
                    this.outputText("Aiko utterly thrashes you with her superior skills, but she’s quite sweet about it, and you manage to pick up a few pointers from her.");
                    this.player.addStatusValue(StatusEffects.Kelt, 1, 2);
                }
                else if (bowSkill < 75) {
                    this.outputText("You give her a good run for her money, but Aiko still manages to come out on top in the end. She’s quite sweet about it though, and offers you a few pointers that might help you out.");
                    this.player.addStatusValue(StatusEffects.Kelt, 1, 1);
                } else {
                    this.outputText("Aiko is stiff competition, but she ends up being no match for your superior skills. You even graciously offer her a few pointers, and she seems grateful for the assistance.");
                    this.flags[kFLAGS.AIKO_AFFECTION] += 5;
                }
            } else {
                this.outputText("The conversation drifts toward your various exploits elsewhere in Mareth, and the two of you chat it up for the better part of an hour before you tell Aiko that you should be heading back to camp. She bids you a fond farewell, and goes back to archery practice as you head out.");
            }
            ...
            this.outputText("You tell her you just want to talk, holding your hands up.\n\n"
                + (this.flags[kFLAGS.AIKO_APOLOGY_SINCERE] == 2 ? "She squints, keeping her bow trained on you the whole time, and says, <i>“Oh, yeah, I’ve heard THAT before.”</i>\n\n" : "")
                + "You assure her that this time is different, keeping your hands held up defensively.\n\n"
                + "<i>“Why should I believe you? So far you’ve turned out to be nothing but an arrogant jackass who does whatever " + this.player.mf("he", "she") + " wants to whoever " + this.player.mf("he", "she") + " wants.”</i>\n\n"
                + (this.silly() ? "You gently hold a finger up and tell her she means “whomever”, not “whoever.” She gives you a “WTF” look and yells, <i>“Whatever!”</i>\n\n" +
                ...
                + "<i>“Of course, you didn’t think mine was the only one, did you? …Never mind. They’re hidden, and you won’t be getting their locations from me; I don’t even know where most of them are. " + ((this.player.tail.type == Tail.FOX && this.player.tail.venom == 9) ? "you should be able to locate them if you want to, you are nine-tails and all." : "") + "”</i>\n\n"
    ...
    private aikoApologize2(): void {
        this.clearOutput();
        this.spriteSelect(SpriteDb.s_aiko);
        this.outputText("She’s right, you say, which seems to surprise her a bit. She has no reason to trust you, but you can see it in her eyes that she’s lonely, and you can help. You tell her if she’s willing to let bygones be bygones, you promise to give her the kind of companionship she’s yearning for right now.\n\n"
    ...
    private aikoTalkAndSex(): void {
        this.clearOutput();
        this.spriteSelect(SpriteDb.s_aiko);
        this.outputText("You tell Aiko you came to talk, but you’d also be interested in sex later on. \n\n"
            + (this.flags[kFLAGS.AIKO_AFFECTION] >= 65 ? "She smiles cheerfully and approaches you, leaning in to give you a light peck on the cheek. <i>“Sounds like a plan to me,”</i> she says, grinning and backing up a bit. <i>“What did you want to talk about?”</i>" :
                (this.flags[kFLAGS.AIKO_AFFECTION] >= 50 ? "She smiles cheerfully, nodding a little. <i>“Sure, we can talk. Play your cards right and I just might take you up on that second part,”</i> she says, but from her playful tone you can tell she’s just teasing. <i>“What did you have in mind?”</i>" :
        ...
        this.talkAndSex = true;
        this.aikoTalk();
    }

    private aikoTalk(): void {
        if (!this.talkAndSex) {
            this.clearOutput();
            this.spriteSelect(SpriteDb.s_aiko);
            this.outputText("You tell Aiko that you just came to talk, nothing more."
                + (this.flags[kFLAGS.AIKO_AFFECTION] >= 65 ? "She smiles cheerfully and approaches you, leaning in to give you a light peck on the cheek. <i>“What did you want to talk about?”</i> she says, grinning and backing up a bit." : (this.flags[kFLAGS.AIKO_AFFECTION] >= 50 ? "She smiles cheerfully, nodding a little. <i>“Sure, we can talk. What did you have in mind?”</i>" : "<i>“Alright, we’ll talk. Well, what did you want to talk about?”</i>")));
        }
        this.flags[kFLAGS.AIKO_AFFECTION] += 3;
        if (this.flags[kFLAGS.AIKO_CORRUPTION_ACTIVE] == 1)
            this.flags[kFLAGS.AIKO_CORRUPTION] -= 5;
        if (this.flags[kFLAGS.AIKO_CORRUPTION] < 0) this.flags[kFLAGS.AIKO_CORRUPTION] = 0;
        ...
        this.menu();
        this.addButton(0, "Giant Bees", this.talkBees).hint("Ask her about the Giant Bees in the forest");
        this.addButton(1, "Tentacle Beasts", this.talkTentacles).hint("Ask her about the Tentacle Beasts in the forest");
        this.addButton(2, "Goblins", this.talkGoblins).hint("Ask her about the Goblins");
        this.addButton(3, "Akbal", this.talkAkbal).hint("Ask her about the Demon Akbal you encountered near here");
        this.addButton(4, "Kitsune Culture", this.talkCulture).hint("Ask her about the Kitsune Culture");
        this.addButton(5, "Aiko", this.talkAiko).hint("Ask her about herself");
        this.addButton(6, "Mansion Sisters", this.talkMansionSisters).hint("Ask her about the Kitsune sisters in the Deep Woods Mansion");
        this.addButton(7, "Nine-Tails", this.talkNineTails).hint("Ask her about the Nine-Tails Kitsune");
        this.addButton(8, "Aiko's Family", this.talkFamily).hint("Ask her about her family");
        this.addButton(9, "Archery", this.talkArchery).hint("Ask her about her Archery skills");
        this.addButton(14, "Leave", this.leave).hint("Leave the clearing");
    }

    /////////////////////////////////////////////////////////////////////
    //////////////////////// TALK TOPICS//////////////////////////////////
    private talkBees(): void {
        this.clearOutput();
        this.spriteSelect(SpriteDb.s_aiko);
        this.outputText("You decide to ask her about some of the local fauna. You ask if she’s had much experience dealing with the giant bee-women that you’ve seen in the forest sometimes.\n\n"
        ...
        if (this.talkAndSex) {
            this.outputText("You tell her that’s alright, and then let the conversation drift to other topics for a bit before you ask if she’s given much thought to your other proposition.");
            this.doNext(this.aikoSex);
        } else {
            this.outputText("You say that that’s alright, thanking her for her time, and tell her that you should be heading back to camp anyway.");
            this.doNext(this.camp.returnToCampUseOneHour);
        }
    }
    private talkTentacles(): void {
        this.clearOutput();
        this.spriteSelect(SpriteDb.s_aiko);
        this.outputText("You decide to ask her about some of the local fauna. You ask her what she knows about tentacle beasts.\n\n"
        ...
        if (this.talkAndSex) {
            this.outputText("Finally, you bring up your previous proposal, asking if she’s still interested.");
            this.doNext(this.aikoSex);
        } else {
            this.outputText("Finally, you bid her farewell, telling her that you have to check in on your camp.");
            this.doNext(this.camp.returnToCampUseOneHour);
        }
    }
    private talkGoblins(): void {
        this.clearOutput();
        this.spriteSelect(SpriteDb.s_aiko);
        this.outputText("You decide to ask Aiko about some of the local fauna. You ask whether she’s ever had any interactions with the ubiquitous goblin-folk that seem to be all over the place.\n\n"
        ...
        if (this.talkAndSex) {
            this.outputText("bring up your previous proposal, asking if she’s still interested.");
            this.doNext(this.aikoSex);
        } else {
            this.outputText("bid her farewell, telling her you should be checking in on your camp again soon.");
            this.doNext(this.camp.returnToCampUseOneHour);
        }
    }
    private talkAkbal(): void {
        this.clearOutput();
        this.spriteSelect(SpriteDb.s_aiko);
        this.outputText("You decide to ask Aiko about some of the local fauna. You ask her what she knows about the demonic panther you’ve encountered occasionally near here.\n\n"
        ...
        if (this.talkAndSex) {
            this.outputText("Thanking her for the information, you decide to change the subject, bringing up your previous proposition again.");
            this.doNext(this.aikoSex);
        } else {
            this.outputText("Thanking her for her time, you tell her that you have to go check back on your camp.");
            this.doNext(this.camp.returnToCampUseOneHour);
        }
    }
    private talkCulture(): void {
        this.clearOutput();
        this.spriteSelect(SpriteDb.s_aiko);
        this.outputText("You tell her you’re interested in "
            + (this.player.kitsuneScore() > 0 ? "expand your knowledge of the culture of your race" : (this.flags[kFLAGS.AIKO_TALK_CULTURE] > 0 ? "hearing what she has to say about kitsune culture" : "learning more about kitsune culture")) + ".\n\n"
            + (this.flags[kFLAGS.AIKO_TALK_CULTURE] == 0 ? "<i>“What, really?”</i>" +
            ...
            + (this.flags[kFLAGS.AIKO_AFFECTION] >= 50 ? "You think about some of the times she’s tried to play jokes on you and coyly ask if that was her way of courting you." +
                (this.flags[kFLAGS.AIKO_AFFECTION] >= 65 ? "She blushes a little and says, <i>“Well, that’s a strong possibility...”</i>" : "She grins and replies back just as coyly, <i>“Could be... Maybe you’re just gullible.”</i>") + "\n\n" : "")
        ...
        this.flags[kFLAGS.AIKO_TALK_CULTURE]++;
        if (this.talkAndSex) {
            this.outputText("The conversation gradually drifts toward other topics before you remind her of your other proposal again.");
            this.doNext(this.aikoSex);
        } else {
            this.outputText("While the experience has been educational, you should be heading back to check on your camp now.");
            this.doNext(this.camp.returnToCampUseOneHour);
        }
    }
    private talkAiko(): void {
        this.clearOutput();
        this.spriteSelect(SpriteDb.s_aiko);
        this.outputText("You tell her that you had a few personal questions, if she doesn’t mind.\n\n"
        ...
        this.flags[kFLAGS.AIKO_TALK_AIKO]++;
        if (this.talkAndSex) {
            this.outputText(" and ask if Aiko has put any thought into your other proposition.");
            this.doNext(this.aikoSex);
        } else {
            this.outputText("; you really should be heading back to check on your camp.");
            this.doNext(this.camp.returnToCampUseOneHour);
        }
    }
    private talkMansionSisters(): void {
        this.clearOutput();
        this.spriteSelect(SpriteDb.s_aiko);
        this.outputText("You decide to ask her about some of the other kitsune you’ve met along your adventures. You tell her all about the three kitsune sisters that you’ve seen in the forest, and ask if they’re from Aiko’s village as well.\n\n"
        ...
        if (this.talkAndSex) {
            this.outputText("You tell her that you might be in the mood for some mischief yourself and remind her of your other proposition.");
            this.doNext(this.aikoSex);
        } else {
            this.outputText("Unfortunately, you’ve been away from camp for too long, and you should probably return to check up on it.");
            this.doNext(this.camp.returnToCampUseOneHour);
        }
    }
    private talkNineTails(): void {
        this.clearOutput();
        this.spriteSelect(SpriteDb.s_aiko);
        this.outputText("<i>“Oh, you want to know about our nobility, hey?”</i> She folds her arms under her large, tightly bound breasts. You can see she's aiming to become one herself, already having [if (flags[kFLAGS.AIKO_BOSS_COMPLETE] >0)eight|seven] tails. [If (player.kitsuneScore() > 0)<i>“You should know stuff like this by now, if you want to be a full fledged kitsune, you need to know who your elders are, heheh”</i> She laughs with her tongue out of her mouth, pulling a mischievous grin.]\n\n"
            ...
            + ((this.player.tail.type == Tail.FOX && this.player.tail.venom == 9) ? " especially since it is so applicable to you.\n\n" : ".\n\n")
        ...
        if (this.talkAndSex) {
            ...
            this.outputText("Thanking her for her time, you tell her that you have to go check back on your camp. <i>“Sayonara, [name].”</i>");
            this.doNext(this.camp.returnToCampUseOneHour);
        }
    }
    private talkFamily(): void {
        this.clearOutput();
        this.spriteSelect(SpriteDb.s_aiko);
        this.outputText("You tell her that you had a few personal questions, if she doesn’t mind.\n\n"
            + "<i>“Go ahead. No guarantees that I’ll answer them all though,”</i> she replies, crossing her arms and grinning coyly.\n\n"
        ...
        if (this.talkAndSex) {
            this.outputText("saying that you can drop the conversation here if she wants.\n\n"
                + "<i>“Yeah, I'd prefer it that way, thanks.”</i>\n\n"
                + "You mention your previous suggestion in the hopes of cheering her up again.");
            this.doNext(this.aikoSex);
        } else {
            this.outputText("saying that you should be heading back to camp anyway.\n\n"
                + "<i>“Yeah, I'd prefer it that way, thanks.”</i>");
            this.doNext(this.camp.returnToCampUseOneHour);
        }
    }
    private talkArchery(): void {
        this.clearOutput();
        this.spriteSelect(SpriteDb.s_aiko);
        this.outputText("You bring up Aiko’s archery skills, and she proudly smiles at the compliment, though you can see a slightly embarrassed blush on her cheeks.\n\n");
        if (this.player.hasKeyItem("Bow") >= 0 || this.player.hasKeyItem("Kelt's Bow") >= 0) {
            this.flags[kFLAGS.AIKO_TALK_ARCHERY]++;
            if (this.player.statusEffectv1(StatusEffects.Kelt) > 75) {
                if (this.flags[kFLAGS.AIKO_TALK_ARCHERY] == 1) {
                    this.outputText("You tell her you’ve been trying to learn as well, pulling out your own bow to show her. She examines it for a bit, smiling as she runs her hand down the side.\n\n"
                        ...
                        + (this.player.statusEffectv2(StatusEffects.Kelt) < 75 ? "remarking on the arrogant centaur’s brash attitude and how awful it is putting up with him." : "sighing almost dreamily as you describe his rugged good looks and no-nonsense teaching methods.")
                    ...
                    this.outputText("You tell her you were wondering if she’d be willing to give you another lesson.\n\n"
                ...
                if (this.flags[kFLAGS.AIKO_TALK_ARCHERY] == 1) {
                    this.outputText("You tell her you’ve been trying to learn as well, pulling out your own bow to show her. She examines it for a bit, smiling as she runs her hand down the side.\n\n"
                        ...
                        + (this.flags[kFLAGS.AIKO_AFFECTION] >= 50 ? "<i>“I’d be happy to give you some pointers, [name].”</i>" : (this.flags[kFLAGS.AIKO_AFFECTION] >= 35 ? "<i>“Okay, I’ll teach you... Just don’t make me regret this.”</i>\n\n" : "<i>“No way am I teaching you anything now, maybe you should try being nicer.”</i>]\n\n")));
                }
                else {
                    this.outputText("You tell her you were wondering if she’d be willing to give you another lesson.\n\n"
                        + (this.flags[kFLAGS.AIKO_AFFECTION] >= 50 ? "She nods happily and gives you a cheerful nod, saying, <i>“Of course, I’ll go set up the targets!”</i>\n\n" : "She thinks about it for a second and then finally says " + (this.flags[kFLAGS.AIKO_AFFECTION] >= 35 ? "<i>“Okay, I’ll teach you some more.”</i>\n\n" : "<i>“No, not today. You want free archery lessons, maybe you should try being nicer.”</i>\n\n")));
                }
            }
            if (this.flags[kFLAGS.AIKO_AFFECTION] >= 35) {
                this.outputText("The two of you take turns shooting at the targets, swapping stories back and forth in between shots. Even if Kelt is a little more skilled, Aiko’s cheery disposition more than makes up for it, and you find yourself grateful to have found an archery partner that isn’t as arrogant, crude, and snide as he. ");
                if (this.player.statusEffectv1(StatusEffects.Kelt) < 33) {
                    this.outputText("In the end, she still proves more than a match for you, but thankfully she doesn’t rub it in your face. You come away feeling like you’ve learned a lot in your short time with her.\n\n");
                    this.player.addStatusValue(StatusEffects.Kelt, 1, 3);
                }
                else if (this.player.statusEffectv1(StatusEffects.Kelt) <= 75) {
                    this.outputText("In the end you give her a decent run for her money, but ultimately it is apparent you still have much to learn from her. You feel like you’ve picked up a few good tips and tricks from her.\n\n");
                    this.player.addStatusValue(StatusEffects.Kelt, 1, 1);

                } else {
                    this.outputText("In the end, she was right; you beat her fair and square, and you can’t say you learned anything new, but it was fun. And you feel a bit closer to her for the experience");
                }
            }
        } else {
            this.outputText("You tell her you would be interested in learning to use a bow as well, if she felt like teaching you. She grins sheepishly, taking out her longbow and holding it close, gently running her hand along it.\n\n"
                + (this.flags[kFLAGS.AIKO_AFFECTION] >= 50 ? "<i>“I would love to, believe me, but... I don’t really have one for you to use, and this one is sort of really important to me. It’s not that I don’t trust you with it, but... I’d be devastated if anything happened to it. Sorry... If you can find one somewhere though, I’d be happy to help you learn to use it!”</i>" : "<i>“Sorry, no can do. I don’t have a spare, and I’m not going to let just anyone touch mine... It’s really important to me, and I’d be devastated if anything happened to it. If you get your own, maybe I’d be willing to give you a few pointers.”</i>")
        ...
        if (this.talkAndSex) {
            ...
            this.outputText(", but unfortunately, you should be getting back to camp now.");
            this.doNext(this.camp.returnToCampUseOneHour);
        }
    }
    ...
    private aikoSex(): void {	// same for e3 onward
        this.clearOutput();
        this.spriteSelect(SpriteDb.s_aiko);
        this.outputText("You tell her that you’d like to have sex with her" + (this.flags[kFLAGS.AIKO_HAS_FOUGHT] > 0 ? ", but you’re not out to force her this time" : "") + ".\n\n");
        if (this.flags[kFLAGS.AIKO_AFFECTION] >= 50) {
            this.outputText("She seems pretty happy to hear you say that, and eagerly begins to strip down.\n\n"
            ...
            this.outputText("<i>“You just can’t get it through your thick skull, can you?”</i> she says, shaking her head. <i>“You know, I just might have let you, if I felt like I could trust you.”</i>\n\n"
            ...
            this.menu();
            this.addButton(0, "Apologize", this.aikoApologize2).hint("Apologize for being a mean-ass fucker.");
            this.addButton(1, "Force", this.aikoFight).hint("You have needs! Force her.");
            this.addButton(14, "Leave", this.leave).hint("Don't force the subject. You screwed up, don't take it out on her.");
        }
    }

    private playerchoice: boolean = true;
    public aikoConsSex(): void {
        // [Missionary] [DoggyStyle] [BJ] [GetLicked] [Tailjob]--[Leave]
        this.menu();
        if (this.player.hasCock()) {
            this.addButton(0, "Missionary", this.aikoSexMissionary).hint("");
            this.addButton(1, "DoggyStyle", this.aikoSexDoggy).hint("");
            this.addButton(2, "BJ", this.aikoSexBJ).hint("Have her pleasure your dick with her mouth.");
        }
        if (this.player.hasVagina())
            this.addButton(3, "Get Licked", this.aikoSexCunni).hint("Let her explore the soft folds of your pussy with her tongue.");
        if (this.flags[kFLAGS.AIKO_AFFECTION] >= 75 && this.player.hasCock())
            this.addButton(4, "Tailjob", this.aikoSexTailjob).hint("");
        if (this.flags[kFLAGS.AIKO_CORRUPTION] > 50 && this.flags[kFLAGS.AIKO_CORRUPTION_ACTIVE] > 0) {
            this.addButton(5, "Submit", this.submitToAiko).hint("Submit to Aiko and see what she does with you");
        }
        this.addButton(14, "Leave", this.leave).hint("Leave the clearing without touching her.");
    }

    private aikoSpar(): void {
        this.clearOutput();
        this.spriteSelect(SpriteDb.s_aiko);
        this.outputText("You smile and suggest maybe you could have a friendly competition, if she was interested.\n\n"
        ...
        this.flags[kFLAGS.AIKO_AFFECTION] += 2;
        this.player.createOrFindStatusEffect(StatusEffects.Spar);
        this.startCombat(new Aiko());
    }

    public sparWithAikoWin(): void {
        this.clearOutput();
        this.spriteSelect(SpriteDb.s_aiko);
        this.outputText("Aiko drops to one knee, panting heavily and supporting herself with her bow. In spite of her obvious battle exhaustion, she is grinning from ear to ear.\n\n"
        ...
        if (this.player.hasStatusEffect(StatusEffects.Spar)) {
            this.flags[kFLAGS.AIKO_SPAR_VICTORIES]++;
        }
        this.aikoConsSex();
    }

    public sparWithAikoLose(aikoLust: number): void {
        this.clearOutput();
        this.spriteSelect(SpriteDb.s_aiko);
        this.outputText("You fall to the ground in a heap, overcome by [if (player.lust >= player.maxLust())lust|exhaustion]. Aiko stands over you for a moment, hands on her hips as she smirks victoriously.\n\n"
        ...
        const options: any[] = [];
        if (this.flags[kFLAGS.AIKO_AFFECTION] >= 60 && aikoLust >= 60) {
            if (this.player.hasCock()) {
                options.push(this.aikoSexMissionary);
                options.push(this.aikoSexDoggy);
                options.push(this.aikoSexBJ);
            }
            if (this.player.hasVagina())
                options.push(this.aikoSexCunni);
            if (this.flags[kFLAGS.AIKO_AFFECTION] >= 75 && this.player.hasCock())
                options.push(this.aikoSexTailjob);
        }
        if (options.length > 0) {
            this.doNext(options[rand(options.length)]);
        } else {
    ...
    private aikoFight(): void {
        this.clearOutput();
        this.spriteSelect(SpriteDb.s_aiko);

        if (this.flags[kFLAGS.AIKO_CORRUPTION] > 50 && this.flags[kFLAGS.AIKO_CORRUPTION_ACTIVE] > 0) {
            this.outputText("You tell her that you’re going to fuck her whether she likes it or not, readying your [weapon]. She grins a wicked grin, seeming to be quite pleased with your enthusiasm, and shouts, <i>“That’s what we like to hear! Come and fuckin’ get it, bitch!”</i>");
        } else {
            this.outputText("Making up your mind, you lunge at her without warning, your [weapon] raised and ready to fight. She jumps back out of the way, narrowly dodging your sudden attack, and raises her bow to defend herself.");
            if (this.flags[kFLAGS.AIKO_AFFECTION] >= 50)
                this.outputText("<i>“H-hey! What the hell?! What are you doing?”</i> she yells in surprise, but your only reply is to take a ready stance, preparing to fight her.");
            else this.outputText("<i>“Alright, we’ll do it your way again!”</i> she barks, prepared to fight.");
            this.flags[kFLAGS.AIKO_AFFECTION] -= 2;
        }
        this.player.createOrFindStatusEffect(StatusEffects.DomFight);
        this.startCombat(new Aiko());
    ...
    public pcWinsDomFight(): void {
        ...
        if (this.player.hasStatusEffect(StatusEffects.DomFight))
            this.player.removeStatusEffect(StatusEffects.DomFight);
        if (this.flags[kFLAGS.AIKO_CORRUPTION] < 50 && this.flags[kFLAGS.AIKO_CORRUPTION_ACTIVE] > 0) {
            this.outputText("Aiko stumbles and falls to the ground, panting hard. Though she puts up a good fight, in the end it seems her muscles are too weak to carry on, and she just stays slumped on the ground, looking up at you with a crazed fire in her eyes.\n\n"
                + "<i>“Eh...heh...haha... So you DO know how to show a girl a good time. Maybe you’re not such a " + this.player.mf("limp-dicked choir boy", "washed-up whore") + " after all.”</i> she says, grinning sarcastically. <i>“Well come on, or are you too much of a wuss to go through with it now that I’m on the ground?”</i>\n\n"
    ...
    public pcLosesDomFight(): void {
        ...
        if (this.flags[kFLAGS.AIKO_CORRUPTION] < 50 && this.flags[kFLAGS.AIKO_CORRUPTION_ACTIVE] > 0) {
            ...
            this.outputText("You fall to the ground, giving in to the crazed vixen’s power. Aiko approaches with a toothy grin and a glimmer of insanity in her eyes as she begins to disrobe, letting her clothes lay haphazardly where they drop. She plants a foot on your chest and pushes you back, giving you a smug little smirk as you collapse backward onto the ground.\n\n"
                ...
                + "To your " + (this.player.findPerk(PerkLib.Masochist) >= 0 ? "delight" : "horror") + ", your foxy dominatrix's hands both clasp firmly around your neck, bearing down on you. You choke and thrash weakly, staring up at her vindictive grin as darkness begins to enclose on your vision. At the edges of your consciousness, you can feel Aiko’s tails slowly sliding down toward your groin, " + (this.player.hasCock() ? "wrapping around your [cocks]" : "") + (this.player.hasVagina() && this.player.hasCock() ? " and " : "") + (this.player.hasVagina() ? "beginning to tease your [clit] with their furry bottle-brush tips" : "") + ".\n\n"
                + "You gasp violently as her hands let up just enough to pull you back from the brink, filling your lungs with sweet fresh air at the exact moment that she begins to pump her tails " + (this.player.hasCock() ? "up and down your [cocks]" : "") + (this.player.hasVagina() && this.player.hasCock() ? " and " : "") + (this.player.hasVagina() ? "into your [vagina]. The worst part is that it feels so GOOD" : "") + ", the combination of the sensations pulling you toward your climax by leaps and bounds while Aiko laughs derisively at your pathetic struggles underneath her.\n\n"
                ...
                + (this.player.hasCock() ? "As your vision darkens once more, you can feel the heat of Aiko’s sopping cunt against the tip of your [cock " + (x + 1) + "], teasing you with their slick depths. The soft, warm lips dilate tantalizingly around your head, and with an abrupt drop, she mounts your member,[if (player.hasKnot(" + x + ")) careful not to let your knot slip in and] squeezing down tightly!\n\n" : "")
                + (this.player.hasVagina() ? "The tails in your [vagina] double their effort, pounding you mercilessly as Aiko’s pleasure begins to boil over.\n\n" : "")
                + "Her hands suddenly release their grip on your throat entirely, and as you hoarsely fill your lungs with sweet, wonderful fresh air, your body is wracked with a tremendous orgasm, sending you into a frenzy from head to toe. Aiko’s tails frizz up with pleasure as she seems to synchronize with you, clawing at her own body with both hands as she rides the wave of her own climax. " + (this.player.cumQ() > 350 && this.player.hasCock() ? "Her abdomen swells with seed as your orgasm goes on for even longer than usual, a side effect of the strange asphyxiation-play that she put you through." : "") + "\n\n"
                + "Released from your pleasurable torment and finally able to climax, you lay panting on the ground, gasping for breath as Aiko slowly dismounts you" + (this.player.cumQ() > 350 && this.player.hasCock() ? ", allowing the pent-up seed to gush out over your lower body" : "") + ". Your last sight is that of her retrieving her robes just before you pass out from exhaustion.");
            if (this.player.hasCock())
                this.player.orgasm('Dick');
            if (this.player.hasVagina())
                this.player.orgasm('vagina');
        } else {
            this.outputText("You fall to the ground in a heap, overcome by your [if (player.lust >= player.maxLust())lust|wounds]. Aiko stands over you for a moment, murmuring an incantation under her breath, and you watch her point her first two fingers at your forehead just before the world goes dark.\n\n");
        }
        this.combat.cleanupAfterCombat();
    }

    public aikoRapeSex(): void {
        // [Fuck Vagina][Fuck Ass][Humiliate][Leave]
        this.menu();
        this.addButton(0, "Fuck Vag", this.aikoRapeFuckVag).hint("Pound ze foxy pussy!").disableIf(!this.player.hasCock(), "Requires a dick you moron!");
        this.addButton(1, "Fuck Ass", this.aikoRapeFuckAss).hint("Get some of that tight, juicy ass!").disableIf(!this.player.hasCock(), "Requires a dick you moron!").disableIf(this.player.cockThatFits(50) < 0, "You dick too huge man, She be delicate in the heinie");
        this.addButton(2, "Humiliate", this.aikoRapeHumiliate).hint("Make her your pet bitch!");
        if (this.flags[kFLAGS.AIKO_BALL_RETURNED] == 1) {
            this.addButton(3, "Fist Her", this.aikoFistHer).hint("Have a feel inside her pussy.");
            this.addButton(4, "Get Licked", this.aikoRapeGetLicked).hint("This time you want Aiko to eat you out!").disableIf(!this.player.hasVagina(), "This scene kind of needs some lady parts, you know...");
        }
        if (this.flags[kFLAGS.AIKO_BALL_RETURNED] != 1)
            this.addButton(14, "Leave", this.aikoRequestsBallAfterLose).hint("Leave the clearing without touching her.");
        else
            this.addButton(14, "Leave", this.leave).hint("Leave the clearing without touching her.");
    }
    ...
    private postRapeUpdate(): void {
        ...
        this.flags[kFLAGS.AIKO_RAPE]++;

        if (this.player.isPureEnough(50)) {
            this.player.dynStats("cor", 5);
        ...
        if ((this.player.cor > this.flags[kFLAGS.AIKO_CORRUPTION] || this.player.hasPerk(PerkLib.Sadist)) && this.flags[kFLAGS.AIKO_CORRUPTION_ACTIVE] == 1) {
            this.flags[kFLAGS.AIKO_CORRUPTION] += 12;
        } else if (this.flags[kFLAGS.AIKO_CORRUPTION_ACTIVE] == 1) {
            this.flags[kFLAGS.AIKO_CORRUPTION] += 8;
        }

        if (this.flags[kFLAGS.AIKO_CORRUPTION] > 100) {
            this.flags[kFLAGS.AIKO_CORRUPTION] = 100;
        }
    }
     ...
     */
    private postSexUpdate(): void {
        this.flags[kFLAGS.AIKO_AFFECTION] += 4;

        if (this.player.isCorruptEnough(50) && this.flags[kFLAGS.AIKO_CORRUPTION_ACTIVE] > 0) {
            this.flags[kFLAGS.AIKO_CORRUPTION] += 4;
        }

        if (this.flags[kFLAGS.AIKO_CORRUPTION_ACTIVE] > 0 && this.flags[kFLAGS.AIKO_CORRUPTION] >= 50 && this.player.isPureEnough(this.flags[kFLAGS.AIKO_CORRUPTION])) {
            this.player.cor += 2;
            this.flags[kFLAGS.AIKO_CORRUPTION] -= 2;
        }
    }

    private aikoRapeFuckVag(): void {
        this.clearOutput();
        this.spriteSelect(SpriteDb.s_aiko);
        const x: number = rand(this.player.cocks.length);
        this.outputText("The temptation to take advantage of the defeated priestess is just too much to pass up. You roughly grab the sash that holds together the lower half of her robes, tugging it until it comes undone, and use it to tie Aiko’s arms behind her back. She doesn’t put up much of a fight, though the worried look on her face tells you she has an inkling what you have in mind.\n\n");
        if (this.flags[kFLAGS.AIKO_TIMES_MET] == 1)
            this.outputText("<i>“I… please, I just wanted my Ball,”</i> she pleads, looking up at you as you turn her onto her back. " + (this.player.isCorruptEnough(50) || this.player.hasPerk(PerkLib.Sadist) ? "You give the impertinent girl a hearty slap across the cheek, telling her to quit blubbering and shut her whore mouth. This is her punishment for trying to trick you, and if you’re feeling generous later, maybe you’ll deign to give it back to her." : "") + " Eagerly tossing her robes aside, you spread her legs to reveal a moist little slit bearing a cute little lotus tattoo just above her pubic mound. With a lascivious grin, you remove your [armor], revealing your [cocks], and begin to rub against her pervertedly.\n\n"
                + "<i>“P-please! Do whatever you want, but… but promise!”</i> she says, small tears welling ");
        else
            this.outputText("<i>“I… please, I didn't want to fight you!”</i> she pleads, looking up at you as you turn her onto her back. " + (this.player.isCorruptEnough(50) || this.player.hasPerk(PerkLib.Sadist) ? "You give the impertinent girl a hearty slap across the cheek, telling her to quit blubbering and shut her whore mouth." : "") + " Eagerly tossing her robes aside, you spread her legs to reveal a moist little slit bearing a cute little lotus tattoo just above her pubic mound. With a lascivious grin, you remove your [armor], revealing your [cocks], and begin to rub against her pervertedly.\n\n"
                + "<i>“P-please! Why are you doing this?!”</i> she says, small tears welling ");
        this.outputText("up at the corners of her eyes as your hands tug at her chest wrap, letting her perky D-cups bounce free. " + (this.player.isCorruptEnough(50) || this.player.hasPerk(PerkLib.Sadist) ? "You smack her around a bit more, telling her to shut up" + (this.flags[kFLAGS.AIKO_TIMES_MET] == 1 ? " about her stupid ball" : "") + ". You could have sworn you told this bitch to keep her damn trap shut! " : "You do your best to shrug off the pangs of guilt, knowing that you’re in it too deep to turn back now.") + " She sniffles a little, biting her lip. It seems she’s prepared to suffer through whatever you plan to do to her, not that she had much choice in the matter." + (this.player.isCorruptEnough(50) || this.player.hasPerk(PerkLib.Sadist) ? " At least now, perhaps she’ll stop whining long enough for you to get off!" : "") + "\n\n"
            ...
            + (this.player.hasKnot(x) ? "When you feel your knot pushing against her lips, you shift your weight to lean over her and begin to pound her pussy with your [cock " + (x + 1) + "]." : "When you feel your hips rocking against hers, you shift your weight to lean over her and begin to pound her pussy with your [cock " + (x + 1) + "].")
            ...
            + (this.flags[kFLAGS.AIKO_BALL_RETURNED] == 0 ? "<i>“You got what you wanted…”</i> she says, almost whispering. <i>“Now will you give me back my Ball?”</i>\n\n"
    ...
    private aikoRapeFuckAss(): void {
        this.clearOutput();
        this.spriteSelect(SpriteDb.s_aiko);
        const x: number = this.player.cockThatFits(50);
        this.outputText("Your eyes trail lasciviously down the fallen priestess’s body, and finally come to rest on the plump cheeks of her ass, exposed by her immodest robes. Making up your mind, you reach down to pull off the lower portion of her robes away, bearing down on her to prevent her from struggling.\n\n"
            + "<i>“Wha, wait, no, what are you—“</i>\n\n"
            + (this.player.isCorruptEnough(50) || this.player.hasPerk(PerkLib.Sadist) ? "You backhand her across the face, reminding her to shut up and be a good little whore if she wants her ball back." : "You give her a disciplinary tap, reminding her to sit still if she wants to get her ball back.") + " She shrinks away from you and obediently raises her plump hind quarters into the air, her tails rising out of the way, revealing her glistening slit and cute pucker. Giving her a firm exploratory slap on the ass, you turn almost giddy with delight as the pillowy cheeks ripple and jiggle for an extended period of time.\n\n"
            ...
            + "You give one last firm shove forward with your hips, forcing a cry from Aiko’s throat as she is finally speared " + (this.player.hasKnot(x) ? "on your full length up to the knot, and that starting to spread her pucker rather wide." : "on the full length of your cock.") + " You’re barely able to control yourself as you squeeze her delicious ass, beginning to pump in and out with a reckless haste.\n\n"
            ...
            + "You lose yourself in the pleasure, your ears deaf to the lovely priestess’s cries as her ass is impaled on your rod again and again, pulling you closer and closer to climax with every thrust.[if(player.isCorruptEnough(50) || player.hasPerk(PerkLib.Sadist)) Smiling vindictively, you lean over her, digging your nails into her back painfully as you pound her ass harder still, her pained yelps filling you with delight.] Finally, you can take it no longer, " + (this.player.hasKnot(x) ? "shoving your hips forward with all your might and burying your knot in her ass as," : "") + " your shaft distending with cum as you begin to pour out your lusts into the girl’s abused fuckhole. " + (this.player.cumQ() > 350 ? "Her belly begins to swell with your seed, gurgling audibly as you dump load after load into her innards. When your cock’s spasms finally end, she is significantly rounder and heavier than previously, weighed down by the enormous volume of semen you filled her with." : "") + "\n\n"
            + (this.player.hasKnot(x) ? "You are held together for a while longer by your distended and sensitive knot, which only starts deflating after several minutes.\n\n" : "")
            ...
            (this.flags[kFLAGS.AIKO_BALL_RETURNED] == 0 ? ", and [if(player.isCorruptEnough(50) ||player.hasPerk(PerkLib.Sadist))pull out her ball, smirking evilly as a despicable idea comes into your mind. You place the ball up against Aiko’s brutalized anus and push it in, eliciting another shameful groan from her throat. Well, she said she wanted it back, and now she has it, you think to yourself with a terrible grin as you set off for camp. |gently drop her ball on the ground next to her, setting off for camp.]" : " and set off back to camp."));
    ...
    private aikoRapeHumiliate(): void {
        this.clearOutput();
        this.spriteSelect(SpriteDb.s_aiko);
        ...
        if (!this.player.hasCock() && this.player.hasVagina())
            this.outputText("You shiver and slowly dismount, standing up and leaving the practically comatose kitsune splayed out pervertedly on the ground, soaked through with your combined juices.\n\n");
        else
            this.outputText("You shiver lightly, prepared to dismount, but a slight throbbing in your groin reminds you that you haven’t yet taken care of your male half.\n\n");
        if (this.player.hasCock()) {
            const x: number = rand(this.player.cocks.length);
            if (!this.player.hasVagina())
                this.outputText("You roughly drag her onto her back by the hair, ignoring the pained cries she lets out as you position yourself over her, head to toe, slowly lowering yourself down to pin her shoulders with your knees. Grasping one of her tails, you begin to coil it up, aiming it toward her glistening twat, and with a [if (player.isCorruptEnough(50))cruel] smirk you force the fuzzy appendage inside.\n\n");
            else
            ...
            if (this.player.cockArea(x) <= 50)
                ...
                this.outputText("You bear down on her forcefully, but as the girl’s struggles grow more and more desperate, you at last concede that you simply won’t be able to cram any more of your massive member into her throat. Displeased with her failure to perform up to your standards, you pull your shaft from her throat, giving her a moment to cough up a wad of precum, and then give her a [if(player.isCorruptEnough(50) ||player.hasPerk(PerkLib.Sadist))firm swat on the side of the face. If she’s too inept to please you with her mouth, then you’ll have to find another way for her to get you off...|gentle smack on the behind, playing up your dominance by scolding her]\n\n"
                    ...
                    + "While it’s no deepthroat, the incredible softness of her near-flawless tit flesh rubbing against your [cocks] soon sends you over the edge. With a strong moan, you release her tail and move your hands to her breasts, taking control as you buck back and forth, " + (this.player.cumQ() <= 150 ? " finally releasing your load onto the fox-girl’s belly, smearing it with your scent" : ((this.player.cumQ() <= 350) ? "pumping out a thick load onto her stomach that spills down her sides and soaks the ground" : "shooting stream after stream of cum all across the fox-girl’s lower body, painting her with a thick and thorough layer of jism")) + ".\n\n");
            this.player.orgasm('Dick');
        }

        this.outputText("You stand back to take a look at your handiwork,[if(player.isCorruptEnough(50) || player.hasPerk(PerkLib.Sadist)) taking a twisted pride in what you’ve done, smirking in satisfaction.| feeling a little disgraceful about your cruel treatment of the poor girl as a nagging guilt descends in your gut.]\n\n"
            + (this.flags[kFLAGS.AIKO_BALL_RETURNED] == 0 ? "You decide that you should reward her obedience, pulling the ball from your pouch and smearing it in some of your leftover essences before [if(player.isCorruptEnough(50) ||player.hasPerk(PerkLib.Sadist))shoving it|placing it gently] in her mouth, then petting and praising her as if she was a dog. You spot a few of her tails wag limply in response to your degrading treatment, her spirit broken, at least for now." : "You pet and praise her as if she was a dog, and spot a few of her tails wag limply in response to your degrading treatment. Her spirit is broken, at least for now.")
    ...
    private aikoRapeGetLicked(): void {
        ...
        this.outputText("She glares up at you " + (this.player.hasCock() ? "[if (player.cocks.length > 1)between|around] your [cocks]" : "") + ", tonguing your [pussy] eagerly as her nine tails swish back and forth behind her. Regardless of any spite she may feel against you, her moans vibrate through your body, and it isn’t long before you can’t stand it any more. You bear down on her face with all your weight, drowning her in your slick girlcum, and she has no choice but to swallow it as quickly as she can. " + (this.player.hasCock() ? "At the same time, your [cocks] swell with pleasure from her continuous stroking, releasing jets of cum that spray onto the ground behind her." : "") + "\n\n"
    ...
    private aikoSexMissionary(): void {
        this.clearOutput();
        this.spriteSelect(SpriteDb.s_aiko);
        const x: number = rand(this.player.cocks.length);
        this.outputText((this.playerchoice ? "You tell Aiko that you want to do it the old-fashioned way today, slowly beginning to take off your [armor]." : "<i>“Today... we’re gonna do it old-school,”</i> Aiko says matter-of-factly, looking you over with a lusty smirk as she begins to remove your [armor].")
            ...
            + (this.flags[kFLAGS.AIKO_AFFECTION] >= 65 ? "You roll off her and collapse next to her on the ground, smiling and staring up into the leafy forest canopy. Turning toward you, Aiko leans in to plant a small kiss on your cheek and curls herself up to wrap against your side, resting her head on your shoulder. You cuddle with her like this for some time, but sadly, all good things must come to an end. With a somber tone, you tell her that you need to go back to check on your camp.\n\n" +
                ...
                (this.flags[kFLAGS.AIKO_AFFECTION] >= 50 ? "You push back from her, sighing with satisfaction, while Aiko continues to lay on her back, legs spread out lazily as she catches her breath.\n\n" +
    ...
    private aikoSexDoggy(): void {
        ...
        this.outputText((this.playerchoice ? "You tell Aiko that this time, you want to take her doggy style." : "Aiko looks you over for a bit, an uncharacteristic lust in her eyes as her gaze lingers over your groin for a moment. <i>“Mn... I’m in the mood for something a bit ‘wild’ today... if you know what I mean,”</i> she says, giving you a coy smirk.")
            ...
            + "The wonderful warmth of her insides wraps around you like a glove, and your hips fall forward involuntarily, " + (this.player.hasKnot(x) ? "driving the full length of your shaft up till the knot" : "driving every last inch of your shaft") + " inside in one stroke. Aiko’s mouth opens wide in a powerful moan, and she begins to rock her hips back and forth, slamming her bountiful bottom against your " + (this.player.hasKnot(x) ? "knot" : "hips") + " over and over again.\n\n"
            ...
            + (this.player.hasKnot(x) && this.player.hasKnot(y) ? "You slam home your knots into her pussy and tight ass, extracting a high pitched, excited screech from her throat at the overly stuffed sensation she is now being subjected to. Your knots swell and locks you thoroughly in place. " : (this.player.hasKnot(x) ? "You slam home your knot into her pussy and she moans at the now very stuffed feeling in her pussy as your knot swells and locks you in place. " : (this.player.hasKnot(y) ? "You slam home your knot into her tight ass and she screams at the now very stuffed feeling in her ass as your knot swells and locks you in place.\n\n" : "")))
            ...
            + (this.player.cumQ() > 350 ? "Her hole[if (" + (y + 1) + " != 0)s] gurgle[if (" + (y + 1) + " == 0)s] audibly as you pump your virile baby cream into [if (" + (y + 1) + " != 0)both] her fuckhole[if (" + (y + 1) + " != 0)s], feeling her abdomen start to swell up. She looks heavily pregnant by the time you finish dumping your load into her, letting her tongue loll out in pure bliss.\n\n" : "")
            + (this.flags[kFLAGS.AIKO_AFFECTION] >= 50 ? "<i>“Th-that… was great…”</i> she says, falling forward " + (this.player.hasKnot(x) || this.player.hasKnot(y) ? "and dragging you with her by your knotted cock before turning onto her side on the grass, both of you panting tiredly.\n\n" : "off your cock[if (" + (y + 1) + " != 0)s] to lay on her side on the grass. You follow soon after her, flopping down beside her with your arms splayed out tiredly, you both panting softly.\n\n") +
                (this.flags[kFLAGS.AIKO_AFFECTION] >= 65 ? (this.player.hasKnot(x) || this.player.hasKnot(y) ? "She wiggles her ass tighter into you, still linked tightly by your knot" : "She curls up against you") + ", as all [if (flags[kFLAGS.AIKO_BOSS_COMPLETE] >0)eight|seven] of her tails wrap around you like a large blanket. She then presses her " + (this.player.hasKnot(x) || this.player.hasKnot(y) ? "nose" : "head") + " into your shoulder sweetly, closing her eyes. The two of you cuddle together on the grass for the remainder of the hour, but you finally have to tell her you need to go back to camp. " + (this.player.hasKnot(x) || this.player.hasKnot(y) ? "You pull out of her, your knot finally having deflated and stand up. " : "") + "She waves goodbye, curling up underneath the tree and wrapping her tails around herself with a warm smile on her face.\n\n" :
                    (this.player.hasKnot(x) || this.player.hasKnot(y) ? "You pull out of her, your knot finally having deflated and stand up. " : "You sit up slowly,") + " and tell Aiko that you should be heading back to camp soon. She nods in understanding, and then sighs a bit, curling up under the tree and wrapping her tails around herself.\n\n") :
                (this.player.hasKnot(x) || this.player.hasKnot(y) ? "A few minutes later you pull out of her, your knot having finally deflated enough to allow it. " : "") + "She pulls away from you, dragging herself up into a seated position, and wraps her tails around herself, catching her breath.\n\n"
    ...
    private aikoSexBJ(): void {
        this.clearOutput();
        this.spriteSelect(SpriteDb.s_aiko);
        const x: number = rand(this.player.cocks.length);
        this.outputText((this.playerchoice ? "You tell Aiko you want her to pleasure you with her mouth this time, eagerly stripping down so you can get things started." : "Pulling you out of your [armor] and setting it aside, Aiko gives you a wry grin as she watches you with lust in her eyes. ")
        ...
        if (this.player.cockArea(x) < 40) {
            this.outputText("Her lips smack loudly as she plants kisses up and down your shaft, sending your senses into a frenzy as her hand works its way up and down your [cock " + (x + 1) + "], dragging her fingertips along the length. [if (player.balls > 0)Cupping your player.ballsDescriptLight(),] Aiko gently glides her tongue up the underside of your shaft one last time, opening her lips into a wide O and engulfing you in her warm, moist mouth.\n\n"
                ...
                + "Letting out an obscene moan, you lose yourself in pleasure as your [cock " + (x + 1) + "] shoots hot ropes of semen into Aiko’s mouth, coating her throat with your virile jizz. She moans, closing her eyes, and presses her lips to the base of your shaft, her throat contracting rhythmically as she endeavors to swallow your load. " + (this.player.cumQ() > 150 ? "She puts up a good effort, but ultimately fails as jets of cum spurt from the corners of her mouth" + (this.player.cumQ() > 350 ? ". As more and more of your semen cascades down her throat, her stomach begins to bulge a little, though far more of it gets on her than in her as it oozes messily down her chin" : "") : "")
                + (this.player.cocks.length > 1 ? ", the rest of your [cocks] spilling their spunk all over Aiko's chest and the ground below you" : "") + "[if (player.hasVagina()), and your vagina spasming, dripping your excretions down the sides of your thighs and onto the ground, forming a messy puddle of combined sexual fluids]"
            ...
            this.outputText("Her lips smack loudly as she plants kisses up and down your shaft, sending your senses into a frenzy as her hand works its way up and down your [cock " + (x + 1) + "], dragging her fingertips along the length. [if (player.balls > 0)Cupping your player.ballsDescriptLight(), ] Aiko gently glides her tongue up the underside of your shaft one last time, opening her lips into a wide O and wrapping them around the head."
                ...
                + "Letting out an obscene moan, you lose yourself in pleasure as your [cock " + (x + 1) + "] shoots out hot ropes of semen all over Aiko’s face and chest, twitching like mad. Stream after stream spills out, several orgasms’ worth of cum finally allowed to fly free, making a thick, warm mess all over her face and breasts. " + (this.player.cumQ() > 350 ? "Your orgasm seems to go on forever, and by the time it is all over, Aiko’s face is almost unrecognizable, hidden under a sticky mask of your cum." : "") + "\n\n"
                + (this.player.cocks.length > 1 ? "The rest of your [cocks] spilling their spunk all over Aiko's chest and the ground below you" + (this.player.hasVagina() ? ", and your" : "") : (this.player.hasVagina() ? "Your" : "")) + "[if (player.hasVagina()) vagina spasming, dripping your excretions down the sides of your thighs and onto the ground, forming a messy puddle of combined sexual fluids.\n\n]"
                + (this.player.cumQ() <= 350 ? " <i>“Mmmm...”</i> Aiko lets out a soft moan, swallowing the cum that landed in her mouth and beginning to clean herself off with the corner of her sleeve." : "<i>“Mn… You can stop now,”</i> Aiko teases, grinning girlishly as she sloughs off a layer of cum, giggling to herself. <i>“Damn, you made a mess,”</i>she adds, starting to clean herself off with the corner of her sleeve."));
        }
        ...
        if (this.flags[kFLAGS.AIKO_TIMES_MET] == 1)
            ...
            this.outputText("Once you have recovered from your satisfied stupor, you begin to gather your things, donning your [armor] again and bidding Aiko farewell as you make your way back to camp.");
        if (this.player.hasStatusEffect(StatusEffects.Spar)) {
            this.player.removeStatusEffect(StatusEffects.Spar);
            if (this.player.hasStatusEffect(StatusEffects.DomFight))
                this.player.removeStatusEffect(StatusEffects.DomFight);
    ...
    private aikoSexCunni(): void {
        this.clearOutput();
        this.spriteSelect(SpriteDb.s_aiko);
        this.outputText((this.playerchoice ? "You tell Aiko you want her to eat you out this time, eagerly stripping down so you can get things started." : "<i>“Pulling you out of your [armor] and setting it aside, Aiko gives you a wry grin as she watches you with lust in her eyes.")
            ...
            + (this.player.cocks.length > 0 ? "Your neglected [cocks] spilling [if (player.cocks.length==1)its|their] spunk all over Aiko's chest and the ground below you" : "")
            + "\n\nPushing herself up to her feet, Aiko, brushes herself off a bit, her hips swaying back and forth gently.\n\n"
            + "<i>“Ah, that was fun,”</i> she says, stretching out and letting out a cute sigh."
            + (this.flags[kFLAGS.AIKO_TIMES_MET] > 1 ? "You can only nod your agreement as you lay there in satisfied exhaustion for some time, until you finally push yourself up and begin to gather your [armor]. Bidding Aiko farewell, you head back to camp." : ""));
    ...
    private aikoSexTailjob(): void {
        this.clearOutput();
        this.spriteSelect(SpriteDb.s_aiko);
        const x: number = rand(this.player.cocks.length);
        this.outputText((this.playerchoice ? "You ask Aiko if she can use her tails this time as you seductively let your [armor] slide off your body, exposing to the skittish kitsune your [cock " + (x + 1) + "]. She instantly blushes and lowers her ears, aww.\n\n"
            ...
            + ((this.player.cocks.length > 1) ? "The " + ((this.player.cocks.length > 2) ? "rest" : "other") + " of your [cocks] are quickly enveloped and caressed by Aiko's tails, being subtly teased and rubbed by the feather-like silky fur of her tails" + (!this.player.hasVagina() ? ".\n\n" : ", and your") : (this.player.hasVagina() ? " Your" : ""))
            + (this.player.hasVagina() ? " [vagina] is tickled by the continuous motion of the tail caressing your [if (player.balls > 0)balls|cock]. Occasionally one of her tails push against your mound, the silky texture feeling quite exquisite against your folds.\n\n" : "\n\n")
    ...
    private submitToAiko(): void {
        this.clearOutput();
        this.spriteSelect(SpriteDb.s_aiko);
        this.outputText("You tell her that you’re just itching for a little " + (this.player.findPerk(PerkLib.Masochist) >= 0 ? "release" : "abuse") + ", and you’re not really looking to fight her for it.\n\n"
        ...
        switch (this.player.gender) {
            case 1: this.doNext(this.aikoFootjob);
                    break;
            case 2: this.doNext(this.aikoWhipping);
                    break;
            case 3: this.doNext((rand(2) == 0 ? this.aikoFootjob : this.aikoWhipping));
                    break;
            default: this.outputText("Wait a minute! How am I supposed to have sex with you when you don't have any of the tools needed to even have sex?! Fuck off!");
                     this.doNext(this.camp.returnToCampUseOneHour);
                     break;
        }
    }

    private aikoFootjob(): void {
        this.clearOutput();
        this.spriteSelect(SpriteDb.s_aiko);
        const x: number = rand(this.player.cocks.length);
        this.outputText("You fall back easily as Aiko shoves you to the ground, grinning smugly. She takes a twisted pleasure in slowly removing your armorName, punctuating her dominant position with an almost blasé approach to undressing you. When you are fully exposed to her “tender mercies,” she begins to drag her nails along your [breast], leaving behind a lingering sting that pushes a shamefully aroused whimper from your chest. Her tails snake out, twisting themselves around your arms and " + (!this.player.isNaga() ? "legs" : "naga tail") + " to hold you firmly in place. For good measure, another tail wraps firmly around your neck, not tight enough to restrict air flow, but seeming to serve as a warning if you were to resist.\n\n"
        ...
        if (rand(4) == 0) {
            this.outputText("<i>“Hehehe, I guess next time you won’t be so selfish then,”</i> she says, pressing her foot down on your groin. No! You’re so close! If she would just keep stroking you for a little longer, you would be able to cum!\n\n"
    ...
    private aikoWhipping(): void {
        this.clearOutput();
        this.spriteSelect(SpriteDb.s_aiko);
        this.outputText("You fall back easily as Aiko shoves you down, grinning smugly. She takes a twisted pleasure in slowly removing your armorName, punctuating her dominant position with an almost blasé approach to undressing you. When you are fully exposed to her “tender mercies,” she begins to drag her nails along your [breast], leaving behind a lingering sting that pushes a shamefully aroused whimper from your chest. Her tails snake out, twisting themselves around your arms and " + (!this.player.isNaga() ? "legs" : "naga tail") + " to hold you firmly in place. For good measure, another tail wraps firmly around your neck, not tight enough to restrict your air flow, but seeming to serve as a warning if you were to resist.\n\n"
            + "<i>“Comfortable? I hope so,”</i> she says, her voice dripping with a sadistic venom.\n\n"
            + "She raises one of her hands and suddenly claws you across the cheek, your face stinging from the sharp slap accompanying it. As fresh blood begins to seep from the wounds and drip down your chin, Aiko grabs you by the " + (this.player.hair.length !== 0 ? "hair" : "ear") + ", holding your head back so she can run her tongue across your face. Her lips are stained cherry red from your blood, a small droplet dribbling from the corner of her mouth when she pulls away, flashing an evil grin.\n\n"
        ...
        if (rand(4) == 0) {
            this.outputText("<i>“Good! Remember that feeling...”</i>\n\n"
            ...
            this.player.dynStats("lus", 50);
            this.player.dynStats("tou", 2, "sen", -12);
            this.player.changeFatigue(30);
        } else {
            this.outputText("<i>“Mmn, you’re lucky I’m feeling generous today my little masochist slut...”</i>\n\n"
            ...
            this.player.orgasm('Vagina');
            this.player.dynStats("tou", 2, "sen", -2);
            this.player.changeFatigue(30);
    ...
    /////////////////////////////////////////////////////////////////////
    ////////////////////// YAMATA INTERACTIONS////////////////////////////
    private yamataTalk(): void {
        this.clearOutput();
        this.spriteSelect(SpriteDb.s_aiko);
        if (this.flags[kFLAGS.AIKO_CORRUPTION] < 50 && this.flags[kFLAGS.AIKO_CORRUPTION_ACTIVE] <= 0) {
            this.outputText("You ask Aiko if there’s anything else she can tell you about her half-sister.\n\n"
    ...
    private yamataPrepare(): void {
        this.clearOutput();
        this.spriteSelect(SpriteDb.s_aiko);
        this.outputText("You tell Aiko that you still need time to prepare, her half-sister is quite the adversary from how she described Yamata. You return to camp to continue preparations for the huge battle to come.");

        this.doNext(this.camp.returnToCampUseOneHour);
    }

    private yamataStart(): void {
        this.clearOutput();
        this.spriteSelect(SpriteDb.s_aiko);
        this.outputText("You tell Aiko you’re ready to go, asking her to lead you to the village now. She seems to perk up when she hears this, and nods.\n\n");
        if (this.flags[kFLAGS.AIKO_CORRUPTION] < 50 && this.flags[kFLAGS.AIKO_CORRUPTION_ACTIVE] <= 0) {
            this.outputText("<i>“Okay, follow me,”</i> she says, taking your hand gently and pulling you along. She takes you from the clearing, leading you down a barely-visible dirt path. After a short walk, you spot a large wooden arch of strange design, seemingly erected in the middle of nowhere. It obviously has been maintained by someone, as its bright red paint shows little signs of fading and discoloration, but looking around, you can’t see any other signs of a civilization.\n\n"
            ...
            this.outputText("<i>“Took you long enough to make up your mind,”</i> she says, taking your hand and pulling you along. She takes you from the clearing, leading you down a barely-visible dirt path. After a short walk, you spot a large wooden arch of strange design, seemingly erected in the middle of nowhere. It obviously has been maintained by someone, as its bright red paint shows little signs of fading and discoloration, but looking around, you can’t see any other signs of a civilization.\n\n"
    ...
    private yamataIntro(): void {
        ...
        this.outputText("The streets of the Kitsune village are deserted, and an eerie silence has settled over the town. Each house you pass is just as dark and empty as the last, and there’s not a single sign of any of the townsfolk. Just as you begin to think you might have been too late, you hear someone cry out in pain!\n\n"
            ...
            + "<i>“" + (this.player.catScore() > 0 ? "Well, well... look who the cat dragged in!" : "Well, well... look who we have here...") + "”</i>\n\n"
            ...
            + "<i>“Looking for this?”</i> she says, grinning condescendingly as she twirls Aiko’s ball on her fingertip. <i>“I see you’ve brought a friend, dear sister. I hope you don’t think " + this.player.mf("he", "she") + "’s going to save you!”</i> Grinning, she shoves her insensate victim aside with a foot, and begins walking toward you and Aiko. <i>“Of course, I can always use more toys for my pleasure... Mn... Yes, I think you’ll be the perfect addition to my playhouse... Champion.”</i>\n\n"
            ...
            + "<i>“[name]! " + (this.flags[kFLAGS.AIKO_CORRUPTION] < 50 || this.flags[kFLAGS.AIKO_CORRUPTION_ACTIVE] == 0 ? "I... I believe in you!" : "Kick that cocky bitch’s ass!") + "”</i>");
        this.startCombat(new Yamata());
    }

    public yamataWins(): void {
        this.clearOutput();
        this.spriteSelect(SpriteDb.s_yamata);
        this.outputText("You grit your teeth in pain as you fall onto your knees. As you fall, you can hear Yamata snickering at you. There's nothing you can do; she's simply too strong.\n\n"
        ...
        if (kGAMECLASS.marbleScene.marbleFollower()) this.outputText("\n\n\"<i>NOOOOO! Sweetie!</i>\" Marble's cries carry from your camp.");
        this.getGame().inCombat = false;
        this.getGame().gameOver();
    }

    public yamataLoses(): void {
        this.clearOutput();
        this.outputText("You relax your shoulders as you see Yamata falling onto her all fours, desperately gasping with her evil eyes opened.\n\n"
            + "<i>“IMPOSSIBLE! You have such strength, " + (this.player.kitsuneScore() == 0 ? "for a lowly mortal!" : "even if you are weaker than I am!") + "”</i>\n\n"
            ...
            + (this.player.findPerk(PerkLib.Masochist) < 0 ? (this.flags[kFLAGS.YAMATA_MASOCHIST] >= 100 ? "You realize that from all the torture Yamata has put you through, you have come to enjoy pain quite a lot. <b>You have become a masochist!</b>\n\n" : "All that pain Yamata put you through almost drove you to masochism, but you defeated her before she could manage to drive you to such depraved thoughts!") : "")
            + "You look at the defeated demon, with her eyes she's pleading mercy. You suppose you can let Yamata go, let Aiko finish her sister" + (this.player.cor > 60 ? ", end her yourself or fuck her before deciding what to do." : "or end her yourself."));

        if (this.player.findPerk(PerkLib.Masochist) < 0 && this.flags[kFLAGS.YAMATA_MASOCHIST] >= 100)
            this.player.createPerk(PerkLib.Masochist, 0, 0, 0, 0);

        this.menu();
        this.addButton(0, "Let her go", this.releaseYamata).hint("Let Yamata go, trusting her to not hurt anyone else.");
        this.addButton(1, "Aiko finish", this.aikoKillSister).hint("It should be up to Aiko what to do with Yamata.");
        this.addButton(2, "Finish her", this.killYamata).hint("Yamata has caused enough pain and misery, end her now!");
        if (this.player.cor > 60) {
            this.addButton(3, "Fuck her", this.fuckYamata).hint("You can take some pleasure first before deciding on such tough matters, that fight got you really worked up!");
        }
    }

    private fuckYamata(): void {
        this.clearOutput();
        this.spriteSelect(SpriteDb.s_yamata);
        this.outputText("PLACEHOLDER TO FUCK YAMATA\n\n");
    ...
    private releaseYamata(): void {
        this.clearOutput();
        this.spriteSelect(SpriteDb.s_yamata);
        this.outputText("<i>“What!? Are you letting her leave!?”</i> Turning her head towards you, Aiko wides her eyes, she really can't believe what you are saying.\n\n"
        ...
        this.flags[kFLAGS.AIKO_AFFECTION] -= 20;
        this.player.dynStats("cor", 10);
        this.flags[kFLAGS.AIKO_BOSS_COMPLETE] = 2;
        this.doNext(this.yamataAftermath);
    }
    private aikoKillSister(): void {
        this.clearOutput();
        this.spriteSelect(SpriteDb.s_yamata);
        this.outputText("You look at Aiko from the corner of your eye and then you nod. She narrows her eyes and coldly releases the arrow. Yamata looks helpless as the projectile travels towards her face and widens her eyes as it pierces her throat. In agony, a look of pure hatred bursts on her face, directed at you and Aiko both as she gasps for air, drowning in her own blood. Finally Yamata collapses to the floor, as Aiko takes a deep breath of relief and closes her eyes.\n\n"
        ...
        this.flags[kFLAGS.AIKO_BOSS_COMPLETE] = 1;
        this.doNext(this.yamataAftermath);
    }
    private killYamata(): void {
        this.clearOutput();
        this.spriteSelect(SpriteDb.s_yamata);
        switch (this.player.weapon.verb) {
            case "shot": this.outputText("You narrow your eyes and raise your [weapon] toward the defeated corrupted kitsune, Aiko and Yamata both widening their eyes in surprise. You then coldly release a shot directly into Yamata's chest, piercing her heart");
                         break;
            case "slash":
            case "stab": this.outputText("You narrow your eyes and dash toward the defeated corrupted kitsune, Aiko and Yamata both widening their eyes in surprise. You then coldly stab the defeated opponent right to the heart");
                         break;
            case "smash": this.outputText("crush the defeated opponent's ribcage");
                          break;
            default: if (this.player.weapon.name.indexOf("staff") >= 0) {
                this.outputText("You raise your [weapon], letting out an immense blast of magic, engulfing Yamata and searing her flesh and crushing her windpipe. Aiko yelps in surprise and gives you a shocked look");
            }
            else
                this.outputText("You narrow your eyes and raise your [weapon] toward the defeated corrupted kitsune, Aiko Yamata both widening their eyes in surprise. You then coldly grab her by chin and the back of her head before twisting sharply and snapping her neck");
                     break;
        }
        this.outputText(". Yamata coughs blood and murmurs a final insult before collapsing to the floor, dead. You then sigh in relief and lower your [weapon], turning to Aiko.\n\n"
    ...
    private yamataAftermath(): void {
        this.clearOutput();
        this.spriteSelect(SpriteDb.s_aiko);
        this.outputText("Before you can go to free the kitsune townsfolk, your attention is drawn by the nefarious blade Yamata was using. “Muramasa”, you recall her naming it. You approach it cautiously, inspecting the blade closely: it's a black and sharp edged katana with deep purplish black swirls moving throughout the blade, it emits a very fierce corrupted energy, making you feel dirty by simply being near it. You think that this weapon may be too corrupted even for Rathazul to cleanse.\n\n"
            + "Aiko arrives and narrows her eyes, peering at the sword. <i>“This is a very powerful weapon... if ever it falls into demonic hands again, it will be disasterous... " + (this.flags[kFLAGS.AIKO_BOSS_COMPLETE] == 1 ? "Perhaps I can purify it with my magic" : "I can use it to locate Yamata, I think I'll be able to use it against her when I'm be ready") + ".”</i> You stand still and watch Aiko grab the hilt of the katana and pull it out of the ground. You are tempted to warn her of its demonic influence, though she probably already knows more about the blade than you do, and can sense the taint regardless. She clearly understands the dangers of her actions.\n\n"
