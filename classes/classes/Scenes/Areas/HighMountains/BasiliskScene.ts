 ...
 */
	...
	{
		public  BasiliskScene(pregnancyProgression:PregnancyProgression, output:GuiOutput)
		{
		...
		public  defeatBasilisk(): void {
			...
			if (player.lust >= 33) {
				...
				if (player.cockThatFits(monster.analCapacity()) >= 0) {
					addButton(1, "Ass (Gentle)", defeatBasiliskAndAnal, false, undefined, undefined, "Fuck the basilisk's ass gently!");
					if (player.isCorruptEnough(66) || player.findPerk(PerkLib.Sadist) >= 0)
						addButton(2, "Ass (Cruel)", defeatBasiliskAndAnal, true, undefined, undefined, "Dominate and fuck the basilisk in the ass! He wouldn't like it though.");
				}
			}
		...
		private  defeatBasiliskAndAnal(corrupt: boolean): void {
			spriteSelect(SpriteDb.s_basilisk);
			clearOutput();
			outputText(images.showImage("monster-basilisk"));
		var  x: number = player.cockThatFits(monster.analCapacity());
			...
			if (corrupt) {
				...
				//<(13 inches or less:) 
				if (player.cocks[x].cockLength <= 13) outputText("gripping his hips and pushing every inch of yourself in and reaching a glorious, skin tingling peak.");
				//(More than 13 inches:)
