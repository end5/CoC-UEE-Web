	...
	export class Minerva extends Monster
	{
		...
		var  damage: number = int(str + game.highMountains.minervaScene.minervaSparIntensity() * 1.85 + 85);
...
//Flying kick:
		...
		var  damage: number = int(str + weaponAttack + (game.highMountains.minervaScene.minervaSparIntensity() * 1.5) + 100);
			damage = player.reduceDamage(damage);
			spe -= 70;
			if (player.getEvasionRoll()) {
...
//Tail-whip
		...
		{
		var  damage: number = int(str + 35 + game.highMountains.minervaScene.minervaSparIntensity());
			damage = player.reduceDamage(damage);
...
//Halberd stab:
		...
		var  damage: number = int(str + weaponAttack);
			damage = player.reduceDamage(damage);
...
//Halberd CHOP:
		...
		var  damage: number = int(str + 40 + (weaponAttack * 2));
			damage = player.reduceDamage(damage);
...
//White Fire
		...
		var  damage: number = int(10 + (inte / 3 + rand(inte / 2)) * ((game.highMountains.minervaScene.minervaSparIntensity() / 30) + 1) * 1.5);
			player.takeDamage(damage, true);
			combatRoundOver();
		}
