...
import { LizanRogue } from "./LizanRogue";
...
export class LizanRogueScene extends BaseContent {

    public lizanIntro(): void {
        this.clearOutput();
        this.credits.modContent = true;
        this.credits.authorText = "Foxxling";
        this.outputText(this.images.showImage("monster-lizanrogue"));
        let lizan: LizanRogue = new LizanRogue();
        if (!this.player.isPureEnough(30)) {
            this.outputText("As you walk through the bog you happen upon a lone lizan male covered in " + this.monster.skin.tone + " scales. Upon hearing your approach he jumps back, your sudden appearance must have surprised him. After an initial moment of surprise the lizan tentatively sniffs the air between the two of you. Without warning he jumps back, flashing his claws and releasing a loud hiss. Apparently he smells something he doesn't like. You ready for an attack but he just sits there, making noise and puffing himself up. For all the ferocity he displays he doesn't attack and you realize he's waiting for you to back down. What will you do?");
            this.menu();
            this.addButton(0, "Fight", this.fightLizan, false, undefined, undefined, "Fight the lizan!");
            this.addButton(1, "Leave", this.leaveLizan, false);
        }
        else {
            this.outputText("As you walk through the bog you happen upon a lone lizan male covered in " + this.monster.skin.tone + " scales. Upon hearing your approach he jumps back, your sudden appearance must have surprised him. After an initial moment of surprise the lizan tentatively sniffs the air between the two of you. ");
            if (this.flags[kFLAGS.MET_LIZAN_FIRST_TIME] <= 0) {
                if (this.player.tallness > 64) this.outputText("He is shorter than you and wearing nothing but a loincloth with a pack belted onto it.");
                else if (this.player.tallness >= 60) this.outputText("He is about the same height as you and wearing nothing but a loincloth with a pack belted onto it.");
                else this.outputText("He is taller than you and wearing nothing but a loincloth with a pack belted onto it.");
                this.outputText("\n\nThis shows off his toned but slim chest, arms and legs covered in " + this.monster.skin.tone + " scales. ");
                this.flags[kFLAGS.MET_LIZAN_FIRST_TIME] = 1;
            }
            this.outputText("After a moment of silent consideration he speaks. \n\n\"<i>Hey there,</i>\" he says, his lizard lips pulling into a big grin, \"<i>... and who might you be?</i>\" You answer with the truth, your name. ");
            ...
            this.menu();
            this.addButton(0, "Spar", this.fightLizan, true, undefined, undefined, "Do a spar session with the lizan!");
            if (this.player.lust >= 33) {
                this.addButton(1, "Sex", this.haveSexWithLizan).hint("Have some fun with the oh-so-sexy lizan.");
            } else {
                this.addDisabledButton(1, "Sex", "You are not aroused enough.");
            }
            this.addButton(2, "Decline", this.leaveLizan, true, undefined, undefined, "Excuse yourself and leave.");
        }
        this.unlockCodexEntry("Lizan", kFLAGS.CODEX_ENTRY_LIZANS);
    }

    public fightLizan(pure: boolean = false): void {
        this.clearOutput();
        this.credits.modContent = true;
        this.credits.authorText = "Foxxling";
        this.startCombat(new LizanRogue());
        if (!pure) this.outputText("You ready your [weapon], prepared to let the little punk know just who it is he is dealing with.");
        else {
            this.outputText("You tell the male lizan you're game as you ready your [weapon]. His grin spreads into the full toothy smile of a rival as he flashes his claws at you, a taunting gesture. You are now sparring with a lizan male.");
            this.monster.createStatusEffect(StatusEffects.Spar, 0, 0, 0, 0);
        }
        this.doNext(this.playerMenu);
    }

    public leaveLizan(pure: boolean = false): void {
        this.clearOutput();
        this.credits.modContent = true;
        this.credits.authorText = "Foxxling";
        if (!pure) {
            this.outputText("You roll your eyes at the overt display. You raise your hands in a gesture of harmlessness and turn around to head home. When you look back over your shoulder the lizan is gone. Such a creature doesn't interest you right now anyway.");
            this.doNext(this.camp.returnToCampUseOneHour);
        }
        else {
            this.outputText("You tell the lizan you aren't up for any games. He shrugs, \"<i>Maybe next time then,</i>\" he says as he rummages around in his pack. When he finds what he's looking for he takes it out and tosses it towards you. You catch the small vial and read that it is labeled Reptilium. When you look up the scaly guy is nowhere to be seen. You head back to camp, smiling at the weird but friendly encounter.");
            this.inventory.takeItem(this.consumables.REPTLUM, this.camp.returnToCampUseOneHour);
        }
    }

    public winAgainstLizan(): void {
        this.clearOutput();
        this.credits.modContent = true;
        this.credits.authorText = "Foxxling";
        ...
        this.outputText("The lizan raises his hands in defeat and you celebrate with a victory dance. ");
        this.menu();
        if (this.player.isCorruptEnough(30)) {
            this.addDisabledButton(0, "Use Dick", "This scene requires you to have cock and sufficient arousal.");
            this.addDisabledButton(1, "Use Pussy", "This scene requires you to have vagina and sufficient arousal.");
            if (this.player.lust >= 33 && this.player.gender > 0) {
                this.outputText("You wonder what you should do to the lizan.");
                if (this.player.hasCock()) this.addButton(0, "Use Dick", this.rapeLizanInTheAss).hint("Anally penetrate him with your [cocks].");
                if (this.player.hasVagina()) this.addButton(1, "Use Pussy", this.rapeLizanWithPussy).hint("Get on top of the lizan and stuff his cock into your [pussy].");
                this.addButton(14, "Leave", this.combat.cleanupAfterCombat);
                return;
            }
            else {
                this.outputText("You rummage through his bag for something valuable.");
                this.combat.cleanupAfterCombat();
                return;
            }
        }
            ...
            if (this.player.lust >= 33) {
                this.outputText("<b>You could have sex with him if you like to.</b> ");
                this.addButton(0, "Sex", this.haveSexWithLizan).hint("Have some fun with the oh-so-sexy lizan.");
                this.addButton(14, "Leave", this.leaveLizanAfterCombat);
                return;
            }
            else this.leaveLizanAfterCombat();
        }
    }

    public leaveLizanAfterCombat(): void {
        this.outputText("After chatting for a bit the two of you part ways, having gotten a good workout in the process. ");
        this.combat.cleanupAfterCombat();
    }

    public loseToLizan(): void {
        this.clearOutput();
        if (!this.player.isPureEnough(30)) {
            this.outputText("Unable to continue fighting, you collapse from " + (this.player.lust >= this.player.maxLust() ? "overwhelming desires" : "your injuries") + ". The lizan steps over to you, intent on punishing you. ");
            this.combat.cleanupAfterCombat();
        }
        else {
            this.outputText("You are forced to raise your hand in defeat and the lizan celebrates with a few acrobatic flips and plenty of bragging. After a quick friendly chat the two of you part ways, having gotten a good workout in the process. ");
            this.dynStats("str", 1, "tou", 1);
            this.combat.cleanupAfterCombat();
        }
    }

    public haveSexWithLizan(continuation: boolean = false): void {
        if (!continuation) {
            this.clearOutput();
            this.outputText("You tell the male lizan you're game as you " + this.player.clothedOrNaked("begin to pull off your [armor]", "show your naked form") + ". His bulge becomes more pronounced as your body comes into view. He quite obviously likes what he sees. Which one of your body parts do you plan to use on the horny male lizan?");
        }
        this.credits.modContent = true;
        this.credits.authorText = "Foxxling";
        this.menu();
        if (this.player.hasCock()) this.addButton(0, "Use Dick", this.consensualButtfuck).hint("Anally penetrate him with your penis.");
        else this.addDisabledButton(0, "Use Dick", "This scene requires you to have cock.");
        if (this.player.hasVagina()) this.addButton(1, "Use Vagina", this.consensualGetFucked).hint("Have him take you vaginally.");
        else this.addDisabledButton(1, "Use Vagina", "This scene requires you to have vagina.");
        this.addButton(2, "Use Ass", this.consensualGetButtFucked).hint("Have him take you anally.");
        if (this.player.hasVagina()) this.addButton(3, "Dbl.Penetration", this.consensualDoublePenetration).hint("Have him stuff both your holes with his dual cocks.", "Double Penetration");
        else this.addDisabledButton(3, "Dbl.Penetration", "This scene requires you to have vagina.", "Double Penetration");
        if (this.flags[kFLAGS.WATERSPORTS_ENABLED] > 0 && this.flags[kFLAGS.LIZAN_ROGUE_SEX_COUNTER] >= 3 && !continuation) this.addButton(4, "Watersports", this.consensualWatersports).hint("Participate into urine activity with him. \n\nNOTE: Contains watersports!");
    }

    public consensualButtfuck(): void {
        this.clearOutput();
        this.credits.modContent = true;
        this.credits.authorText = "Foxxling";
        this.outputText(this.images.showImage("lizanrogue-smex"));
        this.outputText("The lizan watches you with obvious interest as he asks \"<i>How do you like to do this?</i>\"");
        this.outputText("\n\nYou tell him you want his ass, making sure to assert your dominance. He doesn't seem to mind as he turns away and peels off his loincloth, exposing his tight little toned ass with a lift of his glistening " + this.monster.skin.tone + " tail. After he has discarded the meager clothing he turns back around to face you, displaying dual bumpy purple lizan dicks, neither of which are small.");
        ...
        if (this.player.cocks.length == 1) { // One cock
            ...
            if (this.player.balls > 0) this.outputText("You groan as you feel his spit drip down your [balls] and [legs].");
            if (this.player.isBiped()) this.outputText("\n\nPrompting you to spread your [legs] and lean back, letting the lizan know how much you're enjoying this odd tongue job.");
        ...
        this.outputText("\n\nAs you marvel at the lizan's oral talents he gets on his back and wraps his arms around his well defined calf muscles. You feel like the luckiest guy in the world as you watch the flexible lizan practically fold himself in half. The lean globes of his toned " + this.monster.skin.tone + " ass points up at you, a natural part forming to make his naked hole visible. The way his hole winks at you causes [eachCock] to twitch in anticipation.");
        ...
        if (this.player.smallestCockLength() < 7) { // actually, it's not the smallest cock used here, but first...
        ...
        else if (this.player.smallestCockLength() < 12) {
        ...
        this.outputText("\n\nYour thrusts are an echoing smack against his " + this.monster.skin.tone + " ass as you begin fucking him with the entire length of your [cock]. Soon, your abuse of his prostate becomes too much. His rectal passage clenches tightly as his twin shafts burst, spraying dual torrents of pale cream across everywhere as you fuck him through his orgasm. His flexing tail hole and the sight of his exploding lizan dicks push you over the edge.");
        if (this.player.cumQ() < 350) this.outputText("Your [face] twists around a strained groan and you spray your own impressive load deep into the lizan's bowels.");
        else if (this.player.cumQ() < 1000) this.outputText("Your [face] twists around a deep growl as you pour your load deep into the lizan's bowels. As you fuck your seed deeper into his clenching bowls you can feel some of it gushing out and dripping down your [legs].");
        else if (this.player.cumQ() < 2500) this.outputText("Your [face] twists into a chilling howl as you explode deep within the lizan's bowels. As you fuck your seed deep into his bowels his stomach becomes a little less defined, a testament to how much seed you've poured into him. You can even feel it dripping down your [legs].");
        else this.outputText("Your [face] twists into a titanic roar as [eachCock] explodes with enough force to blast out of the lizan's rectal embrace. Torrents of creamy white fluid blast from [eachCock]. Soon the lizan's upturned ass and the base of his tail is covered but you're still going. By the time you're finished the lizan is grinning from ear to ear amidst a small lake of cum.");
        this.outputText("\n\nSpent and breathing hard you collapse. For a second you just lie there [fullChest] against his abs and his legs wrapped around your [hips] while lying in the hot glorious mess of his lizan spunk. Both of you are panting, both of you have been satisfied. Then you push yourself off of him and go to collect your [armor]. He takes it upon himself to clean your [fullChest] with his tongue and as soon as he is done, " + this.player.clothedOrNaked("you pull on your [armor]", "you recollect whatever you had") + ". Before you leave, you reward him with a deep kiss, snaking your [tongue] into his mouth before heading back to camp.");
        this.flags[kFLAGS.LIZAN_ROGUE_SEX_COUNTER]++;
        this.player.orgasm('Dick');
        this.doNext(this.camp.returnToCampUseOneHour);
        if (this.getGame().inCombat) this.combat.cleanupAfterCombat();
    }

    public consensualGetFucked(): void {
        this.clearOutput();
        this.credits.modContent = true;
        this.credits.authorText = "Foxxling";
        this.outputText(this.images.showImage("lizanrogue-smex"));
        this.outputText("The lizan watches you with unabashed interest and asks \"<i>How would you like to do this?</i>\" you tell him you are going to ride him, which causes a huge grin to spread across his face. He confidently peels off his loincloth, exposing two bumpy purple erect lizan dicks not lacking in length or girth. He is a stunning visage with his " + this.monster.skin.tone + " scales, powerful tail and dual cocks all on display for you, his audience of one.");
        ...
        if (this.player.averageVaginalLooseness() <= 1) { // Virgin or tight
            ...
            this.player.cuntChange(24, true, true, true);
        }
        else if (this.player.averageVaginalLooseness() <= 3) { // Loose or stretched
            ...
            this.player.cuntChange(48, true, true, true);
        }
        if (this.player.hasCock()) { // Herm check!
        ...
        if (this.player.averageVaginalLooseness() <= 3) { // Not gaping
            ...
            if (this.player.tail.type > 0) this.outputText("His other lizan cock explodes as well, painting your raised [tail] and [ass] with a generous coating of funky lizan spunk.");
            else this.outputText("His other lizan dick explodes as well, painting your [ass] and lower back with a generous coating of lizan baby batter.");
        }
        else { // Gaping
            this.outputText("\n\nWith a sigh you begin to slide the bumpy purple lizan dick up into your [vagina]. A dick this size would be a hot fuck by itself but the bumpy purple surface is making the experience delightfully erotic. His girthy dick his hot and glorious, you feel as though you could just slide down its bumpy purple surface all day.");
            this.outputText("\n\nYou move slowly, looking down into his yellow eyes as he bites his lip. His other lizan dick slides up and down the crevice of your [ass], not an attempt to enter but a result of the two being so close together. After a few minutes your slow ride becomes unbearable to the lizan. His hands take you by the [hips]. With a groan he begins to pull while thrusting upward into you. You release a hoarse groan as the sensations caused by his dick intensifies. Soon you are trembling and from the way he's fucking you it appears the lizan is enjoying every second of your ecstasy.");
        ...
        if (this.player.hasCock()) {
            ...
            this.outputText("You begin to cum so hard your body trembles. Your [vagina] tingles and your entire being feels as if you're falling through an ocean of pure ecstasy. Your body quakes as your climax rages on, filling your entire being with so much pleasure you can't take it. Just before you feel as if you can't take it anymore the sensation begins to recede, leaving your body achingly sensitive in its wake.");
        }
        this.outputText("\n\nFor a second you lie there [fullChest] against his " + this.monster.skin.tone + " abs. Around the two of you lies the hot glorious mess of your combined sexual fluids. Both of you are panting, both of you are satisfied. Then you push yourself off of him and collect your [armor]. He takes it upon himself to clean your [fullChest] and [vagina] with his long tongue and as soon as he is done, " + this.player.clothedOrNaked("you pull on your [armor]", "you recollect whatever you had") + ". Before you leave you reward him with a deep kiss, snaking your [tongue] into his mouth before heading back to camp.");
        this.player.knockUp(PregnancyStore.PREGNANCY_OVIELIXIR_EGGS, PregnancyStore.INCUBATION_OVIELIXIR_EGGS);
        this.player.createStatusEffect(StatusEffects.Eggs, rand(6), 0, rand(3) + 5, 0);
        this.flags[kFLAGS.LIZAN_ROGUE_SEX_COUNTER]++;
    ...
    public consensualGetButtFucked(): void {
        this.clearOutput();
        this.credits.modContent = true;
        this.credits.authorText = "Foxxling";
        this.outputText("The lizan watches you with obvious interest and asks \"<i>How do you want to do this?</i>\" You tell him you want him to take your back door, which causes a surprised grin to stretch across his face. He confidently peels off his loincloth, exposing two bumpy purple lizan dicks not lacking in length or girth.");
        if (this.player.isBiped()) {
            this.outputText("\n\nTurning away from the lizan you peel off your [armor]. You bend at the waist " + (this.player.tail.type > 0 ? "and lift your tail" : "") + " to show the lizan your [ass]. He wolf whistles, obviously quite pleased. He takes the liberty of pushing you on to your hands and knees. While looking down at the grass you feel his cool scaly hands sliding up your [ass], getting a good long feel before his tongue is pushed into your [asshole].");
        ...
        else if (this.player.looseness(false) < 4) {
        ...
        this.outputText("\n\nYour orgasm seems to spur the lizan on. His thrusts become irregular before he tightens his grip on your waist. He shoves in to the hilt and, yelling like a madman, explodes. You feel his cum surging into your [asshole] as a hot warmth swells " + (this.player.hasVagina() ? "and dribbles out of your [vagina] and " : "") + "down your [legs] in a sticky stream of lizan dick juice.");
    ...
    public consensualDoublePenetration(): void {
        this.clearOutput();
        this.credits.modContent = true;
        this.credits.authorText = "Foxxling";
        this.outputText("The lizan watches you with obvious interest and asks \"<i>How do you want to do this?</i>\" you tell him you're going to ride him until the wheels come off, which causes a surprised grin to stretch across his face. He confidently peels off his loin cloth, exposing two bumpy purple lizan dicks not lacking in length or girth.");
        ...
        this.outputText(this.images.showImage("lizanrogue-win-female"));
        if (this.player.looseness(false) < 2) {
        ...
        if (this.player.hasCock()) {
            ...
            if (this.player.averageVaginalWetness() >= 4) this.outputText("\n\nYour [cock] and [pussy] explode in dual torrents of creamy white ejaculants, both erupting like volcanoes as the lizan continues to work you over.");
            else this.outputText("Your [cock] erupts as your [pussy] floods around the lizan's still thrusting cock.");
            if (this.player.cumQ() >= 1000) this.outputText("The lizan squeezes your still spurting [cock] as beneath the two of you your pearly white dick snot forms a large puddle.");
        }
        else {
    ...
    public consensualWatersports(): void { // Yay watersports!
        this.clearOutput();
        this.credits.modContent = true;
        this.credits.authorText = "Foxxling";
        this.outputText(this.images.showImage("lizanrogue-smex"));
        this.outputText("A kinky idea crosses your mind. You tell him that you'd like to take things a bit further. \"<i>I get it. Some urine activity,</i>\" he says. " + this.player.clothedOrNaked("You slowly disrobe and toss your [armor] into a neat pile on one of the nearby trees.", "You don't waste time showing your naked form to the lizan.") + " The lizan slowly strips his loincloth off, exposing his wonderful dual reptilian cocks.");
        this.outputText("\n\n\"<i>You can go first,</i>\" he says. You thank him for letting you go first. ");
        if (this.player.hasVagina() && !this.player.hasCock()) {
            this.outputText("You squat over the lizan and release your pressure, a stream of warm urine flows from your vagina");
        }
        else if (this.player.hasCock()) {
            if (this.player.cocks.length == 1) this.outputText("You stand in front of the lizan and aim your [cock] at him and release your pressure, a stream of warm urine flows from your penis");
            else this.outputText("You stand in front of the lizan and aim your [cock] at him and release your pressure, streams of warm urine flow from your multiple cocks");
        }
        else { // Just because you're a genderless doesn't mean you can't pee.
            this.outputText("You squat over the lizan and release your pressure, a stream of warm urine flows from your groin despite your genderlessness");
        }
        this.outputText(" and onto his belly. ");
        this.awardAchievement("Urine Trouble", kACHIEVEMENTS.GENERAL_URINE_TROUBLE, true, false);
        ...
        this.menu();
        this.addButton(0, "Continue", this.haveSexWithLizan, true, undefined, undefined, "Continue having sex.");
        this.addButton(1, "Done", this.finishWatersports);

    }

    private finishWatersports(): void {
        this.outputText(" You cuddle with the lizan for a moment before you have to part ways" + this.player.clothedOrNaked(" and redress yourself in your [armor]", "") + ".");
        this.outputText("\n\n\"<i>I hope you come back later,</i>\" the lizan says with a smile. You give him a kiss before you return to your camp.");
        this.doNext(this.camp.returnToCampUseOneHour);
        if (this.getGame().inCombat) this.combat.cleanupAfterCombat();
    }

    public rapeLizanInTheAss(): void {
        this.clearOutput();
        this.credits.modContent = true;
        this.credits.authorText = "Foxxling";
        this.outputText(this.images.showImage("lizanrogue-smex"));
        if (this.monster.lust >= this.monster.maxLust()) {
            this.outputText("The lizan does little to resist. He just continues his intense masturbation as you push him over to make his rectal passage more accessible. The horny guy even lifts his tail, eagerly giving you access to his back door. From the look of things he is neither a virgin nor a frequent taker. ");
            ...
            else if (this.player.smallestCockLength() < 11) {
    ...
    public rapeLizanWithPussy(): void {
        this.clearOutput();
        this.credits.modContent = true;
        this.credits.authorText = "Foxxling";
        this.outputText(this.images.showImage("lizanrogue-smex"));
        this.outputText("With a grin you slip off your [armor], the lizan doesn’t fight it in the least. In fact as you shove him onto his back he breathes a sigh of relief. Despite your having beaten him he is obviously quite attracted to you. He groans when you grab his twin peckers and inspect them. Each irregularly patterned bump is mirrored on its counterpart, like two sexy phallic twins. They grow hard in your hand. With a grin you look up into his face and watch his sexual attraction war with his lofty morals.");
        ...
        if (this.player.looseness(true) < 2) {
        ...
        else if (this.player.looseness(true) < 4) {
