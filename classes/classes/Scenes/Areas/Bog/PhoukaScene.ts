...
export class PhoukaScene extends BaseContent implements TimeAwareInterface, VaginalPregnancy {
    ...
    public timeChange(): boolean {
        if (this.player.statusEffectv1(StatusEffects.PhoukaWhiskeyAffect) > 0) {
            this.player.addStatusValue(StatusEffects.PhoukaWhiskeyAffect, 1, -1); // Count down hours until player is not drunk
            if (this.player.statusEffectv1(StatusEffects.PhoukaWhiskeyAffect) <= 0) {
    ...
    protected phoukaFaerieFire(): void { // In old Irish legends the Phouka are responsible for Will-o’-the-wisps. In the deep woods it’s Kitsune, but in the bog it’s a Phouka
        this.clearOutput();
        this.outputText(this.images.showImage("phouka-encounter"));
    ...
    protected phoukaStuck(): void {
        ...
        this.outputText(this.images.showImage("monster-phouka"));
        ...
        else this.addDisabledButton(0, "Offer Drink", "You have no whiskey to offer.");
    ...
    protected phoukaStuckOfferWhiskey(): void {
        this.clearOutput();
        this.outputText(this.images.showImage("item-whiskey"));
    ...
    protected phoukaStartFightSilenced(): void {
        this.startCombat(new Phouka(this.phoukaName()));
        this.player.createStatusEffect(StatusEffects.WebSilence, 0, 0, 0, 0);
    }
    ...
    protected phoukaTalk(): void {
        ...
        if (((this.player.corAdjustedUp() + rand(125) + (this.player.level - 10)) * 4) <= 110) { // Players with 0% corruption have a 12% chance that the Phouka is willing to talk, players with 100% corruption have a 92% chance
            ...
            this.outputText("\n\nThe " + this.phoukaName());
            switch (this.player.level < 10 ? rand(8) : rand(12)) { // Low level characters don't get the chance to be offered whiskey
                case 0:
                case 1:
                case 2:
                case 3:
                ...
                case 8: // Decides he doesn't want you
                    this.outputText(" takes a long look at you.  Finally he grins and tells you <i>“You know what?  You're not quite what I was lookin for.  Us phouka got varied tastes, ya know.”</i>  Then the phouka lazily flies off, whistling some strange tune to himself.\n\nYou wriggle against your bonds and after a few minutes you are free once more.  You decide that's enough excitement for now and head back to camp.");
                    if (this.flags[kFLAGS.PHOUKA_LORE] == 0) this.flags[kFLAGS.PHOUKA_LORE] = 1; // Now you know what to call them
                    this.doNext(this.camp.returnToCampUseOneHour);  // Return to camp, 1 hour used
                    break;
    ...
    protected phoukaDrinkAccept(): void { // In every path that leads here the character learns the creature is called a phouka, so no phoukaName() calls needed
        ...
        switch (this.consumables.P_WHSKY.phoukaWhiskeyDrink(this.player)) {
            case 0: // Player isn't pregnant
                this.dynStats("lus", rand(5) + 1, "cor", rand(2) + 1); // Minor lust and corruption gains (since you aren’t drinking it all yourself). These gains are permanent
                break;
    ...
    protected phoukaDrinkRefuse(): void { // In every path that leads here the character learns the creature is called a phouka, so no phoukaName() calls needed
        ...
        if ((this.player.pregnancyIncubation == 0) && (this.player.buttPregnancyIncubation == 0) && (this.player.pregnancyIncubation <= 100)) { // Pregnancy is obvious to the phouka
            ...
            if ((this.player.lust100 > 66) || ((this.player.cor > 50) && (this.player.lust > 33))) {
    ...
    protected phoukaHalloweenTreacleEater(): void {
        this.clearOutput();
        this.outputText(this.images.showImage("phouka-adventure"));
    ...
    public vaginalBirth(): void {
        // Picture is here
        this.outputText(this.images.showImage("birth-phouka"));
    ...
    public updateVaginalPregnancy(): boolean { // Belly size doesn't change, instead you get updates on what's going on
    ...
    protected phoukaSexAddStandardMenuChoices(): void { // This happens several times so it's broken out here in case additional options get added later
        this.addDisabledButton(1, "Bunny", "This scene requires you to have vagina.");
        this.addDisabledButton(2, "Horse", "This scene requires you to have vagina.");
        if (this.player.hasVagina()) {
    ...
    public phoukaSexBunny(postCombat: boolean, lustLoss: boolean): void { // Player must have a vagina for bunny-morph sex
        ...
        PhoukaScene.phoukaForm = PhoukaScene.PHOUKA_FORM_BUNNY;
        this.outputText(this.images.showImage("phouka-sex-bunny"));
        ...
        if ((this.player.lust100 > 80) || (this.player.cor > 50))
    ...
    public phoukaSexGoat(postCombat: boolean, lustLoss: boolean): void { // Player does not have a vagina or has chosen anal
        ...
        this.outputText(this.images.showImage("phouka-sex-goat"));
        if (this.player.hasVagina())
        ...
        this.dynStats("cor", rand(1) + (postCombat ? 1 : 3)); // Extra two corruption for being enough of a pervert to want to fuck the phouka
    ...
    public phoukaSexHorse(postCombat: boolean, lustLoss: boolean): void { // Player has a vagina
        ...
        this.outputText(this.images.showImage("phouka-sex-horse"));
        if (this.player.hasVirginVagina())
    ...
    protected phoukaSexFaerie(postCombat: boolean): void {
        ...
        if (this.player.hasVagina()) {
            ...
            this.outputText(this.images.showImage("phouka-sex"));
            this.outputText("You're getting closer and closer to cumming when you feel the " + this.phoukaName() + "'s cock twitch inside you.  The bastard is already there!");
            ...
            this.dynStats("cor", rand(1) + 3); // Extra two corruption for being enough of a pervert to want to fuck the phouka
            if (postCombat) {
                kGAMECLASS.combat.clearStatuses();
                kGAMECLASS.combat.awardPlayer(); // This will provide loot and return to camp, 1 hour used
            }
    ...
    protected phoukaSexPregnate(postCombat: boolean): void { // Whether by horse, bunny or (male) faerie sex it all ends up here if the PC has a vagina
        ...
        if (this.player.isPregnant()) {
            const isFirstTime: boolean = (this.flags[kFLAGS.BIRTHS_PHOUKA] == 0) && (this.flags[kFLAGS.BIRTHS_FAERIE] == 0);
    ...
    protected phoukaSexPregnateEnd(postCombat: boolean): void { // Everything after the sex. Handles awards, gem loss and text for player leaving the bog
        ...
        this.dynStats("cor", rand(1) + (postCombat && (PhoukaScene.phoukaForm != PhoukaScene.PHOUKA_FORM_FAERIE) ? 1 : 3)); // Extra two corruption for being enough of a pervert to want to fuck the phouka
