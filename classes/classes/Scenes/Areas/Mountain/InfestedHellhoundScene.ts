 ...
 */
	...
	export class InfestedHellhoundScene extends BaseContent
	{
		...
		public  infestedHellhoundEncounter(): void
		{
			...
			unlockCodexEntry("Hellhounds", kFLAGS.CODEX_ENTRY_HELLHOUNDS);
		...
		public  infestedHellhoundLossRape(): void
		{
			...
			//[PLAYER'S COCKS ARE BIG ENOUGH TO BE INFECTED]
				...
				if (player.totalCocks() > 1) {
					...
					outputText("each of your " + player.multiCockDescriptLight() + ".  It feels good, better than it has any right to.  ");
					...
					outputText("Your ");
					if (player.balls > 0) outputText(player.ballsDescriptLight() + " shiver as hot doggie-cum and wiggling worms are pumped directly into them.  ");
					...
					outputText("The infested hound repositions itself, blocking out the sun with its dark fur, leaving you with only the pale flames surrounding its fuzzy sack to look at.   The warm wetness of its smooth tongue starts sliding over ");
					outputText("your " + player.multiCockDescriptLight() + ".  It feels good, better than it has any right to.  ");
