...
export class Wanderer extends BaseContent {
    ...
    public wandererRouter(): void {
        this.spriteSelect(SpriteDb.s_markus_and_lucia);
        // First meeting...
        if (!this.player.hasStatusEffect(StatusEffects.MeetWanderer)) {
            this.wandererFirstMeeting();
            this.player.createStatusEffect(StatusEffects.MeetWanderer, 0, 0, 0, 0);
        }
        // Repeat offense!
        else {
            // Chosen demon ending
            if (this.player.hasStatusEffect(StatusEffects.WandererDemon)) {
                this.wandererDemonEpilogue();
                return;
            }
            // Chosen human ending
            if (this.player.hasStatusEffect(StatusEffects.WandererHuman)) {
    ...
    private wandererFirstMeeting(): void {
        ...
        this.menu();
        this.addButton(0, "Help Him", this.wandererHelpHim);
        this.addButton(14, "Leave", this.wandererLeave);
    }
    // Leave
    private wandererLeave(): void {
        this.spriteSelect(SpriteDb.s_markus_and_lucia);
        this.clearOutput();
        this.outputText("Marcus looks disappointed and sighs, hefting his wheelbarrow and waddling away.  Lucia bounces after him, looking like the cat that got the cream.  You wonder what all that was about.   What a strange land.");
    ...
    private wandererRepeatMeeting(): void {
        ...
        this.menu();
        this.addButton(0, "Yes", this.wandererHelpHim);
        this.addButton(14, "Leave", this.wandererLeave);
    }
    // Volunteer to help
    private wandererHelpHim(): void {
        ...
        this.menu();
        this.addButton(0, "Go Demon", this.wandererGoDemon);
        this.addButton(1, "Stay Human", this.wandererStayHuman);
    }
    ...
    private wandererStayHuman(): void {
        ...
        this.outputText("Marcus shakes his head sadly, \"<i>" + this.player.mf("He", "She") + " is right, my soul is immortal and not to be sacrificed so lightly.  Why don't we stick to our original agreement another decade and mull it over then?</i>\"\n\n");
        this.outputText("Lucia pouts, nearly on the verge of tears, \"<i>Fine!  But don't expect me to be happy about it.</i>\"  She pats his swollen balls roughly, smirking down at him, \"<i>and I'm going to let these swell up nice and tight before I take my next meal!</i>\"  ");
        ...
        this.player.createStatusEffect(StatusEffects.WandererHuman, 0, 0, 0, 0);
        this.doNext(this.camp.returnToCampUseOneHour);
    }
    ...
    private wandererGoDemon(): void {
        this.spriteSelect(SpriteDb.s_markus_and_lucia);
        this.clearOutput();
        this.outputText("Lucia breaks into a mischievous smile as you suggest taking her up on her offer.  She sashays over to you, flesh jiggling enticingly the whole way.  She leans close, sliding a slender finger down the center of your chest.  \"<i>Thank you for this.  Should we meet again, I promise rewards fit to make a whore faint.</i>\"\n\n");
        this.outputText("Marcus raises an eyebrow at the exchange, but smiles as his demonic lover returns to his side.  Lucia winks again, and huge wings explode from her back.  She grabs Marcus, who bleats in surprise, and lifts off, flying away with her prize to her lair.");
        this.dynStats("lus", 5, "cor", 1);
        this.player.createStatusEffect(StatusEffects.WandererDemon, 0, 0, 0, 0);
        this.doNext(this.camp.returnToCampUseOneHour);
    }
    ...
    private wandererDemonEpilogue(): void {
        ...
        if (this.player.hasStatusEffect(StatusEffects.WandererDemon)) {
            // First time...
            if (this.player.statusEffectv1(StatusEffects.WandererDemon) == 0) {
                this.clearOutput();
                ...
                this.player.statusEffectByType(StatusEffects.WandererDemon).value1 = 1;
            }
            // Second Encounter
            else if (this.player.statusEffectv1(StatusEffects.WandererDemon) == 1) {
                this.clearOutput();
                ...
                // Catch it
                if (50 < (this.player.spe + rand(60))) {
                    this.outputText("You handily catch a small potion vial.  When you look up, she's gone.\n\n");
                    this.inventory.takeItem(this.consumables.SDELITE, this.camp.returnToCampUseOneHour);
                }
                // Drop it
                else {
                    this.outputText("You dive for the falling bottle, but miss, and it shatters into the sands, the fluids wicking away nearly instantaneously.");
                    this.doNext(this.camp.returnToCampUseOneHour);
                }
            }
        }
    ...
    private wandererEpilogueHuman(): void {
        ...
        if (this.player.hasStatusEffect(StatusEffects.WandererHuman)) {
            // Human Epilogue 1
            if (this.player.statusEffectv1(StatusEffects.WandererHuman) == 0) {
                this.clearOutput();
                ...
                if (this.player.cor < 33) this.outputText("You duck back behind a dune, blushing furiously.");
                else if (this.player.cor < 66) this.outputText("You blush crimson as you swear you see Lucia look right at you and wink.");
                else this.outputText("You openly leer at the crude display, whistling lewdly at the blissful couple.  Marcus looks up and gives a cocky smile, while Lucia licks her lips and gives you a predatory grin.");
                this.dynStats("lus", 10);
                // Value 1 is used to track the status of the end state.
                this.player.statusEffectByType(StatusEffects.WandererHuman).value1 = 1;
                this.doNext(this.camp.returnToCampUseOneHour);
            }
            // Human Epilogue 2
            else if (this.player.statusEffectv1(StatusEffects.WandererHuman) == 1) {
                this.clearOutput();
