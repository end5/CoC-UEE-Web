...
import { PlayerSandTrapPregnancy } from "../../Monsters/pregnancies/PlayerSandTrapPregnancy";
import { PlayerSandTrapFertilePregnancy } from "../../Monsters/pregnancies/PlayerSandTrapFertilePregnancy";
...
export class SandTrapScene extends BaseContent {
    ...
    public encounterASandTarp(): void {
        this.clearOutput();
        this.spriteSelect(SpriteDb.s_sandtrap);
        if (this.flags[kFLAGS.SANDTRAP_LOSS_REPEATS] >= 2 && this.player.eyes.type == Eyes.BLACK_EYES_SAND_TRAP && this.player.wings.type == Wings.GIANT_DRAGONFLY) {
            this.sandTrapBadEnd();
            return;
        }
        ...
        if (this.flags[kFLAGS.TIMES_ENCOUNTERED_SAND_TRAPS] == 1) {
            ...
            this.menu();
            this.addButton(0, "Save", this.saveTheSandTarps);
            this.addButton(1, "Don't Save", this.dontSaveTheTarps);
        }
        else {
            // Standard encounter:
            ...
            if (this.player.spe / 10 + rand(20) >= 16) {
                ...
                // Fight]/[Leave]
                this.menu();
                this.addButton(0, "Fight", this.startSandTarpFight);
    ...
    private saveTheSandTarps(): void {
        ...
        this.startCombat(new SandTrap());
        this.monster.changeStatusValue(StatusEffects.Level, 1, 2);
        this.doNext(this.playerMenu);
    }
    ...
    public sandtrapmentLoss(clear: boolean = false): void {
        if (this.flags[kFLAGS.SFW_MODE] > 0) { // No rape in SFW mode.
            this.clearOutput();
    ...
    public pcBeatsATrap(): void {
        ...
        this.menu();
        this.addDisabledButton(0, "Naga3Some", "This scene requires you to have naga body with genitals and another friendly naga around.", "Naga Treesome");
        this.addDisabledButton(1, "UseYourCock", "This scene requires you to have cock and to be strong enough.", "Use Your Cock");
        this.addDisabledButton(2, "RideVaginal", "This scene requires you to have vagina.");
        this.addDisabledButton(3, "Handjob", "This scene requires you to have genitals.");
        ...
        if (this.player.lust >= 33) {
            ...
            if (this.player.isNaga() && this.player.hasStatusEffect(StatusEffects.Naga) && !this.player.isGenderless() && this.player.face.type == Face.SNAKE_FANGS) {
                this.addButton(0, "Naga3Some", this.nagaThreesomeWithSandTrap, undefined, undefined, undefined, "Call your friend for a treesome.", "Naga Treesome");
            }
            // Requires: Penis and str requirement
            if (this.player.hasCock() && this.player.str >= 60) {
                this.addButton(1, "UseYourCock", this.stickWangInSandgina);
            }
            // \"<i>Ride</i>\" (Z)
            if (this.player.hasVagina()) {
                this.addButton(2, "RideVaginal", this.rideDatSandTarpLikeIts1999);
            }
            // \"<i>Hands</i>\" (Z)
            if (!this.player.isGenderless()) {
                this.addButton(3, "Handjob", this.useSandTarpsHands);
            }
            if (this.player.hasVagina() && this.player.biggestTitSize() >= 4 && this.player.armorName == "lusty maiden's armor") {
                this.addButton(4, "B.Titfuck", (this.player.armor as LustyMaidensArmor).lustyMaidenPaizuri, this.player, this.monster);
            }
        }
    ...
    private dickwieldersLoseToSandTarps(): void {
        this.clearOutput();
        this.spriteSelect(SpriteDb.s_sandtrap);
        this.outputText(this.images.showImage("sandtrap-desert-male-lose-sex"));
        ...
        if (this.player.hasVagina()) this.outputText("  Ignored entirely, your " + this.player.vaginaDescript(0) + " quivers and orgasms in tandem, flexing and wetting fruitlessly against the dry water pressing against it.");
        ...
        this.monster.createStatusEffect(StatusEffects.Fertilized, 0, 0, 0, 0);
    ...
    private chicksLoseToSandTarp(): void {
        ...
        if (this.monster.hasStatusEffect(StatusEffects.Fertilized)) this.outputText("  \"<i>A cute little wwworker hasz fallen into my home.  Wwwell, that isz ok.  Fortunately for hwyou, I already have some eggsz ready.  Hhyou will get to be a queen! Iszn't that exciting?</i>\"");
        ...
        if (this.monster.hasStatusEffect(StatusEffects.Fertilized)) this.sandTrapPregChance();
    ...
    private genderlessLoss(): void {
        ...
        if (this.monster.hasStatusEffect(StatusEffects.Fertilized)) this.outputText("\"<i>Not a worker OR a drone! Wwwell, that isz ok.  Even ants who hhhave nothing can be someone for somebody.  And hhyou can be mommy for my childrrhen!</i>\"");
    ...
    private sandTrapPregChance(): void {
        ...
        if (this.monster.hasStatusEffect(StatusEffects.Fertilized))
    ...
    public birfSandTarps(): void {
        ...
        // Female:
            ...
            this.outputText("  You roll your eyes to the sky and moan, your " + this.player.vaginaDescript(0) + " moistening as you push out the egg.");
            // [(no fukken horses from here)]
            if (!this.player.isTaur()) this.outputText("  Your oily hands push softly into your cleft, fingering your needy " + this.player.clitDescript() + " as you feel the pressure in your bowels intensify again.");
        }
        // Herm:
            ...
            // [(no horses)]
            if (!this.player.isTaur()) this.outputText("  Your oily hands descend upon your genitals and you begin to slowly pump your shaft and finger your needy " + this.player.clitDescript() + " as you feel the pressure in your bowels intensify again.");
        }
    ...
    private useSandTarpsHands(): void {
        ...
        if (this.player.gender == 1) {
            ...
            this.outputText(this.images.showImage("sandtrap-desert-male-win-get-hj"));
            ...
            if (this.player.cocks[0].cockLength < 6) {
                // balls:
                if (this.player.balls == 0) this.outputText("\n\nThe sandtrap only needs one hand to pump your diminutive cock; it barely moves its wrist as it jerks you, occasionally wriggling its oily fingers along the bottom of your " + this.player.cockDescript(0) + " to stimulate you even further.");
                else this.outputText("\n\nThe sandtrap only needs one hand to pump your diminutive cock; it barely moves its wrist as it jerks you, occasionally wriggling its oily fingers along the bottom of your " + this.player.cockDescript(0) + " to incite you even further, whilst it uses its free hand to gently cup your [balls].");
            ...
            // Two cocks:
                ...
                this.outputText("  As the sandtrap begins to pump you faster, your disregarded " + this.player.cockDescript(1) + " bobs in hard, eager concert with your " + this.player.cockDescript(0) + ".  You gasp as the trap slows down momentarily to grasp your second sex.  It is clumsy at first as it moves its lubricated palms in tandem, then with increasing confidence as it finds a position and rhythm that suits it, until it is jerking you off furiously, clutching your butt cheeks so tightly you hazily realize the indents of its slender digits will be with you for hours afterwards.");
                this.outputText("\n\nThe warm, oily double sensation is unbearably good and although you try to hold on to savor it as long as possible, it isn't long before you reach a toe-curling peak and, with a wordless bark, spurt out line after line of jizz.  The trap flinches slightly when your first cumshots hit it in the face and chest, but then goes back to gently rubbing you, guiding you up and down your warm, slippery orgasm.");
            }
            // Three cocks:
            else if (this.player.cockTotal() == 3) {
                this.outputText("\n\nAs the sandtrap begins to pump you faster, your two other, disregarded cocks bob in hard, eager concert with your " + this.player.cockDescript(0) + ".  You gasp as the trap slows down momentarily to reach one of its hands down to grasp your second sex, and then stops entirely as  one of its hands clutching your ass releases and snakes back around to take hold of your " + this.player.cockDescript(2) + ".  It is clumsy at first as it works its way around the rather complex arrangement you have down below, but it moves its lubricated palms with increasing confidence, until it is jerking all three of your cocks in rhythm.");
            ...
            // Four cocks:
                ...
                this.outputText("\n\nAs the sandtrap begins to pump you faster, your three other, disregarded cocks bob in hard, eager concert with your " + this.player.cockDescript(0) + ".  You gasp as the trap slows down momentarily to reach one of its hands down to grasp your second sex, and then stops entirely as both of its hands clutching your ass release and snake back around to take hold of your " + this.player.cockDescript(2) + " and " + this.player.cockDescript(3) + ".  It is clumsy at first as it works its way around the rather complex arrangement you have down below, but it moves its lubricated palms with increasing confidence, until it is jerking all four of your cocks in rhythm.  You laugh into the sky with an almost hysterical edge at how mindbogglingly good getting a quadruple handjob feels; it glances up at you and giggles shyly as you look into each other's eyes and share a thought.  \"<i>At last, I've found somebody who complements my body perfectly.</i>");
            ...
            // More than four cocks:
                ...
                this.outputText("\n\nAs the sandtrap begins to pump you faster, your many other, disregarded cocks bob in hard, eager concert with your " + this.player.cockDescript(0) + ".  You gasp as the trap slows down momentarily to reach one of its hands down to grasp your second sex, and then stops entirely as its two hands clutching your ass snake back around to take hold of your " + this.player.cockDescript(2) + " and " + this.player.cockDescript(3) + ".  It is clumsy at first as it works its way around the incredibly complex arrangement you have down below, but it moves its lubricated palms with increasing confidence, until it is jerking four of your cocks in rhythm.  You laugh into the sky with an almost hysterical edge at how mindbogglingly good getting a quadruple handjob feels, and the trap giggles slightly as well despite itself.  Hazily you wonder if it is too much to wish that this creature, with its excellent wrist action, had just one or two more hands to its name.");
                this.outputText("\n\nThe warm, oily quadrupled sensation is unbearably good and although you try to hold on to savor it as long as possible, it isn't long before you reach a toe-curling peak and, with a wordless bark, spurt out line after line of jizz.  The trap flinches slightly when your first shots hit it in the face and chest, but then goes back to gently rubbing you, guiding you up and down your warm, slippery multiple orgasm as you spurt like a cum sprinkler in front of it.");
            }
        }
        // Female:
        else if (this.player.gender == 2) {
            this.outputText(this.images.showImage("sandtrap-desert-female-win-get-fingering"));
        ...
        // Herm:
            ...
            this.outputText(this.images.showImage("sandtrap-desert-herm-win-get-hj"));
            this.outputText("\n\nOne pair of the creature's hands slowly brush up your inner thighs whilst the other two feel around your \"<i>armor</i>\", clumsily loosening clasps and belts as it finds them.  One hand finds your " + this.player.cockDescript(0) + " and, after pausing for a split second, slides its fingers around your shaft and begins to slowly rub up and down.  You sigh as you quickly harden in the creature's smooth, dry, enveloping grasp.  You look into the sandtrap's face as it works; blushing, it looks down from your gaze and concentrates upon what it is doing.  It begins to pump you faster and you hiss slightly at the abrasion this causes.  At this the sandtrap makes a throaty sound.  Curiously you look down in time to see it bring its other pair of hands up to its mouth and deposit a great dollop of clear ooze into its cupped palms.  Your disgust turns to delight as it swaps its hands and begins to massage its oil into your length, the warm lubricant allowing the creatures' fingers to glide around your shaft and head like they were made of silk.  As it moves its first hand up and down, it tentatively moves its second further between your legs.  It finds your [vagina] and, after pausing for a split second, slides its slender fingers into your moistness.  Although it is clumsy at first, the oil which coats its digits make them glide over your shaft and labia as if they were entirely frictionless.  It quickly gains confidence and begins to gently frig you as it slides its warm, slippery grasp up and down your " + this.player.cockDescript(0) + ", circling your [clit] whilst occasionally dipping into your dripping hole.  You sigh and begin to lose yourself in the wet, silky sensation.");
            // Cock less than 6 inches:
            if (this.player.cocks[0].cockLength < 6) {
                // [no balls:
                if (this.player.balls == 0) this.outputText("\n\nThe sandtrap only needs one hand to pump your diminutive cock; it barely moves its wrist as it jerks you, occasionally wriggling its oily fingers along the bottom of your " + this.player.cockDescript(0) + " to stimulate you even further.");
            ...
            // Cock 6-16 inches:
                ...
                if (this.player.balls == 0) this.outputText("\n\nThe sandtrap glides its hand up and down your " + this.player.cockDescript(0) + ", picking up a warm smooth rhythm.");
                else this.outputText("\n\nThe sandtrap glides its hand up and down your " + this.player.cockDescript(0) + ", picking up a warm smooth rhythm as it gently cups your [balls] with its free hand.]  It slows down intermittently to move its fingers around your length, letting every inch of your dick feel the tips of its delicate fingers.");
            }
            else {
            ...
            // More than three cocks:
                ...
                this.outputText("\n\nAs the sandtrap begins to pump you faster, your many other, disregarded cocks begin to harden in eager concert with your " + this.player.cockDescript(0) + ".  You gasp as the trap slows down momentarily to reach both of its hands clutching your ass back around to take hold of your " + this.player.cockDescript(1) + " and " + this.player.cockDescript(2) + ".  It is clumsy at first as it works its way around the rather complex arrangement you have down below, but it moves its lubricated palms with increasing confidence, until it is jerking three of your cocks whilst fingering you in rhythm.  You laugh into the sky with an almost hysterical edge at how mindbogglingly good getting a quadruple handjob feels, and the trap giggles slightly as well despite itself.  Hazily you wonder if it is too much to wish that this creature, with its excellent wrist action, had just one or two more hands to its name.");
    ...
    private rideDatSandTarpLikeIts1999(): void {
        ...
        this.spriteSelect(SpriteDb.s_sandtrap);
        // Requirement: Vagina
        this.outputText(this.images.showImage("sandtrap-desert-female-win-vag-ride"));
        ...
        this.outputText("\n\n\"<i>You like that, don't you, sand sissy?</i>\"  It is breathing heavily but it still won't look into your face. \"<i>Look me in the eye!</i>\" As it finally does so you push your oil-covered fingers into its mouth and it moans around them as you swab its tongue and cheeks with its own grease.  You press your body against it, your " + this.player.allBreastsDescript() + " pushing into its own flat chest; it is powerless to resist you as its own pheromones take effect upon it, and you slowly push it backwards onto the flat of its back on the sloping sand.  You sit your moist " + this.player.vaginaDescript(0) + " at the base of its pseudo-cock and then slowly lead yourself back, luxuriating in pushing your " + this.player.clitDescript() + " up this oily slide.   It moans and shifts itself upwards needily, but you tut mockingly and stop your slow slide inches away from its tip.");
        this.outputText("\n\n\"<i>Ask for it.</i>\"");
        ...
        this.outputText(this.images.showImage("sandtrap-win"));
    ...
    private stickWangInSandgina(): void {
        ...
        this.outputText(this.images.showImage("sandtrap-desert-male-win-vag-sex"));
        ...
        this.outputText("\n\nYou take a moment to look your strange conquest over.  Even up close you can't tell from its thin, fey beauty whether it is male or female.  Although it is affecting to look angry at you, its sculpted cheeks are quite flushed and there is a definite undercurrent of desire in its eyes; you suspect your display of strength and dominance appeals to it on some perverse, animalistic level.  Still smiling victoriously, you bend your face into it, deliberately invading its personal space, silently demonstrating who is in charge here.  It can't hold your gaze and, blushing, looks downwards.  This submissive demureness sends heat rushing towards your " + this.player.multiCockDescriptLight() + "; the desire to fuck builds in you, to plough, to sink your cock into this strange, eerily beautiful creature, to establish once and for all who is giving and who is receiving around here... the only question is, how?  Its slim, androgyne upper body is devoid of obvious sexual features.  Your eyes roam steadily downwards.  Where its insect half meets its human half, where its cock should be, it instead has a genital slit; from this is emerging a long, black prong, dripping with clear oil.  Your show of dominance is turning this creature on, and it is powerless to disguise it.  It looks up at you with timid hopefulness... but you are in no mood for that.  There is a definite air of femininity to this creature, a pheromone scent which speaks directly to your " + this.player.cockDescript(0) + ", makes you hard");
        if (this.player.balls > 0) this.outputText(", makes your " + this.player.ballsDescriptLight() + " swell in readiness,");
        ...
        this.outputText("\n\n\"<i>Hhhhyes!  That'sz it lion, oh do me, do it to me hard!  Give me yyyyour strong spunk - uh! give hhit to me szo I can sink yyour children into evvryone I catch - yes!  Hhhhso the whole world can know hhwhat a " + this.player.mf("man", "woman") + " you arrrrrhhhhhe!</i>\"  The creature's words subside into a high fluttering cry as it cums.  Its ovipositor spasms and clenches, milking you down your " + this.player.cockDescript(x) + " for all it is worth.  Even if you wanted to, you could not pull back from the brink.  You come with near-painful intensity, your jizz shooting out of you as if your dick were trapped in a vacuum, losing yourself in ecstasy - you feel nothing but the grasping wet around your " + this.player.cockDescript(x) + ", your cream spurting out of you like liquid gold, and the sun beating down on your back.");
    ...
    private nagaThreesomeWithSandTrap(): void {
        ...
        // Female:
            ...
            this.outputText(this.images.showImage("sandtrap-desert-female-win-naga-3some"));
            this.outputText("\n\nYou press your moistening " + this.player.vaginaDescript(0) + " against the creature's lips.  Its eyes still far away, the sandtrap opens its mouth and pushes its soft tongue into your cleft.  It explores your labia and hole before turning its attention to your " + this.player.clitDescript() + ", circling it with gentle pressure whilst giving it the occasional faintest of flicks.");
            // [Squirtr, jotun of flooding:
            if (this.player.wetness() < 4) this.outputText("  Its gentle worship of your " + this.player.vaginaDescript(0) + " soon has you streaming rivulets of girlcum from your oversexed hole, dribbling down the sandtrap's face as you eagerly press yourself down for more.");
            ...
            this.outputText("\n\nThe naga is soon deep within the sandtrap's oil fuge and she slows her thrusting down as it relaxes her and amplifies what she can feel, smiling euphorically as she slowly rotates on the creature's leaking pseudo-cock.  You pace yourself in sympathy; having already reached one ecstatic high you're more than happy to simply sit back and enjoy the sandtrap's worshipful ministrations, tending to every corner of your " + this.player.vaginaDescript(0) + ".  You've also had enough oil rubbed into you to feel quite dreamy yourself; the sand, the sun and even your own vagina seem bigger and more encompassing than they were before.");
    ...
    private sandTrapBadEndFinale(): void {
        this.spriteSelect(SpriteDb.s_sandtrap);
        this.outputText(this.images.showImage("sandtrap-desert-badend"));
