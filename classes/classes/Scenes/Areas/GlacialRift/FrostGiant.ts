	...
	export class FrostGiant extends Monster
	{
		private  giantStrengthLoss(magnitude: number = 0): void {
		...
		public  giantGrabStruggle(): void {
			if (rand(100) >= player.str || rand(10) == 0) giantGrabFail();
			else giantGrabSuccess();
		}
		public  giantGrabFail(struggle: boolean = true): void {
		var  damage: number = 0
			if (struggle) {
				clearOutput();
				if (player.str >= 80) {
		...
		public  giantBoulderFantasize(): void {
		...
		public  giantBoulderHit(mode: number = 0): void {
			clearOutput();
		...
		var  damage: number = (str * 2) + 100 + rand(250);
			damage = player.reduceDamage(damage);
		...
		public  giantBoulderMiss(): void {
			...
			if (player.hasStatusEffect(StatusEffects.GiantBoulder)) player.removeStatusEffect(StatusEffects.GiantBoulder);
		var  damage: number = 10 + rand(str / 2);
			damage = player.reduceDamage(damage);
			player.takeDamage(damage, true);
			outputText("\n\n");
			combatRoundOver();
