 ...
 * @author Kitteh6660
 */
export class Codex extends BaseContent {
    ...
    private PAGE_RACES_1: number = 0;
    private PAGE_RACES_2: number = 1;
    private PAGE_FACTIONS_1: number = 10;
    private PAGE_OTHERS_1: number = 20;

    // CODEX MODULES & APIs
    public headerMain(text: string = ""): void {
        this.outputText("<font size=\"36\" face=\"Georgia\">" + text + "</font>\n\n");
    }
    public headerSub(text: string = ""): void {
    ...
    public setFlags(): void {
        if (this.player.hasStatusEffect(StatusEffects.KnowsCharge) || this.player.hasStatusEffect(StatusEffects.KnowsBlind) || this.player.hasStatusEffect(StatusEffects.KnowsWhitefire) || this.player.hasStatusEffect(StatusEffects.KnowsArouse) || this.player.hasStatusEffect(StatusEffects.KnowsHeal) || this.player.hasStatusEffect(StatusEffects.KnowsMight)) {
            if (this.flags[kFLAGS.CODEX_ENTRY_MAGIC] <= 0) {
                this.flags[kFLAGS.CODEX_ENTRY_MAGIC] = 1;
            }
        }
        if (this.flags[kFLAGS.MARBLE_WARNING] > 0) {
            this.flags[kFLAGS.CODEX_ENTRY_LABOVINES] = 1;
        }
        if (this.flags[kFLAGS.ROGAR_DISABLED] > 0 || this.flags[kFLAGS.ROGAR_PHASE] >= 3) {
            this.flags[kFLAGS.CODEX_ENTRY_ORCS] = 1;
        }
    }

    public checkUnlocked(): number {
        let num: number = 0;
        // Races
        if (this.flags[kFLAGS.CODEX_ENTRY_ANEMONES] > 0) {
            num++;
        }
        if (this.flags[kFLAGS.CODEX_ENTRY_ARACHNES] > 0) {
            num++;
        }
        if (this.flags[kFLAGS.CODEX_ENTRY_BASILISKS] > 0) {
            num++;
        }
        if (this.flags[kFLAGS.CODEX_ENTRY_BEHEMOTH] > 0) {
            num++;
        }
        if (this.flags[kFLAGS.CODEX_ENTRY_COCKATRICES] > 0) {
            num++;
        }
        if (this.flags[kFLAGS.CODEX_ENTRY_ECHIDNAS] > 0) {
            num++;
        }
        if (this.flags[kFLAGS.CODEX_ENTRY_GIANTBEES] > 0) {
            num++;
        }
        if (this.flags[kFLAGS.CODEX_ENTRY_GOBLINS] > 0) {
            num++;
        }
        if (this.flags[kFLAGS.CODEX_ENTRY_GOOGIRLS] > 0) {
            num++;
        }
        if (this.flags[kFLAGS.CODEX_ENTRY_HARPIES] > 0) {
            num++;
        }
        if (this.flags[kFLAGS.CODEX_ENTRY_HELLHOUNDS] > 0) {
            num++;
        }
        if (this.flags[kFLAGS.CODEX_ENTRY_IMPS] > 0) {
            num++;
        }
        if (this.flags[kFLAGS.CODEX_ENTRY_LABOVINES] > 0) {
            num++;
        }
        if (this.flags[kFLAGS.CODEX_ENTRY_LIZANS] > 0) {
            num++;
        }
        if (this.flags[kFLAGS.CODEX_ENTRY_MINOTAURS] > 0) {
            num++;
        }
        if (this.flags[kFLAGS.CODEX_ENTRY_NAGAS] > 0) {
            num++;
        }
        if (this.flags[kFLAGS.CODEX_ENTRY_ORCS] > 0) {
            num++;
        }
        if (this.flags[kFLAGS.CODEX_ENTRY_RHINOS] > 0) {
            num++;
        }
        if (this.flags[kFLAGS.CODEX_ENTRY_SALAMANDERS] > 0) {
            num++;
        }
        if (this.flags[kFLAGS.CODEX_ENTRY_SATYRS] > 0) {
            num++;
        }
        if (this.flags[kFLAGS.CODEX_ENTRY_SHARKGIRLS] > 0) {
            num++;
        }
        if (this.flags[kFLAGS.CODEX_ENTRY_SUCCUBUS] > 0) {
            num++;
        }
        if (this.flags[kFLAGS.CODEX_ENTRY_ZEBRAS] > 0) {
            num++;
        }
        // Factions
        if (this.flags[kFLAGS.CODEX_ENTRY_FETISHFOLLOWERS] > 0) {
            num++;
        }
        if (this.flags[kFLAGS.CODEX_ENTRY_SANDWITCHES] > 0) {
            num++;
        }
        // Others
        if (this.flags[kFLAGS.CODEX_ENTRY_MAGIC] > 0) {
        ...
        if (num >= 26) this.awardAchievement("Scholar", kACHIEVEMENTS.GENERAL_SCHOLAR);
        return num;
    }

    // CODEX MENUS
    public accessCodexMenu(): void {
        this.clearOutput();
        this.setFlags();
        this.menu();
        this.flags[kFLAGS.CODEX_CURRENT_ENTRY] = 0;
        this.outputText("You open your codex. Which topic would you like to read?\n\n");
        this.outputText("Codex entries unlocked: " + this.checkUnlocked() + "/26");
        this.addButton(0, "Races", this.menuRaces);
        this.addButton(1, "Cults", this.menuFactions);
        this.addButton(2, "Others", this.menuItems);
        this.addButton(14, "Back", this.playerMenu);
    }

    private setCodexMenusAfterDisplay(): void {
        if (this.flags[kFLAGS.CODEX_CURRENT_PAGE] == this.PAGE_RACES_1) this.menuRaces();
        if (this.flags[kFLAGS.CODEX_CURRENT_PAGE] == this.PAGE_RACES_2) this.menuRacesII();
        if (this.flags[kFLAGS.CODEX_CURRENT_PAGE] == this.PAGE_FACTIONS_1) this.menuFactions();
        if (this.flags[kFLAGS.CODEX_CURRENT_PAGE] == this.PAGE_OTHERS_1) this.menuItems();
    }

    private addCodexButton(codexEntryName: string, codexEntryId: string, codexEntry: any, flag: number): void {
        let button: number = 0;
    ...
    private codexEntryRhinos(): void {
        ...
        this.headerSub("Corruption");
        this.outputText("Initially the demons learned to fear the rhino morphs. They took in survivors from other villages into their nomadic crashes and protected them with their tough hide and weapon like horns. Yet ultimately their dualistic nature caused them to trust demons that didn’t show themselves to be aggressive. This allowed the demons to walk among them. One thing lead to another and eventually the corruption swept through the crashes like a million birds of prey felling a field of mice petrified with fear.");
    ...
    private codexEntrySalamanders(): void {
        this.flags[kFLAGS.CODEX_CURRENT_ENTRY] = "Salamanders";
        this.clearOutput();
        this.headerMain("Salamanders");
        this.outputText("<b>Genders:</b> Males and Females occur naturally, though transformative items leave none too few Salamanders as hermaphrodites or genderless.\n");
        this.outputText("<b>Body structures:</b> Salamanders stand taller than most humans – usually between 6.5 and 7 feet tall. They have human-like bodies, though their forearms and legs, up to the mid-thigh, are covered in thick, leathery red scales. Their hands and feet have powerful claws that, combined with the natural strength of a Salamander, allows them to easily rend through even some metals. Salamanders also possess a long, prehensile tail that is usually alight with a burning fire.\n");
        this.outputText("<b>Facial structures:</b> Salamanders have human-like faces, save for the smattering of scales on their cheeks (akin to freckles on a human), and frilled, reptilian ears.\n");
        this.outputText("<b>Hair & Eye Coloration:</b> Salamanders have red, orange, and rarely, pink hair. Most members of the species have bright red eyes, though some few manifest yellow or grey-blue eyes.\n");
        this.headerSub("Fire mastery");
        this.outputText("Salamanders have a natural, elemental mastery over fire. Their tails are a natural source of fire, which they are able to extinguish and re-light at-will; a Salamander’s interior is also described as exceedingly hot, often leaving lovers sweating on contact. They are by and large immune to natural fire, including that of dragon-morphs, though spell-fire’s effect on them is unknown.");
        this.headerSub("Berserking");
        this.outputText("Salamanders are notorious for their fiery, impassioned personalities, and this is nowhere more evident than their enraged fighting style. Salamanders throw their heart and souls into combat, letting their rage overcome them until they’re literally half-mad and their bloodthirst seems unquenchable. Even after battle, it is difficult to calm their passions, and most Salamanders prefer – if not need – to engage in visceral activity after combat, usually taking the form of sexual intercourse with friends, loved ones, or defeated enemies.");
        this.headerSub("Sexuality");
        this.outputText("Thankfully, the remote nature of Salamander society has prevented the race from being overexposed to demonic influences. Still, even before the demons came, Salamanders were energetically sexual, and were infamous for their enthusiasm for group sex. Salamanders are a notoriously infertile race, however, and their birth rate was dangerously low. This was only exacerbated by the demonic invasion, and most of the remaining Salamander tribes have crumbled to single-digit populations. A Salamander pregnancy takes many months to complete, though they tend to be less encumbered by their pregnancies than human women are.");
        this.headerSub("History");
        this.outputText("One of Mareth’s less civilized races, even before the demon invasion, the Salamanders lived in hunter-gatherer tribes in the wide open plains, foothills, and mountains. Though exceedingly violent, the Salamanders destructive tendencies were kept in check by the savage gnolls, and the two races have shared a bond of mutual hatred for centuries. They will attack each other on sight, and their bloody battles have kept both races from posing a serious threat to the civilizations of Mareth before the demons, and to Lethice after their arrival.");
        this.outputText("\n\n(Written by: Savin) ");
        this.setCodexMenusAfterDisplay();
    }

    private codexEntrySandWitches(): void {
        this.flags[kFLAGS.CODEX_CURRENT_ENTRY] = "SandWitches";
        this.clearOutput();
        this.headerMain("Sand Witches");
        this.outputText("<b>Genders:</b> Exclusively female\n");
        this.outputText("<b>Height:</b> Sand Witches are typically five to six feet tall.\n");
        this.outputText("<b>Hair:</b> Always sandy blond.\n");
        this.outputText("<b>Eyes:</b> The eye color of Sand Witches varies from witch to witch, but is most often brown.\n");
        this.headerSub("Appearance");
        this.outputText("Sand Witches normally dress in loose fitting robes and little else. They will sometimes wear well-made sandals, and a belt to carry a few simple supplies. They are all young, either they do not age or have found a way to hide its effects. Because of the coloration of their hair and robes, it is difficult to spot a Sand Witch until she is right next to you.");
        this.headerSub("Sexual equipment");
        this.outputText("Sand Witches ALWAYS have four heavily lactating breasts with large prominent nipples. Few creatures can rival their milk production, and those that do wind up captured by the Witches and taken to their remote desert hideout. Somehow all Sand Witches always have multiple vaginas, though most of them only have two. Their methods for procreation and altering the number of vaginas are unknown.");
        this.headerSub("Reproduction");
        this.outputText("Pregnant Sand Witches have never been seen. They have either discovered a perfect contraceptive or kept all the pregnant members of their cult in seclusion.");
        this.headerSub("Social structure");
        this.outputText("Sand Witches live in remote covens deep in the desert. While the specifics of the living arrangements that far out are unknown, it is known that the covens tend to be comprised of at least 25 members, each lead by a leader with the title of ‘Sand-Mother’. The cults are governed together by a council of the 7 Sand-Mothers with the largest covens.");
        this.headerSub("Special attributes");
        this.outputText("Sand Witches are adept users of black AND white magic, and will use the combination to arouse their foes into submission, and then alter them into a form more like their own. They are not necessarily hostile, and will treat any they encounter nicely if they allow the witch to work her magic on them. ");
        this.setCodexMenusAfterDisplay();
    }

    private codexEntrySatyrs(): void {
        this.flags[kFLAGS.CODEX_CURRENT_ENTRY] = "Satyrs";
        this.clearOutput();
        this.headerMain("Satyrs");
        this.outputText("<b>Genders:</b> Males (Females used to exist)\n");
        this.headerSub("Backstory");
        this.outputText("Satyrs were originally a dual-gendered race of \"10% furry\" (legs, tail, horns, possibly ears) goats that came in both male and female. In those days, they were called \"Fauns\". The Fauns believe themselves to be, at least metaphorically, the children of one of Marae's pre-demon demigodlings, a hermaphroditic spirit in the form of an anthro goat named Baphomet. This spirit was a god of pleasure, love, fertility, virility and carnality; as a result, the Fauns were very friendly - and very horny - individuals, constantly playing games, dancing, making music, drinking strong drink, and having sex. They were renowned for their bacchanals and orgies.\n");
        this.outputText("They were also notorious for their mastery of sex-related magic, essentially the Black Magic of Mareth today. There were few races more adept at producing cures for infertility than Fauns.\n");
        this.headerSub("General behavior");
        this.outputText("Fauns live their lives to worship their guardian deity, Baphomet. They preach their religion by planting seeds of the fruit they collect; by brewing an aphrodisiac wine to be used in their parties, and by having sex. They would lure passer-bys with the promise of fun and pleasure by the means of a party to celebrate their god, this would often result in massive orgies since the wine in said parties happens to be their special brew of aphrodisiac wine. In their wake they would leave a few very satisfied lucky souls and small mounds of earth, where they planted new seeds to thank the nature for the gift of nourishment and pleasure.\n");
        this.outputText("Fauns would often be contacted by people who were infertile in hopes of healing whatever prevented them from having children. While it is true that often the Faun magic that would cure them would take the form of sexual contact with a Faun of the opposite sex, this rarely resulted in \"curing\” them by leaving them the mother or father of a baby Faun. Only in the most grievous cases would a Faun offer to serve as a stand-in stud or broodmare for a family in desperate hope for children.\n");
        this.headerSub("The Corruption");
        this.outputText("The corruption had a drastic effect on the Fauns' previously peaceful way of life. Because of the sexual openness of Faun culture, they were among the earliest victims of demonic influence; they literally welcomed the corruption into their souls with open arms; and before they knew it, they were being poisoned by sexual contact with the ever-insatiable corrupt legions. The denizens of Mareth owe the sudden spike in imp population to the Fauns. Eventually the females, weakened by repeated births, perished, leaving only the lust-crazed males behind. The corrupted Fauns, now called simply Satyrs, found a way to circumvent the problems that arose from the death of their females. Through the use of potent black magic, they made their seed extremely potent and able to impregnate anything that they happened to fuck, even each other.\n");
        this.outputText("They have abandoned their former creed of mutual pleasure to be shared amongst the willing, to instead live their lives in pursuit of excess and carnal luxury, just like the demons. When a Satyr is not seeking new mates to impregnate, they are usually drinking and polluting the nearby area with their aphrodisiac wine. They still lure passer-bys with the promise of fun and pleasure, only this time the purpose is solely to lure new victims to become the mothers of more satyrs.\n");
        this.headerSub("Gestation and birth");
        this.outputText("Satyr seed is infused with highly corrupt magic that enables them to literally impregnate anything. When confronted with a female or a herm of another race, the Satyr will try to fertilise them in the usual fashion; because of their tainted seed, pregnancy is all but guaranteed, and high fertility may actually result in the birth of multiple satyrs. However, so virile are satyrs that they are actually capable of anally impregnating male and neuter-gendered individuals. The thick ejaculate floods into the subject’s bowels and solidifies into a tough, leathery, egg-like mass. The interior of this egg is hollow and filled with corrupt fluids that serve as an amniotic \"soup\" in which a near identical clone of the Satyr \"father\" is grown. This infantile satyr grows inside the pseudo-womb like a regular infant would in a normal womb, with all of the usual symptoms - distended midriff, strange cravings, increased lust, sensations of movement, etc. When the infant grows large enough, the egg breaks down and the \"mother\" goes into labor, the liquified pseudo-womb serving as lubricant that, combined with the motions of the anal passage, brings the baby satyr into the world.\n");
        this.outputText("Once exposed to the outer world, the satyr's corrupted body rapidly matures until it attains full reproductive adulthood - puberty. Invariably, it will flatter its mother and offer to leave them \"something to remember him by\", but it retains enough civility to not become violent if they turn down this initial offer. On subsequent meetings, though, the new satyr will strive to incapacitate them and rape them into pregnancy once more.\n");
        this.headerSub("General behavior (Post-corruption)");
        this.outputText("Satyrs are now highly anti-social creatures; when two satyrs meet, invariably, the stronger one will dominate and impregnate the weaker one. They still love to feast and to party, but do so in gluttonous, debauched revels. They are notorious as bandits and thieves, stealing anything they can make off with and assaulting travelers for goods and breeding partners. They are often allied with goblins, one of the few races that can stand to be around them as they are now, with the satyr trading its considerable talents as a stud in exchange for food and liquor. The only reason satyrs as a whole have not sold themselves into slavery to the goblins is because, for many, the ever-increasing supply of willing sluts doesn’t outweigh the fact they can’t breed any sons with goblins.");
        this.setCodexMenusAfterDisplay();
    }

    private codexEntrySharkGirls(): void {
        this.flags[kFLAGS.CODEX_CURRENT_ENTRY] = "SharkGirls";
        this.clearOutput();
        this.headerMain("Shark-Girls & Tigershark-Girls");
        this.outputText("<b>Genders:</b> Mostly female, though there are males and herms. Due to the nature of their conception, the vast majority of tiger sharks are hermaphrodites.\n");
        this.outputText("<b>Height:</b> 5-6 feet tall. Tiger sharks are primarily in the 6 foot region.\n");
        this.outputText("<b>Build:</b> Well-toned and athletic.\n");
        this.outputText("<b>Skin tone:</b> Grey. Light orange with stripes for tiger sharks.\n");
        this.outputText("<b>Hair color:</b> Silver and in rare cases, Black.\n");
        this.outputText("<b>Eye color:</b> Red for both species.\n");
        this.outputText("<b>Weaponry:</b> Fangs, tail and their bare hands.\n");
        this.headerSub("Typical dress");
        this.outputText("Ridiculously skimpy swimwear, which they use to entice victims. Some tiger shark girls will wear grass hula skirts when on land to hide their endowments.");
        this.headerSub("Notable features");
        this.outputText("Retractable shark teeth, a large fin between their shoulders and a shark tail dangling off of their backsides.");
        this.headerSub("Sexual characteristics");
        this.outputText("Despite their slutty nature, shark girls have rather modest endowments in comparison to other such creatures; the majority of them are C-cups, D-cups at most. Though, their hips and buttocks are nice and curvy. Tiger shark girls possess wildly varying bustlines that are larger than their 'sisters', and usually are hyper-endowed with male genitalia.");
        this.headerSub("History");
        this.outputText("Before the corruption truly began, the Shark people were a normal fishing community that lived by the lake. They respected and admired marine life so much that they used magic to morph their forms, allowing them to live both under the sea and on dry land. As the demons began to take control, the Shark people retreated into the lake water to avoid the taint. It was only through sheer bad luck that they wound up as they are now; when the factory was constructed, the chemical run-off was funneled into the lake, causing a drastic change to the mindset of the Shark people and making them near-constantly horny. Those who swam too close to the pollutants found their bodies morphed in unexpected ways, becoming what are now known as tiger shark girls. Even if the factory were to be destroyed, it would take generations for the shark girls to fully lose the effects.");
        this.setCodexMenusAfterDisplay();
    }

    private codexEntrySuccubus(): void {
        this.flags[kFLAGS.CODEX_CURRENT_ENTRY] = "Succubus";
        this.clearOutput();
        this.headerMain("Succubus");
        this.outputText("<b>Genders:</b> Exclusively female.\n");
        this.outputText("<b>Height:</b> Varies\n");
        this.outputText("<b>Hair:</b> Varies\n");
        this.outputText("<b>Eyes:</b> Varies\n");
        this.headerSub("Appearance");
        this.outputText("Succubi ALWAYS appear as attractive, and often slutty women, but they will modify the style of their dress to match what would be expected of a human woman in their current situation. In their true form they often have at least one pair of horns that grow larger and longer as they age. Some succubi have secondary and tertiary horn rows that curve back over their skull. Their skin is always flawless and their teeth white. In their true form their skin ranges from blue, to purple, to rarely midnight black.");
        this.headerSub("Sexual equipment");
        this.outputText("Succubi have what could be described as ‘the perfect vagina’. While it appears to be tight, it has the ability to stretch far beyond what should be possible for such a small piece of flesh. This ensures that no matter how large or small the succubus partner, they are able to enter her and receive enough friction. The vaginal surface is highly malleable and succubi will often add textured rings or wriggling cilia to their internal walls in order to drive their victims to even greater heights of pleasure.");
        this.headerSub("Reproduction");
        this.outputText("Succubi do not reproduce, instead they are created when a mortal woman because so drunk on desire and pleasure that she loses care for anything but her ability to seek pleasure and beauty. The mother succubus will work her victim into a frenzy of desire, until the unsuspecting victim literally cums out her soul and fills the void inside her with corruption. It should be noted that succubi can do this to a male as well, creating an incubus.");
        this.headerSub("Social structure");
        this.outputText("Succubi and Incubi occupy the second lowest ranking of the demonic social structure, just above imps. The are prone to sloth when not ‘hungry’ and are typically never given more power than command over a few imps.");
        this.headerSub("Special attributes");
        this.outputText("A Succubus' Milk can be an extraordinarily powerful aphrodisiac when drank straight from the source. If it is stored, the aphrodisiacal properties rapidly diminish while its mutagenic properties rapidly rise. Some more powerful succubi are able to use their milk with black magic in order to work changes on their partners.");
        this.headerSub("Feeding");
        this.outputText("Like most demons, Succubi gain power from corrupting others, and require a steady supply of sexual fluids to survive. In rare cases, if a Succubus has been deprived of ‘food’ for too long, she may find herself too weak to seduce a partner, and enter into a less than advantageous agreement in order to get her food. Amazingly, most succubi will hold themselves to such an agreement after 'eating' their fill. ");
        this.setCodexMenusAfterDisplay();
    }

    private codexEntryBehemoth(): void {
        this.flags[kFLAGS.CODEX_CURRENT_ENTRY] = "Behemoth";
        this.clearOutput();
        this.headerMain("Behemoths");
        this.outputText("<b>Genders:</b> Male.\n");
        this.outputText("<b>Height:</b> Typically 8 to 10 feet, but 11 feet is not unheard of.\n");
        this.outputText("<b>Build:</b> Muscular and athletic\n");
        this.outputText("<b>Skin tone:</b> Purple\n");
        this.outputText("<b>Hair color:</b> Usually blonde or black.\n");
        this.outputText("<b>Eye color:</b> Yellow\n");
        this.outputText("<b>Notable features:</b> Horns on head, spikes protruding from back and end of tail.\n");
        this.headerSub("Appearance");
        this.outputText("Behemoths typically wear little clothing, such as loincloth. The height, horns and spikes all add up to their fearsome appearance that most of the other races don't even dare to approach them.");
        this.headerSub("Reproduction");
        this.outputText("Like the satyrs, the behemoth are all-male race. Unlike satyrs, they are incapable of impregnating males. To reproduce, they rely on the females of other races as long as they are pure. Gestation period typically lasts two months, much longer than the typical gestation period in Mareth, and it takes two years for the newborn behemoth to fully mature into an adult.");
        this.headerSub("Social structure");
        this.outputText("Behemoths typically live in villages and caves. The leadership of tribes are determined by which is the strongest of all. Once a year, they host \"Feats of Strength\", a contest consisting of several gauntlets and competitions. Among one of them is the \"Cum-Out\" where the objective is for the behemoths to cum as much as they can in a single orgasm.");
        this.setCodexMenusAfterDisplay();
    }

    private codexEntryZebras(): void {
        this.flags[kFLAGS.CODEX_CURRENT_ENTRY] = "Zebras";
        this.clearOutput();
        this.headerMain("Zebra-morphs");
        this.outputText("<b>Genders:</b> Male, Female\n");
        this.outputText("<b>Height:</b> 5 feet 5 inches to 6 feet\n");
        this.outputText("<b>Build:</b> Lean/slender\n");
        this.outputText("<b>Skin:</b> Zebra pattern\n");
        this.outputText("<b>Fur:</b> Zebra pattern with mohawk-style mane\n");
        this.outputText("<b>Eyes:</b> Brown to black\n");
        this.outputText("<b>Diet:</b> Herbivore, specializes on grass, but herbs and shrubs are part of their diet\n");
        this.headerSub("Appearance");
        this.outputText("The head and face is similar to that found on a zebra, though the nose is shorter. The mane looks like mohawk which begin at hairline and extends to middle of back. Legs are digitigrade ending in hooves. Long tail with a tuft of hair at end.Upper body in human (arms, hands, fingers, breasts).");
        this.headerSub("Reproduction");
        this.outputText("Typical reproduction. The stallion treats all women with the attention. When a foal is born, the foal is treated with the same rank his mother has.");
        this.headerSub("Social structure");
        this.outputText("Zebras are polygamous, one stallion with several mares and young offspring form a harem. Before beginning a harem, males are in bachelor groups, females are in natal harems composed of females only until taken into a stallion's harem.\n");
        this.outputText("The rank among mares in a harem is the order they were brought into it. When a new mare is brought into the harem, the other members treat her with disdain, making sure she knows her place, the bottom.\n");
        this.outputText("The acquisition of a new mare to a harem is the stallion impregnation of the mare.\n");
        this.outputText("Stallions are passive in most instances except in dealing with others trying to take away part or all of the stallion’s harem, or conflicts when trying for a new mare for his harem. Other causes exist, of course.");
        this.headerSub("Natural enemies");
        this.outputText("Zebra-morphs show great anxiety when near lion-morphs since millennia have passed where they were the prey to the lion-morphs.");
        this.outputText("\n\n(Written by: Rycharde) ");
        this.setCodexMenusAfterDisplay();
    }

    private codexEntryBasilisks(): void {
        this.flags[kFLAGS.CODEX_CURRENT_ENTRY] = "Basilisks";
        this.clearOutput();
        this.headerMain("Basilisks");
        this.outputText("<b>Genders:</b> Male, though rumours of female and herm basilisks re-emerging are spreading.\n");
        this.outputText("<b>Height:</b> Basilisk tend to grow up to 6 foot 8, with rare basilisks being to be taller. Around the 6 foot 2 mark is"
            + " where they grow to on average.\n");
        this.outputText("<b>Build:</b> Basilisks tend to have thin and wiry frames, taut with muscles. These powerful and lean predators,have slender"
        ...
        this.outputText("<b>Hair:</b> In place of hair Basilisks have a crown of dull spines. Their rubbery texture and darker coloured tips makes"
        ...
        this.outputText("<b>Skin:</b> Basilisks have scales which tend to be a grey-green color, while their underbelly is a yellow color. However"
            + " other scale colors do occur. Green and purple is a combination that high ranking basilisks tend to have.\n");
        this.outputText("<b>Facial structure:</b> Basilisks have a reptilian face with somewhat blunt snout. Their eyes tend to be grey, though higher"
        ...
        this.headerSub("Appearance");
        this.outputText("Basilisks look similar to Lizans, but instead have thin and muscular frames with green-grey scales, sharp sickle like claws"
        ...
        this.headerSub("Reproduction");
        this.outputText("Basilisks do not tend to breed often, usually only breeding for survival. To breed, due to being only male, they must force"
        ...
        this.headerSub("History");
        this.outputText("Despised by lizard and bird races for their proclivity for egg stealing, the basilisks chose to ally themselves with the"
        ...
        this.headerSub("Petrification");
        this.outputText("Before the demons, basilisks could only paralyse others with their gaze. But once the demons had had their way, the gaze of a"
    ...
    private codexEntryCockatrices(): void {
        this.flags[kFLAGS.CODEX_CURRENT_ENTRY] = "Cockatrices";
        this.clearOutput();
        this.headerMain("Cockatrices");
        this.outputText("<b>Genders:</b> Male, female and hermaphrodite. On average 55% of the cockatrices are male, with 35% being female"
            + " and 10% being natural herms.\n");
        this.outputText("<b>Height:</b> Cockatrices can grow between 5 foot 7 and 6 foot 8,"
            + " though their long tails and feathered ears tend to make them look larger.\n");
        this.outputText("<b>Build:</b> Cockatrices tend to have light and athletic builds, with their muscle mass being noticeable particularly"
        ...
        this.outputText("<b>Hair/Feathers:</b> Cockatrices have a layer of feathers coating most of their body, leaving only the belly, tail, forearms"
        ...
        this.outputText("<b>Skin tone:</b> Cockatrice skin tones are unknown. See ‘Hair/Feathers’ for closest info\n");
        this.outputText("<b>Facial structure:</b> Cockatrices have bird like faces complete with beaks. Their face is covered in the same feathered"
        ...
        this.headerSub("Appearance");
        this.outputText("Cockatrices have vibrant feathers covering the majority of their body, with only the belly, forearms, calves and tail being"
        ...
        this.headerSub("Reproduction");
        this.outputText("Female Cockatrices lay eggs, usually in a clutch of up to 4, once every 3 months. While any male can try to fertilize"
        ...
        this.headerSub("Petrification");
        this.outputText("While Cockatrice’s share the petrifying gaze of their basilisk father's, they use it very differently."
