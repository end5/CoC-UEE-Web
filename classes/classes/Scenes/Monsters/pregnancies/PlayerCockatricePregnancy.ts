	...
	/**
	 * Contains pregnancy progression and birth scenes for a Player impregnated by Cockatrice.
	 */
	export class PlayerCockatricePregnancy implements VaginalPregnancy
	{
		private  output:GuiOutput;
		private  pregnancyProgression:PregnancyProgression;
		
		/**
		 * Create a new Cockatrice pregnancy for the player. Registers pregnancy for Cockatrice.
		 * @param	pregnancyProgression instance used for registering pregnancy scenes
		 * @param	output instance for gui output
		 */
		public  PlayerCockatricePregnancy(pregnancyProgression:PregnancyProgression, output:GuiOutput) 
		{
			// needed as an instance variable for refactor test code (detectVaginalBirth)
			this.pregnancyProgression = pregnancyProgression;
			this.output = output;
			
			pregnancyProgression.registerVaginalPregnancyScene(PregnancyStore.PREGNANCY_PLAYER, PregnancyStore.PREGNANCY_COCKATRICE, this);
		}
		...
		{
			pregnancyProgression.detectVaginalBirth(PregnancyStore.PREGNANCY_COCKATRICE);
			kGAMECLASS.highMountains.cockatriceScene.cockatriceBirth();
		}
	}
