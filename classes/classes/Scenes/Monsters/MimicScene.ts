//Mimic, one of the classic encounters from Unnamed Text Game, now modified to be up to modern standards, with new fight.
	...
	export class MimicScene extends BaseContent
	{
		public  mimicAppearance: number;
		
		public  MimicScene() {}
			
		public  mimicTentacleStart(loc: number = 0): void {
			clearOutput();
			//Random appearance :D
		...
		public  mimicTentacle1(): void
		{
			...
			if (rand(100) + Math.floor(player.inte / 1.5) >= 80) {
				...
				var  itemRoll: number = rand(4);
					...
					menu();
					addButton(0, "Fight", function(): any{
		...
		public  findSomeGems(): void {
		var  quantity: number = (rand(30) + 10 + player.level) * (1 + (player.perkv1(PerkLib.AscensionFortune) * 0.1));
