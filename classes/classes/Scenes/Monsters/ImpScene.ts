 ...
 */
	...
	{
		public  ImpScene(pregnancyProgression:PregnancyProgression, output:GuiOutput)
		{
		...
		private  rapeImpWithDick(condomed: boolean = false): void {
		...
		*/
		...
		public  impGangGetsWhooped(): void {
			clearOutput();
		...
		public  impGangabangaEXPLOSIONS(loss: boolean = false): void {
			player.slimeFeed();
		...
		var  titsOrgasm: boolean = false;
			...
			//NOT CENTAUR
				...
				if (rand(2) == 0 && (player.cockTotal() == 0 || player.gender == 3)) {
					...
					outputText("Others stand over your head, their cocks bobbing inches from your face as they jack off. A thick musk wafts off their cocks, and the smell of it makes your sinuses tingle. Two more imps take position between your legs, sliding their cocks along your thighs while stroking your " + player.vaginaDescript(0) + " and flicking your " + player.clitDescript() + ".");
					//(If the player has a cock)
			...
			if (player.lust >= player.maxLust()) {
				...
				outputText(images.showImage("monster-imp"));
					...
					if (player.gender == 1 && rand(2) == 0) {
						...
						if (player.isTaur()) outputText("hocks and knees, your " + player.cockDescript(0) + " hurting from the massive blood pressure caused by your unbridled lust. He approaches you and stops about two feet in front of you, watching with delight your helpless state");
		...
		//IMP WARLORD
		public  impWarlordEncounter(): void {
		...
		//IMP OVERLORD
		public  impOverlordEncounter(): void {
			clearOutput();
		...
		public  loseToAnImpLord(overlord: boolean = false): void {
			clearOutput();
			if (flags[kFLAGS.SFW_MODE] > 0) {
		...
		private  impLordRapeMale(overlord: boolean): void {
			clearOutput();
		...
		//FEMALE LOSE
		private  impLordRapeFemale(overlord: boolean): void {
			clearOutput();
			if      (doSFWloss())  return;
		...
		private  impPetrifyable(): boolean
		{
			return player.isBasilisk() && flags[kFLAGS.CAMP_WALL_PROGRESS] >= 100;
		}

		private  addKillPetrifyButtons(killButtonPos: number = NaN, petriButtonPos: number = NaN): boolean
		{
		...
		private  takeStatue(): void
		{
		var  timeToReturn: number = 1;
