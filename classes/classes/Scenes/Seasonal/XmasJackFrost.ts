...
export class XmasJackFrost extends BaseContent {
    ...
    public meetJackFrostInTheMountains(): void {
        ...
        this.addButton(2, "Later", this.noJizzingInMyCampRightNow);
    }

    // [=Later=]
    public noJizzingInMyCampRightNow(): void {
        this.clearOutput();
        this.outputText("You thank the giant for his offer, but you'll have to decline for now. Maybe next time.");
    ...
    public noJizzingInMyCampPlease(): void {
        this.clearOutput();
        this.outputText("You thank the giant for his offer, but you'll have to refuse.  You really can't take even one day off your quest.");
        this.outputText("\n\n\"<i>Pity... well, if you'll excuse me, this mountain ain't snowy enough just yet!</i>\"  He resumes his furious masturbation, spraying another gush of snow on the side of the mountain.");
    ...
    public processJackFrostEvent(): void {
        ...
        if (this.flags[kFLAGS.JACK_FROST_PROGRESS] == 1) {
        ...
        // No followers
            ...
            this.outputText("You sigh... this really reminds you of back home... you only wish you had someone to share this feeling with... Well, there is no reason you shouldn't enjoy yourself while it snows, so you set about rolling a big ball of snow to make a snowman out of...");
            // Skip to next day...
        ...
        // Rathazul
        else if (this.player.hasStatusEffect(StatusEffects.CampRathazul) && this.flags[kFLAGS.JACK_FROST_PROGRESS] == 2) {
            this.spriteSelect(SpriteDb.s_rathazul);
            this.outputText("Rathazul approaches you.  \"<i>[name]?  What is going on?  It hasn't snowed in Mareth for years now.  And why only around the camp?  I wandered out to investigate, but outside there is no snow at all...</i>\" he asks, concerned.");
        ...
        // Izma
        else if (this.flags[kFLAGS.IZMA_FOLLOWER_STATUS] == 1 && this.flags[kFLAGS.JACK_FROST_PROGRESS] <= 3) {
            this.spriteSelect(SpriteDb.s_izma);
            this.outputText("You wonder where Izma is; as a shark, maybe she doesn't like the cold?  Even as the thought crosses your mind, you note a surprisingly large pile of snow sitting not too far - with a very suspicious striped fin jutting out of it.  You rush over, calling for Izma and asking if she's all right.");
        ...
        // Jojo
        else if ((this.player.hasStatusEffect(StatusEffects.PureCampJojo) || this.getGame().jojoScene.campCorruptJojo()) && this.flags[kFLAGS.JACK_FROST_PROGRESS] <= 4) {
            ...
            if (this.player.hasStatusEffect(StatusEffects.PureCampJojo)) {
                if (this.flags[kFLAGS.JOJO_BIMBO_STATE] < 3) {
                    this.outputText("Jojo is sitting on his usual rock, one hand out to catch snowflakes with a beatific smile of awe on his face.  \"<i>Look, [name], snow!  I haven't seen snow since I was a very, very small boy.</i>\"  He tells you.");
                    ...
                    this.outputText("Joy is sitting on her usual rock, one hand out to catch snowflakes with a beatific smile of awe on her face.  \"<i>Look, [name], snow!  I haven't, like, seen snow since I was a very, very small mouse!</i>\"  She tells you excitedly.");
            ...
            // Corrupt
                ...
                this.outputText("Out in the outskirts of the camp you notice your pet mouse-slut Jojo, casually fucking a small mound of snow like an animal.  You laugh to yourself, now, now... doesn't he know that there are better things to fuck than a pitiful mound of snow on the ground...");
                ...
                this.addButton(4, "Leave Him", this.skipCorruptJojo);
            }
            return;
        }
        // Amily
        // less than 5!
        else if (this.getGame().amilyScene.amilyFollower() && this.flags[kFLAGS.JACK_FROST_PROGRESS] <= 5) {
            this.flags[kFLAGS.JACK_FROST_PROGRESS] = 6;
            kGAMECLASS.amilyScene.amilySprite();
            // Pure
            if (!this.getGame().amilyScene.amilyCorrupt()) {
                this.outputText("Amily is warily prowling across the icy surface, occasionally glancing all around as if trying to spot an ambush.  Other times, she suddenly leaps up in a flurry, whirling around in mid-air as something catches her over-large ears.  All in all, it's quite obvious she's scared.");
                ...
                this.outputText("\n\nIt's not really possible to smirk and kiss at the same time, but your murine lover does her level best to try.  Breaking the kiss with a smile, she tugs you gently towards your " + this.camp.homeDesc() + ".  \"<i>Come on then, lover-" + this.player.mf("boy", "girl") + ", let's go and get warmed up...</i>\" She coos.  Her hands start to remove your clothes, \"<i>So, what do you have in mind, exactly...?</i>\"");
            ...
            // Corrupt
                ...
                this.outputText("Out in the outskirts where your favorite cum-slut lives, you spot her slowly rubbing the snow over her erect nipples, the cold making her shiver in pleasure.");
                ...
                if (this.player.gender > 0) {
                    ...
                    if (this.player.hasCock()) {
                        this.outputText("\n\nYou strip off your lower garments, exposing your " + this.player.multiCockDescriptLight() + " to the hungry eyes of your cumslut.  You tell her you would like her to give you a \"<i>hand</i>\", with the extraction process...");
                        ...
                        if (this.player.balls > 0) this.outputText(", even tickling your [balls]");
                        else if (this.player.hasVagina()) this.outputText(", even tickling your [cunt] for added tantalization");
                        this.outputText(".");
        ...
        // Tainted Ember
        else if (this.getGame().emberScene.followerEmber() && this.flags[kFLAGS.JACK_FROST_PROGRESS] <= 6) {
            this.flags[kFLAGS.JACK_FROST_PROGRESS] = 7;
            kGAMECLASS.emberScene.emberSprite();
            this.outputText("The dragon is wandering your camp in open fascination.  \"<i>What is this weird white stuff?  It's cold and wet... but soft and fluffy, too,</i>\" [ember ey] says, gently catching a snowflake.");
        ...
        // Marble (Written by TDM himself)
        else if (this.flags[kFLAGS.JACK_FROST_PROGRESS] <= 7 && this.player.hasStatusEffect(StatusEffects.CampMarble)) {
            this.flags[kFLAGS.JACK_FROST_PROGRESS] = 8;
            kGAMECLASS.marbleScene.marbleSprite();
            // With Kids
            if (this.flags[kFLAGS.MARBLE_KIDS] > 0 && this.flags[kFLAGS.MARBLE_NURSERY_CONSTRUCTION] >= 100) {
                this.outputText("You find Marble simply staring into the sky, a look of wonder covering her face.  Dancing around her in excitement ");
                if (this.flags[kFLAGS.MARBLE_KIDS] > 1) this.outputText("are ");
                else this.outputText("is ");
                this.outputText("your kid");
                if (this.flags[kFLAGS.MARBLE_KIDS] > 1) this.outputText("s");
                this.outputText(", playing in the snow, blowing the snow, and experimenting with the snow.  Exactly what ");
                if (this.flags[kFLAGS.MARBLE_KIDS] == 1) this.outputText("she");
                else this.outputText("they");
                this.outputText(" are up to at any given moment is really hard to say, and you're not even sure that the little girl");
                if (this.flags[kFLAGS.MARBLE_KIDS] > 1) this.outputText("s");
                this.outputText(" know");
                if (this.flags[kFLAGS.MARBLE_KIDS] == 1) this.outputText("s herself.");
                else this.outputText(" themselves.");
                ...
                this.outputText(" playing together once more.  A moment later, Marble puts her hand to her expansive backend again, before suddenly exclaiming again in realization.  \"<i>Sweetie, watch our kid");
                if (this.flags[kFLAGS.MARBLE_KIDS] > 1) this.outputText("s");
                this.outputText(" for a moment, I'll be right back!</i>\" and she leaves the camp as quickly as she can, without slipping and falling on her ass again.");

                this.outputText("\n\nYou certainly don't mind staying behind for now, it is certainly a lot of fun spending time with your kid");
                if (this.flags[kFLAGS.MARBLE_KIDS] > 1) this.outputText("s");
                this.outputText(" and just having fun.  Your cow-girl daughters are certainly not lacking in energy in the cold snow, their fur covered legs and built in shoes probably help a lot with this.  ");
                if (this.flags[kFLAGS.MARBLE_KIDS] > 1) this.outputText("They");
                else this.outputText("She");
                this.outputText(" listen");
                if (this.flags[kFLAGS.MARBLE_KIDS] == 1) this.outputText("s");
                this.outputText(" with rapt attention when you explain what snow is, and about all the things you use to do in it back home.  Hands-on demonstrations are also in high demand.");
                ...
                if (this.flags[kFLAGS.MARBLE_KIDS] > 1) this.outputText("s");
                this.outputText(" to ride on!  Happy cries of joy fill the air for some time, as you and Marble take turns in giving rides on the sled.");

                // PC strength check:
                // (>=85)
                if (this.player.str >= 85) {
                    this.outputText("\n\nIt would seem that your strength has won you more favor with ");
                    if (this.flags[kFLAGS.MARBLE_KIDS] > 1) this.outputText("the girls");
                    else this.outputText("your daughter");
                    this.outputText(" when it comes to giving sled rides, Marble doesn't seem to mind too much though.  Your mate actually decides to join in the riding part after a while!");
                }
                // (60-84)
                    ...
                    this.outputText(" in deciding who gives the best rides.  In the end, a clear winner is never decided, but everyone agrees that it was a lot of fun.");
                }
                // (<=59)
                else {
        ...
        // if she ain't a follower and is a lover, just have her visit.
        else if (this.flags[kFLAGS.JACK_FROST_PROGRESS] <= 9 && (this.getGame().helScene.fuckBuddyHel() || this.getGame().helFollower.followerHel())) {
            kGAMECLASS.helScene.spriteChooser();

            this.outputText("\"<i>Hey, champ, what's going on here?</i>\"  Helia calls as she sees you approach.  Gentle trails of steam are wafting from the salamander's body as her own internal heat melts any snow that builds up on her.");
            ...
            // (Speed check)
            if (this.player.spe / 20 + rand(20) > 11) {
                ...
                this.outputText("\n\nShe laughs as she surfaces amidst a spray of slush, whipping her long hair around.  \"<i>That's the way to do it, lover-" + this.player.mf("boy", "girl") + "!</i>\" she crows.");
            }
            else {
            ...
            if (this.player.gender > 0) {
                this.outputText(", as evidenced by ");
                if (this.player.hasCock()) this.outputText("your erect " + this.player.multiCockDescriptLight());
                if (this.player.gender == 3) this.outputText(" and ");
                else if (this.player.hasVagina()) this.outputText("your ");
                if (this.player.hasVagina()) this.outputText("dripping [vagina]");
            }
            this.outputText(".");
            ...
            // (If PC has a cock)
            if (this.player.hasCock()) this.outputText("  A torrent of seed splashes against Helia's taut belly, as [eachCock] throbs and spews its seed while sandwiched between the two of you.");
            // (if PC has a pussy)
            if (this.player.hasVagina()) this.outputText("  Your [vagina] quivers and spill your juices with the force of a wave.  Moisture falls on the ground to join in with Helia's own discharge.");
        ...
        // Isabella
        else if (this.getGame().isabellaFollowerScene.isabellaFollower() && this.flags[kFLAGS.JACK_FROST_PROGRESS] <= 10) {
            this.spriteSelect(SpriteDb.s_isabella);
            this.outputText("The bovine adventurer Isabella is staring at the snow around her with a sad look on her face, one hand gently raised to catch snowflakes.  She watches them gather in her palm and heaves a huge sigh.  You quietly approach her and ask what's wrong; is she homesick?");
            this.outputText("\n\n\"<i>");
            if (this.getGame().isabellaFollowerScene.isabellaAccent()) this.outputText("Da, [name], I am,");
            else this.outputText("Yes, [name], I am,");
            this.outputText("</i>\" she replies.  ");
            if (this.getGame().isabellaFollowerScene.isabellaAccent()) this.outputText("\"<i>Back home, in mine land, it vould be snowing all over at this time of year, and ve vould be having big vinter festival to celebrate ze ending of old year and coming of new year.  This land, it is so dry and hot...  I had forgotten it, but seeing zis snow makes me remember it and all I have lost by being stuck here.</i>\"  She sighs.");
            else this.outputText("\"<i>Back home, in my old land, it would be snowing all over at this time of year, and we would be having a big winter festival to celebrate the ending of the old year and the coming of the new one.  This land, it's so dry and hot...  I had forgotten it, but seeing this snow makes me remember it and all I have lost by being stuck here.</i>\"  She sighs.");
        ...
        // Kiha
        else if (this.getGame().kihaFollower.followerKiha() && this.flags[kFLAGS.JACK_FROST_PROGRESS] <= 11) {
            kGAMECLASS.kihaFollower.kihaSprite();
            this.outputText("\"<i>Ah-ah-ahchoo!</i>\"");
        ...
        // Feast time! Just a collection of words on the pleasant day...
            ...
            this.outputText("This small holiday of yours was much needed.  You feel reinvigorated and even more determined to put an end to this struggle with demons.");
            ...
            if (this.camp.loversCount() + this.camp.slavesCount() == 1) {
            ...
            else if (this.camp.loversCount() + this.camp.slavesCount() == 0) {
                this.outputText("\n\nYou take care of the preparations and cooking, whipping up a delightful meal for you and your companion");
                if (this.camp.companionsCount() > 1) this.outputText("s");
                this.outputText(".  You can tell that you were not the only one needing this break... but unfortunately all good things must come to an end, and so you bid your companion");
                if (this.camp.companionsCount() > 1) this.outputText("s");
                this.outputText(" good night and retire for the night.");
            }
            //// more than one sexable follower.
            else {
                this.outputText("You take care of the preparations and cooking, whipping up a delightful meal for you and your companions.  For a moment you look around - this small band you've formed feels enough like a family that you wouldn't mind having more days like this in the future.  You vow to work extra hard to make this a reality.");
    ...
    public kihaXmasFuck(): void {
        ...
        this.outputText("Stripping off your [armor], you tell the dragon-girl that a good workout is all she needs to get nice and warm.  She snorts in dismissal, but the way she's looking at you is a pretty good indication she's willing to try it.  \"<i>So, what do you have in mind?</i>\" she blusters.  You stroke your quickly erecting " + this.player.multiCockDescriptLight() + " and, smirking, ask her if your intentions aren't obvious?");
        ...
        this.getGame().kihaFollower.kihaKnockUpAttempt();
        this.player.orgasm('Dick');
        this.doNext(this.processJackFrostEvent);
    }
    // [=Lick Boobs and Finger Pussy=]
    // All sexes!
    public kihaXmasFingering(): void {
        this.clearOutput();
        kGAMECLASS.kihaFollower.kihaSprite(true);
        this.outputText("You examine Kiha.  A small snowflake lands on one of her nipples and she yelps with a start at the sudden contact.   \"<i>Hey, I asked you to help warm me up, not to stare at me!</i>\" she complains. ");
    ...
    public penetradePureMouseWaifu(): void {
        ...
        if (x < 0) x = this.player.smallestCockIndex();
        this.outputText("Stripping down your [armor] and indicating Amily should do the same... you tell her that you were hoping for some good old-fashioned sex between a horny " + this.player.mf("man", "herm") + " and " + this.player.mf("his", "her") + " lovely mousette.  You playfully ask Amily if she has any objections?");
        this.outputText("\n\n\"<i>Well, it's a little counterproductive to take your clothes off if you want to get warm,</i>\" Amily notes.  Despite her words, she's stripping off without the slightest hesitation.  You reply that just the sight of her getting nude is already making you a little warmer... thrusting your hips so she can see your erect " + this.player.multiCockDescriptLight() + ".  \"<i>Flattering flirt; you always think with your dick, don't you?</i>\" the mouse laughs.  That's rich; you can see her pussy dripping from here");
        if (this.flags[kFLAGS.AMILY_WANG_LENGTH] > 0) this.outputText(", not to mention her dick's pretty hard, too");
        this.outputText(".");

        this.outputText("\n\nSitting down on your bedroll, you open your arms, inviting Amily over.  The mouse promptly approaches");
        if (this.flags[kFLAGS.AMILY_WANG_LENGTH] > 0) this.outputText(", her own " + this.getGame().amilyScene.amilyCock() + " jutting out in anticipation");
        this.outputText(", wriggling into your arms and angling her pussy over your " + this.player.cockDescript(x) + ".  With the ease of long practice, she slides smoothly down its length, her warm, wet depths greedily inhaling your masculinity.  \"<i>Ah, nothing like a nice hard cock in your cunt to make you feel warmer,</i>\" she coos.");
        ...
        if (this.flags[kFLAGS.AMILY_WANG_LENGTH] > 0) this.outputText(" and painting your belly white with futa-spooge");
        this.outputText(".");
    ...
    public makeOutWithPureMouseWaifu(): void {
        this.clearOutput();
        this.outputText("You strip down your [armor] and suggest Amily should do the same.  \"<i>Well, it's a little counterproductive to take your clothes off if you want to get warm,</i>\" Amily notes.  Despite her words, she's stripping off without the slightest hesitation.  \"<i>So, what's on your mind, lover-" + this.player.mf("boy", "girl") + "?</i>\" she asks.");
        ...
        if (this.flags[kFLAGS.AMILY_WANG_LENGTH] > 0) this.outputText("Smiling sheepishly you  tell her to mind the erection... though you're not actually looking for sex, you can't help with how cute she looks...  \"<i>Flirt.</i>\"  Amily giggles, but otherwise doesn't comment on it.  ");
        this.outputText("The mouse promptly walks over and sits down just in front of you, her back facing you and giving you a perfect opportunity to hug her.  Not keen on missing such an opportunity, you do so.");
    ...
    public penetradeCorruptJojoAss(): void {
        ...
        if (this.player.cumQ() >= 700) this.outputText(" all over his own cum-engorged gut and ");
        this.outputText("onto the snowy earth underneath him.  He cums and cums until he's exhausted even his prodigious balls, creating a salty quagmire of semi-melted snow and spooge under him that he promptly collapses face-first into.");
    ...
    public rideCorruptJojoCock(): void {
        ...
        if (this.player.hasCock()) this.outputText(", your own futa-cock spewing its load, which you deliberately spray in his face for amusement");
        this.outputText(".  Your orgasm triggers Jojo's own; though he has cum without permission, at least he waited for you to cum first like a good slut should.  Rope after rope of mousy sperm bats your cervix, trying to get past the barrier and into the waiting womb inside, even as Jojo tries his best to fuck you over and over as he spills his load, until he finally slumps into the ground, panting, with his mouth open and his tongue lolling out, eyes glazed over in a pleasured daze.");
    ...
    public spankNFingerCorruptJojoAss(): void {
        ...
        this.dynStats("lus=", this.player.maxLust(), "scale", true);
        this.flags[kFLAGS.JACK_FROST_PROGRESS] = 5;
        this.doNext(this.processJackFrostEvent);
    }
    ...
    // [=Skip Jojo=]
    // Available to all.
    public skipCorruptJojo(): void {
        this.clearOutput();
        this.outputText("You shake your head and leave Jojo back to fucking the snow mound..");
        this.flags[kFLAGS.JACK_FROST_PROGRESS] = 5;
        this.doNext(this.processJackFrostEvent);
    }
}
