...
export class Thanksgiving extends BaseContent {

    public isItThanksgiving(): boolean {
        return ((this.date.date >= 21 && this.date.month == 10) && (this.date.date < 30 && this.date.month == 10) || this.flags[kFLAGS.ITS_EVERY_DAY] > 0);
    }

    public datTurkeyRumpMeeting(): void { // Introduction: -McGirt
        if (this.flags[kFLAGS.TURKEY_FUCK_YEAR_DONE] > 0) {
            this.turkeyGirlTwoTheTurkeningBySavinWhatADickInAButt();
            this.flags[kFLAGS.TURKEY_FUCK_YEAR_DONE] = this.date.fullYear;
            return;
        }
        this.flags[kFLAGS.TURKEY_FUCK_YEAR_DONE] = this.date.fullYear;
        this.outputText(this.images.showImage("thanksgiving"));
    ...
    public letZeTurkeyApproach(): void { // Let her Approach
        ...
        this.menu();
        this.addButton(0, "Baste Her", this.basteThatTurkeyBooty);
        this.addButton(1, "No Thanks", this.noThanksTurkeyGal);
    }
    ...
    public basteThatTurkeyBooty(): void { // Baste Her -McGirt, reluctantly
        ...
        this.outputText(this.images.showImage("thanksgiving-baste"));
        ...
        this.dynStats("sen", -1);
        this.doNext(this.basteThatTurkeyBootyII);
    }
    public basteThatTurkeyBootyII(): void {
        this.clearOutput();
        this.outputText(this.images.showImage("thanksgiving-basteII"));
        this.outputText("\"<i>G-gobble?</i>\"");
    ...
    public stuffDatTurkeyWithSpoo(): void { // Stuff Her -McGirt, this is awful by the way.
        ...
        this.outputText("\n\n...Gravy?");
        this.outputText(this.images.showImage("thanksgiving-stuff"));
        ...
        if (this.flags[kFLAGS.HEL_FUCKBUDDY] == 1 && this.flags[kFLAGS.HEL_REDUCED_ENCOUNTER_RATE] == 0) {
            ...
            this.menu();
            this.addButton(0, "Spit-Roast", this.spitRoastThatTurkey);
            this.addButton(1, "That's Enough", this.thatllDoTurkey);
        }
        else this.doNext(this.thatllDoTurkey);
    }

    public spitRoastThatTurkey(): void { // SPITROAST HER! GangHel Style / (Needs friendly/lover Hel)
        this.clearOutput();
        this.outputText("As you contemplate what to do with the insatiable turkey-girl, you hear another rustling at the edge of camp.  For a moment both worried and aroused by the thought of a whole flock of these sluts, you glance up in time to see a certain salamander poking her way through your bevy of traps.");
        ...
        this.outputText(this.images.showImage("thanksgiving-spit-roast"));
    ...
    public femaleLetZeTurkeyGalApproach(): void { // Let her Approach
        this.clearOutput();
        this.outputText(this.images.showImage("thanksgiving-approach"));
        ...
        this.menu();
        this.addButton(0, "Appetizer", this.turkeyAppetizer);
        this.addButton(1, "Main Course", this.femaleAndTurkeyMainCourse);
        this.addButton(2, "Dessert", this.haveTurkeyGirlDesertInYourCunt);
    }

    public turkeyAppetizer(): void { // Appetiser
        this.clearOutput();
        this.outputText("Biting back a moan as the girl continues to tease you, your own hands reach out to her wrists, reluctantly pulling her hands away and placing them on her own thighs.  She seems disappointed as you shift in your seat, but before she can voice her displeasure you slide up to her, snaking an arm around her back as you start to play with her oversized rack.  She gasps slightly as you heft her plush breasts, kneading and squeezing them as your fingers circle around her hardening nipples.");
        ...
        this.outputText(this.images.showImage("thanksgiving-appetizer"));
    ...
    public femaleAndTurkeyMainCourse(): void { // Main Course
        this.clearOutput();
        this.outputText("Fully disrobed, the girl runs her hands down her chest, fingers clamping around her dribbling nipples once again.  With another squeeze, gravy is rolling down her chest once more, slathering her fingers with her own mouthwatering juices.  Unable to hold yourself back, your mouth clamps onto her breast, hands shooting to her soft, plush rear as you pull her bodily into your lap.  Her eyes almost light up in her new position, her hands running through your [hair] as you suck for all you're worth.  Her thick, rich gravy fills your mouth with its delicious mix of flavours.  There's so many different, wonderful tastes mixing together that you could almost believe you were back in Ingnam, gorging yourself on the gigantic spread that the village always strives to put on, regardless of how well the harvest went.");
        ...
        this.outputText(this.images.showImage("thanksgiving-main-course"));
    ...
    public haveTurkeyGirlDesertInYourCunt(): void { // DESERT
        this.clearOutput();
        this.outputText("Barely able to contain yourself, you nearly tear your [armor] from your burning body, desperate to feel the girl with your bare flesh.  She reclines lazily on the ground, legs spreading wide as a hand moves from her dribbling tits, sliding its way down her toned stomach to her swollen lips.  She continues to tease herself whilst you struggle with your clothes, little moans and grunts of pleasure leaving you weak at the knees as she dreamily circles her reddening snatch with a finger.  Her other hand busies itself with her chest, pinching and squeezing just hard enough to release another spurt of gravy that rolls its way down the curve of her breast, dripping onto her belly in a thick glob.");
        ...
        this.outputText(this.images.showImage("thanksgiving-dessert"));
        ...
        if (this.player.findPerk(PerkLib.Cornucopia) < 0) {
            this.outputText("\n\n(<b>Gained Perk: Cornucopia - increases vaginal and anal capacities by 30.</b>)");
            this.player.createPerk(PerkLib.Cornucopia, 0, 0, 0, 0);
        }
        this.player.changeFatigue(-100); // Fatigue to 0?
        this.player.HPChange(3000, false); // HP set to full
        this.doNext(this.camp.returnToCampUseTwoHours);
    }

    public turkeyGirlTwoTheTurkeningBySavinWhatADickInAButt(): void { // Turkey Girl II: Return of the Cockgobbler (Cockwielders) / {Thanksgiving Day event, nat. Cockmeisters only.}
        this.clearOutput();
        this.outputText(this.images.showImage("thanksgiving"));
        ...
        this.doNext(this.turkeyGirlTwoTheTurkeningBySavinWhatADickInAButtII);
    }
    public turkeyGirlTwoTheTurkeningBySavinWhatADickInAButtII(): void {
        this.clearOutput();
        this.outputText(this.images.showImage("thanksgiving-encounter"));
        ...
        this.outputText("\n\n“<i>G-gobble?</i>”  she asks quietly, her big blue eyes pleading at you.");
        this.awardAchievement("Gobble Gobble", kACHIEVEMENTS.HOLIDAY_THANKSGIVING_II, true, true, false);
        this.menu();
    ...
    public helloAgain(): void {
        this.clearOutput();
        this.outputText("\"<i>Oh, hey, you,</i>\" you say as the seasonal little near-harpy waddles up.  You pat your thigh invitingly, letting Gobbles approach.");
        ...
        this.outputText(this.images.showImage("thanksgiving-hello-again"));
        this.outputText("\n\n\"<i>Gobble,</i>\" you answer, pushing her head down toward your [cock].  She giggles giddily as you free your prick, already hard enough to bop her on the nose as she buries her face in your groin.  The turkey takes a deep breath, wallowing in the potent musk of your " + this.player.multiCockDescriptLight() + ", venturing to lick along the side of your shaft.  You let out a soft moan, stroking her auburn locks as the turkey happily suckles on your prick, taking the head ");
        if (this.player.cockArea(0) <= 60) this.outputText("and down to the base");
        ...
        this.menu();
        this.addButton(0, "Next", this.gravyBoatDatTurkturk); // Gravy boat?
    }

    public gravyBoatDatTurkturk(): void { // Gravy Boat
        this.clearOutput();
        this.outputText("You cup the turkey's chin, pulling her up off your cock enough to get her looking up at you with her big, questioning eyes.  \"<i>Gobble?</i>\" she ventures nervously, straining against your hand to get back at your shaft.");
        ...
        if (this.player.cocks[0].cockThickness >= 5) this.outputText(", her stomach bulging obscenely with your inhuman girth");
        this.outputText(".");
        this.outputText(this.images.showImage("thanksgiving-hello-again"));
    ...
    public seasonHerDatTurkeyBitch(): void { // Season Her
        ...
        if (this.player.hasKeyItem("Deluxe Dildo") >= 0) this.outputText("Tamani's dildo");
        else if (this.player.hasItem(this.consumables.L_DRAFT)) this.outputText("a bottle of Lust draft");
        else if (this.player.hasItem(this.consumables.F_DRAFT)) this.outputText("a bottle of Fuck draft");
        else this.outputText("A FUCKIN ERROR");
        this.outputText(".  You pull it out and squeeze a copious load of the bubblegum pink aphrodisiac out into your hand.  The second it touches your skin, you feel a palpable warmth spreading out through your nerves, skin burning with sensitivity as the roiling, viscous substance pools in your cupped hand.  You can't help but shudder as the pink veno's effect reaches out through your body, setting your whole person to tingling - and especially your " + this.player.multiCockDescriptLight() + ", which rises to an almost painful hardness.");
        ...
        this.outputText("\n\nInstantly, you can see her pale flesh reddening, burning with desire.  Her fem-slime practically pours out of her over-excited pussy as her breath quickens, chest heaving her tits up and around your " + this.player.multiCockDescriptLight() + ".  Perfect! You pull your hand back out of her love-tunnel, slapping both her butt cheeks to wipe off the last of the lusty cream on her bare derriere, which flushes as red as her hair.  \"<i>Gobble!</i>\" she screams as you swat her ass, slamming her big hips down on your face, trying to get you to eat her needy box again even as she's panting and twitching, playing with her tits and your cock with shameless abandon.");
    ...
    public turkeyDesertBitches(): void { // Dessert
        this.clearOutput();
        this.outputText("\"<i>G-gobble!?</i>\" Gobbles whines at your moment of hesitation, wiggling her upraised botty at you enticingly, hot flesh jiggling just close enough to brush your " + this.player.multiCockDescriptLight() + " with her lust-coated cheeks.");

        this.outputText(this.images.showImage("thanksgiving-dessert-bitches"));
        ...
        if (this.player.biggestTitSize() > 1) this.outputText(", cupping a tit");
        this.outputText(" as the venom takes control, flushing your skin and forcing out all thoughts but one: fuck that turkey! There's nothing for it, now: you grab Gobbles' hips and plunge in");
        if (this.player.cockTotal() == 2) this.outputText(" with both cocks, one ramming into her abused cunny and the other right up her vulnerable little bumhole");
        else if (this.player.cockTotal() > 2) this.outputText(" with every cock you've got, half in the pink and half in the stink, spreading her as wide as you are in both barrels");
        this.outputText(", shocking her so much that she can't hold back, screaming and cumming on the spot! You groan, thrusting forward as her spasming muscles contract around your cock");
        if (this.player.cockTotal() > 1) this.outputText("s");
        this.outputText(", milking you as hard as she can while her orgasm overtakes her.  Gravy geysers out of her swollen tits as fem-cum splashes out around your deep-buried cock");
        if (this.player.cockTotal() > 1) this.outputText("s");
        this.outputText(", drenching your [hips] and [legs] in creamy girl-spunk, tinged pink by the poison you fist-fucked up her cunt.");

        this.outputText("\n\nThe squawking turkey seems to be depthless, easily swallowing up every inch of your " + this.player.multiCockDescriptLight() + " until ");
        if (this.player.cockArea(0) >= 50) {
            this.outputText("her stomach is bulging obscenely with the sheer volume of cockflesh rammed up her cunt");
            if (this.player.cockTotal() > 1) this.outputText("and ass");
        }
        else this.outputText("your hips are slapping into hers, making her butt bounce with the impact");
        this.outputText(".  She cries her one word to the heavens as you pound into her hole");
        if (this.player.cockTotal() > 1) this.outputText("s");
        this.outputText(", thrusting your cock");
        if (this.player.cockTotal() > 1) this.outputText("s");
        this.outputText(" deep inside.  Her avian legs break free of her hold, wrapping around your [hips] and pulling you in, locking you in her spasming hole");
        if (this.player.cockTotal() > 1) this.outputText("s");
        this.outputText(" as she cums and cums, pleading, \"<i>Gobble!</i>\" at the top of her lungs.  You switch your grip to her bountiful bosom, roughly squeezing her tits as you hump your cock");
        if (this.player.cockTotal() > 1) this.outputText("s");
        this.outputText(" in and out of her slick slit{and quivering behind}, pinching her nipples to give the squealing turkey just a little more pleasure.");
        ...
        if (this.player.cockTotal() > 1) this.outputText(" and ass");
        this.outputText(", her legs spurring you on every time you withdraw, muscles clenching when you pause, never letting you rest.  Needy slut! But she's more than willing to do her part, thrusting her hips up to meet you, squeezing and milking your cock");
        if (this.player.cockTotal() > 1) this.outputText("s");
        this.outputText(" every time you pierce her, working your shaft");
        if (this.player.cockTotal() > 1) this.outputText("s");
        this.outputText(" so expertly that you know you won't be able to hold on for long - and neither will she, the way she's panting and gasping along with you!");

        this.outputText("\n\nYou draw nearer to the breaking point, hips unrelentingly slamming your shaft");
        if (this.player.cockTotal() > 1) this.outputText("s");
        this.outputText(" deep into Gobbles' eager depths, urged on by her happy squawks and gleeful cries.  Lust-venom-addled as the two of you are, orgasm approaches like a wave of relief, crashing through your bodies in beautiful unison.  \"<i>Gobbles!</i>\" she cries as her cunt ");
        if (this.player.cockTotal() > 1) this.outputText("and ass ");
        this.outputText("squirm around your cock");
        if (this.player.cockTotal() > 1) this.outputText("s");
        this.outputText(", wringing the cum from you with powerful spastic contraction.  Her arms pull you down into the valley of her gravy-laden bosom, squeezing your face between her massive mounds as she cums, drawing the spunk right out of you.  With a potent, feral roar, you cum, " + this.player.multiCockDescriptLight() + " blasting its salty load deep into Gobbles' womb");
        if (this.player.cockTotal() > 1) this.outputText(" and bowels");
        this.outputText("; her back arches, teats wobbling around your head as she takes your cum, squawking and gobbling gaily as more and more of your virile spunk pours into her, mixing with her own fem-cum and the remnants of the lusty draft your fucked into her.");

        this.outputText("\n\n\"<i>Gobble!</i>\" she sighs happily as your cock");
        if (this.player.cockTotal() > 1) this.outputText("s deposit the last of their");
        else this.outputText(" deposits the last of its");
        this.outputText(" cream into her, fully stuffing your turkey-girl.  Panting, you let your head rest between her tits a moment long, shaft");
        if (this.player.cockTotal() > 1) this.outputText("s");
        this.outputText(" wilting inside her until like a broken dam, her cunt");
        if (this.player.cockTotal() > 1) this.outputText(" and ass");
        this.outputText(" begin");
        if (this.player.cockTotal() == 1) this.outputText("s");
        this.outputText(" to drool spunk, pooling onto the hungry ground with the gallons of spilt gravy and fem-spooge.");
    ...
    public pigSlutRoastingGreet(): void {
        ...
        this.outputText("As you enter The Wet Bitch you’re accosted by an incredibly busty waitress. You can’t really discern what exactly she is as your eyes are solely focused on the massive melons jutting out from her chest and jiggling proudly with mile deep cleavage to boot.");
        this.outputText("\n\n\"<i>Hey there " + this.player.mf("handsome", "cutie") + "! We've got quite a unique special going on right now!</i>\" she exclaims in a cheery voice.");
        ...
        this.awardAchievement("Thankslutting", kACHIEVEMENTS.HOLIDAY_THANKSGIVING_I, true, true);
        this.dynStats("lus", 5); // {Lust = lust + 10}
        this.menu();
        if (this.player.gems >= 1) this.addButton(0, "Throw gem", this.getARoastPiggueOinkOinkOinkMotherfucker);
        this.addButton(1, "Nah", this.getGame().telAdre.barTelAdre);
        this.addButton(2, "Never", this.disablePigSlut);
    }

    public disablePigSlut(): void {
        this.clearOutput();
        this.outputText("Pig sluts are not really your thing. You tell her that you have no interest in her. She seems to be a bit heartbroken.");
        this.flags[kFLAGS.PIG_SLUT_DISABLED] = 1;
        this.doNext(this.getGame().telAdre.barTelAdre);
    }

    public getARoastPiggueOinkOinkOinkMotherfucker(): void { // {Return to The Wet Bitch menu} / Ham Roast -> In For A Gem
        this.clearOutput();
        this.player.gems -= 1;
        this.outputText("You quickly approach the gooey bucket and toss a single, shiny gem inside. Another waitress winks at you, almost equally as busty, nodding her head in the direction of the back rooms.");
        ...
        this.outputText("our nostrils flare, taking in great amounts of the salacious stench.");
        this.outputText(this.images.showImage("thanksgiving-pigslut"));
    ...
    public morePigRoast(): void {
        this.clearOutput();
        this.outputText(this.images.showImage("thanksgiving-pigslut-more"));
    ...
    public tenderizeDatPigSloot(): void { // [ Tenderize Her ] / Ham Roast -> Tenderize Her (Female and Genderless versions)
        this.clearOutput();
        this.outputText("\"<i>Turn over.</i>\" you say rather plainly, eager to see that jiggling booty of hers.");
        this.outputText(this.images.showImage("thanksgiving-pigslut-tenderize"));
        ...
        if (this.player.tail.type > Tail.NONE) this.outputText("You even bring your tail from behind your back to join in, the unfamiliar texture causing the sausage slut to let out surprised, high-pitched squeaks. ");
        this.outputText("You notice the pigslut moving backwards, her ass being pushed into the air as she attempts to feel the full power of your spanking, her pussy spraying juices onto the soaked sheets.");
        ...
        this.dynStats("lus", 25, "scale", false);
        this.menu();
        this.addButton(0, "Softcore", this.hamRoastTenderizeHerSoftcore);
        this.addButton(1, "Hardcore", this.tenderizeHerHardcore);
    }

    public tenderizeHerHardcore(): void { // Ham Roast -> Tenderize Her -> Hardcore
        this.clearOutput();
        this.outputText(this.images.showImage("thanksgiving-pigslut-tenderize-hardcore"));
        this.outputText("You can't help but maliciously smirk, your eyes resting on a fetish whip and some bondage equipment. You give the piggy one last double-handed spank on her ample ass, leaving her a pitifully moaning and quivering wreck. You quickly snatch up the whip, some rope, and a kinky ballgag.");
        ...
        this.dynStats("lus", 25, "scale", false);
        this.menu();
        this.addButton(0, "Next", this.spankEpilogueColonYoureAHorriblePersonForAbusingADumbPigGirl); // {Route to: here}
    }

    public hamRoastTenderizeHerSoftcore(): void { // Ham Roast -> Tenderize Her -> Softcore
        this.clearOutput();
        this.outputText(this.images.showImage("thanksgiving-pigslut-tenderize-softcore"));
        this.outputText("You can't help but let a salacious smile grace your lips, your eyes resting on a paddle in the corner. You give the piggy one last double-handed spank on her ample ass, leaving her a pitifully moaning and quivering wreck. You quickly snatch the somewhat heavy paddle, feeling its heft and grinning like a madman.");
    ...
    public spankEpilogueColonYoureAHorriblePersonForAbusingADumbPigGirl(): void {
        this.clearOutput();
        this.outputText("Eventually you stop your ceaseless romping of her rear, the piggy whore blubbering into the now thoroughly torn pillow from her biting and tearing. Several strips of flesh from her reddened ass look practically raw from your activities. You lay an experimental finger on the tortured flesh, the blazing heat radiating from it telling you all you need to know.");
    ...
    public spankEpilogueForRizzles(): void {
        this.clearOutput();
        this.outputText(this.images.showImage("thanksgiving-pigslut-spank-epilogue"));
    ...
    public getDoneEatingOutPigSlut(): void {
        this.clearOutput();
        this.outputText(this.images.showImage("thanksgiving-pigslut-ending"));
        this.outputText("Finally you remove yourself from the still slightly dribbling juicebox, giving the whore one last spank as she seems to have passed out from the bliss. The bed is entirely a mess, covered with her sticky juices and torn from the spanking.");
        ...
        if (this.player.thickness < 100) {
            this.outputText("  You notice that you seem a bit more... jiggly, a slight paunch in your stomach now as you grow a bit more plump than you once were.");
            if (this.player.thickness <= 10) this.player.thickness += 10;
            if (this.player.thickness <= 20) this.player.thickness += 10;
            if (this.player.thickness <= 30) this.player.thickness += 10;
            if (this.player.thickness <= 40) this.player.thickness += 10;
            if (this.player.thickness < 50) this.player.thickness += 5;
            changed = true;
        }
        if (this.player.butt.rating < 35) {
            this.outputText("  You notice your ass filling out, gaining more mass as it visibly grows before your eyes.");
            if (this.player.butt.rating < 4) this.player.butt.rating += 3;
            if (this.player.butt.rating <= 10) this.player.butt.rating += 2;
            if (this.player.butt.rating <= 15) this.player.butt.rating += 1;
            if (this.player.butt.rating <= 20) this.player.butt.rating += 1;
            if (this.player.butt.rating <= 35) this.player.butt.rating += 1;
            changed = true;
        }
        if (this.player.hips.rating < 35) {
            this.outputText(" Your hips grow too, widening and giving you more sexy curves.");
            if (this.player.hips.rating < 4) this.player.hips.rating += 3;
            if (this.player.hips.rating < 10) this.player.hips.rating += 2;
            if (this.player.hips.rating < 15) this.player.hips.rating += 1;
            if (this.player.hips.rating < 20) this.player.hips.rating += 1;
            if (this.player.hips.rating < 35) this.player.hips.rating += 1;
            changed = true;
        }
        if (this.player.biggestTitSize() < 20) {
            this.outputText(" Your boobs also grow, gaining size as it seems that every part of your body wants to join in on the growth.");
            if (this.player.biggestTitSize() < 3) this.player.growTits(3, this.player.bRows(), false, 1);
            if (this.player.biggestTitSize() < 6) this.player.growTits(2, this.player.bRows(), false, 1);
            if (this.player.biggestTitSize() < 12) this.player.growTits(2, this.player.bRows(), false, 1);
            if (this.player.biggestTitSize() < 20) this.player.growTits(2, this.player.bRows(), false, 1);
            changed = true;
        }
        this.outputText("\n\nYou sigh");
