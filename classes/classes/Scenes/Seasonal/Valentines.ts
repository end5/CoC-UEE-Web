...
export class Valentines extends BaseContent {

    public isItValentine(): boolean {
        return ((this.date.date >= 13 && this.date.date <= 15 && this.date.month == 1) || this.flags[kFLAGS.ITS_EVERY_DAY] > 0);
    }
    ...
    public crazyVDayShenanigansByVenithil(): void {
        ...
        this.outputText("Moving through the streets of Tel'Adre, you turn your steps towards the Wet Bitch, only to find that there's some odd event going on nearby.  Deciding the town is a safe enough place to let yourself be led astray by curiosity, you decide to investigate a bit closer.");
        ...
        this.awardAchievement("Valentine", kACHIEVEMENTS.HOLIDAY_VALENTINE);
        // [Help them][Leave]
        this.menu();
    ...
    public helpValentinesDayIII(): void {
        this.clearOutput();
        this.outputText("You spend the rest of the evening participating in holiday games and showing people around the stall, listening to Scylla giving a talk or two about love and the celebration of it.  All in all, the little popularization of the holiday turned out to be a moderate success, but a success after all thanks to the joint effort of the four of you.");
    ...
    public cuddleWithScyllaVDay(): void {
        ...
        // [if (hasVagina = true)]
        if (this.player.hasVagina()) this.outputText("  Your [vagina] is starting to get moist, making a small mess in your underclothes.");
        // [if (hasCock = true)]
        if (this.player.hasCock()) this.outputText("  You realize it'd probably be a done deal if you didn't resist the pull of your hardening cock towards one of Scylla's orifices, but you decide to spare her any temptations and endure yourself.");
        this.outputText("  In time, your ignited lust slowly changes to constant arousal, and Scylla just sitting there, watching the sun with a beautiful smile on her face, clearly content to just have someone in her arms on this evening is helping you control yourself a lot better.");
    ...
    public makeOutWithScyllaVDay(): void {
        ...
        // [if (cocks = 1)
        if (this.player.cockTotal() == 1) this.outputText("your [cock] is completely hard and stiff, forming a bulge in your clothing");
        else if (this.player.cockTotal() > 1) this.outputText("[eachCock] is by now completely hard, eager to be touched and wanting to be caressed by the nun you're kissing");
        this.outputText(".");
    ...
    public feedScyllaVDay(): void {
        ...
        this.outputText("You feed Scylla what feels like a gallon of jizz!");
    ...
    public goVisitAbby(): void {
        this.clearOutput();
        this.outputText("\"<i>You... you're kidding, right?  That fatty's words messing with yah head?  I always figured you were somewhat less... messed up in the head than Suck-a-Lot and Always-Drunk.  Was I wrong?</i>\"");
        ...
        this.outputText("\n\n\"<i>Ahhh, I see.  " + this.player.mf("He", "She") + ", the hero, saving a nun from the evil clutches of the demonic overlord!  She says thank you through growin' monstrous curves and providing extra sloppy blowjobs!  Dat right?</i>\"  You giggle a bit at that interpretation, and say it's not really all that far from the truth, although it is cheapening things... as is her previous interpretation of \"<i>Marethian kinkyness</i>\" cheapening love somewhat.  Abby brings a hand to her lips.  \"<i>Well, she does, at least, LOOK like she could do some... stuff... to a cock.</i>\"  She says, blushing, before you detour that train of thought with a question on how she's gotten herself here.");
        ...
        if (this.player.hasCock()) {
            const x: number = this.player.cockThatFits(46);
    ...
    public fuckAbbyVDay(): void {
        this.clearOutput();
        let x: number = this.player.cockThatFits(46);
        if (x < 0) x = this.player.smallestCockIndex();
        this.outputText("Figuring that the goblin girl's felt so out of place amid a celebration of love, you might as well show her a little affection the only way she's likely to understand.  Pulling aside your [armor] enough to let [eachCock] spring free, you step up behind the bent-over girl and rest your hands on her thick thighs.  The emerald girl's breath quickens as the heat of your manhood presses between the jiggling globes of her ass cheeks and a whimper escapes her throat before she realizes it.  Coughing, in an attempt to cover the moment of weakness, she turns her head to the side to give you a disapproving eye.  “What is this, romance?  Just jam it in, already!” she commands, gruffly.  Her face is flushed a deeper forest hue, pink irises twinkling in the dim light of her spartan home.");
    ...
    public pleasureAbbyVDay(): void {
        ...
        // ([Naga tongue]
        if (this.player.tongue.type == Tongue.SNAKE) this.outputText("  Your fingers make it into her pussy alongside your tongue, and the prehensile thing, longer and more flexible than a human's, ravages the insides of her pussy, tasting everywhere as Abby shakes in her orgasm.  Her pussy eagerly tries to milk both your fingers and your tongue, clenching and gripping at them as it drips her pussy juices all over your face.  Abby screams out in a loud, uninhibited orgasm at last, expressing her ecstasy.");
        // ([Demon/Dragon Tongue]
        else if (this.player.hasLongTongue()) this.outputText("  Abylon shakes in excitement, pleasure, and fear as you extend your inhuman tongue to reach deep into the recesses of her pussy.  She cannot hold out a moment longer, her cunt gripping your inhuman muscle tightly, the hole and muscles inside quivering as you can taster her orgasmic juices with every single point of your tongue.  She screams, shudders and moans, tongue lolling out from the intense pleasure as you force your own tongue to roll around inside her and stretch her in multiple ways. Only you holding her stops Abylon from slumping forward, it seems...");
        // ([Human tongue]
        else this.outputText("  Hooking your fingers at the front and back of her cunny, you plunge your tongue in and wiggle it eagerly.  Abby clamps down on all three invaders and loudly moans out, screaming to her content at last, as your face is flooded with pussy juice.");
    ...
    public goVisitPastyVDay(): void {
        this.clearOutput();
        this.outputText("You lean over the table Pastie is no longer dancing, but rather laying, on, and, conspiratorially, ask her if she'd like to go and get something to drink.");
    ...
    public pastieValentineIntro(choice: string = ""): void {
        this.clearOutput();
        if (choice == "vag") {
            this.outputText("You tell Pastie that indeed, she could get her fix, as you have something you're sure every fairy likes to drink not far away... between your legs.  \"<i>T-that's an... unusual drink proposition...  Umm, and I'm more susceptible to girls than most fairies, so... Ohhh, who am I kidding, that sounds pretty good right now.</i>\"");
        ...
        // ({Any other drink}.
            ...
            this.doNext(Valentines.curry(this.valentineDrinkPastie, itype));
        }
    }
    ...
    public valentineDrinkPastie(itype: ItemType): void {
        this.clearOutput();
        this.outputText("One you're away from any prying eyes, Pastie turns to you and rubs her little fairy hands together, apparently barely able to hold her enthusiasm to finally get a bit drunk.  You're afraid it may not end at one drink, either, and for a moment, wonder about whether this is all right or not...");
        // Corruption 0-20]
        if (this.player.cor < 33) this.outputText("  It probably isn't.");
        // [Corruption 21-75]
        else if (this.player.cor < 75) this.outputText("  You guess as long as she's fine afterwards, it should be okay.");
        // [Corruption 76-100]
        else this.outputText("  As long as she's in the mood to drink more than just the alcohol afterwards, you don't really care.");
        ...
        this.valentineDayGetPastieDrink(itype);
    }

    public valentineDayGetPastieDrink(itype: ItemType): void {
        // {Black Cat Beer}
        if (itype == this.consumables.BC_BEER) {
        ...
        // {Bimbo Champagne}
        else if (itype == this.consumables.BIMBOCH) {
        ...
        // {Goblin Ale}
        else if (itype == this.consumables.GOB_ALE) {
    ...
    public fuckPastieForVDay(): void {
        ...
        this.outputText("With a semi-sadistic grin, you watch Pastie saunter towards you, clearly allured by the scent of your genitals.  You stroke your " + this.player.cockDescript(x) + ", quickly bringing it to an erection, imagining how tight even this little thing will feel inside Pastie's small, but now unnaturally stretchable body.");

        this.outputText("\n\nBeing able to resist no longer, you grab her little bum and direct her towards your cock, not really hearing whether she's cheering, complaining, or asking you to stop; honestly, you could care less when you press your " + this.player.cockDescript(x) + " against the entrance to her diminutive vagina and start pushing.");
        this.outputText("\n\nAfter a while of fruitless pressing against her little legs, bum, and clit, Pastie finally gives way. Even your " + this.player.cockHead(x) + " is enough to stretch her already, but there's not much else to do now other than pushing it in deeper.");

        this.outputText("\n\nThe tightness of Pastie's confines is amazing, even as her body balloons out to more than twice its usual thickness, stretching and quivering around you, trying to either accommodate your " + this.player.cockDescript(x) + " or push it out completely.  It almost borders on painful, but also feels like a divine cum milking machine meant to draw your seed out of you.  You gently press Pastie down and pull her up your erection, essentially using her body as a masturbation aid, moaning all the while...");
    ...
    public rubPastieOnYourWangDawg(): void {
        ...
        if (this.player.balls > 0) this.outputText("  Your [balls] lurch, [skin] contracting as liquid passion rises within you.");
        this.outputText("  Your [ass] clenches in anticipation, heralding your climax, so you slide the fairy up your shaft, pulling free from the grip of her arms and legs, holding the confused girl directly in front of your " + this.player.multiCockDescriptLight() + " while loosing your grasp to bear her entire, curvaceous form to the open air.  With a shuddering gasp, you erupt your intoxicating cream directly into the fey girl, your orgasm");
        // low cum production:
        if (this.player.cumQ() < 250) this.outputText(" easily drenching the tiny fairy, in plump pearls up and down her naked flesh.");
        else if (this.player.cumQ() < 750) this.outputText(" leaving her and the arm you're holding her in dripping with a sticky alabaster lacquer.");
        // high cum production:
        else this.outputText(" almost blasting her right out of your hand in the torrent of white that spills forth from your mighty member.");
        this.outputText("  The spunk-plastered pixie shudders in your palm, overwhelmed by the spoo drenching her.  She masturbates, feverishly, climaxing in seconds and then again a moment later.  You watch her for a few minutes, the batter-basted bimbo jilling herself silly from the heightened sensitivity your spoo shower has given her.");
    ...
    public goForAPushayDivePasty(): void {
        ...
        // [if (hascock = true)
        if (this.player.hasCock()) this.outputText("  All the while, you continue to jerk off [eachCock], preparing yourself for an orgasm on both sides.");
        this.outputText("  She finally takes a full dive inside, and you moan out in pleasure, feeling her little body roll and explore in there, her tongue tasting every bit of fluid she can from what's moisturizing your inner passage.");
        ...
        // [if (hascock = true)]
        if (this.player.hasCock()) this.outputText("  At the same time, your cock deposits its own load onto the ground as your hermaphrodite genitals achieve a simultaneous orgasm.");
        // [if (isSquirter = true)
        if (this.player.wetness() >= 4) this.outputText("  She doesn't have much to say in the matter when your female ejaculation truly shoots in, though, sending her rolling out of your [vagina] and onto the ground.");
