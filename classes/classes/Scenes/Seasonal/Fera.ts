...
export class Fera extends BaseContent implements Encounter {
    ...
    public isItHalloween(): boolean {
        return ((this.date.date >= 28 && this.date.month == 9) || (this.date.date < 2 && this.date.month == 10) || this.flags[kFLAGS.ITS_EVERY_DAY] > 0);
    }

    public pumpkinFuckEncounter(): void {
        this.clearOutput();
        this.awardAchievement("Pump-kin-kin-kin", kACHIEVEMENTS.HOLIDAY_HALLOWEEN_I);
        this.outputText("Just ahead, in the middle of the path, lies a seeming innocuous gourd - a pumpkin to be precise.  The growth is oddly out of place.  Nothing else grows on the soft animal-forged path you now tread, and the pumpkin and its vines are moist, as if wet with morning dew.  They are the only plants in the vicinity to bear such moisture, gleaming dully in the muted arboreal light.");
        if (this.flags[kFLAGS.PUMPKIN_FUCK_YEAR_DONE] > 0) this.outputText("  An old, haunting memory surfaces, and you realizes this seems... familiar.  Did you find this last year?  Yes... it could have been this very same pumpkin...");
        ...
        this.menu();
        this.addDisabledButton(0, "Fuck It");
        this.addDisabledButton(1, "Mount It");
        if (this.player.hasCock()) {
            ...
            this.addButton(0, "Fuck It", this.pumpkinFuck);
        }
        ...
        if (this.player.hasVagina()) {
            ...
            this.addButton(1, "Mount It", this.mountPumpkin);
        }
        if (this.player.isHerm()) this.outputText("\n\nSo many choices, so little time.");
        // If PC has neither
    ...
    // Unfinished scene.
    public pumpkinSmash(): void {
        this.clearOutput();
        this.outputText("You have no idea but to be safe, you pick up the pumpkin and throw the pumpkin at the tree, shattering the pumpkins into several pieces.");
    ...
    public pumpkinFuck(): void {
        this.clearOutput();
        this.flags[kFLAGS.PUMPKIN_FUCK_YEAR_DONE] = this.date.fullYear;
        this.outputText("Well, fuck it.  You're horny, and this little pumpkin doesn't look like it could stop you from fucking that gushing gourd-pussy.  As you shed your [armor], a few of the rough, slime-sweating vines feebly rise up to embrace you, but you brush them away, having no time for corrupt foolishness.  You're going to bust a nut in this fruit's sweet, supple folds, not let some malformed tentacle-horror molest you.");
        ...
        if (this.player.cumQ() < 200) this.outputText("Your weak load disappears into the plant's sucking gullet, wicked away as fast as you fire.");
        else if (this.player.cumQ() < 1000) this.outputText("Your thick load disappears into the plant's sucking gullet almost as fast as you're firing it, though you can see a bit of white creampie forming around the edges of your cock.");
        else {
            this.outputText("Your massive load floods the pumping, a deluge of semen squirting around your [cock biggest] even as the pumpkin starts to enlarge.  Every titanic spooge-squirt pumps the gourd up another level.  Soon, the rind is rubbing your belly.  A few blasts later, it's lifting you from the ground, your still-spurting cock lodged firmly in its cunt.  The suction feels so much stronger now - you're not sure you could pull out now if you wanted to, and you don't!");
        }
    ...
    public pumpkinFuckPartII(): void {
        ...
        if (this.player.cockTotal() > 1) {
            this.outputText(", her free hand working ");
            if (this.player.cockTotal() == 2) this.outputText("your other");
            else this.outputText("another");
            this.outputText(" dick with child-like glee");
        }
        this.outputText(".  \"<i>");
        if (this.player.race != "human") this.outputText("I know what you were, virile suitor.  ");
        this.outputText("There's never too much of a good thing.  Of course, my champion must have the proper train of thought...</i>\"");
    ...
    public pumpkinFuckPartIII(): void {
        this.clearOutput();
        this.outputText("SLAP!  The crack of a palm on your face rouses you, a red handprint left on your cheek.  Fera smirks at you as you stir woozily in your plant-woven prison.  \"<i>Did you enjoy yourself?</i>\"  You slowly nod, feeling... liberated, free.  The situation is less worrying than before.  Fera just wanted to thank you, right?");
        this.outputText("\n\nThe sultry orange-haired maiden ");
        if (this.player.balls > 0) this.outputText("pats your sloshing sack");
        else this.outputText("caresses a sensitive spot beneath your dick");
        this.outputText(" as the vines unwind, allowing you to stand on your own [feet].  You go to wipe the cum from your nose and upper lip as soon as you're free.  It's dry.  Did any of that even happen?");
    ...
    public mountPumpkin(): void {
        ...
        if (this.player.pregnancyIncubation > 0 || (this.player.wetness() < 2 && this.player.totalFertility() < 30)) {
            this.outputText("Well, fuck it.  You're horny, and this little pumpkin doesn't look like it could stop you from fucking that gushing stem-rod.  As you shed your [armor], a few of the rough, slime-sweating vines feebly rise up to embrace you, but you brush them away, having no time for corrupt foolishness.  You're going to mount this fruit's hard, pulsing phallus, not let some malformed tentacle-horror molest you.");
            ...
            (this.player as Player).cuntChange(50, true, false, false);
        ...
        // Female characters don't have an easy analogue to cum production unless we count milk production, and female gender doesn't require either breasts or lactation to play.  So I just went for high vaginal wetness, not pregnant, and likely to become pregnant if she had sex with a male creature.  I hope that works out OK.
            ...
            this.outputText("\n\nA faint rumbling is the only warning you get (or need).  You jump back and ready yourself for anything, ");
            if (this.player.weaponName != "fists") this.outputText("picking up your [weapon]");
    ...
    public pumpkinMountingLevelII(): void {
        this.clearOutput();
        this.outputText("The pumpkin's titanic stem shivers, the mighty shaft trembling immensely before the pumpkin splits from the top causing it to topple.  The crack goes around the rind in multiple directions, causing it to split open like a flower as strings of orangey-purple goo hang wetly between the pieces for a moment.  A moment later, the sectioned fruit falls apart, at last revealing the corrupt core.  No trace of the mammoth copulation remains.");
        ...
        if (this.player.biggestTitSize() >= 2) this.outputText(", her free hand working your [fullChest] with glee");
        this.outputText(".  <i>\"There's never too much of a good thing.  Of course, my champion must have the proper train of thought...\"</i>");
    ...
    public mountPumpkinsIII(): void {
        this.clearOutput();
        this.outputText("SLAP!  The crack of a palm on your face rouses you, a red handprint left on your cheek.  Fera smirks at you as you stir woozy in your plant-woven prison.  <i>\"Did you enjoy yourself?\"</i>  You slowly nod, feeling... liberated... free.  The situation is less worrying than before.  Fera just wanted to thank you, right?");
    ...
    public exploreFerasWonderland(): void {
        this.clearOutput();
        this.awardAchievement("Fera's Wonderland", kACHIEVEMENTS.HOLIDAY_HALLOWEEN_II);
        this.flags[kFLAGS.FERAS_GLADE_EXPLORED_YEAR] = this.date.fullYear;
        this.outputText("Straightening to your full height, you take a few experimental sniffs and begin to walk back towards the middle of the meadow, brushing aside garish pink blooms that rise up past your waist.  That smell is getting stronger now.  How could you have missed it when you first arrived? It's heavenly, like a combination of every sweet treat you like with an undercurrent of passionate sexual musk blended in underneath.  You close in, " + this.player.mf("[hips] swaggering excitedly", "[hips] sashaying excitedly") + ", [skin] flushing.  A few baby blue sunflowers bump your shoulders as you muscle past them to hone in your new favorite aroma.  You've just got to know what bloom could possibly release such a heavenly scent!");
    ...
    public fuckDemCorruptFlowerLips(): void {
        ...
        if (this.player.longestCockLength() >= 20) {
            this.outputText("easily ramming your huge endowment");
            if (this.player.cockTotal() > 1) this.outputText("s");
            this.outputText(" straight into the erotic-looking mouth");
        }
        else if (this.player.longestCockLength() >= 8) {
            this.outputText("slipping your large endowments");
            if (this.player.cockTotal() > 1) this.outputText("s");
            this.outputText(" right into the erotic-looking mouth");
        }
        else if (this.player.longestCockLength() >= 4) {
            this.outputText("easing the top hal");
            if (this.player.cockTotal() > 1) this.outputText("ves");
            else this.outputText("f");
            this.outputText(" of your phallus");
            if (this.player.cockTotal() > 1) this.outputText("es");
            this.outputText(" into the erotic-looking mouth");
        }
        else {
            this.outputText("barely managing to slip your tip");
            if (this.player.cockTotal() > 1) this.outputText("s");
            this.outputText(" into the erotic-looking mouth");
        }
        this.outputText(". The contact is everything you thought it would be: surprisingly warm, soaking-wet, and squeezing just tight enough to make your pillar");
        if (this.player.cockTotal() > 1) this.outputText("s");
        this.outputText(" of lust twitch in excitement.");
        ...
        if (this.player.balls > 0) this.outputText("[balls] against the cushy lower lip, wreathing them in syrupy sap or nectar.  You're not really sure what the appropriate name for such a substance is, and you're well past caring.");
        else this.outputText("loins against the cushy, sap-slicked lips, compressing their hot, smooth surfaces against your [skin].  You're not really sure if sap is the appropriate name for such a substance, but you're far beyond caring at this point.");
        this.outputText("  The fast-diminishing light doesn't even register, as your eyes have closed as you subsume yourself in pleasure.");
        ...
        this.outputText("\n\nYour motions slow as the ambrosial syrup is wicked into your body, frying your synapses with ecstasy until only lust, pleasure, and perfect heat are occupying your few remaining thoughts. By the time the tendril pulls free, glazing your face in sappy goo, your [hips] have stopped their thrusts entirely, leaving your " + this.player.multiCockDescriptLight() + " to pulsate and ache with need despite their owner's inability to remember how to hump. Best of all, the few times that you do manage to summon up a coherent thought, the flashing, undulating colors unravel it, letting you revel in unthinking arousal.");

        this.outputText("\n\nThe deluge of drugs, arousal, sexual stimulation, and mind-scattering visual patterns has made your mind as pliant and vulnerable as the lips your " + this.player.multiCockDescriptLight() + " ");
        if (this.player.cockTotal() > 1) this.outputText("are");
        else this.outputText("is");
        this.outputText(" buried in, so hot, wet, and soft. With each passing moment, you become more relaxed, more open and exposed, and more aroused than before. You would probably be afraid if you were capable of any kind of reason.");
        this.outputText("\n\nA perfectly smooth, perfectly dry hand traces down your back in a comforting yet sensual gesture, followed a moment later by another upon your [butt]. It squeezes you, kneading your cheek in its supple, dextrous fingers, letting long, sharp nails just barely dig into your " + this.player.skinFurScales() + ". Then the hands move down to your [legs] to continue their massage, dodging around the tentacle-like stamen so that they can reach your [feet]. There, the hands stop and separate from your body, vanishing until a familiar face leans in front of you, interrupting the slew of mind-tranquilizing colors briefly.");
    ...
    public dickBoostingFuntimes(): void {
        ...
        if (this.player.cockTotal() > 1) this.outputText("s");
        this.outputText(" just in time to take the first thrust from your hips, burying yourself into them and immediately pulling back, setting an easy, average rhythm.  At the same time, the suction starts, causing your ");
        if (this.player.cockTotal() > 1) this.outputText("heads");
        else this.outputText("[cockHead]");
        this.outputText(" to swell up a little, and when the rest of your length is plowed through the tight, slippery ring, the tugging vacuum increases, plumping your dick");
        if (this.player.cockTotal() > 1) this.outputText("s");
        this.outputText(" bigger, harder, and fatter than ");
        if (this.player.cockTotal() == 1) this.outputText("it");
        else this.outputText("they");
        this.outputText(" should be.");
        ...
        if (this.player.balls > 0) this.outputText("empty");
        else this.outputText("empties");
        this.outputText(" so that you can start the cycle anew.  You love that, and this is the perfect place for that kind of behavior, isn't it? Everybody in this world wants to fuck, they just like to fight to see who gets to be on top, and you're good at winning, aren't you, pet champion?</i>\" Fera ");
        if (this.player.balls > 0) this.outputText("cups your [sack] tenderly");
        else this.outputText("tickles your taint");
        this.outputText(" to spur you to thrust harder while she fills your head with lurid thoughts.");
        ...
        this.outputText(" as you do, whispering, \"<i>Good " + this.player.mf("boy", "girl") + ",</i>\" into your ears as dark energies flow into you, refilling your fast-emptying reservoirs faster than they can empty.  \"<i>Don't stop yet, [name].  You're going to be doing this a lot, so I'm giving you lots of practice.</i>\"");
        ...
        for (let x: number = 0; x < this.player.cocks.length; x++) {
            ...
            if (this.player.cocks[x].cockThickness < this.player.cocks[x].cockLength / 5) {
                this.player.cocks[x].cockThickness = this.player.cocks[x].cockLength / 5;
            }
        }
        this.player.orgasm('Dick');
    ...
    public postSeederUpgrade(): void {
        ...
        // Add 50 lust.
        this.dynStats("lus", 25, "cor", 10, "scale", false);
    ...
    public stickAStamenInIt(): void {
        ...
        else if (this.player.isGoo()) this.outputText("goo");
        else this.outputText("knees");
        this.outputText(", you drag yourself up until the 'floor' flattens out and you nearly ram into the pursed lips, stopping yourself abruptly once you realize you've reached your goal.");
        ...
        if (this.player.hasCock()) this.outputText("or ");
        this.outputText("a tight, dick-milking rape-mother");
        if (!this.player.hasCock()) this.outputText(", or a libidinous dick-girl, sowing your seed everywhere you can and saving your cunt for only the strongest of mates");
        this.outputText("?</i>\"");
        ...
        // [Gape] [Tight] [Dick]
        this.dynStats("lus=", this.player.maxLust(), "cor", 2);
        this.menu();
    ...
    public gapeMePleaseFera(): void {
        ...
        if (this.player.vaginas[0].vaginalLooseness >= Vagina.LOOSENESS_LEVEL_CLOWN_CAR) this.outputText(", spreading its well-widened lips as wide as they've ever been");
        else if (this.player.vaginas[0].vaginalLooseness >= Vagina.LOOSENESS_GAPING) this.outputText(", spreading your lips well beyond the widest they've ever been and still farther.");
        else if (this.player.vaginas[0].vaginalLooseness >= Vagina.LOOSENESS_LOOSE) this.outputText(", meeting a bit of token resistance at your unprepared entryway and slithering stamen after stamen inside, the forerunners widening you until the entirety of the column can pass inside, gaping you almost inhumanly wide.");
        else this.outputText(", meeting more than a little resistant at your unstretched entry and finding a way in anyhow. First one tentacle slithers in, then a second, then a fourth, and so on. One after another, they spread you wider and wider, opening you until the entire wiggling column is buried deep, pulling your lips ridiculously wide.");
        ...
        // STEAL VIRGINITY!
        this.player.cuntChange(90000, true, true, false);
        ...
        if (this.player.lowerBody.type == LowerBody.HUMAN) this.outputText("toe");
        else if (this.player.tail.type > Tail.NONE) this.outputText("tail");
        ...
        if (this.player.wetness() < 4) this.outputText(" even though you're not a squirter");
        this.outputText(".  They pump on with intense, unrelenting energy, squishing wetly with every thrust, splattering juice everywhere as they ruin your poor, convulsing cunny.");
        this.player.cuntChange(90000, true, true, false);
        ...
        // (cum!)
        // Minimum size to 4
        this.player.createPerk(PerkLib.FerasBoonWideOpen, 0, 0, 0, 0);
        ...
        if (this.player.vaginas[0].vaginalWetness < Vagina.WETNESS_SLICK) this.player.vaginas[0].vaginalWetness = Vagina.WETNESS_SLICK;
        if (this.player.vaginas[0].vaginalLooseness < Vagina.LOOSENESS_GAPING_WIDE) this.player.vaginas[0].vaginalLooseness = Vagina.LOOSENESS_GAPING_WIDE;
        // Add five corruption.
        this.player.orgasm('Vaginal');
        this.dynStats("cor", 3);
        // [Next]
    ...
    public gapeByFeraEpilogue(): void {
        this.clearOutput();
        this.outputText("You awaken fully dressed with a familiar ache between your legs. Damn, that was a good nap.  You rise, stretching as you rub your back, sore from lying against a tree for so long.  You suppose you could hunt down a tentacle monster and see if it'd really give you a filling");
        if (this.player.pregnancyIncubation == 0) this.outputText(", but you'd rather fuck something that'll get you good and pregnant with a well-hung fuck-monster, like a minotaur!");
        else this.outputText(".");
        this.outputText("  You idly reach into your [armor] and stuff four fingers into yourself before sighing and removing them.  It just isn't the same as being full of cock.");
        ...
        // Add ten more corruption.
        this.dynStats("lus", 20, "cor", 10, "scale", false);
        this.doNext(this.camp.returnToCampUseFourHours);
    }
    ...
    public tightLikeAToigar(): void {
        this.clearOutput();
        this.outputText("Fera smiles.  \"<i>I had hoped we might do that, little champion.  Now, you simply relax and enjoy the pleasure that only my normal weak-willed prey are allowed to experience.  You will be the first to taste it and be allowed free.  How wonderful!</i>\"  She pets your forehead.");
        ...
        if (this.player.vaginas[0].vaginalLooseness >= Vagina.LOOSENESS_LEVEL_CLOWN_CAR) this.outputText(", slipping past your capacitive entrance with ease");
        else if (this.player.vaginas[0].vaginalLooseness >= Vagina.LOOSENESS_GAPING) this.outputText(", sliding into your welcoming entrance fairly easily");
        else if (this.player.vaginas[0].vaginalLooseness >= Vagina.LOOSENESS_LOOSE) this.outputText(", slowly working its way into your decently unstretched entrance");
        else this.outputText(", slowly burrowing past your tight, restrictive entrance");
        this.outputText(" and leaving a trail of slick, black corruption in its wake.  The tendril pushes the whole way up to your cervix, gently butting up against your womb's portal before dragging back out, sputtering more inky cum all the way.  When it pops out, the head catches the tip of your [clit], causing your [vagina] to spasm and contract, squeezing down on the foreign fluid with such strength that a geyser of sable spunk erupts from your quivering quim.");
        ...
        // (cum!)
        this.player.orgasm('Vaginal');
        this.dynStats("lib", 5, "sen", 20, "cor", 3);
        // Maximum size to 2
        this.player.createPerk(PerkLib.FerasBoonMilkingTwat, 0, 0, 0, 0);
        if (this.player.vaginas[0].vaginalLooseness > Vagina.LOOSENESS_LOOSE) this.player.vaginas[0].vaginalLooseness = Vagina.LOOSENESS_LOOSE;
    ...
    public growADickForFera(): void {
        ...
        if (this.player.tail.type > Tail.NONE) this.outputText(" and press your tail to the ground");
        this.outputText(".  You feel the wetness of the hollow tube as it presses against you along with a lance of pure desire and heat in the very center, undoubtedly caused by the tainted needle as it slips through your unprotected flesh.  The thing is compressed so tightly against you that it looks like the entire injector was able to bury itself in your body.");
        ...
        // (cum!)
        this.player.orgasm('Dick');
        this.dynStats("lib", 10, "sen", 10, "cor", 3);
        ...
        if (rand(2) == 0) this.player.cocks[0].cockType = CockTypesEnum.TENTACLE;
        // Add five corruption.
        this.player.createPerk(PerkLib.FerasBoonSeeder, 0, 0, 0, 0);
        this.menu();
