...
import { ActionScriptRNG } from "../../internals/ActionScriptRNG";
import { BlackCock } from "./Bazaar/BlackCock";
...
import { ChillySmith } from "./Bazaar/ChillySmith";
import { GretasGarments } from "./Bazaar/GretasGarments";
import { SlipperySqueeze } from "./Bazaar/SlipperySqueeze";
import { kFLAGS } from "../../GlobalFlags/kFLAGS";
import { BazaarGatekeeper } from "../Areas/Plains/BazaarGatekeeper";
...
export class Bazaar extends BaseContent {
    ...
    public roxanne: Roxanne = new Roxanne(this.rng);
    ...
    public findBazaar(): void {
        ...
        if (this.flags[kFLAGS.BAZAAR_ENCOUNTERED] == 0) {
            ...
            this.outputText("Once again you smell a campfire through the tall grass, and as you close the distance you hear the familiar sounds of the traveling bazaar. You peek through the weeds and behold the caravan's gate-keeper - a red-skinned, muscular giant of a man.");
        }
        this.outputText("\n\nDo you approach?");
        // [YES] [NOOOO]
    ...
    private approachBazaarGuard(): void {
        ...
        if (!this.player.isCorruptEnough(33)) {
            this.outputText("\"<i>Leave at once. You are not yet ready for the wonders of the Bazaar.</i>\"");
            this.menu();
            this.addButton(0, "FIGHT!", this.initiateFightGuard).hint("Fight the guard to forcibly gain entry into the bazaar.");
        } else {
            this.outputText("\"<i>Welcome to the Bizarre Bazaar. Enter, but be mindful of your actions within.</i>\"");
            this.menu();
            this.addButton(0, "Enter", this.enterTheBazaar).hint("Enter the Bizarre Bazaar and take a good look around.");
        }
    ...
    public enterTheBazaar(): void {
        if (this.getGame().time.hours == 19 || this.getGame().time.hours == 20) {
            ...
            if (this.flags[kFLAGS.COUNTDOWN_TO_NIGHT_RAPE] % 4 == 0 && (this.player.gender == 1 || (this.player.gender == 3 && this.player.mf("m", "f") == "m"))) {
    ...
    public initiateFightGuard(): void {
        this.clearOutput();
        this.outputText("You ready your " + this.player.weaponName + " and assume a combat stance! He grabs his dual scimitars! It's a fight!");
        this.startCombat(new BazaarGatekeeper());
    }

    public winAgainstGuard(): void {
        this.clearOutput();
        this.outputText("With the gatekeeper defeated, you walk right past the unconscious guard and enter...");
        this.combat.cleanupAfterCombat(this.enterTheBazaarAndMenu);
    }
    ...
    public enterTheBazaarAndMenu(demons: boolean = true): void {
        ...
        this.outputText(this.images.showImage("location-bazaar"));
        ...
        this.chillySmith.smithButton();
        this.roxanne.RoxanneAppearance();
        this.benoit.setBenoitShop();
        this.fapArena.fapAppearance();
        this.blackCock.blackCockDescription();
        // Set menu. The top row is always available.
        this.menu();
        this.addButton(0, "Shops", this.shopMenu).hint("Browse the shops and access their services.");
        this.addButton(1, (this.flags[kFLAGS.FAP_ARENA_RULES_EXPLAINED] > 0 ? "Fap Arena" : "Tent"), this.fapArena.fapArenaGOOOO).hint(this.flags[kFLAGS.FAP_ARENA_RULES_EXPLAINED] > 0 ? "Visit the tent that is also known as 'Fap Arena' to many of the well-hung guys." : "Investigate the large tent. What could it even be?");
        this.addButton(2, "Food Tent", this.blackCock.enterTheBlackCock).hint("The incredible smell seems to come from that tent.", "The Black Cock");
        if (this.flags[kFLAGS.PRISON_ENABLED] == true) {
            this.addButton(4, "Back Alley", this.investigateBackAlley).hint("That back alley looks suspicious. Do you dare investigate?");
        } else {
            this.addButton(4, "Back Alley", this.investigateBackAlleyNoPrison).hint("That back alley looks suspicious. Do you dare investigate?");
        }
        // Cinnabar
        if (this.getGame().time.hours >= 15 && this.getGame().time.hours <= 20) this.addButton(5, (this.flags[kFLAGS.CINNABAR_NUMBER_ENCOUNTERS] > 0 ? "Cinnabar" : "Rat"), this.cinnabar.cinnabarAppearance(false));
        // Griping Demons
        if (this.flags[kFLAGS.BAZAAR_DEMONS_LISTENED_IN] == 0 && rand(4) == 0 && demons) {
            this.overHearDemonsAboutSyrena();
            return;
        }
        if ((this.flags[kFLAGS.BAZAAR_DEMONS_LISTENED_IN] == 1 || this.flags[kFLAGS.BAZAAR_DEMONS_LISTENED_IN] == 2) && demons && rand(10) == 0) {
            // [Repeat Variant]
            this.outputText("\n\n<b>The familiar sounds of the two griping demons can be heard nearby. Do you listen in again?</b>");
            this.addButton(6, "GripingDemons", this.overHearDemonsAboutSyrena).hint("Overhear the conversation of the two griping demons.", "Griping Demons");
        }
        // Lilium
        if (this.lilium.LiliumText(false) != undefined) {
            this.addButton(7, (this.flags[kFLAGS.LILIUM_TIMES_TALKED] > 0 ? "Lilium" : "Demon"), this.lilium.LiliumText(false));
        }
        // Roxanne
        this.addButton(8, (this.flags[kFLAGS.ROXANNE_MET] > 0 ? "Roxanne" : "Lizans"), (this.flags[kFLAGS.ROXANNE_MET] > 0 ? this.roxanne.RoxanneChooseApproachOrRepeat : this.roxanne.Roxanne1stApproach));
        // Bimbo Niamh
        if (this.flags[kFLAGS.NIAMH_STATUS] > 0 && this.flags[kFLAGS.NIAMH_MOVED_OUT_COUNTER] == -1) {
            ...
            this.addButton(9, "Niamh", this.getGame().telAdre.niamh.bazaarNiamh);
        }
        this.addButton(14, "Leave", this.camp.returnToCampUseOneHour);
        if (this.flags[kFLAGS.GRIMDARK_MODE] > 0) this.addButton(14, "Leave", this.leaveBazaarGrimdark);
    }
    ...
    private shopMenu(): void {
        this.menu();
        this.benoit.setBenoitShop(true);
        this.addButton(1, "G. Garments", this.gretasGarments.gretasGarments).hint(this.flags[kFLAGS.FOUND_SOCKS] > 0 ? "Visit 'Greta's Garments' run by the succubus which is a shop stocked with cock-socks of varying colours." : "From the look of the sign, it's probably a clothing shop. Investigate the shop?", "Greta's Garments");
        this.addButton(2, "S. Squeeze", this.slipperySqueeze.theSlipperySqueeze).hint(this.flags[kFLAGS.BAZAAR_SLIPPERY_SQUEEZE_VISITED] > 0 ? "Visit 'The Slippery Squeeze' which is a massage parlour run by Joey the bunny." : "If the sign is of any indication, it's probably a massage parlour. Investigate the parlour?", "Slippery Squeeze");
        this.chillySmith.smithButton(true);
        this.addButton(4, "Back", this.enterTheBazaarAndMenu);
    }

    private overHearDemonsAboutSyrena(): void {
        this.clearOutput();
        if (this.flags[kFLAGS.BAZAAR_DEMONS_LISTENED_IN] == 0) {
        ...
        // [Listen in repeat]
        else if (this.flags[kFLAGS.BAZAAR_DEMONS_LISTENED_IN] == 1) {
        ...
        // [Listen in Repeat 2]
        else if (this.flags[kFLAGS.BAZAAR_DEMONS_LISTENED_IN] >= 2) {
        ...
        this.flags[kFLAGS.BAZAAR_DEMONS_LISTENED_IN]++;
    }
    ...
    public nightBazaarButtfuck(): void {
        ...
        this.menu();
        this.addButton(0, "Escape", this.escapeFromCertainRape).hint("Book it and make a dash! Run like the wind back to the safety of the public space!");
        this.addButton(1, "Wait", this.waitOnStuff).hint("Do nothing and wait instead. However, you have a feeling this might lead to a certain rape.");
        this.addButton(2, "Assault", this.assaultYoRapistYo).hint("Turn the tables against the burly tiger. Nobody is taking you today!");
    }
    ...
    private assaultYoRapistYo(): void {
        ...
        // [(If strength is 50 or more)
            ...
            if (this.player.isCorruptEnough(50) || this.player.hasPerk(PerkLib.Sadist)) this.addButton(0, "Abuse Ass", this.abuseHisAss);
        }
    }
    ...
    private abuseHisAss(): void {
        ...
        this.outputText("\n\nYou decide to have a little fun with the muscular ass in front of you. You hold the struggling tiger's wrists with one hand and fish out your " + this.player.multiCockDescriptLight() + " free from your [armor]. Your eyes drink in how the burly tiger man writhes under you. He's completely at your mercy.");
        // [(If multicock)
        if (this.player.cockTotal() > 1) this.outputText(" Your hand strokes along your " + this.player.multiCockDescriptLight() + " and press them to the warm cheeks exposed before you.");
        else this.outputText(" You pump your " + this.player.multiCockDescriptLight() + " and milk a pearl of pre-cum out to drip onto the enticing ass before you.");
        ...
        if (this.player.smallestCockLength() >= 16) {
            ...
            if (this.player.totalCocks() == 1) this.outputText(" You squeeze your " + this.player.multiCockDescriptLight() + " between his warm ass cheeks, stroking yourself with his exposed bottom. The tiger whimpers meekly under you.");
        ...
        // [[(If largest cock is smaller than 16)
            ...
            else this.outputText(" Your " + this.player.multiCockDescriptLight() + " pushes in, your pre-cum-slicked length sliding in with a wet noise.");
            ...
            if (this.player.balls > 0) this.outputText(" swatting your " + this.player.ballsDescriptLight() + " against him,");
            this.outputText(" making him shout out in discomfort. Your " + this.player.multiCockDescriptLight() + " stretching him out");
    ...
    private collectSomeButtSmex(): void {
        ...
        if (this.player.hasCock()) {
            this.outputText(" [EachCock] grows stiff as everyone watches. You wince to try and fight off the draft, but soon you're dripping pearls of pre-cum.");
        }
        if (this.player.hasVagina()) this.outputText(" Your " + this.player.vaginaDescript(0) + " grows slick in your excitement.");
        ...
        if (this.player.hasCock()) this.outputText(" wrap around your " + this.player.multiCockDescriptLight() + ".");
        else this.outputText(" rub over your " + this.player.assholeDescript() + ".");
        ...
        else this.outputText("\n\nHis fingers slip past your puckered flesh and dip into your " + this.player.assholeDescript() + " and spread apart, loosening you for what feels like ages.");
        this.outputText(" Without warning the hand is gone");
        if (this.player.cockTotal() > 1) this.outputText(" and your " + this.player.multiCockDescriptLight() + " wetly slaps your stomach with need");
    ...
    private finalGayFinallee(road: number = 0): void {
        this.outputText("\n\nWith a wet pop, the satyr pulls his spent manhood from your " + this.player.assholeDescript() + ". He smears his softening cock over your " + this.player.buttDescript() + " and leaves a white streak of cum across your cheeks. He huffs a content breath and moves over to the side. \"<i>I'm good, you guys decide who gets to fuck the slut next.</i>\" You look up at the tiger and bear men. Their eyes light up a moment before turning to look at your " + this.player.buttDescript() + " with hungry lust. Your spine shivers under their gaze, but before they can move a green blur runs past them.");

        this.outputText("\n\n\"<i>Zug want hole!</i>\" The large green orc runs to your behind, slapping his massive foot-long against your " + this.player.buttDescript() + " as if to call 'dibs!'. You bite your lips at the sight of the throbbing twelve inches sawing your ass cheeks apart, the dark olive flesh glistening with a thick layer of prespunk and slick, cool lubricant. The orc's bulky muscles twitch menacingly as he stares the tiger and bear down. The two of them stay where they are and, after a few moments, the orc grunts. He looks down and lines himself up with your hole. His thick, orcish girth presses against your " + this.player.assholeDescript() + ", pushing in forcefully as the satyr's cum and rough pounding opened you just wide enough for the orc to slide in with little pain.");

        this.outputText("\n\nYou give a muffled groan as the giant orc cock impales you, the stiff erection grinding over your stretched-out, silken walls. His fat orc sack swats your upturned " + this.player.buttDescript() + ". It coaxes whimpers from your lips as the green cock pulls out and slams back in, the brute of an orc focusing on getting further into you as his massive phallus plunges deeper and deeper with each thrust. The colossal girth rides on the satyr's cum trails and pushes past it, stuffing you full to bursting with his orc meat. His hands grip your hips and he roughly yanks you into a good angle as he begins to abuse your " + this.player.buttDescript() + " with long, pummeling lunges. He lets out guttural groans as his hefty cock drools orc cream, mixing it with the satyr's spooge as your hole begins to resemble a used condom.");
        this.player.buttChange(30, true, true, false);
        this.outputText("\n\nYou can barely focus on pleasuring the two cocks in your hands as your lower body receives a slut's treatment. \"<i>Ragh! Hole so tight around orc cock! Make good orc bitch!</i>\" The large green man groans and you feel his heavy sack clench against your cheeks. Your eyes go wide as you're suddenly flooded with orc spunk, the sensation and sound of the cum overflowing from your hole to splatter against the Orc's balls and thighs overwhelming your senses. The Orc lets out a \"<i>Wrahhg!</i>\" as he pins your hips, hilting his orc cock as he continues to unload his thick, fertile seed into you. Your belly soon distends and bulges from the amount of cum shooting up your ass. The orc man pants over you as you feel your " + this.player.assholeDescript() + " dripping orc spunk like a leaking dam.");
        ...
        if (road == 0) {
            ...
            if (this.player.hasCock()) this.outputText("His hand dips down to play with your " + this.player.multiCockDescriptLight() + ", stroking you off as he gives a few more brutal pounds of his hips. ");
            else if (this.player.hasVagina()) this.outputText("Fingers ram into your " + this.player.vaginaDescript(0) + ", thrusting into you as he fucks your other hole. ");
        ...
        // ((Ending 3 , suck none, //No furry plx))
            ...
            if (this.player.hasCock()) this.outputText(" His hand drips down to clumsily play with your " + this.player.multiCockDescriptLight() + ", stroking you off as he gives you a few more brutal pounds of his hips.");
            else if (this.player.hasVagina()) this.outputText(" Fingers dip into your " + this.player.vaginaDescript(0) + ", ramming into you as he fucks your other hole.");
