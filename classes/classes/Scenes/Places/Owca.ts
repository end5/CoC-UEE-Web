...
export class Owca extends BaseContent {
    ...
    private modOwcasAttitude(change: number): number {
        this.flags[kFLAGS.OWCAS_ATTITUDE] += change;
        if (this.flags[kFLAGS.OWCAS_ATTITUDE] > 100) this.flags[kFLAGS.OWCAS_ATTITUDE] = 100;
        if (this.flags[kFLAGS.OWCAS_ATTITUDE] < 0) this.flags[kFLAGS.OWCAS_ATTITUDE] = 0;
        return this.flags[kFLAGS.OWCAS_ATTITUDE];
    }
    ...
    private agreeToFollowRebecFirstTime(): void {
        ...
        // Yes/No
        this.menu();
        this.addButton(0, "Yes", this.acceptRebeccsPlea, this.flags[kFLAGS.TIMES_IN_DEMON_PIT] == 0, true);
        this.addButton(1, "No", this.declineRebeccsPlea);
    }
    // Refuse plea (Z)
    private declineRebeccsPlea(): void {
        ...
        // [Attitude is set to 50]
        if (this.flags[kFLAGS.OWCAS_ATTITUDE] > 5) this.modOwcasAttitude(-5);
    ...
    private acceptRebeccsPlea(firstTime: boolean = false, sacrificed: boolean = false): void {
        ...
        if (this.player.cor > 70 && this.player.gender > 0) {
            ...
            if (this.player.hasVagina()) this.outputText(this.player.vaginaDescript(0));
    ...
    private intoTheDemonPit(sacrifice: boolean = true): void {
        this.clearOutput();
        this.spriteSelect(SpriteDb.s_vapula);
        // N is the number of hours left before night
        if (this.getGame().time.hours < 21) {
            const passed: number = 21 - this.getGame().time.hours;
            this.outputText("<b>" + Owca.Num2Text(passed) + " " + (passed > 1 ? "hours pass" : "hour passes") + "...</b>\n");
            this.getGame().time.hours = 21;
        ...
        this.menu();
        this.addButton(0, "Fight", this.fightZeDemons, sacrifice).hint("Break free from the chains and engage in combat with the demons!");
        this.addButton(1, "Submit", this.loseOrSubmitToVapula).hint("Let the demons have their way with you. " + (this.flags[kFLAGS.VAPULA_SUBMISSIVENESS] >= 100 ? "Given your submissiveness towards the demons, this will lead to a bad end." : "This will make you more submissive towards the demons."));
    }
    // Submit/Fight
    private fightZeDemons(sacrifice: boolean = true): void {
        ...
        if (sacrifice) {
            // Remove weapon
            this.player.createStatusEffect(StatusEffects.Disarmed, 0, 0, 0, 0);
            this.flags[kFLAGS.PLAYER_DISARMED_WEAPON_ID] = this.player.weapon.id;
            this.player.setWeapon(WeaponLib.FISTS);
            this.monster.createStatusEffect(StatusEffects.BowDisabled, 0, 0, 0, 0);
            if (this.player.str < 80 && this.player.spe < 80) {
                this.monster.createStatusEffect(StatusEffects.AttackDisabled, 0, 0, 0, 0);
                this.monster.createStatusEffect(StatusEffects.RunDisabled, 0, 0, 0, 0);
                this.monster.createStatusEffect(StatusEffects.PhysicalDisabled, 0, 0, 0, 0);
            }
            else {
                if (!this.player.canFly()) this.monster.createStatusEffect(StatusEffects.RunDisabled, 0, 0, 0, 0);
            }
        }
    ...
    public loseOrSubmitToVapula(): void {
        this.clearOutput();
        this.outputText("Vapula taunts you as she circles around you.  \"<i>Look at the slutty pet!  Ain't you a slutty pet?  Yes, you are!  Don't pretend you're not hungry for some fat demon cock, I know you are.</i>\"  As she speaks, the crowd gathers closer.  A few creatures show some temerity, giving you pinches and gropes as they near.  The cock-belted imp unties his tentacle; the horror wriggles and squirms as it drops to the ground and slithers toward you.  The tip of the absurdly long pecker inspects your body, pressing itself against your flesh, massaging you in the most sensual places, wetting you with sap-like pre-cum and teasingly grinding itself against your mouth, and then your " + this.player.assholeDescript());
        if (this.player.hasVagina()) this.outputText(", followed by your " + this.player.vaginaDescript(0));
        if (this.player.hasCock()) this.outputText(", before finally wrapping around your " + this.player.cockDescript(0) + " and stroking it; the friction uncontrollably arouses you, and you find yourself reaching full erectness");
        ...
        this.outputText("\n\nSoon you find yourself completely overwhelmed by hot demon hands and dicks touching every part of your body.  In no time at all two imps bend you over and forcefully ram your " + this.player.assholeDescript() + " with their mammoth peckers.");
        ...
        // [if antennae]
        if (this.player.antennae.type > Antennae.NONE) this.outputText("  Your antennae are being harshly pulled and twisted; some imps, in a crazy show of libertinism, start inserting your sensitive peduncles down their bloated urethrae. The intimate friction and the sudden jolts when the internal walls slather your appendages in slick, hot pre-cum are driving you mad with irregular shots of unbearable pleasure.");
        // [if horns]
        if (this.player.horns.value > 0) this.outputText("  You feel your horns being used as leverage to slap your head with even more rock-hard dick.  The impacts stun you until your whole forehead feels numb and coated with a mixture of ballsweat and pre-cum.");
        // [if vagina
        if (this.player.hasVagina()) {
            ...
            this.outputText("  A dick found a way into your " + this.player.vaginaDescript(0) + " and is pushing further inside.  Wait, what's this?  A second, and then a third!  There's no way these titanic columns of flesh will... this is too much... you fear you will be torn in half, but at the last moment, you feel someone spilling the content of a flask over your nether-lips.  As if you had lost control of your " + this.player.vaginaDescript(0) + ", it automatically starts leaking girl-cum in prodigious amounts, and you let out a stifled moan as a delicious shiver runs teasingly across your body.  Your fuck-hole widens under the corrupting effect of the weird mixture you were administered.  ");
        ...
        if (this.player.hasCock()) {
            ...
            if (this.player.cockTotal() >= 7) {
                this.outputText("  Sadly, surrounded as you are, no one will be able to find extra room to ride your remaining cocks; fortunately, you feel warm, long demonic tongues being pressed against your " + this.player.cockDescript(6));
        ...
        // [if cocks]
        if (this.player.hasCock()) {
            this.outputText("  " + this.player.SMultiCockDesc() + " pulsates and throbs");
            // [[if balls]
        ...
        this.outputText("\n\nAfter what seems like hours of forceful penetration, the gigantic members inside you release a milky torrent of corrupted seed, completely packing your colon through your " + this.player.assholeDescript());
        ...
        // [if vagina]
        if (this.player.hasVagina()) this.outputText("\n\nYour own " + this.player.vaginaDescript(0) + " clenches too, doing its best to waste as little semen as possible.  Your muscles quiver as they feel the incoming last release.  Then, in climax, you squirt your own girl-cum, splattering the demon dicks with your juice.");
        // [if lactating breasts]
        if (this.player.lactationQ() > 0) this.outputText("\n\nThe combined efforts of a dozen demons proves too much for your poor " + this.player.chestDesc() + " and your nipples let out a white creamy flood, which is immediately gobbled by thirsty imps.  ");
        else this.outputText("\n\n");
        ...
        else if (this.player.hasStatusEffect(StatusEffects.LostVillagerSpecial)) this.doNext(this.morningAfterRape);
    ...
    public defeetVapulasHorde(): void {
        ...
        if (this.monster.hasStatusEffect(StatusEffects.AttackDisabled)) {
        ...
        if (this.player.isCorruptEnough(60) && this.player.gender > 0) {
    ...
    private rapeZeVapula(): void {
        ...
        if (this.player.hasCock()) {
            ...
            // [if cocks]
            if (this.player.hasCock()) this.outputText("\n\nYou gently stroke your " + this.player.cockDescript(0) + ", bringing it to full erectness.  ");
            // [if vagina]
            if (this.player.hasVagina()) this.outputText("You stick a finger in your own " + this.player.vaginaDescript() + " in order to lubricate it.  ");
            ...
            this.outputText("\n\nWith lazy nonchalance you grab hold of another demon crawling at your feet, another succubus; although not so tall or big-breasted as Vapula, her provocative curves are a delight to your eyes.  You bend her over and, without ceremony, start working her tight pucker, filling her insides with your " + this.player.cockDescript(0) + ".");
            ...
            if (this.player.cumQ() > 1000) this.outputText("  Some of it spurts by little jets outside of her, even as she instinctively contracts her muscles to keep as much spooge as possible and her belly accommodates by bloating out absurdly.");
            ...
            this.outputText(" cumdumpster and grab another slut.  This time you feel like you could use a blowjob to clean off the demonic sweat and slime from your cock; you forcefully open her jaw and shove your " + this.player.cockDescript(0) + " down her throat without any care.  The sudden influx of dickflesh blocking her air-flow makes her choke and cough, but a withdrawal and a good cock-slap reminds her of her place.  \"<i>You fucking bitch!  You goddamn worthless slut!  What good are you if you can't even give proper head?  Lick, you whore!  I want your tongue all over my crotch!  You can breathe when you've properly moistened my ");
            this.outputText(this.player.balls > 0 ? "balls" : "[skin]");
            this.outputText("!</i>\"  The poor demoness, cheek stained with semen, mutters an apology and proceeds to lick your " + this.player.cockDescript(0) + ", suckling the tip, ");
            if (this.player.balls > 0) this.outputText("fondling your " + this.player.ballsDescriptLight() + ", ");
            // [if multicocks]
            ...
            if (this.player.cockTotal() > 1) this.outputText("In an unholy sex-frenzy, you want more; you start grabbing creatures by groups and alternatively pump in and out of their asses with your " + this.player.cockDescript(0) + ", always hungry for more anal.");
        ...
        // Victory rape using vagina/anus [Take Dicks in All Holes](Z)
            ...
            this.outputText("\n\nWith lazy nonchalance you grab hold of another demon crawling at your feet, an incubus; shorter and younger than Vapula, his tight, pert body is a delight to your eyes.  Without ceremony you shove him to the ground and grind your " + this.player.vaginaDescript() + " into his crotch.  He looks at you, semi-comatose and bewildered, as you get a rise out of his dick, which hardens against your damp strokes.  You pin him down as you sink onto his red length, sighing as you rapidly engulf him, then begin to ride him ruthlessly.  ");
            ...
            this.outputText("\n\nYou keep working the incubus's hot cock in front of the once-powerful dominatrix, panting as you accelerate the pace until you finally reach your climax, clenching your thighs tightly to his fit body as your " + this.player.vaginaDescript() + " clenches and spasms.  ");
            // [if squirter]
            ...
            this.outputText("\n\nYou throw your expendable cock toy to one side and grab another slut, this time a succubus.  Your semen-dribbling cunt is now in need of a cleaning, and without any ado you sit yourself straight down onto her face, banging the back of her head against the ground.  She whines into your " + this.player.vaginaDescript() + ", to which you respond by pulling her hair.  \"<i>You fucking bitch!  You worthless slut!  What good are you if you can't even munch rug properly?  Lick, you whore!  I want your tongue to clean up what your useless fuckhead of a friend couldn't keep to himself.</i>\"  The poor demoness mutters an apology and proceeds to lap up the juices drooling out of your hole, licking your labia and mons clean of what the incubus left, darting her wet muscle across your " + this.player.clitDescript() + " before reaching into your vagina, carefully teasing out and drinking every drop of jizz she can find, making your own juices run afresh.  You enjoy her ministrations while staring Vapula in the eyes; she has trouble looking back at you as her body trembles and her eyes widen at your sheer display of depravity.  Your " + this.player.nippleDescript(0) + "s harden");
            if (this.player.biggestTitSize() >= 2) this.outputText(" and your " + this.player.breastDescript(0) + " bounce gently");
    ...
    private owcaMainScreenOn(): void {
        ...
        if (Owca.rand(10) <= 2 && this.flags[kFLAGS.DAYS_SINCE_LAST_DEMON_DEALINGS] >= 7 && this.flags[kFLAGS.OWCA_SACRIFICE_DISABLED] == 0) {
            ...
            this.menu();
            this.addButton(0, "Yes", this.acceptRebeccsPlea, false, true);
            this.addButton(1, "No", this.declineRebeccsPlea);
            return;
        }
        this.outputText(this.images.showImage("location-owca"));
        // [if attitude > 80]
        ...
        this.menu();
        if (this.getGame().time.hours >= 16 && this.flags[kFLAGS.OWCA_SACRIFICE_DISABLED] == 0) {
            ...
            this.addButton(0, "Pit", this.zePit).hint("Volunteer to be the sacrifice tonight and earn the town's trust.");
        } else {
            this.addDisabledButton(0, "Pit").hint("You cannot volunteer to be the sacrifice at the moment. " + (this.flags[kFLAGS.OWCA_SACRIFICE_DISABLED] > 0 ? "The demons will no longer trouble the village of Owca again." : "Come back at four in the evening or later."));
        }
        if (this.flags[kFLAGS.OWCAS_ATTITUDE] >= 50) {
            ...
            this.addButton(1, "Herds", this.herds).hint("Take the opportunity to work at the sheep herd.");
        } else {
            this.addDisabledButton(1, "Herds").hint("The village doesn't seem to trust you at the moment. Consider earning more trust by being the sacrifice.");
        }
        ...
        this.addButton(2, "Rebecc", this.rebeccMenu);
        ...
        if (this.flags[kFLAGS.OWCAS_ATTITUDE] >= 10) {
            ...
            this.addButton(3, "Tavern", this.owcaTavern).hint("Give the tavern a visit and see what kinds of food and beverages are there. Rebecc also hangs out there.");
        } else {
            this.addDisabledButton(3, "Tavern").hint("The tavern appears to be closed at this time.");
        }

        if (this.flags[kFLAGS.GRIMDARK_MODE] > 0) {
            this.addButton(14, "Leave", this.leaveOwcaGrimdark);
        } else {
            this.addButton(14, "Leave", this.camp.returnToCampUseOneHour);
        }
    }

    public leaveOwcaGrimdark(): void {
        this.inRoomedDungeonResume = this.getGame().dungeons.resumeFromFight;
        this.getGame().dungeons._currentRoom = "plains";
    ...
    public owcaTavern(): void {
        this.clearOutput();
        this.outputText(this.images.showImage("location-owca-tavern"));
        this.menu();
        ...
        this.outputText("\n<i>Sheep Milk Bottle: " + (180 - this.flags[kFLAGS.OWCAS_ATTITUDE]) + " gems</i>");
        if ((180 - this.flags[kFLAGS.OWCAS_ATTITUDE]) > this.player.gems) {
            this.addDisabledButton(0, "Sheep Milk");
        } else {
            this.addButton(0, "Sheep Milk", this.owcaBuySetup, this.consumables.SHEEPMK);
        }

        this.outputText("\n<i>Goblin Ale: " + (60 - Math.round(this.flags[kFLAGS.OWCAS_ATTITUDE] / 2)) + " gems</i>");
        if ((60 - Math.round(this.flags[kFLAGS.OWCAS_ATTITUDE] / 2)) > this.player.gems) {
            this.addDisabledButton(1, "Goblin Ale");
        } else {
            this.addButton(1, "Goblin Ale", this.owcaBuySetup, this.consumables.GOB_ALE);
        }

        if (Owca.rand(100) > this.flags[kFLAGS.OWCAS_ATTITUDE]) {
            this.outputText("\n<i>Bro Brew: 2000 gems</i>");
            if ((2000) > this.player.gems) {
                this.addDisabledButton(2, "Bro Brew");
            } else {
                this.addButton(2, "Bro Brew", this.owcaBuySetup, this.consumables.BROBREW);
            }
        }

        this.outputText("\n<i>Minotaur Cum: " + (300 - this.flags[kFLAGS.OWCAS_ATTITUDE]) + " gems</i>");
        if ((300 - this.flags[kFLAGS.OWCAS_ATTITUDE]) > this.player.gems) {
            this.addDisabledButton(3, "MinotaurCum");
        } else {
            this.addButton(3, "MinotaurCum", this.owcaBuySetup, this.consumables.MINOCUM);
        }

        this.outputText("\n<i>Clovis: " + (80 - (this.flags[kFLAGS.OWCAS_ATTITUDE] / 2)) + " gems</i>");
        if ((80 - (this.flags[kFLAGS.OWCAS_ATTITUDE] / 2)) > this.player.gems) {
            this.addDisabledButton(4, "Clovis");
        } else {
            this.addButton(4, "Clovis", this.owcaBuySetup, this.consumables.CLOVERS);
        }

        this.addButton(14, "Back", this.gangbangVillageStuff);
    }
    ...
    private owcaBuySetup(item: ItemType): void {
        ...
        else if (item == this.consumables.CLOVERS) this.buyOwcaShit(item, (80 - Math.round(this.flags[kFLAGS.OWCAS_ATTITUDE] / 2)));
    ...
    private rebeccMenu(): void {
        ...
        this.menu();
        this.addButton(0, "Appearance", this.rebeccAppearance).hint("Get a good look at the sheep-girl's appearance.");
        this.addButton(1, "Bath", this.rebeccBathScene).hint("Relax your tension with a nice milk bath. This could get a bit teasing though.");
        if (this.player.isCorruptEnough(66) || this.player.findPerk(PerkLib.Sadist)) {
            this.outputText("\n\nYou could try and force yourself on her, but you might not be welcome in town after...");
            this.addButton(3, "Surprise Sex", this.rapeRebecc, false).hint("Molest the sheep girl? This might not be a good idea.");
        } else {
            this.addDisabledButton(3, "Surprise Sex").hint("You're not corrupt or sadistic enough to consider doing that.");
        }
        this.addButton(14, "Back", this.gangbangVillageStuff);
    }
    // Rebecc Appearance (Z)
    ...
    private rebeccBathScene(): void {
        ...
        if (this.player.gender == 0) this.outputText(".");
            ...
            // [if cock]
            if (this.player.hasCock()) this.outputText("; your " + this.player.cockDescript(0) + " grows to full erect size");
            if (this.player.hasVagina() && this.player.hasCock()) this.outputText(" and ");
            else if (this.player.hasVagina()) this.outputText("; ");
            if (this.player.hasVagina()) this.outputText("your " + this.player.vaginaDescript() + " leaks a little trickle of girl-juice");
    ...
    private rapeRebecc(outside: boolean = false): void {
        ...
        if (this.player.hasCock()) {
            ...
            this.outputText("\n\nAfter a few minutes of resistance, her body tenses up and you ease yourself into effectively butt-fucking her.  You pull her white fuzzy hair, seize her squeezable hips for more leverage and start raping her ass; the thrusts come hard and fast, almost bringing your " + this.player.cockDescript(0) + " out before jamming it all the way inside again.  This forceful and tigerish motion makes her shriek in excruciating ecstasy; she groans and moans rhythmically at the pace of your regular ass-pounding.  The torment you're inflicting on her poor interior walls is too much to bear and she twists violently to escape the dick-beast dominating her pucker; however, all she can do is wave her appreciable butt, bringing extra sensations to your " + this.player.cockDescript(0) + " and adding extra spice to your already ardent anal-rape.");
            this.outputText("\n\nThe cadenced wobbling of her rump as well as your constant pumping is bringing you to a higher state of pleasure; the heavenly tingle your " + this.player.cockDescript(0) + " is receiving is making you heedless of anything else but the burning heat of her insides cooking your meat.  You feel your lust rise as you keep riding the buttslut below you, ignoring her muffled complaints and whining, her tight confines gradually taking over any control you have over your rod.  You pound Rebecc's ass as if your life depended on it, clutching her curvy hips instinctively like an imp in rut.  Your groin impacts her buttocks with a loud SLAP at every thrust");
            // [if balls]
            if (this.player.balls > 0) this.outputText(", and your " + this.player.ballsDescriptLight() + " almost feel sore from colliding with her dribbling cunt");
            ...
            this.outputText("\n\nWith a last, satisfied sigh, you fall over, " + this.player.cockDescript(0) + " deeply buried inside Rebecc's butt as you feel her warm, curvy body slowly waggling below you.  ");
        ...
        // Rebecc Female Rape Scene (Z)
            ...
            this.outputText("Within three strides you are upon Rebecc; you slap her twice on her cheeks, brutally yank her hair and in a harsh gesture, you turn her around.  The woman yells, \"<i>W-what are you doing?  No, please stop!</i>\"  Completely caught unaware, she gasps and jolts in surprise, crying loudly until you slap her again to shut her up.");
            this.outputText("\n\nYou quickly proceed to undress her, tearing her coarse peasant dress in order to reveal her bountiful, jiggling breasts.  This enormous rack is begging to be touched; you ruthlessly grope it, forcefully bringing her nipples into their erect state.  She tries to struggle away but you hold her tight, her resistance and endless writhing making your " + this.player.vaginaDescript() + " moist.");
    ...
    private desperateVillages(): void {
        ...
        this.menu();
        this.addButton(0, "Yes", this.acceptRebeccsPlea, false, true);
        this.addButton(1, "No", this.fightZeVillagers);
    }
    ...
    private fightZeVillagers(): void {
        ...
        this.monster.createStatusEffect(StatusEffects.GenericRunDisabled, 0, 0, 0, 0);
        this.playerMenu();
    ...
    public beatUpOwca(): void {
        ...
        this.menu();
        if (!this.player.isGenderless() && this.player.lust >= 33 && (this.player.isCorruptEnough(60) || this.player.findPerk(PerkLib.Sadist) >= 0)) {
            this.addButton(0, "Rape Rebecc", this.rapeRebecc, true).hint("Serve the sheep-girl right for setting you up.");
        } else {
            this.addDisabledButton(0, "Rape Rebecc").hint("You need to have either sexes and be of sufficient lust to consider doing that. In addition, you need to be corrupt or sadistic enough.");
        }
        if (this.player.hasStatusEffect(StatusEffects.KnowsWhitefire) || this.player.hasStatusEffect(StatusEffects.KnowsBlackfire) || this.player.findPerk(PerkLib.FireLord) >= 0 || this.player.findPerk(PerkLib.Hellfire) >= 0 || this.player.findPerk(PerkLib.EnlightenedNinetails) >= 0 || this.player.findPerk(PerkLib.CorruptedNinetails) >= 0) {
            this.addButton(1, "Torch Village", this.torchOwcaMotherFuckers).hint("Burn baby burn! Set the village on fire and you can be confident it will be destroyed by the time you return to camp.");
        } else {
            this.addDisabledButton(1, "Torch Village").hint("Unfortunately, you lack the spells or tools to start a fire.");
        }
        this.addButton(4, "Leave", this.leaveOwcaAfterWhupping);
    }
    ...
    private torchOwcaMotherFuckers(): void {
        ...
        this.menu();
        if (!this.player.isGenderless() && this.player.lust >= 33 && (this.player.isCorruptEnough(60) || this.player.findPerk(PerkLib.Sadist) >= 0)) {
            this.addButton(0, "Abuse Her", this.rapeRebecc, true).hint("Serve the sheep-girl right for setting you up.");
        } else {
            this.addDisabledButton(0, "Abuse Her").hint("You need to have either sexes and be of sufficient lust to consider doing that. In addition, you need to be corrupt or sadistic enough.");
        }
        this.addButton(1, "Leave", this.torchUpVillagersAndLeave);
    }
    ...
    public loseToOwca(): void {
        ...
        this.player.createStatusEffect(StatusEffects.LostVillagerSpecial, 0, 0, 0, 0);
    }
    // Post-Mob Encounter (Z)
    private morningAfterRape(): void {
        this.player.removeStatusEffect(StatusEffects.LostVillagerSpecial);
        this.clearOutput();
        ...
        this.menu();
        this.addButton(0, "Forgive", this.forgiveOwca).hint("Show the sheep some MERCY. Grudges are not to be held after all.");
        this.addButton(1, "Rape", this.rapeRebecc, false).hint("Whether the sheep-girl did it or not, teach her a lesson.");
        this.addButton(2, "Leave", this.fuckThisShit).hint("You don't really want to have anything with the sheep-girl.");
    }
    // Option: Forgive (Z)
    private forgiveOwca(): void {
        ...
        // To main owca menu
        if (this.getGame().inCombat)
            this.combat.cleanupAfterCombat(this.gangbangVillageStuff);
    ...
    private rebeccsLastPlea(): void {
        ...
        this.menu();
        this.addButton(0, "Rape Her", this.rapeRebecc, true).hint("You don't give a fuck about what's happening but you could take advantage of her helplessness. However, you'll no longer be able to return to the village.");
        this.addButton(1, "Face Them All", this.faceDownHordes).hint("Take on the horde of demons to make up for it.");
        this.addButton(2, "Leave", this.leaveRebeccToBeRaped).hint("Leave the sheep-girl to her fate and choose not to return to the village.");
    }
    // Option: Leave (Z)
    ...
    private subdueVapula(): void {
        ...
        this.menu();
        this.addButton(0, "Disband", this.disbandHorde).hint("Permanently disband the horde. The demons shall bother the village no more.");
        if (!this.player.isGenderless() && (this.player.isCorruptEnough(66))) {
            this.addButton(1, "EnslaveVapula", this.enslaveVapulaWithYourWang).hint("Make the succubus your slave and bring her to your camp. This will also disband her horde permanently.");
        } else {
            this.addDisabledButton(1, "EnslaveVapula", "This scene requires you to have genitals and high enough corruption.");
        }
        if (!this.player.isGenderless() && this.player.lust >= 33) {
            this.addButton(2, "JustFuckEm", this.rapeZeVapula);
        } else {
            this.addDisabledButton(2, "JustFuckEm", "This scene requires you to have genitals and high enough arousal.");
        }
        this.addButton(3, "Skip Out", this.combat.cleanupAfterCombat).hint("None of the options seem that appealing. Just leave the horde.");
    }
    // Option: Disband (Z)
