//Prisoner Mod WIP
	...
	export class Prison extends BaseContent implements TimeAwareInterface
	{
		...
		public  prisonCombatWinEvent = undefined;
		public  prisonCombatLoseEvent = undefined;
		...
		public  get inPrison(): boolean { return flags[kFLAGS.IN_PRISON] > 0; }
		...
		{
		var  successChance: number = 5 + rand(5);
			if (player.statusEffectv2(StatusEffects.PrisonRestraints) == 1) {
		...
		public  prisonRoom(allowsEvents: boolean = true): void {
			...
			if (flags[kFLAGS.PRISON_DIRT_ENABLED] > 0) {
				outputText("\n\nThe room is ");
			var  cleanlinessLevel: number = Math.floor(player.statusEffectv2(StatusEffects.PrisonCaptorEllyStatus) / 25);
		...
		{
		var  fight = doPrisonEscapeFight;
		var  seduce = doPrisonEscapeSeduce;
		var  bribe = doPrisonEscapeBribe;
		var  sneak = doPrisonEscapeSneak;
		var  run = doPrisonEscapeRun;
		var  quest = doPrisonEscapeQuestRun;
			clearOutput();
		...
		var  i: number = 1;
			...
			if (fight != undefined) outputText("You think you might be able to win if you fought your guard. " + prisonWillCostDescript(20) + "\n");
			if (seduce != undefined) outputText("It occurs to you that if you seduce your guard you could get away afterwards. " + prisonWillCostDescript(10) + "\n");
			if (bribe != undefined) outputText("Given that your sizable stack of gems is probably with your gear you consider trying to pay for your freedom. " + prisonWillCostDescript(10) + "\n");
			if (sneak != undefined) outputText("If you were quick and quiet enough you think you might be able to sneak out when a guard enters the room. " + prisonWillCostDescript(15) + "\n");
			if (run != undefined) outputText("Maybe you could just walk out the door? " + prisonWillCostDescript(5) + "\n");
			if (quest != undefined) outputText("You've been given a task to do, so you can just walk out the door. \n");
		...
		{
		var  charmScore: any = NaN;
		var  bribeCost: any = NaN;
		...
		var  temp: string = "";
			if (flags[kFLAGS.PRISON_STORAGE_ARMOR] != 0) {
			var  tempArmor: any = ItemType.lookupItem(flags[kFLAGS.PRISON_STORAGE_ARMOR]);
				if (tempArmor != undefined) player.setArmor(tempArmor);
		...
		var  temp: string = "";
			if (flags[kFLAGS.PRISON_STORAGE_WEAPON] != 0) {
			var  tempWeapon: any = ItemType.lookupItem(flags[kFLAGS.PRISON_STORAGE_WEAPON]);
				if (tempWeapon != undefined) player.setWeapon(tempWeapon);
		...
		var  temp: string = "";
			if (flags[kFLAGS.PRISON_STORAGE_SHIELD] != 0) {
			var  tempShield: any = ItemType.lookupItem(flags[kFLAGS.PRISON_STORAGE_SHIELD]);
				if (tempShield != undefined) player.setShield(tempShield);
		...
		public  prisonEscapeFightStart(combatID:Monster = undefined): void
		{
			if (!combatID)
			{
		...
		public  prisonEscapeFightAutoLose(combatID:Monster = undefined): void
		{
			if (!combatID)
			{
			var  combatID:Monster = prisonGuard.guardCombatID;
		...
		public  prisonEscapeSeduceAutoLose(combatID:Monster = undefined): void
		{
			if (!combatID)
			{
			var  combatID:Monster = prisonGuard.guardCombatID;
		...
		{
		var  eventOccurred: any = undefined;
			eventOccurred = false;
			if (flags[kFLAGS.PRISON_PUNISHMENT] == 1)
			{
				if (rand(8) == 0)
		...
		{
		var  trainingLevelCounter: any = undefined;
			if (flags[kFLAGS.IN_PRISON] > 0)
			{
