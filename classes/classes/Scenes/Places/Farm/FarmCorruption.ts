...
export class FarmCorruption extends AbstractFarmContent {
    ...
    public whitneyCorruption(mod: number = 0): number {
        ...
        if (this.flags[kFLAGS.FARM_CORRUPTION_APPROACHED_WHITNEY] == 0 && this.flags[kFLAGS.WHITNEY_CORRUPTION] > 30) {
            this.flags[kFLAGS.WHITNEY_CORRUPTION] = 30; // Whitney at 30 corruption clamp.
        }
        else if (this.flags[kFLAGS.WHITNEY_LEAVE_0_60] == 0 && this.flags[kFLAGS.WHITNEY_CORRUPTION] > 60) {
            this.flags[kFLAGS.WHITNEY_CORRUPTION] = 60; // Whitney at 60 corruption clamp.
        }
        else if (this.flags[kFLAGS.WHITNEY_LEAVE_61_90] == 0 && this.flags[kFLAGS.WHITNEY_CORRUPTION] > 90) {
            this.flags[kFLAGS.WHITNEY_CORRUPTION] = 90; // Whitney at 90 corruption clamp.
        }
        else if (this.flags[kFLAGS.WHITNEY_MENU_91_119] == 0 && this.flags[kFLAGS.WHITNEY_CORRUPTION] > 119) {
            this.flags[kFLAGS.WHITNEY_CORRUPTION] = 119; // Whitney at 119 corruption clamp.
        }
    ...
    public collectTheGoodies(): void {
        ...
        this.addButton(14, "Back", this.rootScene);
    }

    private getItemObj(flag: number): Consumable {
    ...
    private takeItems(flag: number): void {
        const item: Consumable = this.getItemObj(flag);
    ...
    public takeoverPrompt(): boolean {
        ...
        if (this.player.isCorruptEnough(70) && this.corruptFollowers() >= 2 && this.player.level >= 12) {
            if (this.flags[kFLAGS.FARM_CORRUPT_PROMPT_DISPLAY] == 0) {
                this.clearOutput();
                if (this.flags[kFLAGS.KELT_BREAK_LEVEL] >= 4) {
                    this.takeoverPromptKelly();
                }
                else if (this.player.hasStatusEffect(StatusEffects.MarbleRapeAttempted)) {
    ...
    public takeoverPromptNow(): void {
        ...
        if (this.flags[kFLAGS.KELT_BREAK_LEVEL] >= 4 || this.player.hasStatusEffect(StatusEffects.MarbleRapeAttempted)) {
        ...
        if (!this.player.hasStatusEffect(StatusEffects.NoMoreMarble)) this.flags[kFLAGS.FOLLOWER_AT_FARM_MARBLE] = 0; // Don't have to care about recruitment paths -- she'll fuck off based on corruption before the player can corrupt the farm.
    ...
    public farmMenu(): void {
        ...
        if (!this.player.hasStatusEffect(StatusEffects.MarbleRapeAttempted) && !this.player.hasStatusEffect(StatusEffects.NoMoreMarble) && this.player.hasStatusEffect(StatusEffects.Marble) && this.flags[kFLAGS.MARBLE_WARNING] == 0) this.addButton(1, "Marble", this.farm.meetMarble);

        if (this.player.hasStatusEffect(StatusEffects.Kelt) && !this.player.hasStatusEffect(StatusEffects.KeltOff)) {
        ...
        if (this.player.hasKeyItem("Breast Milker - Installed At Whitney's Farm") >= 0) {
            if (this.player.hasStatusEffect(StatusEffects.Milked)) {
                this.outputText("\n\n<b>Your " + this.player.nippleDescript(0) + "s are currently too sore to be milked.  You'll have to wait a while.</b>");
            }
    ...
    private corruptingTheFarmExplore(): void {
        ...
        this.addButton(14, "Back", this.farmMenu);
    }
    ...
    private dogeNotCorruptYetMenu(): void {
        ...
        if (this.whitneyCorruption() <= 60 && this.flags[kFLAGS.WHITNEY_LEAVE_0_60] == 0) this.addButton(14, "Back", this.dogeNotCorruptLeaveFirstTime);
        else if (this.whitneyCorruption() <= 90 && this.flags[kFLAGS.WHITNEY_LEAVE_61_90] == 0) this.addButton(14, "Back", this.dogeNotCorruptLeave6190);
    }
    ...
    private prosperityGoNotCorrupt(): void {
        ...
        let lowProtection: boolean = false;
    ...
    private investmentMenu(): void {
        ...
        if (!this.whitneyCorrupt()) this.addButton(14, "Back", this.dogeNotCorruptYetMenu);
        else this.addButton(14, "Back", this.dogeCorruptedMissionComplete);
    }
    ...
    private firstOralTraining(): void {
        ...
        this.outputText(" there has ever been. Doubt wages war with aroused apprehension on Whitney’s face.");
    ...
    private firstVaginaOralTraining(): void {
        ...
        if (this.player.getClitLength() > 3) this.outputText(" despite the fact it must be practically poking her in the eye");
    ...
    private firstVaginaOralTrainingStageThree(): void {
        ...
        if (this.player.getClitLength() >= 4) this.outputText(" She uses her new anchorage to take your bulging fem-dick entirely into her mouth, sucking on it avidly; she hums as she does it, sending ecstatic pleasure thrumming through you.");
    ...
    private vaginaOralTrainingMaxed(): void {
        ...
        if (this.player.butt.rating < 4) this.outputText("firm butt");
        if (this.player.butt.rating < 8) this.outputText("supple rump");
        ...
        this.player.orgasm('Vaginal');
        this.dynStats("sen-", 1);
        if (this.player.wetness() < 5 && rand(4) == 0) this.player.vaginas[0].vaginalWetness++;
        this.doNext(this.camp.returnToCampUseOneHour);
    }
    ...
    private whitneyDomPleasure(): void {
        ...
        this.doNext(scenes[rand(scenes.length)]);
    }
    ...
    private whitneyDomStraponDoggy(): void {
        ...
        else this.outputText("<b>SOMETHING DERPED UP!</b>"); // trace("Couldn't determine body type, welp.");
        ...
        // wet and/or loose
        const capacity: number = (this.player.hasVagina()) ? this.player.vaginalCapacity() : this.player.analCapacity();
        if (capacity >= 36) {
    ...
    private brandingMenu(): void {
        ...
        if (this.flags[kFLAGS.FARM_CORRUPTION_BRANDING_MENU_UNLOCKED] == 0) {
            ...
            this.addButton(14, "Back", this.dogeCorruptedMissionComplete);
        }
    }
    ...
    public brandSelect(slot: number): void {
        ...
        this.addButton(14, "Back", this.brandSlotSelect);
    }
    ...
    public amilyBrandSelect(slot: number): void {
        ...
        this.addButton(14, "Back", this.amilyBrandSlotSelect);
    }
    ...
    public jojoBrandSelect(slot: number): void {
        ...
        this.addButton(14, "Back", this.jojoBrandSlotSelect);
    }
    ...
    public bimboSophieBrandSelect(slot: number): void {
        ...
        this.addButton(14, "Back", this.bimboSophieSlotSelect);
    }
    ...
    public vapulaBrandSelect(slot: number): void {
        ...
        this.addButton(14, "Back", this.vapulaSlotSelect);
    }
    ...
    public kellyBrandSelect(slot: number): void {
        ...
        this.addButton(14, "Back", this.kellySlotSelect);
    }
    ...
    public smallMilkyBrandSelect(slot: number): void {
        ...
        this.addButton(14, "Back", this.smallMilkySlotSelect);
    }
    ...
    public bigMilkyBrandSelect(slot: number): void {
        ...
        this.addButton(14, "Back", this.bigMilkySlotSelect);
    }
    ...
    private orgyRoomSubMassage(): void {
        ...
        if (this.player.butt.rating >= 6) this.outputText("soft");
        ...
        if (this.player.isBiped()) {
            ...
            if (!(this.player.lowerBody.type == LowerBody.HOOFED || this.player.lowerBody.type == LowerBody.NAGA || this.player.lowerBody.type == LowerBody.GOO || this.player.lowerBody.type == LowerBody.PONY)) {
                ...
                if (this.player.lowerBody.type == LowerBody.DEMONIC_CLAWS || this.player.lowerBody.type == LowerBody.LIZARD || this.player.lowerBody.type == LowerBody.HARPY || this.player.lowerBody.type == LowerBody.DRAGON) this.outputText(" claws");
        ...
        // Naga:
            ...
            if (this.player.tongue.type == Tongue.SNAKE) this.outputText(" You grin, flicking your forked tongue out to smell her arousal.");
