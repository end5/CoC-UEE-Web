...
export class Kelly extends AbstractFarmContent implements TimeAwareInterface {
    ...
    public timeChange(): boolean {
        ...
        if (this.getGame().time.hours > 23) {
            if (this.flags[kFLAGS.KELLY_REWARD_COOLDOWN] > 0 && this.getGame().time.days % 3 == 0) this.flags[kFLAGS.KELLY_REWARD_COOLDOWN] = 0;
    ...
    public kellySprite(nude: boolean = false): void {
        if (!this.pregnancy.isPregnant) {
            if (this.flags[kFLAGS.KELLY_BONUS_BOOB_ROWS] == 0) this.spriteSelect(SpriteDb.s_kelly);
            else this.spriteSelect(SpriteDb.s_kelly_brst);
        }
        else {
            if (this.flags[kFLAGS.KELLY_BONUS_BOOB_ROWS] == 0) this.spriteSelect(SpriteDb.s_kelly_preg);
            else this.spriteSelect(SpriteDb.s_kelly_brst_preg);
        }
    }
    ...
    public breakingKeltOptions(): void {
        ...
        if ((!this.player.hasCock() && this.flags[kFLAGS.KELT_BREAK_LEVEL] == 0) || this.flags[kFLAGS.NEVER_RESIST_KELT] == 1 || this.player.statusEffectv2(StatusEffects.Kelt) >= 40 || !this.player.hasStatusEffect(StatusEffects.Kelt)) {
        ...
        this.menu();
        this.addButton(0, "Endure", this.farm.keltScene.keltEncounter).hint("Swallow your pride and endure his disrespect.  Mastering the bow is far more important than some petty remarks.");
        this.addButton(1, "Resist Kelt", this.resistKeltsBSBreakHimIntro).hint("Find a way to turn the tables and make that arrogant asshole your bitch.  You would need lots of succubi milk to pull this off. A pink egg might help, too.");
        this.addButton(2, "Never Resist", this.neverBreakKeltIntoKelly).hint("Just accept that Kelt will never change and move on.  No point in getting upset about things you can't change.");
        this.addButton(4, "FIGHT!", this.fightToBeatKelt);
        this.addButton(10, "Appearance", this.keltAppearance);
    }

    // Appearance
    public keltAppearance(): void {
        this.clearOutput();
        this.credits.authorText = "Stygs";
        this.outputText("Kelt is a 8 foot tall centaur with a muscular and well trained body.");
        ...
        this.menu();
        this.addButton(0, "Next", this.breakingKeltOptions);
    }

    public fightToBeatKelt(): void {
        this.clearOutput();
        this.outputText("Deciding you've had enough with Kelt, you ready your " + this.player.weaponName + ". Kelt readies his bow. \"<i>Do you really think you can beat me, slut?</i>\"");
        this.flags[kFLAGS.KELT_KILL_PLAN] = 1;
    ...
    private breakKeltGo(): void {
        ...
        this.outputText(" small droplets of pre-cum begin dripping out of your " + this.player.multiCockDescriptLight() + " before being swallowed by the potent, feminine mixture down below.  The white spots are drowning in that milky goo, but you're confident that Kelt will be able to recognize the taste.");
    ...
    public defeatKellyNDBREAKHIM(): void {
        ...
        if (this.monster.lust >= this.monster.maxLust()) this.outputText("Kelt moans, mauling at his mantits in his lust before he realizes what's going on");
        ...
        this.outputText(" your erect " + this.player.multiCockDescriptLight() + ".");
    ...
    private finalKeltBreaking(): void {
        ...
        this.outputText("\n\nYou laugh triumphantly as you whip your " + this.player.multiCockDescriptLight() + " out of your [armor]; the slut is more than ready!");
        ...
        this.outputText("\n\nThe cum-flow eventually starts to ebb; less and less goo pours from your " + this.player.multiCockDescriptLight() + " and your orgasm gradually fades out.  When your euphoric pleasure wears off, you finally get a good look at the creature standing before you.");
    ...
    private keltFucksShitUpII(): void {
        this.clearOutput();
        this.outputText("You awaken at the periphery of the farm, thankful to be alive.  Kelt is nowhere to be seen.  You have to wonder if Whitney saved you or the dumb beast was too stupid to finish you off.  Whatever the case, you head back to camp to lick your wounds.  ");
        if (this.flags[kFLAGS.KELT_KILL_PLAN] != 1) {
            this.outputText("<b>The worst indignity of all is that he broke a lot of your succubi milks.</b>  He'll likely have regained some more of his maleness by the time you're ready to attempt teaching him another lesson.");
            this.player.consumeItem(this.consumables.SUCMILK, 5);
    ...
    private approachKelly(): void {
        ...
        if (this.flags[kFLAGS.KELLY_VAGINALLY_FUCKED_COUNT] > 0 && this.flags[kFLAGS.TIMES_PUNISHED_KELLY] == 0 && rand(3) == 0 && this.player.hasCock()) {
        ...
        if (this.getGame().time.hours >= 9 && this.getGame().time.hours <= 11 && this.flags[kFLAGS.KELLY_KIDS] >= 2) {
            this.outputText("Kelly is standing in the shadow of her barn, an expression of blissful contentment on her face as she nurses your ");
        ...
        // Standard:
            ...
            this.outputText("As soon as you enter the big, grassy expanse beyond the barns, your centaur slave canters over, radiating happiness and hunger.  \"<i>[Master], you've come to visit! Is it feeding time?</i>\"");
        }
        this.menu();
        this.addButton(10, "Appearance", this.kellyAppearance);
        if (this.player.lust >= 33) this.addButton(0, "Sex", this.kellySexMenu);
        else {
            this.outputText("\n\n<b>You aren't aroused enough to pursue sex with your toy right now.</b>");
            this.addDisabledButton(0, "Sex", "This scene requires you to have sufficient arousal.");
        }
        if (this.flags[kFLAGS.KELLY_CUNT_TYPE] == 0) {
            if (this.player.hasItem(this.consumables.EQUINUM)) {
                ...
                this.addDisabledButton(5, "Give Equinum", "You need Equinum for this.");
            }
        }
            ...
            if (this.player.hasItem(this.consumables.SUCMILK)) {
                ...
                this.addDisabledButton(5, "Give SucMilk", "You need a Succubi Milk for this.");
            }
        }
        ...
        else this.addDisabledButton(6, "Give CanineP", "You need a Canine Pepper for this.");
        ...
        else this.addDisabledButton(7, "Punish", "You have no reason to punish Kelly right now.");
        if (this.flags[kFLAGS.TIMES_PUNISHED_KELLY] > 0 && this.flags[kFLAGS.KELLY_REWARD_COOLDOWN] == 0 && rand(3) == 0) {
        ...
        else this.addDisabledButton(8, "Reward", "You have no reason to reward Kelly right now.");

        // Showing up resets Kelly's desire not to fap without you
        this.flags[kFLAGS.KELLY_DISOBEYING_COUNTER] = 0;

        if (this.flags[kFLAGS.FARM_CORRUPTION_STARTED] == 0) this.addButton(14, "Leave", this.camp.returnToCampUseOneHour);
        else this.addButton(14, "Back", this.farm.farmCorruption.rootScene);
    }

    private kellySexMenu(): void {
        this.menu();
        this.clearOutput();
        this.outputText("Kelly is standing before you, her emerald green eyes watching your every move with interest.  \"<i>What you do want to do, [Master]?</i>\"");
        if (this.player.hasCock() && this.player.lust >= 33) {
            if (this.player.cockThatFits(300) >= 0 || this.flags[kFLAGS.KELLY_CUNT_TYPE] == 1) {
                ...
                if (this.player.countCocksOfType(CockTypesEnum.TENTACLE) >= 2) this.addButton(1, "TentaFuck", this.tentaFuckKelly);
                else this.addDisabledButton(1, "TentaFuck", "This scene requires you to have at least 2 tentacle cocks.");
            }
            ...
            else this.addDisabledButton(2, "Titfuck", "This scene is not available for taurs.");
            this.addButton(3, "Blowjob", this.kellyBJsAhoy);
            this.addButton(4, "Talk And HJ", this.talkNHandToKelly);
        }
        this.addButton(14, "Back", this.approachKelly);
    ...
    private fuckKellysCunt(): void {
        ...
        this.outputText("Holding your " + this.player.cockDescript(x) + " in one hand, you press your " + this.player.cockHead(x) + " against her entrance.   The steamy cunt pulses wetly around your girth, slowly spreading its thick lips to accept your rigid endowment.");
        ...
        this.outputText("  You slowly slide inside the tight tunnel, feeling Kelly's powerful muscles squeeze delightfully around you, massaging your " + this.player.cockDescript(x) + " with slow undulations.");
    ...
    private tentaFuckKelly(): void {
        this.clearOutput();
        this.outputText("With a mischievous grin, you remove your [armor], fully revealing your " + this.player.multiCockDescriptLight() + ".  The plant-like appendages wriggle around the horny centauress, inspecting her body from every angle.  You feel your own lust rising as the tentacle peckers grow harder, their green heads turning pink in arousal.  Kelly stammers, confused: \"<i>W-what are you-</i>\"");
        ...
        this.outputText("\n\nSLAM!  You profit from the slut's babbling to shove your " + this.player.cockDescript(one) + " right into her gaping mouth, forcefully filling her throat with dick-flesh.  Her lips instinctively cling to your shaft, sucking it like a popsicle.  Drops of pre-cum are quickly swallowed and ingested by the hungry whore as they escape your urethra.  You begin thrusting in and out of her warm mouth, but her lips squeeze your rod so tight that you have to fight for every inch.  As you stuff her full of your " + this.player.cockDescript(one) + ", you can hear her endless muffled moans; the slut is obviously getting off to the taste of your meaty junk.  Completely driven with lust, the green tentacle prick is vibrating on its own as it fucks Kelly's throat, sending you soft, prickling sensations.  You can't help but release a grunt as the centaur slut fellates you so expertly.");
        this.outputText("\n\nKelly is doing an incredible job with her mouth, teasing you into oblivion.  Your " + this.player.cockDescript(two) + " is convulsing recklessly in frustration, begging to be stuffed into some warm hole.  Therefore, you naturally direct it toward the centauress' wet snatch; it is easily spottable due to the regular streams of juices that ooze from it.  Moreover, the slut is incredibly turned on from sucking you off, and the fem-spunk trails are even thicker than usual.  It doesn't take long for your secondary tentacle dong to reach the entrance of her moist love-tunnel.  You grind the tip of your " + this.player.cockDescript(two) + " against it, using her genitals as a masturbation aid.  When it is hard as bark and ready to go, you ram it through Kelly's fuck-hole, utterly piercing her from behind.");

        this.outputText("\n\nYou ream the centaur slut from both sides, making her whinny and thrash violently.  She tramples the floor with her hooves, arches her back and screams in ecstasy through the inches of dick-flesh blocking her throat; she is completely heedless of the world beside the two hard vegetal poles fully impaling her on either end.  You pump away in rhythm, synchronizing each thrust in both holes.  Her twat is almost inhumanly hot, and gloriously tight.  However, the slut tries her best to wrap her lips even tighter around your main flexible pecker, and her tongue is teasing your " + this.player.cockDescript(one) + " in the most excruciating way.  You stuff the mare slut full of your green appendages, treating her body like a bipolar cocksleeve.");

        // [if (cocks > 2)
        if (three >= 0) {
            this.outputText("\n\nThe display of Kelly trembling in pleasure as you spit her like a pig on your duo of plant-like pricks is enticing, and your " + this.player.cockDescript(three) + " is aching with need.  It almost moves on its own volition toward the woman's perky tits, ready for a good boobjob.  The supple green wang lodges itself between the busty melons; the slut instantly begins to squeeze her heavy globes around the rod of dick-flesh, fully entrapping it in her bosom.  Her tits are so tender and soft you feel like your tentacle pecker is about to melt.  Groaning in pleasure, you thrust your " + this.player.cockDescript(three) + " up and down as the titty-fucked whore accompanies your motion.  You grind in rhythm so the slut doesn't forget your other tentacle dongs.  Fortunately, Kelly quickly picks up your intention and soon you can't tell which member is providing you with the most pleasure; her mouth, vagina and soft tits are all working in conjunction, and you are fully lost in a never-ending spiral of ecstasy.");
        }
        // if (cocks > 3)
        if (four >= 0) {
            this.outputText("\n\nYour " + this.player.cockDescript(four) + " trembles furiously, unable to find some body part that isn't taken by its colleagues.  You grin wickedly as an idea comes to your mind.  The protruding appendage writhes as it threads its way toward Kelly's right hand; the slut is still focusing on your " + this.player.cockDescript(three) + ", squeezing her tits as tightly as she can.  Needless to say, she is surprised when another tentacle shoves itself in the way, right under her palm.  She doesn't lose her composure, though; using her forearm to press on her chest, her hand quickly grabs hold of your tree-like pecker and begins stroking your length.  Although her handjob doesn't feel as warm and tight as the wet depths of her love-tunnel or her throat, or even the squelching contact of her tender boob-flesh, she teases you efficiently with her fingers.  Her hand massages your " + this.player.cockDescript(four) + " and clutches it like a vice, milking some drops of pre-cum out of your shaft.  She takes care of the most sensitive points and often makes you moan when she squeezes the towering rod.");
        }
        // [if (cocks > 4)
        if (five >= 0) {
            this.outputText("\n\nKelly still has a free hand, and your " + this.player.cockDescript(five) + " is left alone, flustered.  The squirming tentacle quickly finds a comfortable spot near her boob, but this time you don't need to do anything; the eager slut catches the vegetal rod and begins to stroke it furiously.  Her hands pump along your lengths in turn; both pricks are so hard that you don't know which one will cum first.  Her fingers move across the surface of your meaty green dongs with expert care, teasing you more and more.  You enjoy the double handjob, your duo of plant-like dongs ready to explode in a mess of cum.");
        }
        // [if (cocks > 5)
        if (this.player.countCocksOfType(CockTypesEnum.TENTACLE) > 5) {
    ...
    private punishKelly(): void {
        ...
        this.menu();
        if (this.flags[kFLAGS.TIMES_RIDDEN_KELLY_FOR_PUNISHMENT] > 0 && this.player.statusEffectv1(StatusEffects.TelAdre) < 1) {
    ...
    private getARimjobFromKelly(): void {
        ...
        // [Centaur:]
        else if (this.player.isTaur()) this.outputText("\n\nYou gesture at the ground.  \"<i>Lie down,</i>\" you command.  Kelly carefully folds her body up to do so, until her horse half is prostrate and only her human half is propped up.  You lead yourself back until your rear end is near to her face; grinning to yourself, you slowly brush your semi-erect " + this.player.cockDescript(x) + " against her soft face, teasing her with the idea that you've changed your mind; you cannot see her but from the small bitten off grunts and moans you know she's struggling with the urge to wrap her lips around it.  Finally, you carefully lean downwards, so that instead of your dick she's faced with your [butt].");

        // [Biped/Other:
        else this.outputText("\n\nYou gesture at the ground.  \"<i>Lie down,</i>\" you command.  Carefully Kelly folds her body up to do so, until her horse half is prostrate and only her human half is propped up.  You step forwards and for a few moments slowly brush your semi-erect " + this.player.cockDescript(x) + " against her soft face, teasing her with the idea that you've changed your mind; she struggles with the urge to wrap her lips around it, emitting small bitten off grunts and moans to your ministrations.  Finally, with a grin you pull away and turn around, so that her mouth is now facing your [butt].");
        ...
        this.outputText("\n\nHer tongue probes deeper and deeper into you, her plump lips pressing into your [asshole] until she touches something sensitive.  Your " + this.player.cockDescript(x) + " immediately reacts, twitching upwards restlessly.");
        ...
        this.outputText(".  Grunting, you reach your peak.  Your " + this.player.cockDescript(x) + " opens wide and you ejaculate streams of cum, splattering into the dust in front of you, your whole body clenching and seizing around the warm, wet muscle wedged in your [butt].");
        // [Pussy:]
        ...
        // [Repeat:
            ...
            this.outputText("\n\n\"<i>Th-thank you for the lesson, [Master],</i>\" she says.  She knows the drill by now; she quickly gets to her feet and then trots over to the off-white slimy mess you have created.  What a good girl.  Before she can get started you present your " + this.player.cockDescript(x) + "; she murmurs thanks for the gift and licks your oozing head spotless first before getting on with the main meal.  You rub her behind the ear as she licks away at the spent seed at your feet and then leave her to it.");
            if (this.player.cumQ() >= 1000) this.outputText("  She is, after all, going to be here for a while.");
        }
    ...
    private rideKellyForPunishment(): void {
        ...
        if (this.flags[kFLAGS.TIMES_RIDDEN_KELLY_FOR_PUNISHMENT] == 1) {
            ...
            // [PC not rider:
                ...
                if (!this.player.isTaur()) {
                    ...
                    this.outputText("\n\nNeither of you are in any condition to keep this up for long, and after five minutes of fevered fucking, you groan as you clutch Kelly's ass firmly and pour yourself into her, your " + this.player.cockDescript(x) + " surging line after line of cum into her.  She moans breathily and happily to the heavenly sensation of being filled with your seed, her vagina milking you for every drop it can get.  You don't even know how many times she orgasmed; her pussy dribbled and gushed the entire time you were inside her.");
                }
                // Centaur:
                else {
                    this.outputText("\n\nEventually you arrive back at camp, and not a moment too soon.  Your nostrils are full of the smell of Kelly, the sight of her hot, moist skin and you are practically champing at the bit yourself; even her rippling horse body speaks directly to your own, urging you irresistibly to mount her.  You bring her to a shuddering halt, rip off your [armor], and then without any further ado rear onto her back and sink your straining " + this.player.cockDescript(x) + " straight into her cunt.  It is absolutely soaked with arousal and you encounter virtually no resistance as you thrust your hips into her soft butt and immediately begin to fuck her as hard as you can.  In your sex daze you realize you are still clutching the reigns; you pull at them intermittently as you push as much of yourself in the centauress hot, grasping warmth as you can, making her rear, squealing and then screaming around her gag to the dual sensation of her [master]'s dick burying itself in her and the harness pulling tight upon her head and bit, thrusting back into you as best she can.");
                    this.outputText("\n\nYou wish you could live forever in these luscious moments; it is so indefinably right to be reared over your eager submissive, encompassing her with your own flesh as you penetrate her deep wetness, both your human and horse parts cry out for it; mounting her, breeding her, imprinting yourself upon her and making her yours.  Neither of you are in any condition to keep it up for long though, and after five minutes of this fevered fucking you groan as you clutch Kelly's breasts hard and pour yourself into her, your " + this.player.cockDescript(x) + " surging line after line of cum into her.  She moans breathily and happily to the heavenly sensation of being filled with your seed, her vagina milking you for every drop it can get.  You don't even know how many times she orgasmed; her pussy dribbled and gushed the entire time you were inside her.");
                }
            }
            this.kellyPreggers();
            // All go to:
        ...
        // First repeat:
            ...
            // [Not centaur:
            if (!this.player.isTaur()) this.outputText("   You sigh and enjoy the sight of her pretty head bobbing over your groin, your " + this.player.cockDescript(x) + " encapsulated in soft shifting blankets of wet mouth flesh, your ears filled with her slurps and muffled sighs.  You are quickly pushed towards a new high by the worshipful treatment; you give her what she wants and cum in her mouth, letting her savor your texture and flavor; she closes her eyes and swallows, orgasm shuddering through her as it hits her stomach.  You smile and hold her face as she basks in an afterglow earned from pure debasement.");
            else this.outputText("  You sigh and enjoy your " + this.player.cockDescript(x) + " encapsulated in soft shifting blankets of wet mouth flesh, your ears filled with her slurps and muffled sighs.  You are quickly pushed towards a new high by the worshipful treatment; you give her what she wants and cum in her mouth, letting her savor your texture and flavor; she closes her eyes and swallows, orgasm shuddering through her as it hits her stomach.  You smile, turn and hold her face as she basks in an afterglow earned from pure debasement.");
        ...
        // Second+ repeat:
            ...
            if (this.player.tallness < 78 && !this.player.isTaur()) {
                ...
                this.outputText("\n\nYou lead her down a side alley - giving a hard look to the onlookers which makes most of them remember they have business elsewhere- and then, braced against her formidable weight, slowly dip your " + this.player.cockDescript(x) + " into her cunt until it is completely slathered in girl slime.  She sighs to the agonisingly gentle treatment and then gives out a muffled cry out of shock as you withdraw completely and, using the lubrication she provided, push into her tight asshole.");
    ...
    private takeKellysVirginity(): void {
        ...
        if (!this.player.isTaur()) this.outputText("get down behind her and line your " + this.player.cockDescript(x) + " with her dribbling lips.");
        else this.outputText("lie down behind her, sliding your hindlegs behind hers in order to properly align your " + this.player.cockDescript(x) + " with her dribbling lips.");
    ...
    private kellyPregSex(): void {
        ...
        this.outputText("\n\nYou remove your [armor] and watch your " + this.player.multiCockDescriptLight() + " grow into a rock-hard erect state under the aphrodisiac scent; you're ready to stuff the breeding mare before you.  Driven mad with lust, you jump the centaur's behind and slap her angrily, making her moan and snort in submissive pleasure.  You can't take it anymore and with feverish excitement, you grab hold of [oneCock] and plunge it deep into Kelly's dark recesses.");
    ...
    private giveKellyAppleSauce(): void {
        ...
        // Repeat:
            ...
            this.outputText("You go to the barn and grab your trusty metal bowl.  Kelly knows what this means: she clasps her hands eagerly, her horse half clopping backwards and forwards impatiently as she watches you walk casually towards the forest.");
        ...
        if (!this.player.isTaur()) {
            ...
            // 1 < cock:
            if (y >= 0) {
                this.outputText("\n\nYour eyes fasten upon her tight, puckered asshole as lust grips you closer, a tiny puddle of human pink buried in her heaving brown horse flesh.  Your breath coming heavily now you withdraw your " + this.player.cockDescript(x) + " from her wet softness, dripping with her lubrication.  Quickly you push it against her puckered anus whilst lining up your " + this.player.cockDescript(y) + " with her gaping vagina.");
                this.outputText("\n\nYou wait for Kelly's head to descend towards the bowl again and then thrust forward, penetrating both her holes at the same time.  She cries out from the intense sensation and then bucks back into you, helping you sink your " + this.player.cockDescript(x) + " deeper and deeper into her ass, spreading her deliciously tight anal passage whilst your " + this.player.cockDescript(y) + " slides effortlessly into the fleshy folds of her cunt.  You slap her ass exultantly, making her squeak again, as you find your limit before resuming the same rhythm, fucking both her sopping vagina and tight, hot ass now with deep, slow strokes.");
            }
            ...
            this.outputText("as you push towards your reward.  Panting, Kelly thrusts back into you as best she can, resisting your frenetic tugs to her hair so that the big, final slather of apple sauce can slide out of the bowl and into her mouth.  She screams around the wet, gooey mouthful and her vagina quivers and seizes your bulging " + this.player.cockDescript(x) + " in milking ripples");
            if (y >= 0) this.outputText(" as her anus clenches around your " + this.player.cockDescript(x));
        ...
        // Centaur:
            ...
            this.outputText("\n\nYou tut mockingly and pull it away until she lowers her hands, then bring it in close to her face.  Her hands fall on her erect nipples as she dips her head into the bowl, scoops up a long lap of the green mash with her tongue and guides it into her wet mouth.  Her throat works and she closes her eyes.  The intermittent dripping of her femcum on the straw behind you turns into a steady trickle and your own arousal urgent now, you push your " + this.player.cockDescript(x) + " against her sopping pussy gently.  She is radiating heat and her wet depths suck at your " + this.player.cockHead(x) + " deliciously.");
            ...
            this.outputText("\n\n\"<i>Oh [Master]...</i>\" Kelly groans, arching her head back from her feed so it spoons into your neck.  A series of clenches grip your " + this.player.cockDescript(x) + " and a sudden gush of fluid warms your ");
            if (this.player.balls > 0) this.outputText("[balls]");
            else this.outputText("thighs");
            ...
            // 1 < cock:
            if (y >= 0) {
                this.outputText("\n\nYou think about her tight, puckered asshole beneath you as lust grips you closer, a tiny puddle of human pink buried in her heaving brown horse flesh.  Your breath coming heavily now you withdraw your " + this.player.cockDescript(x) + " from her wet softness, dripping with her lubrication.  Quickly you push it against her puckered anus whilst lining up your " + this.player.cockDescript(y) + " with her gaping vagina.  You wait for Kelly's head to descend towards the bowl again and then thrust forward, penetrating both her holes at the same time.  She cries out from the intense sensation and then bucks back into you, helping you sink your " + this.player.cockDescript(x) + " deeper and deeper into her ass, spreading her deliciously tight anal passage whilst your " + this.player.cockDescript(y) + " slides effortlessly into the fleshy folds of her cunt.  You grab one of her soft tits and squeeze exultantly, making her squeak again, as you find your limit before resuming the same rhythm, fucking both her sopping vagina and tight, hot ass now with deep, slow strokes.");
            }
            ...
            this.outputText("\n\nPanting, Kelly thrusts back into you as best she can, resisting your frenetic tugs to her hair so that the big, final slather of apple sauce can slide out of the bowl and into her mouth.  She screams around the wet, gooey mouthful and her vagina quivers and seizes your bulging " + this.player.cockDescript(x) + " in milking ripples");
            if (y >= 0) this.outputText(" as her anus clenches around your " + this.player.cockDescript(y));
        ...
        if (!this.player.isTaur()) {
            ...
            if (this.player.balls > 0 && this.player.hasVagina()) {
                ...
                // [Huge balls:
                    ...
                    // [Dominika dreams:
                    if (this.flags[kFLAGS.NUMBER_OF_TIMES_MET_SCYLLA] > 0 && this.flags[kFLAGS.DOMINIKA_STAGE] > 0) this.outputText("  You are gripped by a sudden but thrilling velvet-edged sense of déjà vu.  Has this scene not repeated somewhere else?");
                }
            }
            // Vagina:
