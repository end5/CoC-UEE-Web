...
export class Niamh extends TelAdreAbstractContent implements TimeAwareInterface {
    ...
    public timeChange(): boolean {
        ...
        if (this.player.hasStatusEffect(StatusEffects.BimboChampagne)) {
            this.player.addStatusValue(StatusEffects.BimboChampagne, 1, -1);
            if (this.player.statusEffectv1(StatusEffects.BimboChampagne) <= 0) {
                this.consumables.BIMBOCH.removeBimboChampagne();
                needNext = true;
            }
        }
        if (this.player.statusEffectv1(StatusEffects.BlackCatBeer) > 0) {
            this.player.addStatusValue(StatusEffects.BlackCatBeer, 1, -1);
            if (this.player.statusEffectv1(StatusEffects.BlackCatBeer) <= 0) {
                this.consumables.BC_BEER.blackCatBeerExpires();
                needNext = true;
            }
        }
        return needNext;
    ...
    public telAdreNiamh(): void {
        if (this.flags[kFLAGS.MET_NIAMH] == 0) {
            ...
            if (this.getGame().time.hours <= 8) this.outputText("massive N-cup breasts, spilling over the table");
            else if (this.getGame().time.hours <= 9) this.outputText("huge M-cup breasts, taking up much of the table");
            else if (this.getGame().time.hours <= 10) this.outputText("great L-cup breasts, advancing far onto the table");
            else if (this.getGame().time.hours <= 11) this.outputText("heavy K-cup breasts, lying on the table for support");
            else if (this.getGame().time.hours <= 12) this.outputText("hefty J-cup breasts, resting comfortably on the table");
            else if (this.getGame().time.hours <= 13) this.outputText("weighty I-cup breasts, dangling to lie on the table");
            else if (this.getGame().time.hours <= 14) this.outputText("squeezable H-cup breasts, settling burdensomely on her chest");
        ...
        // This scene only plays if the PC has either Talked to Niamh or has gotten at least 1 mug of Black Cat Beer from her.
            ...
            if (this.getGame().time.hours <= 8) this.outputText("her titanic N-cup 'kegs' resting against the thankfully sturdy table in front of her.  Her teats are engorged with the beer she's cursed to carry, and the stuff dribbles out whenever she shifts.");
            else if (this.getGame().time.hours <= 14) {
                this.outputText("her ");
                if (this.getGame().time.hours <= 9) this.outputText("M-cup");
                else if (this.getGame().time.hours <= 10) this.outputText("L-cup");
                else if (this.getGame().time.hours <= 11) this.outputText("K-cup");
                else if (this.getGame().time.hours <= 12) this.outputText("J-cup");
                else if (this.getGame().time.hours <= 13) this.outputText("I-cup");
    ...
    public approachNiamh(): void {
        ...
        if (rand(5) == 0 && this.flags[kFLAGS.MET_NIAMH] > 0 && (this.player.hasItem(this.consumables.BIMBOLQ) || this.player.hasItem(this.consumables.SUCMILK))) {
        ...
        // [Talk] [Get Beer] [Leave]
        this.menu();
        this.addButton(0, "Talk", this.talkToNiamh);
        if (this.player.gems >= 2) {
            this.addButton(1, "Get Beer", this.getANiamhBeer);
        } else {
            this.addDisabledButton(1, "Get Beer", "You're too poor for beer.");
        }
        this.addButton(14, "Leave", this.leaveNiamh);
    }
    ...
    public getANiamhBeer(): void {
        ...
        this.menu();
        this.addButton(0, "Here", this.drinkNiamhsBeerInTelAdre);
        if (this.player.gems >= 2) {
            this.addButton(1, "To Go", this.blackCatBeerToGo);
        } else {
            this.addDisabledButton(1, "To Go", "You're too poor to buy the mug.");
        }
    }
    ...
    private drinkNiamhsBeerInTelAdre(): void {
        this.clearOutput();
        if (this.player.lactationQ() >= 300 && this.player.biggestTitSize() >= 5 && rand(2) == 0 && this.flags[kFLAGS.MET_NIAMH] > 0) {
            ...
            this.menu();
            this.addButton(0, "SellYourBooze", this.sellYourBooze);
            this.addButton(14, "Leave", this.leaveWithBeerTits);
            return;
        }
    ...
    private talkToNiamh(): void {
        ...
        if (this.flags[kFLAGS.TALKED_NIAMH] == 0) {
            ...
            if (this.getGame().time.hours < 15) this.outputText("who usually sits over yonder ");
        ...
        this.menu();
        if (this.player.gems >= 2) {
            this.addButton(0, "Beer", this.getANiamhBeer);
        } else {
            this.addDisabledButton(0, "Beer", "You can't afford this!");
        }
    ...
    public niamhPostTelAdreMoveOut(): void {
        ...
        this.menu();
        this.addButton(0, "Yes", this.niamhCorruptedMobileSnackDrinkTime);
        this.addButton(14, "Leave", this.niamhCorruptMobileSnackTurnDown);
    }
    ...
    private niamhCorruptedMobileSnackDrinkTime(): void {
        ...
        if (this.flags[kFLAGS.NIAMH_STATUS] == 2) {
            ...
            this.consumables.BIMBOCH.applyEffect(this.player, false, false);
    ...
    public bazaarNiamh(): void {
        ...
        this.menu();
        if (this.player.gems >= 2) {
            this.addButton(0, "Get A Drink", this.bazaardNiamhDrink);
        } else {
            this.addDisabledButton(0, "Get A Drink", "You're too poor to get a drink.");
        }
        if (this.player.hasCock()) {
            this.addButton(1, "Tit-Fuck", this.bazaarSex);
        } else {
            this.addDisabledButton(1, "Tit-Fuck");
        }
        this.addButton(14, "Back", this.getGame().bazaar.enterTheBazaar);
    }
    ...
    private bazaardNiamhDrink(): void {
        ...
        if (this.flags[kFLAGS.NIAMH_STATUS] == 2) {
            ...
            this.menu();
            this.addButton(0, "Drink", this.drinkFromZeTap);
            this.addButton(1, "Bottle", this.getBimboChampFromNiamh);
        }
        // Corrupt
        else {
    ...
    public seanBimboBrewing(): void {
        ...
        if (this.player.gems < 500) {
            ...
            this.addDisabledButton(0, "Yep", "You can't afford this!");
        } else {
            this.addButton(0, "Yep", this.yeahSeanLetsBimbooze);
        }
        this.addButton(14, "Back", this.getGame().dungeons.deepcave.incubusShop);
    }

    // yeah I got this
    public yeahSeanLetsBimbooze(): void {
        this.clearOutput();
    ...
    public getBimboozeFromSean(): void {
        ...
        this.inventory.takeItem(this.consumables.BIMBOLQ, this.getGame().dungeons.deepcave.incubusShop);
    }
    ...
    private sellYourBooze(): void {
        ...
        if (this.temp > 40) this.temp = 40 + rand(3);
        ...
        this.dynStats("lus", 10 + this.player.sens / 5, "scale", false);
        // [If player has cocks]
        ...
        this.outputText(".");
        // If lust is low
        if (this.player.lust100 < 50 || this.player.gender == 0) {
            this.player.growTits(2, this.player.bRows(), false, 2);
        ...
        // If lust is high
            ...
            if (this.player.hasCock()) {
                this.outputText("pre-cum drizzles down your " + this.player.multiCockDescriptLight() + " as the need to fuck a pussy grows stronger.  A few wandering hands reveals some apparent interest amongst the folks in the bar in seeing your genitals as active as your nipples.");
            }
            // [FUCK THE BAR] [LEAVE]
            this.menu();
            this.addButton(0, "Fuck Bar", this.barBeerOrgyTits);
            this.addButton(14, "Leave", this.leaveWithBeerTits);
        }
    }
    ...
    private barBeerOrgyTits(): void {
        ...
        if (this.player.butt.rating >= 20) this.outputText("  She slaps your ass to emphasize her meaning, and the action sends jiggling waves through each immense cheek.");
        ...
        if (this.getGame().time.hours < 15 && this.flags[kFLAGS.URTA_COMFORTABLE_WITH_OWN_BODY] > 0 && this.player.hasVagina()) {
        ...
        if (this.player.statusEffectv1(StatusEffects.Edryn) >= 5 && this.player.gender == 3 && this.getGame().time.hours == 14 && this.flags[kFLAGS.URTA_COMFORTABLE_WITH_OWN_BODY] > 0) {
            ...
            else if (this.player.cockTotal() > 1) this.outputText("  The heads of your cocks press in between Edryn's labia causing her to cry out, \"<i>Oh gawd!  How many dicks are y-you f-f-fucking meeee with???</i>\"  Edryn's orgasm cuts her voice off and every muscle in her cunt contracts around your " + this.player.multiCockDescriptLight() + ".");
            ...
            if (this.player.butt.rating >= 22) this.outputText("  The fat of your ass cheeks jiggles and quakes even as Urta practically stuffs her hips into your crack.  \"<i>Damn, you've got a bigger ass than Edryn,</i>\" she chides as she spanks you.  \"<i>Hey!</i>\"  Edryn calls out with a false tone of hurt pride.");
        ...
        else if (this.player.hasVagina() && this.getGame().time.hours < 15 && this.flags[kFLAGS.URTA_COMFORTABLE_WITH_OWN_BODY] > 0) {
