...
public  rubiIntros() {
	...
	if (flags[kFLAGS.RUBI_DISABLED] > 0 || flags[kFLAGS.RUBI_PROSTITUTION] == -2) return undefined;
	outputText("\n");
	//(Haven't met yet)
...
var  gifts: any[] = [];
	...
	if (closet.length > 0) {
		...
		for (var x: number = 0; x < closet.length; x++) outputText("\n" + closet[x]);
		outputText("\n\n");
	}
...
//(Give Clothes)
public  giveRubiClothes(itype:ItemType = undefined): void {
	clearOutput();
	rubiSprite();
