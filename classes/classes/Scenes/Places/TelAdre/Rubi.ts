...
export class Rubi extends TelAdreAbstractContent {
    ...
    private rubiSprite(): void {
        if (this.flags[kFLAGS.RUBI_HORNTYPE] > 0) this.spriteSelect(SpriteDb.s_rubi_horns);
        else this.spriteSelect(SpriteDb.s_rubi_hornless);
    }
    ...
    private specialRelationship20scene(): void {
        ...
        // (Accept) or (Reject)
        this.menu();
        this.addButton(0, "Accept", this.acceptRubi);
        this.addButton(1, "Reject", this.rejectRubi);
    }
    // [If Accept]
    ...
    public rubiIntros() {
        ...
        else if (this.rubiBimbo() && this.flags[kFLAGS.RUBI_PROSTITUTION] == 0 && rand(9) == 0) {
    ...
    public approachRubiScenes(): void {
        this.outputText(this.images.showImage("rubi-at-cafe"));
        this.clearOutput();
        this.rubiSprite();
        // (First time)
        ...
        if (this.player.gems >= 3) {
            this.addButton(0, "Milk", this.buyRubiMilk);
        } else {
            this.addDisabledButton(0, "Milk", "You cannot afford milk.");
        }
        if (this.player.gems >= 6) {
            this.addButton(1, "Tea", this.getTeaFromRubi);
        } else {
            this.addDisabledButton(1, "Tea", "You cannot afford tea.");
        }
        this.addButton(2, "Chat", this.chatWithRubi);
        if (this.flags[kFLAGS.RUBI_ADMITTED_GENDER] >= 1) {
            this.addButton(3, "Rubi's Place", this.rubisFuckingHouseYouPervert);
        } else {
            this.addDisabledButton(3, "Rubi's Place", "Maybe if you spend more time with her...");
        }
        this.addButton(14, "Back", this.telAdre.bakeryScene.bakeryuuuuuu);
    }
    ...
    public rubisFuckingHouseYouPervert(): void {
        this.outputText(this.images.showImage("rubi-at-house"));
        this.clearOutput();
        this.rubiSprite();
        ...
        // Bimbo Shopping!
        else if (this.rubiBimbo() && (this.player.findPerk(PerkLib.BimboBrains) >= 0 || this.player.findPerk(PerkLib.FutaFaculties) >= 0) && rand(7) == 0) {
        ...
        if (this.player.isNaga() && this.flags[kFLAGS.RUBI_BIMBO] == 0 && this.flags[kFLAGS.RUBI_COCK_SIZE] < this.player.biggestCockLength() && this.player.hasCock() && this.flags[kFLAGS.RUBI_SHE] == 0 && this.player.isCorruptEnough(85) && this.flags[kFLAGS.RUBI_BREAST_SIZE] <= 2) {
    ...
    private rubiSexMenu(): void {
        // [Fuck Rubi (if player has cock, OR at least a 4</i>\" clit)] [Dildo Fuck (If player has Deluxe Dildo)] [Tease] [Release (Only if Normal or Incubus Rubi who has been teased)] [Titfuck (Bimbo Rubi only)] [Give Item]
        this.menu();
        this.addDisabledButton(0, "Fuck", "This scene requires you to have cock or big enough clit and sufficient arousal.");
        this.addDisabledButton(1, "Dildo Fuck", "This scene requires you to have Deluxe Dildo and sufficient arousal.");
        // 2 - always on
        this.addDisabledButton(3, "Release", "This scene requires Rubi to be pent up.");
        this.addDisabledButton(4, "Train", "This scene requires you to have overly large cock and high enough Rubi affection.");
        this.addDisabledButton(5, "Get Fucked", "This scene requires Rubi to have decently sized cock and high enough affection.");
        this.addDisabledButton(6, "Hotdogging", "This scene requires you to have cock.");

        if (this.player.lust >= 33) {
            if (this.player.hasCock() || (this.player.hasVagina() && this.player.getClitLength() >= 4)) this.addButton(0, "Fuck", this.fuckRubi);
        ...
        this.addButton(14, "Back", this.rubisFuckingHouseYouPervert);
    }
    ...
    public fuckRubi(): void {
        this.outputText(this.images.showImage("rubi-fuck"));
        let x: number;
        ...
        this.rubiSprite();
        // (-100 Arousal)
        this.outputText("You take Rubi by the hand and head into the bedroom, clutching [rubi em] close.  You sit on the edge of the soft fluffy bed and pull the little demon " + this.rubiMF("boy", "girl") + " onto your lap.  The two of you kiss, letting your " + this.player.tongueDescript() + " explore the depths of your partner's mouth.");
        ...
        if (this.player.hasCock()) {
            ...
            this.outputText(this.player.cockDescript(x));
        }
        else
        ...
        let size: number = 0;
        const cock: boolean = this.player.hasCock();
        ...
        if (this.player.hasCock()) {
            ...
            if (this.player.getClitLength() <= 4) size = 0;
            else if (this.player.getClitLength() <= 12 || this.flags[kFLAGS.HYPER_HAPPY]) size = 1;
            else size = 2;
        }
        if (size == 0) {
        ...
        if (this.player.hasCock()) this.outputText(this.player.cockDescript(x));
        else this.outputText("[clit]");
        ...
        if (this.player.hasCock() && this.player.cockArea(x) <= this.rubiCapacity()) {
            this.outputText("Your " + this.player.cockDescript(x) + " engorges for a second before erupting deep inside your demonic little fucktoy.  ");
            ...
            this.outputText("Your " + this.player.cockDescript(x) + " engorges for a second before erupting beneath Rubi, splattering [rubi em] and the blankets ");
        ...
        // (Player has clit?)
        else this.outputText("  Your " + this.player.clitDescript() + " throbs and engorges briefly as your " + this.player.vaginaDescript(0) + " reflexively contracts and a warm feeling rocks through your entire body.  Femjuices run down your [legs] and relief overtakes you like a powerful wave upon a beach.");

        this.outputText("\n\nYou stumble backwards, pulling your " + this.player.cockClit(x) + " out of Rubi and collapse on the bed next to [rubi em].  [rubi Ey] cuddles up next to you, in a mixed puddle of spunk, and hugs you close.  \"<i>That was ");
    ...
    public dildoFuckRubi(): void {
        this.outputText(this.images.showImage("rubi-dildo-fuck"));
        this.clearOutput();
        this.rubiSprite();
        // (If PC has no cock)
        ...
        // (If player has pussy)
        if (this.player.hasVagina()) this.outputText("  Your " + this.player.clitDescript() + " stiffens, aching for attention.  It peeks from your folds like a shy little lewd faerie.");
        ...
        else if (this.player.hasVagina()) this.outputText("your " + this.player.clitDescript() + ", rubbing it frantically, in desperate need of release.");
        ...
        // (PC has pussy?)
        if (this.player.hasVagina()) this.outputText("  Your " + this.player.vaginaDescript(0) + " clenches down on the enormous invader, which at this point feels like an enormous steel rod lodged firmly inside you, and a spray of fem-juices erupts from your nethers, coating the already slick dildo and Rubi.");
        // (PC lactating?)
    ...
    public teaseRubi(): void {
        this.outputText(this.images.showImage("rubi-tease"));
        this.clearOutput();
        this.rubiSprite();
    ...
    public releaseRubi(): void {
        this.outputText(this.images.showImage("rubi-orgasm-denial-release"));
        this.clearOutput();
        this.rubiSprite();
    ...
    public cheatingRubi(): void {
        ...
        // [Find Her] [Don't Care]
        this.menu();
        this.addButton(0, "Find Her", this.findBimboCheatster);
        this.addButton(1, "Don't Care", this.dontCareAboutNoCheatingRubis);
    }
    ...
    private findBimboCheatster(): void {
        ...
        // [Interrupt] [Wait]
        this.menu();
        this.addButton(0, "Interrupt", this.interruptTheNTRsYouCrazyFool);
        this.addButton(1, "Watch n Wait", this.waitAndGetNTRedLikeTheBoyBitchYouAre);
    }
    // (Interrupt)
    ...
    private waitAndGetNTRedLikeTheBoyBitchYouAre(): void {
        this.outputText(this.images.showImage("rubi-get-ntr-ed"));
        this.clearOutput();
        this.rubiSprite();
    ...
    private NTRbimboBitchResolution(): void {
        ...
        this.menu();
        this.addButton(0, "No", this.noBimboNTR);
        this.addButton(1, "Yes", this.yesBimboNTR);
        this.addButton(2, "Break Up", this.breakUpWithRubi);
        this.addButton(4, "Pimp", this.pimpOutRubi);
    }
    ...
    private chocoRubiMassage(): void {
        this.outputText(this.images.showImage("rubi-get-massage"));
        this.clearOutput();
        this.rubiSprite();
    ...
    private rubiAppearance(): void {
        this.outputText(this.images.showImage("rubi-inspect-appearance"));
        this.clearOutput();
        this.rubiSprite();
        ...
        if (this.flags[kFLAGS.RUBI_COCK_SIZE] > 0) {
            this.outputText(this.rubiCock() + ", which starts to perk up under your scrutiny. From experience, you know it can reach an impressive " + this.flags[kFLAGS.RUBI_COCK_SIZE] + " inches when erect.  ");
    ...
    private playDressUp(): void {
        ...
        while (button < 14 && button < buttonNames.length) {
            if (button < 13 || closet.length < 14) this.addButton(button, buttonNames[button], this.dressUpRouter, closet[button]);
            else this.addButton(13, "More", this.playDressUp2);
            button++;
        }
        this.addButton(14, "Back", this.rubiAppearance);
    }
    private playDressUp2(): void {
        ...
        let button: number = 12;
        while (button - 12 < 14 && button < buttonNames.length) {
            if (button == 12) this.addButton(0, "Previous", this.playDressUp);
    ...
    private putOnSluttySwimwearYouSkank(): void {
        ...
        else this.outputText("strain thanks to Rubi's ample bosom");
        this.outputText(".");
    ...
    private anallyTrainYourBitchBoySlutHowToBeAnalForDCL(): void {
        ...
        if (this.flags[kFLAGS.RUBI_TIMES_ANALLY_TRAINED] == 1) {
            ...
            this.dynStats("lus", 20 + this.player.lib / 5, "scale", false);
        }
        // Repeatable
        // Must have a cock greater than Rubi's capacity.
        // Rubi's capacity won't go larger than 100.
    ...
    private getFuckedByRubi(): void {
        this.outputText(this.images.showImage("rubi-fucks-you"));
        this.clearOutput();
        this.rubiSprite();
        ...
        if (this.player.hasCock()) this.outputText(", ignoring your " + this.player.multiCockDescriptLight() + " completely,");
        ...
        else if (this.rubiGetCockType() == CockTypesEnum.CAT) this.outputText("spiked tip");
        else if (this.rubiGetCockType() == CockTypesEnum.ANEMONE) this.outputText("tentacle-ringed crown");
    ...
    private rubiHotdogging(): void {
        this.outputText(this.images.showImage("rubi-hotdogged"));
        this.clearOutput();
        this.rubiSprite();
    ...
    private teaseButtjobs(): void {
        this.outputText(this.images.showImage("rubi-tease-buttjob"));
        this.clearOutput();
        this.rubiSprite();
    ...
    private popButtjobs(): void {
        this.outputText(this.images.showImage("rubi-pop-buttjob"));
        this.clearOutput();
        this.rubiSprite();
        ...
        else if (this.flags[kFLAGS.RUBI_BLU_BALLS] <= 9) this.outputText(" which is followed up by several more, each seemingly more powerful than the one previous.  Rubi's body twitches and shudders with every jet, until [rubi eir] entire torso, and the couch, is covered in the musky jizz");
    ...
    private giveRubiATFItem(itype: ItemType): void {
        ...
        if (itype == this.consumables.SUCMILK || itype == this.consumables.P_S_MLK) {
            ...
            if (rand(3) == 0 && this.flags[kFLAGS.RUBI_NO_CUNT] > 0) {
            ...
            if (this.flags[kFLAGS.RUBI_FEET] == 0 && rand(3) == 0 && itype == this.consumables.SUCMILK) {
            ...
            // (If Rubi gets horns)
            if (rand(3) == 0 && this.flags[kFLAGS.RUBI_HORNTYPE] == 0 && itype == this.consumables.SUCMILK) {
            ...
            if (rand(3) == 0 && this.flags[kFLAGS.RUBI_SKIN] != 1 && itype == this.consumables.SUCMILK) {
            ...
            // (If Rubi has no penis; instead)
                ...
                this.flags[kFLAGS.RUBI_COCK_SIZE] = 5;
                this.flags[kFLAGS.RUBI_COCK_TYPE] = CockTypesEnum.HUMAN;
            }
            // (If Vagina removed)
            if (this.flags[kFLAGS.RUBI_NO_CUNT] == 0 && rand(3) == 0) {
            ...
            // (If Rubi gets red skin)
            if (rand(3) == 0 && this.flags[kFLAGS.RUBI_SKIN] != 1 && itype == this.consumables.INCUBID) {
                this.outputText("\n\nRubi clutches [rubi eir] stomach suddenly as you notice red spots appearing on [rubi eir] ");
                if (this.flags[kFLAGS.RUBI_SKIN] != 2) this.outputText("skin");
                else this.outputText("coat of fur");
                this.outputText(".  The blotches soon spread, rapidly overtaking [rubi eir] body.  ");
            ...
            // (If Rubi gets horns)
            if (rand(3) == 0 && this.flags[kFLAGS.RUBI_HORNTYPE] == 0 && itype == this.consumables.INCUBID) {
                this.outputText("\n\nRubi's hands suddenly dart up, clutching [rubi eir] head as though [rubi ey] has a massive headache.  [rubi Ey] feels around blindly, trying to locate the source of the pain, and then you see it.  Two small bulges appear on [rubi eir] forehead, just at the hairline.  They grow larger and larger until the skin splits cleanly open revealing shiny black bone.  Horns.  They grow up and backwards, forming light ridges.  They don't quite grow completely past [rubi eir] head, however, before the growth stops.  Rubi calms and stands.  [rubi Eir] hands roam over every inch of [rubi eir] new addition as glances in the mirror, marveling at [rubi eir] demonic appearance.");
                this.flags[kFLAGS.RUBI_HORNTYPE] = 1;
            }
        }
        ...
        // Grows cock. (As Incubus Draft above)
            ...
            if (this.rubiGetCockType() != CockTypesEnum.HORSE) {
                ...
                this.flags[kFLAGS.RUBI_COCK_SIZE] += 3 + rand(5);
            }
        }
        // Trap Oil
        ...
        if (itype == this.consumables.PURPEAC) {
            ...
            // (Any changes? Yes)
            if (this.flags[kFLAGS.RUBI_EAR_TYPE] != 2 || this.flags[kFLAGS.RUBI_SKIN] != 3 || (this.flags[kFLAGS.RUBI_COCK_SIZE] > 0 && this.rubiGetCockType() != CockTypesEnum.ANEMONE)) {
    ...
    private rubiGrowPlusBreasts(): void {
        this.outputText(this.images.showImage("rubi-gro-plus-tits"));
        this.clearOutput();
        this.rubiSprite();
        ...
        if (this.flags[kFLAGS.RUBI_BREAST_SIZE] < 7 || (this.flags[kFLAGS.RUBI_BREAST_SIZE] < 30 && this.flags[kFLAGS.HYPER_HAPPY])) {
            ...
            this.flags[kFLAGS.RUBI_BREAST_SIZE] += 2 + rand(2);
            if (this.flags[kFLAGS.HYPER_HAPPY]) {
    ...
    private rubiPenisGroPlus(): void {
        this.outputText(this.images.showImage("rubi-gro-plus-penis"));
        this.clearOutput();
        this.rubiSprite();
        ...
        if (this.flags[kFLAGS.RUBI_COCK_SIZE] < 12 || (this.rubiGetCockType() == CockTypesEnum.HORSE && this.flags[kFLAGS.RUBI_COCK_SIZE] < 20) || (this.flags[kFLAGS.RUBI_COCK_SIZE] < 30 && this.flags[kFLAGS.HYPER_HAPPY])) {
            ...
            this.flags[kFLAGS.RUBI_COCK_SIZE] += 2 + rand(2);
    ...
    private pickAnItemToFeedRubi(): void {
        ...
        if (this.flags[kFLAGS.RUBI_BIMBO_MINIDRESS] == 1)
            ...
            if (this.player.hasItem(this.armors.BIMBOSK)) this.addButton(button++, "BimboSk", this.giveRubiClothes, this.armors.BIMBOSK);
        }
        if (this.flags[kFLAGS.RUBI_BONDAGE_STRAPS] == 1)
            closet[closet.length] = "Bondage Straps";
        else {
        ...
        if (closet.length > 0) {
            ...
            for (let x: number = 0; x < closet.length; x++) this.outputText("\n" + closet[x]);
            this.outputText("\n\n");
        }

        if (gifts.length > 0) {
            this.outputText("After a quick glance around Rubi's things, you bet [rubi ey] could really use... " + gifts[rand(gifts.length)] + ".\n\n");
        }
        else {
            ...
            this.awardAchievement("Dress-tacular", kACHIEVEMENTS.GENERAL_DRESSTACULAR);
            this.outputText("\n\n");
        }
    ...
    public giveRubiClothes(itype: ItemType): void {
        ...
        else if (itype == this.armors.BIMBOSK) this.flags[kFLAGS.RUBI_BIMBO_MINIDRESS] = 1;
    ...
    public hypnoBimboficationForRubiSloots(): void {
        ...
        if (this.getGame().jojoScene.isJojoCorrupted()) {
