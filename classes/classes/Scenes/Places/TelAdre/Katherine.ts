...
public  katherineAtUrtas(): void {
	...
	if (isAt(KLOC_URTAS_APT)) { //This option happens if you and Urta have no children
		...
		switch (rand(4)) {
			...
			var  kids: number = kGAMECLASS.urtaPregs.urtaKids();
				if (kids == 1)
...
//Talk
private  talkToKatherine(from = undefined): void {
...
private  talkLoversUrta(): void {
	...
	var  urtaKids: number = kGAMECLASS.urtaPregs.urtaKids();
...
var  button: number = 0;
	...
	//Dyes
	if (page == 3 || page == 4) {
	var  dyeButton = function(color: string, item:ItemType, func): void {
