...
export class YaraPiercingStudio extends TelAdreAbstractContent {
	...
	private  buildLocChoices(fun, inverse: boolean=false): boolean {
	...
	function  check(toCheck: number): boolean{
		...
		if (player.hasVagina())
		{
			if (check(player.vaginas[0].clitPierced)) {
				addButton(0, "Clit", fun,LOC_CLIT);
				buttonsAdded = true;
			}
		}
		if (player.totalCocks() > 0)
		{
			if (check(player.cocks[0].pierced)) {
				addButton(1, "Dick", fun, LOC_DICK);
				buttonsAdded = true;
			}
		}
		if (check(player.earsPierced)) {
			addButton(2, "Ears", fun, LOC_EARS);
			buttonsAdded = true;
		}
		if (check(player.eyebrowPierced)) {
			addButton(3, "Eyebrow", fun, LOC_EYEBROW);
			buttonsAdded = true;
		}
		if (check(player.lipPierced)) {
			addButton(4, "Lip", fun, LOC_LIP);
			buttonsAdded = true;
		}
		if (check(player.nipplesPierced)) {
			addButton(5, "Nipples", fun, LOC_NIPPLES);
			buttonsAdded = true;
		}
		if (check(player.nosePierced)) {
			addButton(6, "Nose", fun, LOC_NOSE);
			buttonsAdded = true;
		}
		if (check(player.tonguePierced)) {
			addButton(7, "Tongue", fun, LOC_TONGUE);
			buttonsAdded = true;
		}
		if (player.hasVagina())
		{
			if (check(player.vaginas[0].labiaPierced)) {
				addButton(8, "Labia", fun, LOC_VULVA);
				buttonsAdded = true;
			}
		}
	...
	private  chooseMaterials(loc: number, type: number): void {
		...
		menu();
		addButton(0, "Amethyst", areYouSure, loc, type,MAT_AMETHYST);
		addButton(1, "Diamond", areYouSure, loc, type,MAT_DIAMOND);
		addButton(2, "Gold", areYouSure, loc, type,MAT_GOLD);
		addButton(3, "Emerald", areYouSure, loc, type,MAT_EMERALD);
		addButton(4, "Jade", areYouSure, loc, type,MAT_JADE);
		addButton(5, "Onyx", areYouSure, loc, type,MAT_ONYX);
		addButton(6, "Ruby", areYouSure, loc, type,MAT_RUBY);
		addButton(7, "Steel", areYouSure, loc, type,MAT_STEEL);
		if (player.gems >= 1000) {
			addButton(8, "Rare Menu", chooseAdvancedMaterials,loc,type);
		} else {
	...
	private  chooseAdvancedMaterials(loc: number, type: number): void {
		...
		addButton(0, "Lethite", areYouSure, loc, type, MAT_LETHITE);
		addButton(1, "Fertite", areYouSure, loc, type, MAT_FERRITE);
		addButton(2, "Furrite", areYouSure, loc, type, MAT_FURRITE);
		addButton(3, "Crimstone", areYouSure, loc, type, MAT_CRIMSTONE);
		if (player.gems >= 2000) {
	...
	private  applyPiercing(loc: number, type: number = TYPE_NONE, shortP: string = "", longP: string = ""): void {
		switch (loc) {
			case LOC_CLIT:
	...
	function  doRemove(loc: number): void {
			clearOutput();
	...
	var  x: number = player.cockThatFits(36);
		...
		if (!girl && player.hasCock()) {
			outputText(" as your " + player.cockDescript(x) + " rises to attention, her eyes fixed upon the stiffened tool.  You resist the urge to grab her thin-yet-girlish hips and power into her right then and there, curious enough to allow her teasing.");
		} else {
