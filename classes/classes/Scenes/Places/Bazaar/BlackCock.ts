 ...
 * @author Kitteh6660
 */
export class BlackCock extends BazaarAbstractContent {
    protected get changes(): number { return this.mutations.changes; }
    protected set changes(val: number) { this.mutations.changes = val; }
    protected get changeLimit(): number { return this.mutations.changeLimit; }
    protected set changeLimit(val: number) { this.mutations.changeLimit = val; }

    // Will eventually be used to display sprites.
    public displayAnitaSprite(): void {
        // Anita's sprite code goes here
    }
    public displayAndySprite(): void {
        // Andy's sprite code goes here
    }
    public displayHarrySprite(): void {
        // Harry's sprite code goes here
    }
    public displayFridaSprite(): void {
        // Frida's sprite code goes here
    }

    private applyAndysSmokeEffect(): void {
        if (this.player.hasStatusEffect(StatusEffects.AndysSmoke)) {
            if (this.player.statusEffectv2(StatusEffects.AndysSmoke) > -15) { // Maximum speed loss is -15.
                this.player.addStatusValue(StatusEffects.AndysSmoke, 2, -5);
                this.dynStats("spe", -5);
            }
            if (this.player.statusEffectv3(StatusEffects.AndysSmoke) < 15) { // Maximum intelligence boost is 15.
                this.player.addStatusValue(StatusEffects.AndysSmoke, 3, 5);
                this.dynStats("int", 5);
            }
            this.player.changeStatusValue(StatusEffects.AndysSmoke, 1, 8);
        }
        else {
            this.player.createStatusEffect(StatusEffects.AndysSmoke, 8, -5, 5, 0);
            this.dynStats("spe", -5, "int", 5);
        }

    }

    public blackCockDescription(): void {
        if (this.isHolidays()) this.outputText("\n\nThere is also a large marquee-style tent made with black canvas. Smoke rises out of an open flap in the rear. There is a wooden sign next to the front flap depicting a small black rooster; beneath its yellow feet the words \"The Black Cock\" are etched into the old sign. Around the sign there is a festive Christmas wreath with flickering candles. The smell of greasy food and sweets wafts out of the open entrance.");
    ...
    public enterTheBlackCock(): void {
        this.clearOutput();
        this.spriteSelect(undefined);
        this.credits.modContent = true;
        this.credits.authorText = "Foxxling";
        this.credits.show();
        this.outputText(this.images.showImage("location-bazaar-theblackcock"));
        // General descriptor
        this.outputText("You enter the tent to find much of what you'd expect: A bunch of wooden tables and a sectioned-off portion in the back where you presume the cooking happens. Next to the flap leading into the kitchen there is a big \"keep out\" sign stuck into the ground through the tent's bottom. ");
        if (this.isHalloween()) this.outputText("The interior is lit by several carved Jack O'Lanterns on each table, emitting a warm glow from flickering flames that cast dancing shapes along the walls.");
        else if (this.isHolidays()) this.outputText("The interior is lit by a combination of black rooster-shaped candles on each table and a large globe sitting in the center of the tent. It looks like they just dumped the glowing creamy fluid from all the small globes into a larger one. You also see a big festive tree tucked into the corner decorated with glowing frosting… oh... wait, that's not frosting.");
        else this.outputText("The interior is lit by what looks like round globes on each table, each of which is filled with a thick creamy liquid that emits a pale glow.");
        // Staff descriptor
        if (this.flags[kFLAGS.BLACK_COCK_MET_ANITA] == 0) this.outputText("\n\nThere is a harpy lady sitting next to a sign promising she'll sing if you pay her 5 gems.");
        else this.outputText("\n\nAnita sits next to a sign promising she'll sing for you if you pay her 5 gems.");

        if (this.flags[kFLAGS.BLACK_COCK_MET_ANDY] == 0) this.outputText("\n\nThere is an odd satyr going about the place serving customers.");
        else this.outputText("\n\nAndy walks around the tables serving customers.");

        if (this.flags[kFLAGS.BLACK_COCK_MET_FRIDA] == 0) this.outputText("\n\nThere is also an echidna-morph leaning on a counter towards the back of the tent.");
        else this.outputText("\n\nFrida leans on a counter towards the back of the tent, looking a little bored.");
        // General menu
        this.menu();
        this.addButton(0, "Check Menu", this.checkFoodMenu);
        this.addButton(1, "Staff", this.checkStaff);
        this.addButton(2, "Patrons", this.checkPatrons);
    ...
    private addFoodPlate(foodName: string, gems: number, tooltip: string, foodEatDesc: string): void {
        ...
        this.addButton(button, foodName, this.orderFoodPlate, foodName, gems, foodEatDesc, tooltip, header);
    }

    private orderFoodPlate(foodName: string, gemCost: number, foodEatDesc: string): void {
        this.clearOutput();
        if (this.player.gems < gemCost) {
            this.outputText("<b>You do not have sufficient gems to order that. Damn!</b>");
            this.doNext(this.checkFoodMenu);
        ...
        if ((this.player.hunger100 >= 90 && this.flags[kFLAGS.HUNGER_ENABLED] > 0) || (this.player.hasStatusEffect(StatusEffects.Fullness) && this.flags[kFLAGS.HUNGER_ENABLED] == 0)) {
            this.outputText("<b>You are too full to consider eating that.</b>");
            this.doNext(this.checkFoodMenu);
            return;
        }
        this.player.gems -= gemCost;
        this.statScreenRefresh();
        this.eatFoodNomNom(foodName, foodEatDesc);
    }

    private eatFoodNomNom(foodName: string, foodEatDesc: string): void {
        this.outputText(foodEatDesc);
        switch (foodName) {
            case "Rhino Steak":
                this.consumables.RHINOST.useItem();
                break;
            case "Echidna Cake":
                this.consumables.ECHIDCK.useItem();
                break;
            case "Giant Burger":
                this.player.refillHunger(70);
                if (this.flags[kFLAGS.HUNGER_ENABLED] == 0) this.player.modThickness(100, 4);
                if (rand(3) == 0 && this.player.hair.length < 16) this.player.hair.length += 0.5;
                break;
            case "P. Potatoes":
                this.player.refillHunger(70);
                if (this.flags[kFLAGS.HUNGER_ENABLED] == 0) this.player.modThickness(100, 2);
                if (this.flags[kFLAGS.HUNGER_ENABLED] == 0 || (rand(2) == 0 && this.player.hunger100 >= 80)) this.player.butt.rating++;
                break;
            case "Spicy Chilli":
                this.player.refillHunger(60);
                if (this.flags[kFLAGS.HUNGER_ENABLED] == 0) this.player.modThickness(100, 1);
                if (this.flags[kFLAGS.HUNGER_ENABLED] == 0 || (rand(2) == 0 && this.player.hunger100 >= 80)) this.player.hips.rating++;
                break;
            case "Prot. Shake":
                this.player.refillHunger(30);
                this.player.modTone(100, 1);
                this.player.modThickness(0, -1);
                break;
            case "Funnel Cake":
                this.player.refillHunger(50);
                this.player.modTone(0, 2);
                if (this.flags[kFLAGS.HUNGER_ENABLED] == 0) this.player.modThickness(100, 1);
                break;
            case "R.Tk. Dinner":
                if (this.player.hunger < 60) this.player.hunger = 60;
                this.player.refillHunger(40);
                this.player.modTone(0, 5);
                if (this.flags[kFLAGS.HUNGER_ENABLED] == 0) this.player.modThickness(100, 2);
                break;
            case "G.Ham Dinner":
                if (this.player.hunger < 60) this.player.hunger = 60;
                this.player.refillHunger(40);
                this.player.modTone(0, 2);
                if (this.flags[kFLAGS.HUNGER_ENABLED] == 0) this.player.modThickness(100, 5);
                break;
            case "MysteryMeat":
                this.player.refillHunger(50);
                this.mysteryMeatTFs();
        ...
        if (!this.player.hasStatusEffect(StatusEffects.Fullness)) this.player.createStatusEffect(StatusEffects.Fullness, 4, 0, 0, 0);
        else this.player.changeStatusValue(StatusEffects.Fullness, 1, 4);
        this.awardAchievement("Dining In", kACHIEVEMENTS.GENERAL_DINING_IN, true, true, false);
    ...
    // STAFF
    // ------------
    private checkStaff(): void {
        this.clearOutput();
        this.menu();
        // Anita Roswell
        if (this.flags[kFLAGS.BLACK_COCK_MET_ANITA] == 0) {
            this.outputText("There is a scantily clad harpy with pale rosy skin whose feathers and short-styled hair are both a dirty blond color. She wears a blue flannel shirt with long sleeves, the bottom of which is tied in the front, pushing her soft milky white tits together and showing off her trim and lightly-toned stomach. Her blue pants are cut off, showing off a pair of stunning legs as well as a firm, pert rump. She is apparently some sort of singer, as the sign next to her promises a song for 5 gems.\n\n");
            this.addButton(0, "Harpy", this.meetAnitaRoswell);
        }
        else {
            this.outputText("You see Anita Roswell, the sexy slutty songbird of The Black Cock. She is wearing her usual breast-exposing front-tied blue flannel and tight tiny cut-off pants to show off her athletic yet feminine figure. The sign next to her promises a song for 5 gems.\n\n");
            this.addButton(0, "Anita", this.meetAnitaRoswell);
        }
        // Andy Mathis
        if (this.flags[kFLAGS.BLACK_COCK_MET_ANDY] == 0) {
            this.outputText("The waiter is a Satyr. Unlike the table full of his pot-bellied kinsmen his waist is trim and his tall-standing horns are polished to a shine. Curly dark brown hair frames his face but fails to conceal his pointy ears. His goatee is neatly trimmed and tastefully styled. From the look of things he takes great pride in his appearance. He wears nothing more than a simple black shirt with the sleeves cut off; his heavy balls are visible just below the sheath that conceals his presumably dog-like manhood.\n\n");
            this.addButton(1, "Satyr", this.meetAndyMathis);
        }
        else {
            this.outputText("You also see Andy Mathis, the waiter at The Black Cock. Unlike the table full of his beer-bellied kinsmen his waist is trim and his tall-standing horns are polished. His masculine face is framed by a cloud of curly brown hair. His goatee is also neatly trimmed and tastefully styled. He wears nothing more than a simple black shirt with the sleeves cut off; his heavy balls are visible just below the sheath that conceals his dog-like manhood.\n\n");
            this.addButton(1, "Andy", this.meetAndyMathis);
        }
        // Harry Roswell
        if (this.flags[kFLAGS.BLACK_COCK_MET_HARRY] == 0) {
            this.outputText("You see the sign saying \"Keep Out\" leading to the back. You assume the cook is back there.\n\n");
            this.addButton(2, "Sneak", this.meetHarryRoswell).hint("Sneak into the kitchen?");
        }
        else {
            this.outputText("You see the sign saying \"Keep Out\" leading to the back portion of the tent. You know that Harry Roswell, the beefy rhino-morph cooking the food, is back there and doesn't mind you ignoring the sign.\n\n");
            this.addButton(2, "Harry", this.meetHarryRoswell);
        }
        // Frida
        if (this.flags[kFLAGS.BLACK_COCK_MET_FRIDA] == 0) {
            ...
            this.addButton(3, "Echidna", this.meetFrida);
        }
        else {
            this.outputText("Behind the counter you see Frida, going about her business as usual. When the hard-working girl notices you she gives you a brief wave and a quick smile before returning to tidying up her area.");
            this.addButton(3, "Frida", this.meetFrida);
        }
        this.addButton(14, "Back", this.enterTheBlackCock);
    }
    ...
    // Anita Roswell
    // ------------
    private meetAnitaRoswell(): void {
        this.clearOutput();
        this.displayAnitaSprite();
        this.outputText(this.images.showImage("anita-intro"));
        if (this.flags[kFLAGS.BLACK_COCK_MET_ANITA] == 0) {
            this.outputText("You catch the sky blue eyes of the songbird and summon her to your table with a nod. As she walks towards you her hard gait causes her tits, twin B-cups by the looks of it, to bounce up and down in her tied and knotted flannel shirt.");
            ...
            this.flags[kFLAGS.BLACK_COCK_MET_ANITA] = 1;
        }
        else {
            this.outputText("You catch Anita's eye and wave her over. With a big smile she struts, her soft tits rising and falling with every step, toward your table. She leans in, proudly displaying her bust and crushing them against your table as she smiles big and says, \"<i>Well if it ain't my favorite customer, [name]. What can I do ya for, hun?</i>\"");
        }
        this.unlockCodexEntry("Harpies", kFLAGS.CODEX_ENTRY_HARPIES);
        this.menu();
        this.addButton(0, "Song", this.listenToAnitaSong).hint("Listen to Anita's performance. This will cost you five gems.");
        if (this.player.lust >= 33) this.addButton(1, "Sex", this.rompUpAnitaOffer);
        else this.addButtonDisabled(1, "Sex", "You aren't aroused enough to do that.");
        this.addButton(4, "Nevermind", this.nevermindToAnita);
    }

    private nevermindToAnita(): void {
        this.clearOutput();
        this.outputText("You decide not to take the slutty songbird up on her offer… either of them. You tell her you were just saying hi and she pouts slightly, \"<i>Well, if you need anything,</i>\" She leans in close and says in a sultry whisper, \"<i>... and I mean anything,</i>\" her eyes glance meaningfully towards your crotch, \"<i>...you don't hesitate to call me back over.</i>\"");
        this.outputText("\n\nThen she leaves, walking back and swinging her ass every which way as she heads back to her stool.");
        this.doNext(this.checkStaff);
    }

    private listenToAnitaSong(): void {
        this.clearOutput();
        this.outputText(this.images.showImage("anita-song"));
        if (this.player.gems < 5) {
            this.outputText("You realize you're a bit short on gems. \"<i>Come back when you have more gems, ya hear?</i>\" Anita says.");
            this.doNext(this.enterTheBlackCock);
            return;
        }
        this.outputText("You decide that you'd like to hear the little songbird sing. When you hand her the gems and tell her to give you a show she takes them with a lick of her soft pink lips. She heads back to her chair and, with a glance to make sure you're still watching, leans over it.");
        ...
        switch (rand(2)) {
            case 0: // Squirtin Like a Fountain
                ...
                this.dynStats("lus", 10, "scale", false);
                this.dynStats("lus", (this.player.lib / 5) + (this.player.cor / 10));
                break;
            case 1: // Rockin Robin
        ...
        this.player.gems -= 5;
        this.statScreenRefresh();
        this.flags[kFLAGS.BLACK_COCK_ANITA_SONGS_LISTENED]++;
        this.doNext(this.camp.returnToCampUseOneHour);
    }

    private rompUpAnitaOffer(): void {
        this.clearOutput();
        if (this.flags[kFLAGS.BLACK_COCK_ANITA_SEX_INTRO] == 0) {
            this.outputText("Picking up on the slutty little songbird's obvious flirtation you decide to test it by asking if she likes to taste her customers.");
            ...
            this.flags[kFLAGS.BLACK_COCK_ANITA_SEX_INTRO] = 1;
        }
        else {
            this.outputText("To answer her question you tell her she could get naked and show you her pussy. She gives you a sultry grin and says, \"<i>I love it when you talk like that.</i>\"");
        ...
        this.menu();
        if (this.player.canFly() && this.player.gender > 0) this.addButton(0, "Eagle Fuck", this.eagleFuckAnita).hint("Take a wild trip in the skies with Anita and fuck her!");
        else this.addButtonDisabled(0, "Eagle Fuck", (!this.player.canFly() ? "You'll need wings for that." : "You need either a penis or a vagina to do that."));
        if (this.player.hasCock()) this.addButton(1, "Vaginal", this.vaginalFuckAnita);
        else this.addButtonDisabled(1, "Vaginal", "You'll need a penis for that.");
        if (this.player.isBiped() || this.player.isNaga() || this.player.isGoo()) this.addButton(2, "Sixty Nine", this.sex69Anita);
        else this.addButtonDisabled(2, "Sixty Nine", "You won't be able to do this with your unusual body shape.");
        this.addButton(4, "Nevermind", this.nevermindToAnita);
    }

    private eagleFuckAnita(): void { // Eagle fuck, must have wings
        this.clearOutput();
        this.outputText(this.images.showImage("anita-sex-eaglefuck"));
        this.outputText("Looking into her eyes you feel around your [armor] as she does the same. The two of you blindly disrobe as you hold each other's gaze. It is both a little awkward and undeniably sexy, and once the two of you are completely naked Anita's wings flare out, opening wide in a very royal-looking gesture. She fixes those big blue country-girl eyes on you and asks, \"<i>Sky fuck?</i>\"");
        ...
        if (this.player.hasCock()) { // Males and hermaphrodites
            ...
            if (this.player.cumQ() >= 2500) this.outputText("Your chest swells with pride as you watch Anita fly, her snatch trailing a sticky trail of your glistening baby-batter as her swollen stomach and loosened lady parts empty.");
        }
        ...
        this.flags[kFLAGS.BLACK_COCK_ANITA_SEX_COUNTER]++;
        this.doNext(this.camp.returnToCampUseOneHour);
    }

    private vaginalFuckAnita(): void { // Vaginal fuck, must have penis
        this.clearOutput();
        this.outputText(this.images.showImage("anita-sex-vaginalfuck"));
        this.outputText("Before she says anything you press your lips against hers, shoving her against the tall fence behind her with a lip-locked growl. You throw off your [armor] and push her little unbuttoned cut-offs down. She gasps against you, her voice a husky groan as you trail kisses down her neck, hungry to be inside her.");
        ...
        if (this.player.isBiped() || this.player.isGoo() || this.player.isDrider()) {
            ...
            if (this.player.cockTotal() == 1) {
                ...
                else if (this.player.biggestCockLength() >= 10 && this.player.biggestCockLength() < 18) {
    ...
    private sex69Anita(): void { // 69 with Anita, must not be a taur.
        this.clearOutput();
        this.outputText(this.images.showImage("anita-sex-sixtynine"));
        this.outputText("Before she says anything you press your lips against hers, shoving her against the tall fence behind her with a lip-locked growl. Still connected at the lips you throw off your [armor] and push her little unbuttoned cut-offs down. She gasps against you, her voice a husky groan as you begin to trail kisses down her neck, hungry to both taste her and to be tasted.");
        ...
        if (this.player.hasCock()) { // Males and hermaphrodites
            ...
            if (this.player.hasVagina()) {
                this.outputText("She even begins to finger your [vagina], The added sensation pushes you over the edge and your [cock] swells as you reach climax. ");
            }
            else {
                this.outputText("She begins to suck harder, using her hands to massage your [legs] and pull you harder against her. Your [cock] slides, free of her mouth as she kisses its rigid underside. The added sensation pushes you over the edge and your [cock] swells as you reach climax. ");
            }
            if (this.player.cumQ() >= 250 && this.player.cumQ() < 1000) this.outputText("You feel the hot evidence of your climax hit your jaw as you unload, painting hot streaks of cum from your chin to her perky tits.");
            else if (this.player.cumQ() >= 1000 && this.player.cumQ() < 2500) this.outputText("You feel the hot evidence of your climax slam into your chin, a huge glob of your cum painting your jaw white. You groan as you orgasm rages on, painting both your bodies in a liberal coating of jism as Anita continues to lick the belly of your [cock].");
            else if (this.player.cumQ() >= 2500) this.outputText("A tidal wave of baby-batter fills your vision with white, your [cock] expelling gigantic torrents of seed with every contraction that rains back down to the earth. By the time your orgasm has subsided you're quietly convulsing with you and Anita covered in spunk and surrounded by a pool of hot seed.");
        }
        else if (this.player.hasVagina()) { // Females
    ...
    // Andy Mathis
    // ------------
    private meetAndyMathis(): void {
        this.clearOutput();
        this.displayAndySprite();
        this.outputText(this.images.showImage("andy-intro"));
        if (this.flags[kFLAGS.BLACK_COCK_MET_ANDY] == 0) {
            this.outputText("With a flick of your fingers you catch the waiter's attention. The satyr walks towards you and pulls out his notepad and a little black pencil.");
            ...
            this.flags[kFLAGS.BLACK_COCK_MET_ANDY] = 1;
        }
        else {
            this.outputText("You call Andy over and when he sees you his face pulls into a warm inviting smile.");
            this.outputText("\n\n\"<i>Hey [name], so what'll it be today?</i>\" he asks, pen poised.");
        }
        this.unlockCodexEntry("Satyrs", kFLAGS.CODEX_ENTRY_SATYRS);
        this.menu();
        this.addButton(0, "Stash", this.checkAndyStash).hint("Go out the back of the tent and check out Andy's stash?");
        if (this.player.lust >= 33) this.addButton(1, "Sex", this.rompUpWithAndyTheSatyr).hint("Proposition to have sex with the satyr.");
        else this.addButtonDisabled(1, "Sex", "You aren't aroused enough to consider this.");
        this.addButton(4, "Nevermind", this.nevermindToAndy);
    }

    private nevermindToAndy(): void {
        this.clearOutput();
        this.outputText("You decide to not take Andy up on his offer or make any offers of your own so you tell him you'll call him back over when you're actually ready. He nods and with a charming smile leaves you to it.");
        this.doNext(this.checkStaff);
    }

    private checkAndyStash(): void {
        this.clearOutput();
        if (this.flags[kFLAGS.BLACK_COCK_ANDY_STASH_STATE] == 0) {
            this.outputText("You decide to take a look at Andy's \"stash\". He gives you a wicked grin in response and says, \"<i>Oh, you are in for a treat.</i>\" He nods towards the flap in the back of the tent, the one with the keep out sign and says, \"<i>Well… follow me,</i>\" and with that he leads you through a hot cluttered kitchen past a big furiously-swearing rhino and out the back of the tent.");
            ...
            this.flags[kFLAGS.BLACK_COCK_ANDY_STASH_STATE] = 1;
        }
        else {
            this.outputText("You tell Andy you want to check out his stash again. He steps back with an exaggerated bow, silently telling you to lead the way. You " + (this.player.isNaga() ? "slither" : (this.player.isGoo() ? "slide" : "walk")) + " as silently as your [feet] will allow as you enter the tent flap, sneaking past a humming rhino-morph that is dancing as he cooks. Once you've successfully made it out the back of the tent, you breathe a sigh of relief. Andy comes from behind you and walks to his stash.");
        ...
        this.doYesNo(this.smokeSomeHookah, this.dontSmokeItsBad);
    }

    private smokeSomeHookah(): void {
        this.clearOutput();
        if (this.player.gems < 5) {
            this.outputText("You realize you are short on gems. Andy gives you a bit of disappointed look and escorts you back to the tent.");
            this.doNext(this.enterTheBlackCock);
            return;
        }
        this.outputText(this.images.showImage("andy-smoke"));
        if (this.flags[kFLAGS.BLACK_COCK_ANDY_STASH_STATE] < 2) {
            this.outputText("You nod, telling Andy you want to smoke his Nepenthe. He gives you a big smile as he scoots closer to you and shows you what to do, \"<i>It's pretty easy. When the vapor fills this chamber you want to empty your lungs and put it to your lips. Pull this divider up and inhale… simple.</i>\" He then demonstrates and once he's done he's left with a smiling face, \"<i>Also, hold it in your lungs for a second. That makes it feel better.</i>\"");
            ...
            this.flags[kFLAGS.BLACK_COCK_ANDY_STASH_STATE] = 2;
        }
        else {
            this.outputText("You hold your hand out for the pipe. Andy scoots closer to you and passes it, leaning back against the barrels. As you take your hit the warm happy heat blooms in your [fullChest] and you hand the pipe back with a giant smile.");
        }
        this.dynStats("lus", 10, "scale", false);
        this.dynStats("lus", 10 + (this.player.lib / 5) + (this.player.cor / 10));
        switch (rand(2)) {
            case 0:
            ...
            case 1:
                ...
                if (this.flags[kFLAGS.TIMES_TRANSFORMED] < 3 || this.player.humanScore() >= 5) this.outputText("You nod, telling him that you look the same as when you walked through the portal.");
                else this.outputText("You shake your head and tell the curious satyr you've changed since coming to Mareth.");
        ...
        this.player.gems -= 5;
        this.statScreenRefresh();
        this.applyAndysSmokeEffect();
        this.outputText("\n\nWhat do you want to do?");
        this.menu();
        this.addButton(0, "Grab It", this.grabAndysDongAfterSmoking).hint("That cock looks fun to play with!");
        this.addButton(1, "Do Nothing", this.doNothingAfterSmoking);
    }

    private doNothingAfterSmoking(): void {
        this.clearOutput();
        this.outputText("You can't help laughing at the horny satyr's predicament as he exhales, this time creating a female dog morph, her fingers attached to her wispy snatch, \"<i>Yeah,</i>\" He says with a grin, \"<i>I gotta start wearing pants.</i>\"");
    ...
    private grabAndysDongAfterSmoking(): void {
        this.clearOutput();
        this.outputText("Feeling adventurous you wrap a hand around the foot long doggy dong standing at attention next to you. Andy gives a soft gasp as you squeeze his hot dick. After giving it a few pumps he gently tugs your hand off of his leaking dog cock.");
        ...
        if (this.player.hasCock()) { // Male/herm
            ...
            if (this.player.vaginas[0].vaginalLooseness >= 2) {
            ...
            // Biped or naga
            if (this.player.isBiped()) this.outputText("You spread your legs as you settle down and take a deep hit, enjoying the wet teasing fingers and the lung full of warm joy.");
            else if (this.player.isNaga()) this.outputText("You wrap your coils around his waist, enjoying the wet teasing fingers and the lung full of warm joy.");
            // Anal looseness check
            if (this.player.ass.analLooseness < 2) { // Virgin or tight
            ...
            else if (this.player.ass.analLooseness < 4) { // Loose
    ...
    private stayAfterNoSmoke(): void {
        ...
        this.dynStats("lus", 20, "scale", false);
        this.menu();
        if (this.player.hasCock()) this.addButton(0, "Suck my Cock", this.haveAndySuckYouOff).hint("Have Andy suck you off.");
        else this.addButtonDisabled(0, "Suck my Cock", "You'll need a cock to have Andy do this.");
        if (this.player.hasVagina()) this.addButton(1, "Lick my Pussy", this.haveAndyLickYouOut).hint("Have Andy lick your pussy.");
        else this.addButtonDisabled(1, "Lick my Pussy", "You'll need a vagina to have Andy do this.");
        this.addButton(2, "Rim my Ass", this.haveAndyRimYouOmgItsGross).hint("Have Andy rim your ass. \n\nNOTE: Contains rimjob! You have been warned.");
        this.addButton(4, "Do Nothing", this.declineAndyOral);
    }

    private haveAndySuckYouOff(): void {
        this.clearOutput();
        this.outputText(this.images.showImage("andy-sex-blowjob"));
        this.outputText("He looks at you with his boyishly cute face surrounded by a cloud of curly dark brown hair. You begin pulling off your [armor] so as to free your [eachCock]. As you do Andy's mouth drops.");
        ...
        this.outputText("\n\nYou wag your dick at him to display your impatience; he gets the message. It's as if he has snapped out of a trance the way he suddenly tugs his shirt off and kneels before you with the reverence of a servant bowing before his " + this.player.mf("king", "queen") + ". With patient hands he reaches forward, taking your [cock] out of your hand and holding it out with a firm grip as his lips descend.");
        // Cocks check
        if (this.player.findFirstCockType(CockTypesEnum.DOG) >= 0) {
        ...
        else if (this.player.findFirstCockType(CockTypesEnum.CAT) >= 0) {
        ...
        else if (this.player.findFirstCockType(CockTypesEnum.HORSE) >= 0) {
        ...
        else if (this.player.findFirstCockType(CockTypesEnum.RHINO) >= 0) {
        ...
        else if (this.player.findFirstCockType(CockTypesEnum.LIZARD) >= 0) {
        ...
        else if (this.player.findFirstCockType(CockTypesEnum.TENTACLE) >= 0) {
        ...
        else if (this.player.biggestCockLength() >= 7 && this.player.biggestCockLength() < 12) {
    ...
    private haveAndyRimYouOmgItsGross(): void {
        this.clearOutput();
        this.outputText(this.images.showImage("andy-sex-rimjob"));
        this.outputText("With a grin on your face you ask him how much he really likes eating ass.");
    ...
    private declineAndyOral(): void {
        this.clearOutput();
        this.outputText("Despite his obvious flirtation you're not really interested in his oral skills at the moment. After a few seconds of awkward silence he takes one last deep drag and blows more smokey shapes. The pleasing scent washes over you whenever he does and soon you're light headed and happy. After chatting with the satyr about nothing important he decides he has to get back to work. He lets you out of the large swinging gate and you wave goodbye as you depart for camp.");
        this.doNext(this.camp.returnToCampUseOneHour);
    }

    private leaveAfterNoSmoke(): void {
        this.clearOutput();
        this.outputText("You tell the satyr that you think you'd best be leaving. He smiles as he toys with his lighter before saying, \"<i>Alright… Feel free to come back whenever you want [name].</i>\"");
        this.doNext(this.camp.returnToCampUseOneHour);
    }

    private rompUpWithAndyTheSatyr(): void {
        this.clearOutput();
        if (this.flags[kFLAGS.BLACK_COCK_ANDY_SEX_COUNTER] == 0) {
            this.outputText("You tell Andy that you’d like to accompany him out back.");
            ...
            this.outputText("You tell Andy that you’re going to accompany him out the back. You make sure to give him a suggestive look, your [eyes] glancing at his fuzzy balls just long enough to see the angry red tip push through the slit of his sheath. He gives you a conspiratorial grin. He once again sneaks you out the back, dog cock hanging out as you follow him, watching his soft little fuzzy cheeks move against one another.");
        ...
        this.menu();
        if (this.player.hasCock()) this.addButton(0, "Fuck Him", this.fuckAndyTheSatyr).hint("Take Andy from behind and put your cock to good use.");
        else this.addButtonDisabled(0, "Fuck Him", "You will need a penis for that.");
        this.addButton(1, "Get Fucked", this.haveAndyFuckYouPrompt).hint("Get Andy to fuck you good!");
    }

    private fuckAndyTheSatyr(): void {
        this.clearOutput();
        this.outputText(this.images.showImage("andy-sex-analpitch"));
        // Height
        if (this.player.tallness < 64) this.outputText("You look up into Andy’s eyes and tell him you’re going to be fucking him today. He smiles as he looks down into your eyes and says, \"<i>So long as you fuck me hard, lil’ " + this.player.mf("guy", "gal") + "..</i>\" He obviously likes the idea of having your [cock] stuffed into his tailhole; you’ll have to oblige him and fuck the cocky bastard as hard as you can for insinuating that your height might be a problem.");
        else if (this.player.tallness >= 64 && this.player.tallness < 68) this.outputText("You look straight into Andy’s eyes and tell him he’s going to be taking your [cock] today. He nods as you grab his trim waist and says, \"<i>Yes " + this.player.mf("sir", "ma'am") + "</i>\". From the way his dick jumps at the brief contact you can tell he quite likes the idea of you stuffing your [cock] into his tailhole.");
        else this.outputText("You grab Andy by his polished horn and make him look up, tearing his gaze away from your body and directing it toward your face. His mouth drops open as you tell him you’re going to be fucking his little satyr ass today and his only response is a husky, \"<i>Oh fuck yes.</i>\" You can see his little knotted dog dick twitch at the thought of you ramming your [cock] into his tailhole. When you release his horns he rubs the spot with a smile; he is obviously a fan of the rough treatment, and maybe even a little giddy about you being taller than him.");
        // Taur or not?
        if (this.player.isTaur()) {
            ...
            if (this.player.balls > 0) this.outputText("Your [balls] churn with your need for release as hot tingles shoot up your spine. ");
            if (this.player.hasVagina()) this.outputText("Even your [vagina] feels fit to explode as you mercilessly pound the sexy little satyr’s furry rump.");
            ...
            if (this.player.cockTotal() > 1) {
                this.outputText("[eachCock] spits hot white semen, most of which travels through your [cock] and inseminates Andy’s upturned backside. ");
                if (this.player.cumQ() >= 2500) this.outputText("His belly swells dangerously as each of your dicks spits, creating a puddle of mixed man cream as Andy passes out beneath you. ");
            }
            else {
                this.outputText("You hold your [cock] deep inside Andy’s wrecked hole, pressed against him like a husband and wife snuggling for warmth. Your body trembles like a scared mouse as you unload into his tightly clenching bowels. ");
                if (this.player.cumQ() >= 2500) this.outputText("From your vantage point above the two of you you can see Andy’s belly swelling dangerously. Baby batter leaks from his wrecked hole as well, creating an impressive puddle while making him look pregnant at the same time. ");
            }
        }
        else {
            ...
            if (this.player.biggestCockLength() < 7) {
                ...
                if (this.player.cockTotal() == 2) this.outputText("He takes your remaining [cock 2] into his hand and fists it with a sure and practiced hand. ");
                else if (this.player.cockTotal() > 2) this.outputText("He takes two of your remaining dicks and fists them with two sure and practiced hands. ");
                this.outputText("He hums as he sucks you in, drool leaking from the side of his mouth as you grab his curly hair and pull his talented mouth up and down your [cock], fucking his face hard due to his reaction to your size.");
            }
            else if (this.player.biggestCockLength() >= 7 && this.player.biggestCockLength() < 13) {
            ...
            else if (this.player.biggestCockLength() >= 12 && this.player.biggestCockLength() < 19) {
            ...
            if (this.player.hasKnot()) {
                ...
                if (this.player.cumQ() >= 2500) this.outputText("You watch with relish as the little guy’s stomach swells, your baby batter filling him up from the back door and pushing his still bent legs forward, forcing him to let them go. ");
                this.outputText("Andy Mathis looks up at you with a lazy smile as he holds his hand up, asking for a high five. With a chuckle and a shake of your head you give it to him before tugging your knot, uncocking his insides and letting your potent seed spread onto the floor. Feeling a little tired and sweaty you look down at your cum covered and lightly dozing fuck buddy, watching his chest rise and fall before you gather your [armor] and exit through the back gate, headed back for camp now that you’ve had your fill.");
            }
            else {
                ...
                if (this.player.cumQ() >= 2500) this.outputText("Your body trembles as you soak his innards with blast after blast of ejaculent, some of which you can feel streaming down your [legs] as the little guy’s stomach swells with however much of your seed doesn’t fall into the puddle you’re creating below. ");
                this.outputText("Andy Mathis looks up at you with a lazy smile as he holds his hand up, asking for a high five. With a chuckle and a shake of your head you give it to him before tugging your [cock], out of his cum greased backside. Feeling a little tired and sweaty you look down at your cum covered and lightly dozing fuck buddy, watching his chest rise and fall before you gather your [armor] and exit the building out the back door, headed back for camp now that you’ve had your fill.");
            }
        }
        this.player.orgasm('Dick');
        this.dynStats("sens", -1);
    ...
    private haveAndyFuckYouPrompt(): void {
        if (this.player.hasVagina()) {
            this.clearOutput();
            this.outputText("\"<i>Well alright... So you want me to fuck you? Which of your holes would you like me to fuck?</i>\" Andy prompts.");
            this.addButton(0, "Vaginal", this.haveAndyFuckYou, true);
            this.addButton(1, "Anal", this.haveAndyFuckYou, false);
        }
        else {
            this.haveAndyFuckYou(false);
        }
    }

    private vagOrAss(vaginal: boolean): string {
        if (vaginal) return this.player.vaginaDescript();
        else return this.player.assDescript();
    }

    private haveAndyFuckYou(isVaginal: boolean): void {
        this.clearOutput();
        if (isVaginal) this.outputText(this.images.showImage("andy-sex-vaginalcatch"));
        else this.outputText(this.images.showImage("andy-sex-analcatch"));
        const ANDY_COCK_AREA: number = 12 * 1.5; // 18
        ...
        if (this.player.isTaur()) { // Tauric lower body
            ...
            // Looseness check
            if ((this.player.hasVagina() && this.player.vaginas[0].vaginalLooseness < 2 && isVaginal) || (!this.player.hasVagina() && this.player.ass.analLooseness < 2)) {
                this.outputText("\n\nYou hear a low whistle and Andy says, \"<i>Wow, you’re tight… hold on a sec.</i>\" You look back in time to watch the satyr squat and shove his face into your " + (this.vagOrAss(isVaginal)) + ". Your voice catches in your throat as you feel his tongue invading your tender opening. The strong muscle is soon joined by a finger that slips into you and swirls, the sensation causing you to swoon. Soon he leans back and spits before fingering the cold wetness into your " + (this.vagOrAss(isVaginal)) + " with dual fingers. Your sexy groans seem to be egging him on as his dual fingers sink into you, spread and then retract. Once the sensation becomes familiar you tell him to stop playing and put his dick in you. His fingers obediently retract and he leans in, planting a wet kiss on your " + (this.vagOrAss(isVaginal)) + " before putting the bullet shaped head of his dog cock up against your hungry hole, pushing in without delay. ");
                if (isVaginal) this.player.cuntChange(ANDY_COCK_AREA, true);
                else this.player.buttChange(ANDY_COCK_AREA, true);
                this.outputText("\n\nYour head swims as his dick widens your " + (this.vagOrAss(isVaginal)) + ". Your sharp gasp slows his advance; he’s a very attentive lover, so eager to please. He moves with a deliberate slowness, his fingers holding your [ass] firmly in place as he pulls back and pushes forward. He isn’t fucking you with his entire length but instead is slowly working it into you, allowing you to get used to his dog dick’s long thickness. Soon enough you feel his knot bump against your " + (this.vagOrAss(isVaginal)) + " and he begins fucking you with deep eager thrusts that send your head reeling as you do everything you can to not take a step forward. In almost no time you feel the tell tale tingle of your impending orgasm and, from the way his thrusts are becoming sloppy and less timed, you’d assume Andy feels the same.");
            }
            else if ((this.player.hasVagina() && this.player.vaginas[0].vaginalLooseness < 4 && isVaginal) || (!this.player.hasVagina() && this.player.ass.analLooseness < 4)) {
                this.outputText("\n\nAndy spits into his hand and you turn to see him slickening his foot long cock. He smiles back at you and hooks a free finger inside your " + (this.vagOrAss(isVaginal)) + ". You watch his bitten lip as his finger moves in and out of you. He adds one, and then a third, pushing through your insides and spreading your entrance in preparation for his thick dog cock. It is a pleasurable sensation, there’s no doubt about that, but not the one you wanted. As he catches your eye and smiles at you like a happy kid with a new toy you give him an impatient glare. He obediently dislodges his fingers from your " + (this.vagOrAss(isVaginal)) + " and grabs his spit slickened member.");
                ...
                this.outputText("Andy wastes no time and begins sliding his rigid length in and out of your " + (this.vagOrAss(isVaginal)) + " with fevered need. Every time you feel his swollen knot bump against your entrance you squeeze, driving the satyr mad as he begins pounding you harder, his long dick making a glorious mess of your " + (this.vagOrAss(isVaginal)) + " as you push back, his knot threatening to spread you open even further every time it slams against your " + (this.vagOrAss(isVaginal)) + " before it is snatched away by Andy’s swiftly thrusting hips become sloppy and poorly timed. He’s close but a familiar tingle creeping up your spine tells you you are as well.");
            }
            else {
                this.outputText("\n\nAndy slides a spit soaked hand against your " + (this.vagOrAss(isVaginal)) + " with a whistle, \"<i>Oh I’m going to like this.</i>\" He says as he pulls your loose lips apart and gazes at your entrance with hungry eyes. He wastes no time and grabs his leaking dick and shoves it into your " + (this.vagOrAss(isVaginal)) + " with a snarl. ");
                if (isVaginal) this.player.cuntChange(ANDY_COCK_AREA, true);
                else this.player.buttChange(ANDY_COCK_AREA, true);
                this.outputText("\n\nAndy thrusts forward hard and swirls his dick around inside of you. Looking back you see his eyes are shut tight and his long tongue is hanging out of his open mouth as he pounds your hole with body jarring strokes. The surprising strength behind his thrusts is only matched by the sensations tingling through your body. You can feel his swollen knot slamming against your " + (this.vagOrAss(isVaginal)) + ", threatening to enter you before it is snatched away by his rapidly thrusting hips. Soon your body begins to quiver as you stand on the edge of your impending climax. From the sounds he’s making you’d guess he is as well.");
            }
            // Genitals check
            if (this.player.hasCock() && this.player.hasVagina() && isVaginal) { // Hermaphrodite
                ...
                if (this.player.cumQ() >= 1000 && this.player.cumQ() < 2500) this.outputText("As you unload your cum pools around your [feet], spreading out in a giant puddle beneath you.");
                else if (this.player.cumQ() >= 2500) this.outputText("As you unload you feel [eachCock] spasm and coat your [legs] and [feet] in hot pearly goo. Your orgasm rages on for what seems like an eternity, [eachCock] creating an ocean of melted pearl that stretches out and covers the ground around you.");
            ...
            else if (this.player.hasCock()) { // Male
                ...
                if (this.player.cumQ() >= 1000 && this.player.cumQ() < 2500) this.outputText("As you fire blast after blast of baby batter across the floor the cum spewing from your body pools around your [feet], leaving a pearly puddle below you.");
                else if (this.player.cumQ() >= 2500) this.outputText("Your cum explodes out of [eachCock] like fireworks. Groaning like a dying man your shiver in ecstasy as your cum soaks the earth beneath your [feet], spreading out like a small lake made of moonlight.");
                this.outputText("\n\nThere you are breathing like you just ran a marathon. You feel an incredible amount of satyr spunk sloshing around in your horse half. Your body feels heavy but at the same time you’re blissfully aware of the glorious post orgasmic ache spreading throughout your body. You can feel Andy lying against your horse half, catching his breath before he stands and tugs his dog cock. You canter back, his swollen knot lodged inside your [ass] and holding the two of you together.");
            }
            else if (this.player.hasVagina() && isVaginal) { // Female
            ...
            this.outputText("\n\nOnce he’s down you can plainly see his tall standing erection. You guess his knotted dog dick to be a foot long. The guy is leaking like a sieve. Just looking at the tall standing canine cock makes your " + (this.vagOrAss(isVaginal)) + " yearn to be filled. Andy is obviously as hungry for your " + (this.vagOrAss(isVaginal)) + " as you are for his dog cock, you can tell by the way he palms your tender flesh. With a smile you lower yourself, reaching down to tug his knotted shaft towards your waiting " + (this.vagOrAss(isVaginal)) + ".");
            // Looseness check
            if ((this.player.hasVagina() && this.player.vaginas[0].vaginalLooseness < 2 && isVaginal) || (!this.player.hasVagina() && this.player.ass.analLooseness < 2)) {
                this.outputText("\n\nThe bullet shaped head of Andy’s dog prick stretches your " + (this.vagOrAss(isVaginal)) + " to the max. The girth of it alone is almost enough to make you hop off and leave him there. Yet looking down at him, the way his eyes widen as he feels your tightness enveloping his cock, gives you the strength to continue. After dropping a line about what you’ll do to him should he move you slowly sink down onto the foot long member, your teeth clenched and you descend. ");
                if (isVaginal) this.player.cuntChange(ANDY_COCK_AREA, true);
                else this.player.buttChange(ANDY_COCK_AREA, true);
                ...
                this.outputText("\n\nAs the bullet shaped head of Andy’s dog prick pushes into your " + (this.vagOrAss(isVaginal)) + " he reaches forward to grab your hips, presumably to pull your body up and down his tall standing erection. All it takes is a look to stop him and you tell the satyr to relax. He shudders inside of you as you sink down his rigid pole with deliberate slowness. ");
                if (isVaginal) this.player.cuntChange(ANDY_COCK_AREA, true);
                else this.player.buttChange(ANDY_COCK_AREA, true);
                this.outputText("\n\n\"<i>Oh Marae.</i>\" He gasps as you sink, stopping at his swollen knot. You feel blissfully full and let him know with a deep tongue wrestling kiss. When you come up for air he just smiles up at you, trembling with excitement as he verbally reminds himself not to move. Ignoring his chanting you begin bouncing up and down his swollen dog prick, using him like your own personal dildo. His trembling actually spurs you to ride faster, your " + (this.vagOrAss(isVaginal)) + " making a squelching sound as you ride his leaking cock. He begins to whine and you can’t help but sympathize with him. You could ride this living dildo forever, feel its slick heat as it penetrates your " + (this.vagOrAss(isVaginal)) + " at exactly the perfect pace, controlled by your [legs]. Yet from the tell tale tingle creeping up your spine you know it’ll end soon.");
            }
            else {
                this.outputText("\n\nAs the bullet shaped head of Andy’s dog prick sinks into your " + (this.vagOrAss(isVaginal)) + " you sink down the entire length without a problem. His eyes go wide as he looks up at you with his jaw hanging open. Then his hands stretch towards your hips like a boy hesitantly reaching for a present he can scarcely believe is real. You tell him he’s going to sit still and let him know what you’ll do to him if he doesn’t. Tucked inside you he complies as best he can. Even though he holds himself still he can’t seem to help trembling beneath you but as his unconscious tremors vibrate his cock it’s not something you’re worried about. It only makes him a better toy. ");
                if (isVaginal) this.player.cuntChange(ANDY_COCK_AREA, true);
                else this.player.buttChange(ANDY_COCK_AREA, true);
                this.outputText("\n\n\"<i>Oh my… ahhnnn, fuck.</i>\" he whines as you begin to slide up and down his doggy dick. The foot long cock is a hot rod of slick heat you ride at a leisurely pace, enjoying the teasing tickle caused by the trembling satyr beneath you. Once you’ve had your fill of torturing the poor guy you throw caution to the wind and begin to really ride him, enjoying the way he cringes every time your " + (this.vagOrAss(isVaginal)) + " completely envelop his dog cock. The slap, slap, slap of your weight crashing down on his pelvis fills the air and you watch the trembling satyr hold on for dear life. The entire time you’re expecting him to pop at any moment but somehow he manages to hold on until you feel your satisfaction preparing to culminate in a climax of epic proportions.");
            }
            // Genitals check
            if (this.player.hasCock() && this.player.hasVagina() && isVaginal) { // Hermaphrodite
                this.outputText("\n\nGroaning like a madman Andy suddenly thrusts forward, his swollen knot penetrating your vaginal walls and pushing you over the edge. With a simultaneous cry the two of you unload together. [eachCock] fires round after round of dick milk and your [vagina] screams in tingling triumph as the wet heat of Andy’s satyr spunk coats your insides. ");
                if (this.player.cumQ() >= 250 && this.player.cumQ() < 1000) this.outputText("As you unload Andy opens his mouth to catch your seed as it paints his chest and chin in streaks of pearly white baby batter.");
                else if (this.player.cumQ() >= 1000 && this.player.cumQ() < 2500) this.outputText("As you unload the Andy opens his mouth to catch your seed as it paints his chest, chin and abs in a thick coating of white goo.");
                else if (this.player.cumQ() >= 2500) this.outputText("As you unload the satyr practically drowns in your baby batter, coated from horns to abs in a heavy coating of white spooge that pools around the two of you like a lake of melted pearls.");
            ...
            else if (this.player.hasCock()) { // Male or herm choosing anal
                ...
                if (this.player.cumQ() >= 250 && this.player.cumQ() < 1000) this.outputText("As you unload Andy opens his mouth to catch your seed as it paints his chest and chin in streaks of pearly white baby batter.");
                else if (this.player.cumQ() >= 1000 && this.player.cumQ() < 2500) this.outputText("As you fire blast after blast of baby batter across Andy’s body you manage to liberally coat his entire form from his chin to his abs, [eachCock] spent as you drool out your last bit of spunk.");
                else if (this.player.cumQ() >= 2500) this.outputText("Your cum explodes out of [eachCock] like fireworks and as you cry out so does Andy, sputtering as your titanic orgasm leaves him covered head to abs in your copious load.");
        ...
        if (this.player.hasVagina()) this.player.knockUp(PregnancyStore.PREGNANCY_SATYR, PregnancyStore.INCUBATION_SATYR, 50); // 2x chance of pregnancy
        else this.player.buttKnockUp(PregnancyStore.PREGNANCY_SATYR, PregnancyStore.INCUBATION_SATYR, 50);
    ...
    protected haveHarryFuckYou(): void {
        ...
        if (this.player.analCapacity() < 40) {
            ...
            if (this.player.hasCock() && this.player.hasVagina()) { // Hermaphrodite
                ...
                if (this.player.balls > 0) this.outputText("His powerful grip begins to slide down your [cock]. They continue down to your [balls] and give them a firm but gentle squeeze, one that sends you bucking back into his greasy cock as it spills gobs of precum onto your [ass]. ");
                else this.outputText("His powerful grip slides down your [cock] with firm squeezes, his thumb rubbing the top of your dick as it travels south probably searching for a pair of gonads you don't have; once his fruitless search is over he returns to tugging your [cock]. ");
                ...
                if (this.player.vaginas[0].vaginalWetness >= 4) this.outputText("Every time you gush fluid onto his fingers he vibrates his palm, sending uncontrollable shuddering screams into the air as the sensations intensify. ");
                this.outputText("Once he's satisfied with the number of times you've spilled girl cum onto his kitchen floor he pulls his finger from your pleasantly tingling [pussy].");
            ...
            if (this.player.analCapacity() >= 40 && this.player.analCapacity() < 55) {
                ...
                this.player.buttChange(HARRY_COCK_AREA, true);
            ...
            else if (this.player.analCapacity() >= 55) {
        ...
        this.flags[kFLAGS.BLACK_COCK_HARRY_SEX_COUNTER]++;
        this.player.orgasm('Anal');
        this.doNext(this.camp.returnToCampUseOneHour);
    }

    private nevermindToHarrySex(): void {
        this.clearOutput();
        this.outputText("You put your hands up in an attempt to look innocent as you tell the man he's crazy and you weren't thinking any such thing.");
    ...
    // Frida
    // ------------
    private meetFrida(): void {
        this.clearOutput();
        this.displayFridaSprite();
        this.outputText(this.images.showImage("frida-intro"));
        if (this.flags[kFLAGS.BLACK_COCK_MET_FRIDA] == 0) {
            this.outputText("You approach the echidna-girl's side of the counter, taking a seat in one of the barstools. She approaches you, a pleased smile decorating her visage.");
            ...
            this.flags[kFLAGS.BLACK_COCK_MET_FRIDA] = 1;
        }
        else {
            this.outputText("Frida smiles at you as you take a seat in one of the barstools.");
            if (this.flags[kFLAGS.BLACK_COCK_FRIDAS_CAKE_EATEN_COUNTER] > 0) this.outputText("\n\n\"</i>You come here for another piece of cake?</i>\"");
            else this.outputText("\n\n\"<i>You ready for a piece of cake now, hun?</i>\"");
        }
        this.unlockCodexEntry("Echidna-Morphs", kFLAGS.CODEX_ENTRY_ECHIDNAS);
        this.doYesNo(this.orderFridasCake, this.noThanksToCake);
    }

    private orderFridasCake(): void {
        this.clearOutput();
        this.outputText(this.images.showImage("frida-order-cake"));
        if (this.player.gems < 10) {
            this.outputText("You count out your gems and realize you're short on these at the moment.");
            this.doNext(this.enterTheBlackCock);
            return;
        }
        if ((this.player.hunger100 >= 90 && this.flags[kFLAGS.HUNGER_ENABLED] > 0) || (this.player.hasStatusEffect(StatusEffects.Fullness) && this.flags[kFLAGS.HUNGER_ENABLED] == 0)) {
            this.outputText("<b>You are too full to consider ordering that that.</b>");
            this.doNext(this.enterTheBlackCock);
            return;
        }
        this.outputText("You drop 10 gems into Frida's waiting hand. She stuffs them in one of her apron pockets as she removes the lid from the cake stand. She takes a dull knife from a nearby knife block and cuts you a piece, putting it on a plate. She sits the plate in front of you before retreating to the back of the tent to pour your milk. Within seconds, she returns with the milk, putting it down next to your plate.");
        this.outputText("\n\n\"<i>Enjoy!</i>\"");
        ...
        this.player.refillHunger(35);
        if (this.flags[kFLAGS.HUNGER_ENABLED] == 0) this.player.modThickness(100, 2);
        this.player.modTone(0, 1);
        if (!this.player.hasStatusEffect(StatusEffects.Fullness)) this.player.createStatusEffect(StatusEffects.Fullness, 2, 0, 0, 0);
        else this.player.changeStatusValue(StatusEffects.Fullness, 1, 2);
        this.flags[kFLAGS.BLACK_COCK_FRIDAS_CAKE_EATEN_COUNTER]++;
        this.doNext(this.camp.returnToCampUseOneHour);
    }

    private noThanksToCake(): void {
        this.clearOutput();
        if (this.flags[kFLAGS.BLACK_COCK_FRIDAS_CAKE_EATEN_COUNTER] == 0) this.outputText("You politely decline, not feeling very hungry at the moment.");
        else {
            this.outputText("You politely decline, explaining that you were just stopping by to say hello. Frida waves at you as you get up.");
    ...
    // PATRONS
    // ------------
    private checkPatrons(): void {
        let patronCount: number = 0;
        this.clearOutput();
        this.menu();
        if (this.getGame().time.hours >= 19) {
            this.outputText("There is a large group of loud satyrs taking up a table, throwing back mugs of beer and belching loudly, obviously too absorbed in their drinking to pay attention to anyone else.\n\n");
            this.addButton(0, "Satyrs", this.satyrGangIntro);
            patronCount++;
        }
        if (patronCount == 0) this.outputText("There appears to be nobody interesting here.");
        this.addButton(14, "Back", this.enterTheBlackCock);
    }

    private satyrGangIntro(): void {
        this.clearOutput();
        this.outputText(this.images.showImage("satyr-gang-intro"));
        ...
        if (this.player.lust >= 33) {
            if (!this.player.isTaur()) {
                this.addButton(0, "Let Them", this.letTheSatyrsHaveTheirWayWithYou, false, undefined, undefined, "Let the satyrs have their way with you. This is going to be rough and intense. ");
                if (this.player.hasVagina()) this.addButton(5, "No Rimjobs", this.letTheSatyrsHaveTheirWayWithYou, true, undefined, undefined, "Let the satyrs have their way with you. This is going to be rough and intense. Also signal that you don't want to rim one of the satyr's asses.");
            }
            else this.addButtonDisabled(0, "Let Them", "You doubt you'll be able to participate into the orgy with your tauric body.");
        }
        else this.addButtonDisabled(0, "Let Them", "You aren't horny enough to consider that.");
        this.addButton(1, "No", this.dontLetTheSatyrsFuck);
    }

    private letTheSatyrsHaveTheirWayWithYou(noRimjob: boolean = false): void {
        this.clearOutput();
        this.outputText(this.images.showImage("satyr-gang-bang"));
        if (!this.player.isTaur()) {
            this.outputText("Greedy hands yank you back first over the table. The rough satyrs position you so your [ass][if (hasTail = true) \" and tail\"] hang off the edge of the table. Immediately they begin pulling off your [armor]. As your body comes into view they make grunts of approval; they all begin high fiving and you hear several brash comments about your body including, \"<i>[He]'s a beaut isn't [he]?</i>\" and \"<i>SKULLFUCK, SKULLFUCK, SKULLFUCK.</i>\"");
            ...
            this.outputText("\n\nFinally exhausted, you slump on one of the satyrs, embracing his warm body. The rest of the satyrs are finally spent as well. When you've finally recovered from the ordeal, you get yourself cleaned up" + this.player.clothedOrNaked(", redress yourself") + " and return to your camp.");
        }
        this.player.orgasm('VaginalAnal');
        this.player.changeFatigue(25);
        this.dynStats("lib", 1, "sens", 2, "cor", 1);
        if (this.player.hasVagina()) this.player.knockUp(PregnancyStore.PREGNANCY_SATYR, PregnancyStore.INCUBATION_SATYR);
        this.player.buttKnockUp(PregnancyStore.PREGNANCY_SATYR, PregnancyStore.INCUBATION_SATYR);
        this.doNext(this.camp.returnToCampUseOneHour);
    }

    private dontLetTheSatyrsFuck(): void {
        this.clearOutput();
        this.outputText("Pulling your hands from their grip you tell the satyrs you weren't looking their way and make an abrupt U-turn, disappearing out the doors amidst a musical theme of laughter and feeling embarrassed as all hell.");
        this.doNext(this.enterTheBlackCock);
    }
