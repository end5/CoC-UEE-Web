...
export class SharkGirlScene extends AbstractBoatContent {
    ...
    */
    ...
    public sharkGirlEncounter(exploreLoc: number = 0): void {
        ...
        if (this.flags[kFLAGS.IZMA_ENCOUNTER_COUNTER] == 0) this.flags[kFLAGS.IZMA_ENCOUNTER_COUNTER] = 1;
        if (!this.player.hasStatusEffect(StatusEffects.SharkGirl)) this.player.createStatusEffect(StatusEffects.SharkGirl, 0, 0, 0, 0);
        else if (this.player.statusEffectv1(StatusEffects.SharkGirl) >= 7 && this.player.totalCocks() > 0) {
            this.spriteSelect(SpriteDb.s_sharkgirl);
            this.sharkBadEnd();
            return;
        }
        ...
        // Rowboat
        if (exploreLoc == 1) {
            this.outputText("While rowing the boat across the lake you spy a shark fin heading your way.  Worried it might damage the small boat, you hastily row back to shore, jumping out of the boat.  The shark shows no signs of slowing, and the fin disappears just before coming ashore.  ");
        }
        // Lake
        else {
            this.outputText("You wander around the sandy shores of the lake before raising your eyebrow at a strange sight: A solitary fin, circling around in the water. You take a step back in surprise when the fin suddenly starts barrelling toward the shore at inhuman speeds.  ");
        }
        ...
        this.unlockCodexEntry("Shark-girls & Tigershark-girls", kFLAGS.CODEX_ENTRY_SHARKGIRLS, false, true);
    ...
    public sharkWinChoices(): void {
        ...
        if (this.monster.HP < 1) {
            this.outputText("The shark-girl falls, clearly defeated.");
        }
        // Lust win
        else {
            this.outputText("The shark-girl begins masturbating, giving up on dominating you.  The sight is truly entrancing.");
            this.dynStats("lus", 15);
        }

        this.menu();
        this.addDisabledButton(0, "Use Dick", "This scene requires you to have cock and sufficient arousal.");
        this.addDisabledButton(1, "Pussy Oral", "This scene requires you to have vagina and sufficient arousal.");
        this.addDisabledButton(2, "Dildo Rape", "This scene requires you to have the Deluxe Dildo.");

        if (this.player.lust >= 33) {
            if (!this.player.isGenderless())
                this.outputText("  Do you have your way with her or leave?");
            if (this.player.hasCock()) this.addButton(0, "Use Dick", this.sharkgirlDickFuck);
            if (this.player.hasVagina()) this.addButton(1, (this.player.isNaga() ? "Pussy Oral" : "Pussy w/69"), this.sharkgirlSixtyNine);
            if (this.player.hasKeyItem("Deluxe Dildo") >= 0) this.addButton(2, "Dildo Rape", this.sharkGirlGetsDildoed);
        }
    ...
    private sharkgirlDickFuck(): void {
        this.player.addStatusValue(StatusEffects.SharkGirl, 1, 1);
        this.clearOutput();
        this.spriteSelect(SpriteDb.s_sharkgirl);
    ...
    private sharkgirlSixtyNine(): void {
        ...
        if (this.player.isNaga()) {
            ...
            // [if female]
            if (this.player.gender == 2) this.outputText(", pulling out every few minutes to lick your " + this.player.clitDescript() + ".");
            // [if herm]
            if (this.player.gender == 3) this.outputText(". You remind her not to neglect your " + this.player.cockDescript(0) + ", and the shark girl responds by thoroughly licking your hard erection and sucking at your " + this.player.ballsDescriptLight() + ".");
            this.outputText("\n\n");
            ...
            if (this.player.gender == 2) this.outputText(", juices spraying from your sex and coating the girl's face.");
            // [if herm]
            else this.outputText(" and stand up, gripping your " + this.player.cockDescript(0) + " and splattering thick jets of cum onto her face.");
            this.outputText("  The shark slut licks up your fluids hungrily.\n\n");

            this.outputText("Thoroughly satisfied, you leave the shark girl on the ground covered in your fluids and depart for your camp.");
        }
        this.combat.cleanupAfterCombat();
    ...
    public sharkGirlGetsDildoed(): void {
        this.clearOutput();
        this.outputText("You grin from ear to ear, revealing enough teeth to make even the defeated shark-girl shiver.  Advancing upon the prone monster-woman, you ");
        if (this.player.weaponName != "fists") this.outputText("put away your " + this.player.weaponName + " and ");
        this.outputText("draw out a glistening pink dildo from your pouches, as if it were a weapon.   She looks up at you, at once knowing your intent.  ");
        if (this.monster.lust >= this.monster.maxLust()) this.outputText("Her legs spread invitingly wide and she tears off her bikini bottom in a lusty frenzy.  You hold the dildo over her and give it a tiny squeeze, wringing out a few drops of pink fluid from the special dildo.  They land on her exposed cunt and in seconds she's writhing underneath you, humping the air.");
        else this.outputText("She holds her legs together defiantly, but you pry them open, tear off her bikini bottom and expose her bald nether-lips.   You hold the dildo over her and squeeze, squirting a gush of pink fluid onto her exposed cunt.  The effect is immediate, and in moments she's humping the air and wet with her own fluids.");
        this.outputText("\n\n");

        this.outputText("'<i>What a wonderful toy</i>,' you muse as you kneel between your ");
        if (this.player.cor > 50) this.outputText("latest ");
        this.outputText("victim's legs, inhaling her scent.  It smells surprisingly salty, reminding you of the sea.   Sand-papery skin brushes against your ankle.  You don't even have to look down to recognize the feeling of her tail curling around your leg and thigh.  Before she can use it to her advantage, you gently part her outer folds with one hand and insert the artificial member with the other.  The shark-girl's tail immediately drops away as her body shivers and relaxes, barely managing a few twitches.\n\n");

        this.outputText("The sea-smell from her groin intensifies and you feel the dildo bloat up in your hand.  She's orgasmed already!\n\n");

        this.outputText("You slap her left ass-cheek hard, the rough texture of her skin giving the perfect *crack* of skin on skin as you scold her, \"<i>You're not supposed to cum so hard!  Now you're really going to get stretched, skank!</i>\"\n\n");

        this.outputText("The aquatic monster-woman whimpers, feeling herself getting stretched wider and wider by the growing toy.   Intent on punishing her for trying to rape you AND for cumming too soon, you grab the dildo in a double-handed grip and force it in to the hilt.   Her tail flops and thrashes as the dildo sinks in until you bottom out.  She flops about like a dying fish, though perhaps the only thing about her 'dying' is a few brain cells from the bliss her cunny's feeling.  Satisfied with the degree of penetration, you grab it around the base and yank back on it, struggling with how tightly it's lodged in her cunt.\n\n");

        this.outputText("You wiggle it back and forth, working it partway out before ramming it back in.  Moaning and rocking underneath you, the shark-girl's eyes cross as she tries to fuck your hands.  Both her hands climb up under the thin black fabric of her top and wrap around her modest breasts, squeezing and caressing them.  She gasps when her fingers find her nipples, and starts drooling down her chin.\n\n");
        this.outputText("Her salty pussy clamps down hard, but the slick juices pouring from her hole makes it easier and easier to work the bloated sex-toy in and out of her love-hole.   Tired of the slut's moans and gasps of pleasure, you decide to finish her off.   You pull the dildo out, and let out a gasp of surprise when her twitching cunt gapes open obscenely, allowing you to see the entire depth of her love-tunnel, all the way to her cervix.   She groans at the sudden loss of sensation, but before she has a chance to jam a finger into her needy hole, you squeeze the perverted toy HARD, wringing a thick gush of pinkish fluid directly into her baby-maker.\n\n");

        this.outputText("You plunge the jiggling sex-toy back into her stretched cunt, pistoning the fluid directly into her womb and setting off an orgiastic cry with enough volume to make you cringe.  She trembles madly and you have to sit down on her tail to keep the tip of it from cracking you upside your head.  The dildo in your hands puffs up wider and wider until it pops itself out, exposing the pink-coated inner flesh of the shark-woman's cunt.  You can watch the muscles rippling and squeezing, though thanks to her stretching, you don't think she'll be able to squeeze anything smaller than a minotaur.  After a few more seconds her body goes still, save for the occasional twitch, and you realize she's passed out.\n\n");

        this.outputText("Damn that was hot!  You'll need to sate yourself once you get back to camp for sure.  Maybe you should give this dildo a whirl?  It still smells of your victim.");
    ...
    private sharkBadEnd(): void {
        ...
        this.outputText("The girls look at each other and grin before the one straddling you pulls out an odd-looking Shark's Tooth. \"<i>Oh you'll see. Open wide...!</i>\"\n\n");
        this.outputText("<b>Do you let the shark-girl put the odd-looking tooth into your mouth? You suspect it might end your adventures...</b>");
        this.doYesNo(this.sharkBadEnd2, this.rejectSharkBadEnd);
    }
    ...
    private sharkBadEnd2(): void {
        ...
        this.outputText("\"<i>Fresh meat!</i>\"");
        this.getGame().gameOver();
    }

    private rejectSharkBadEnd(): void {
        this.clearOutput();
        this.outputText("A thought flares up in your mind and you scowl at the shark-girls before you shove the shark-girl as hard as you can before making a dash for safety! One of the shark-girls begins to sniffle and contemplates what's with the sudden rejection.\n\n");
        this.outputText("The last you hear is a series of indistinct shouts before fading with the growing distance. Crisis averted!");
        this.player.addStatusValue(StatusEffects.SharkGirl, 1, -1);
    ...
    public sharkLossRape(): void {
        ...
        // Female:
        if (this.player.hasVagina() && (this.player.totalCocks() == 0 || rand(2) == 0)) {
            this.outputText("You slump down in defeat, too ");
            // [defeat via HP]
            if (this.player.HP < 1) this.outputText("hurt ");
            else this.outputText("horny ");
            ...
            this.outputText("The shark girl giggles and moves over to you, tugging at your " + this.player.armorName + "  impatiently. Her tail swishes around and smacks your " + this.player.assDescript() + ". \"<i>You're gonna make me very happy, you hear? Otherwise...</i>\" she opens her mouth wide and you see her fangs glinting menacingly in the light. You gulp hard and nod, bringing a smile from the shark girl.\n\n");
            this.outputText("Wasting no time, she removes her skimpy swimwear and your own gear.  ");
            // [if herm]
            if (this.player.gender == 3) this.outputText("Seeing your " + this.player.cockDescript(0) + " puts a smile on the shark girl's face as she takes a firm grip on your erection. \"<i>Well, you're just full of surprises, aren't you? Maybe I'll give this bad boy a whirl sometime. For now though...</i>\"  ");
            this.outputText("Her gaze drifts over to your " + this.player.vaginaDescript(0) + " and she licks her lips in delight. \"<i>Now that's what I'm looking for! Tell you what dear, you get me wet and I might just give you some pleasure too.</i>\"\n\n");
            ...
            this.outputText("The shark girl stands to leave and winks at you before diving back into the water. You eventually pass out from the exertion.");
