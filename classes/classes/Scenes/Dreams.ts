...
export class Dreams extends BaseContent {

    // Returns true if needs to END SHIT
    public dreamSelect(): boolean {
        let inBimbo: boolean = false;
        if (this.getGame().bimboProgress.ableToProgress()) inBimbo = true;

        let daydream: number = 0;
        const choices: any[] = new Array();
        // dream quantity
        let dreamtemp: number = 0;
        dreamtemp = rand(this.player.lib / 10 + this.player.cor / 20) + this.player.cor / 20;

        // BUILD UP CHOICES ARRAY
        this.temp = this.player.humanScore();
        // Add human numbers to choices
        while (this.temp > 0) {
            choices[choices.length] = 0;
            this.temp--;
        }
        this.temp = this.player.dogScore();
        while (this.temp > 0) {
            choices[choices.length] = 2;
            this.temp--;
        }
        this.temp = this.player.horseScore();
        while (this.temp > 0) {
            choices[choices.length] = 1;
            this.temp--;
        }
        this.temp = this.player.cowScore();
        while (this.temp > 0) {
            choices[choices.length] = 3;
            this.temp--;
        }
        this.temp = this.player.catScore();
        while (this.temp > 0) {
            choices[choices.length] = 4;
            this.temp--;
        }
        this.temp = this.player.demonScore();
        while (this.temp > 0) {
        ...
        if (this.player.minotaurAddicted()) {
            ...
            choices[choices.length] = 6;
            if (this.player.findPerk(PerkLib.MinotaurCumAddict) >= 0) choices[choices.length] = 6;
        }
        // Akbal
        if (this.flags[kFLAGS.AKBAL_SUBMISSION_COUNTER] > 4) {
            if (this.flags[kFLAGS.AKBAL_SUBMISSION_COUNTER] > 10) choices[choices.length] = 7;
            if (this.flags[kFLAGS.AKBAL_SUBMISSION_COUNTER] > 13) choices[choices.length] = 7;
            if (this.flags[kFLAGS.AKBAL_SUBMISSION_COUNTER] > 15) choices[choices.length] = 7;
            if (this.flags[kFLAGS.AKBAL_SUBMISSION_COUNTER] > 18) choices[choices.length] = 7;
            choices[choices.length] = 7;
            choices[choices.length] = 7;
        }
        // Exgartuboobs
        if (this.player.statusEffectv1(StatusEffects.Exgartuan) == 2) {
            choices[choices.length] = 8;
            choices[choices.length] = 8;
            choices[choices.length] = 8;
        }
        // Exgartucock
        if (this.player.statusEffectv1(StatusEffects.Exgartuan) == 1 && this.player.hasCock()) {
            choices[choices.length] = 9;
            choices[choices.length] = 9;
            choices[choices.length] = 9;
        }
        // Latexy Skinz
        if (this.player.skin.adj == "latex" || this.player.skin.adj == "rubber") {
        ...
        // Dom + Scylla
        if (this.flags[kFLAGS.NUMBER_OF_TIMES_MET_SCYLLA] > 0 && this.flags[kFLAGS.DOMINIKA_STAGE] > 0) {
            choices[choices.length] = 12;
            choices[choices.length] = 12;
            choices[choices.length] = 12;
            if (this.player.hasCock() && this.player.cor > 50) {
        ...
        if (this.getGame().anemoneScene.kidAXP() >= 40 && this.player.lust100 >= 70 && this.player.gender > 0) {
            choices[choices.length] = 15;
            choices[choices.length] = 15;
            choices[choices.length] = 15;
        }
        // Sand trap
        if (this.player.sandTrapScore() >= 2) {
            choices[choices.length] = 16;
            choices[choices.length] = 16;
            choices[choices.length] = 16;
            choices[choices.length] = 16;
        }
        if (this.player.mouseScore() >= 3) {
            choices[choices.length] = 17;
            choices[choices.length] = 17;
            choices[choices.length] = 17;
            choices[choices.length] = 17;
        }
        if (this.player.bimboScore() > 0 && inBimbo) {
            this.temp = this.player.bimboScore();
            while (this.temp > 0) {
        ...
        if (this.player.findPerk(PerkLib.Lusty) >= 0) dreamLust += (dreamtemp / 3);

        // Well Adjusted cuts by half
        if (this.player.findPerk(PerkLib.WellAdjusted) >= 0) dreamLust = (dreamLust / 2);

        if (dreamLust > 0) this.dynStats("lus", dreamLust);
        ...
        if (dreamtemp > 5) {
            ...
            if (daydream == 0) {
                if (this.player.cor <= 33) {
                    ...
                    if (rand(2) == 0) this.outputText("tender sex with a sweetheart back home.  ");
                    else {
                        this.outputText("beautiful nude ");
                        if (this.player.gender <= 1 || (this.player.gender == 3 && rand(2) == 0)) this.outputText("women ");
                        else {
                            this.outputText("men ");
                            if (inBimbo) this.player.orgasm('Vaginal', false);
                        }
                        this.outputText("massaging you, slowly moving their hands over your most intimate places.  ");
                    }
                }
                if (this.player.cor > 33 && this.player.cor <= 66) {
                    this.outputText("being violently raped and used by demons.  ");
                    if (inBimbo) this.bimboWetDreamOrgasm();
                }
                if (this.player.cor > 66) {
                    dreamtemp = rand(5);
                    if (dreamtemp <= 3) this.outputText("giving yourself to the demons fully, allowing yourself to be tied down and owned as your body is warped for their twisted pleasures.  ");
                    else this.outputText("being captured and taken to a rusted building with dark smokestacks that belch sweet purplish smoke.  Inside are rows of multi-dicked, huge breasted humans, approximately thirty of them.  Each is shackled with their legs spread and torso bent over, and each has tight fitting suction tubes fitting over their tits and cocks.  Every set of tubes is pulsing with suction, drawing rivers of sticky white fluids from its slave's over-endowed breasts and balls.  You shudder in horror and arousal as you realize the victims seem to be arranged by age, ending with an empty machine next to the youngest slut-cow.  The inhuman strength of your captors easily overpowers your struggles as you are forced into your shackles, the metal locks clicking with finality.  A funnel is forced into your mouth, force-feeding you slick corrupted fluids that taste like sex and make your head swim.  Your vision fades as you feel heat in your chest and groin, making you swoon from the drugged cocktail and pleasure of your new best friends – the suction tubes. All you can hear is your own desperate moans... no, wait... that was a dream... but it was so twisted and hot that you're still panting with lust.  ");
                    if (inBimbo) this.bimboWetDreamOrgasm();
                }
            }
            // canine
                ...
                if (this.player.gender <= 1 || (this.player.gender == 3 && rand(2) == 0)) {
                    if (rand(2) == 0) this.outputText("locking a thick knotted cock inside a female, the pheromones of her heat making your maleness twitch and flex, the knot bulging obscenely as you begin to impregnate her.  ");
                    else this.outputText("stroking a knotted doggie-prick, gently stroking and squeezing it, yipping in pleasure as your pointed dog-cock leaks steady streams of fluids.  ");
                }
                // female
                else {
                    // heat dream!
                    if (this.player.inHeat) {
                        this.outputText("being pregnant, your belly bulging obscenely with the fruits of all your frequent copulations, and your breasts swollen with breast milk.  You imagine your condition enhancing your libido, driving you seek out sexual partners willing to pleasure your distorted form.  ");
                    }
                    // normal dream
                    else {
                        this.outputText("being in heat, your cunt sopping wet with moisture and desire, intense pheromones pouring off you to make all the males rigid and ready.  ");
                    }
                    if (inBimbo) this.player.orgasm('Vaginal', false);
                }
            }
            ...
            // Scylla + dom + dong
                ...
                if (this.player.gender <= 1) {
                    ...
                    // Male Trap Dream #2:
                        ...
                        if (this.player.hasCock()) this.outputText("  [EachCock] is fully erect and throbbing.");
                        else if (this.player.hasVagina()) this.outputText("  Your [vagina] dripping wet with need.");
                        this.outputText("  You shudder as you recall the erotic dream you were just having, a hand caressing your aching loins.");
                        if (inBimbo) this.bimboWetDreamOrgasm();
                        // end dream
                        // lust increase
                    }
                ...
                // Dream about living in a borrow and having lots and lots of kids.  Many furred, big eared bodies are running around and you feel the pregnant belly of your mate, or feel your own pregnant belly.
                    ...
                    this.outputText("You have a great larder, able to feed everyone as much as they need.  It has never been a problem to keep it full.  It's a veritable paradise for a rodent such as yourself.  Then you find your way into the master bedroom and see its massive bed, the site where your great family was brought into existence.\n\n");

                    if (this.player.gender == 1 || (this.player.gender == 3 && rand(2) == 0)) {
                        this.outputText("Things blur for a moment, and you find your head resting against a pregnant belly, covered in fine fur.  The results of your virility as a father, and soon to be added to the great brood that you helped bring into this world...\n\n");
                    } else {
                        this.outputText("Things blur for a moment, and you find yourself laying on the bed with your hands wrapped around your heavily pregnant belly.  The product of your fertility and the virility of your great mate, and soon to be added to the great brood you've already brought into the world...\n");
                    }
                }
            }
            else if (daydream == 18) {
    ...
    public dayTenDreams(): void {
        ...
        if (this.player.hasCock()) {
            if (this.player.cor < 50) {
                this.outputText("Your task completed, you return through the portal to your home.  Exhausted but happy, the sight of your village fills you with a surge of emotions.  Most prominently, and perhaps most enjoyably: pride.  You did it.\n\n");

                this.outputText("You drink and celebrate with your people long into the night, feasting and receiving accolades and praise.  It is a party the likes of which you have never seen, and rightfully so.  Never again will your village be threatened by the demons.  Late in the evening, you arrive at the home of your childhood and greet your mother.\n\n");

                this.outputText("\"<i>Welcome back,</i>\" she smiles.\n\n");

                this.outputText("\"<i>Thank you, Mom,</i>\" you respond, wrapping your arms around her and kissing her deeply.  Your hands take hold of her rear, gripping-\n\n");

                this.outputText("Shocked awake, you blink as the light of your dying fire flickers dimly across the campsite.  That was...\n\n");

                this.outputText("This place is getting to you.");
            }
            else {
                this.outputText("Your task completed, you return through the portal to your home.  Exhausted but happy, the sight of your village fills you with a surge of emotions.  Most prominently, and perhaps most enjoyably: pride.  You did it.\n\n");
                ...
                this.outputText("\"<i>Thank you, Mom,</i>\" you respond, wrapping your arms around her and kissing her deeply.  Your hands take hold of her rear, gripping it in great handfuls and squeezing her possessively.  Nearly throwing her in the bed, you delight in her playful squeals.  Mounting her, you tear away her dress and plunge into her cunt.\n\n");

                this.outputText("\"<i>Oooh, god!</i>\" she exclaims,  \"<i>Such a good boy!  Such a big boy!</i>\"\n\n");

                this.outputText("\"<i>That's right, Mom,</i>\" you grunt, taking hold of her breast and sucking harshly on it.  \"<i>All grown up.</i>\"  You continue to grope her roughly as you pound into her cunt, slapping your balls into her thighs.  Her pussy squeezes you with a mother's love, and you soon find yourself dumping heaping loads of your spunk within her.  You rise, and bring your dick to her mouth so that she can clean you.\n\n");

                this.outputText("As your mother laps at your shaft, coated in her juices, you look confidently down at her belly.  Though you love your mother, you're looking forward to having a daughter, and training her to love you just as much as a mother does. One day, perhaps even to replace her.\n\n");

                this.outputText("You awaken in a sweat.  Fuck.");
            }
        }
        else {
            if (this.player.cor < 50) {
                ...
                this.outputText("\"<i>Welcome back,</i>\" he smiles.\n\n");

                this.outputText("\"<i>Thank you, daddy,</i>\" you respond, wrapping your arms around him and kissing him deeply.  Your hands slip into his pants, lowering-\n\n");
                ...
                this.outputText("\"<i>Thank you, daddy,</i>\" you respond, wrapping your arms around him and kissing him deeply.  Your hands slip into his pants, lowering them and exposing his crotch.  Too excited to control yourself, you drag your lips down his chest before wrapping them around his cock and taking him into your mouth.  You pull him deep, teasing his balls with your lower lip.  Though it isn't necessary to gag - you could have swallowed him easily - you do anyway.  Daddy deserves a show.\n\n");

                this.outputText("His cum splatters into your throat.\n\n");

                this.outputText("You smile with glazed teeth as you straddle him in his bed and mount him.  Some disappointment lingers in you that you didn't just let daddy take your ass then and there, but the prospect of his cum flooding your womb and fertilizing you is too delightful to pass up.\n\n");

                this.outputText("\"<i>Do you like it?</i>\" you ask, before moaning his name.\n\n");

                this.outputText("\"<i>You're a good girl,</i>\" he answers.\n\n");

                this.outputText("His balls gurgle as they fill you.  You squeal in delight and lick his chest, demonstrating your affection.  Clenching him tightly, you make sure not to spill daddy's seed, and imagine giving birth to a sexy, busty daughter, teaching her to love him.  One day, teaching her to replace you.\n\n");

                this.outputText("You wake up in a sweat.  Fuck.");
            }
        }
    ...
    public displacerDreamII(): void {
        ...
        if (this.silly()) this.outputText("k");
        this.outputText("not a drop of jism has escaped.");
    ...
    public displacerDreamIII(): void {
        this.clearOutput();
        this.outputText("The details of the dream immediately begin to slip away.  You strain your brain, trying to commit everything to memory, but it slips out of your mind like water through a sieve.");
        ...
        let x: number = this.player.cockTotal();
        while (x > 0) {
            x--;
            if (this.player.cocks[x].sock == "amaranthine" && this.player.cocks[x].cockType != CockTypesEnum.DISPLACER) {
                if (this.player.cocks[x].cockType != CockTypesEnum.DOG) this.player.cocks[x].knotMultiplier = 1.5;
    ...
    public bimboDream(): void {

        this.getGame().dynStats("lus", rand(20) + rand(this.player.sens / 2));

        const dreamType: number = rand(5);
        const bimboScore: number = this.player.bimboScore();
        ...
        if (dreamType == 0) { // Lips
            ...
            if (intensity == 0) {
                ...
                this.outputText(" approaching a strange door. You feel familiar tingling all over your face, and you lick your lips as you turn the door knob. You enter a salon and your eyes are immediately fixed on a huge minotaur cock. It's so gorgeous and tasty! Your feel strong pulsation in your groin as you approach it and instinctively kneel down. From your position on your knees, it looks very massive; it's over two feet long, and has three rings of prepuce spaced around its length. You can just barely span your entire hand around its thickness.\n\n");
                this.outputText("You open your mouth wide and lean forward, taking the thick, spongy head into your mouth. The taste was incredibly strong, and its musk was as thick as the minotaur's dick itself. You moan in pleasure, and start bobbing your head back and forth, taking more and more of the dick into your mouth. You stroke the rest of the cock in time with your sucking. As you work your mouth around the first ring, you opened up the top section of your " + this.player.armorName + ", freeing up your " + this.player.breastDescript(0) + ". With each pass, you were able to take more and more of the bull dick down your throat. You caress your " + this.player.breastDescript(0) + " while moaning around the dick. When you get your mouth around the second ring, the minotaur's cock throbbed sharply, and his cum exploded in your mouth. Your eyes bulged in shock, and you pulled off the cock as quickly as you could, swallowing the first several shots of thick, rich minotaur cum, while the rest shoots all over your face and " + this.player.breastDescript(0) + ". You wake up in a heat, still feeling strong taste on your mouth.\n\n");
            }
        ...
        if (dreamType == 1) { // Anal
            ...
            if (intensity == 0) {
                this.outputText(" something very sexual. As you wake up, you feel tingling sensation around your [asshole], and think of how strangely empty it feels.\n\n");
            }
            else if (intensity == 1) {
        ...
        if (dreamType == 3) { // Tits
            if (intensity == 0) {
                this.outputText(" something very sexual. As you wake up, you feel tingling sensation around your [nipples] and feel pressure in your sensitive " + this.player.breastDescript(0) + ".\n\n");
            }
            else if (intensity == 1) {
                if (this.player.hasVagina()) {
                    this.outputText("wandering through the forest, cradling your full and lactating breasts, searching for the right person to give your milk to.  You come across a cute human boy and invite him to your breast.  He eagerly jumps into your arms and starts to suckle from your ample bosom.  You then guide his erect cock into your " + this.player.vaginaDescript(0) + " and engulf him.  The two of you stand there in eternal bliss as he suckles on your chest and thrusts into your womanhood, until you wake from the dream.  ");
        ...
        if (dreamType == 4) { // BimboLiqueur
            ...
            if (intensity == 0) {
                this.outputText("something very sexual. As you wake up, you feel waves of pleasure running through your body. You quite enjoy yourself.\n\n");
            }
            else if (intensity == 1) {

                this.outputText("getting lost in mountains. You climbing up and down, your muscles sore, your mouth dry. You desperately want to drink something, otherwise you'll die of thirst. As you make your way round a large rock, you see a little pond, filled by a stream from above. You run to it, kneel down and start drink water with greedy gulps. It has refreshing taste, and you feel how your strength returns, and with it your whole body starts to tingle from passion. You quickly remove your " + this.player.armorDescript() + " and bend lower, plunging your face into the liquid, and drinking. You arc your back as your ass is totally exposed, and you are feeling how it becomes bigger, wider. You are feeling the touch of the stone with your nipples, the added weight to your expanded breasts. Your thirst is quenched, and you start washing your body with the water from the pond, feeling how your skin becomes very sensitive. You enjoy your new bigger tits, you fuller ass and rounder hips, until finally you wake up.\n\n");

            }
            else {
                this.outputText(" celebrating your victory. You pour glittering liquid in your glass, and drink, enjoying a spiced scent that paints visions of a slutty slave-girl's slightly-spread folds. You take bottle from the table, open wide and guzzle it down, feeling the fire of alcohol burning a path to your belly.  The burning quickly fades to a pleasant warmth that makes you light-headed and giggly. Everyone stares at you, as you grab a lock of the silken strands and watch open-mouthed while streaks so blonde they're almost white flow down the " + this.player.hair.color + " hair. It goes faster and faster until your hair has changed into perfectly bimbo-blonde, flowing locks.\n\n");
                this.outputText("You raise, and begin to sway your hips from side to side, putting on a show for everyone here. You just feel so... sexy.  Too sexy to hide it. Your body aches to show itself and feel the gaze of strangers.  Mmmm, it makes you so wet!  Wait!?  Wet?  You touch yourself between the " + this.player.legs() + " and groan when your fingers sink into a sloppy, wet cunt. You sigh and run one hand over your " + this.player.nippleDescript(0) + "s, admiring how sensitive they're getting.  The big breasts start getting bigger and bigger, soft chest-flesh practically oozing out between your fingers as the squishy mammaries sprout like weeds, expanding well beyond any hand's ability to contain them.  The supple boobs still manage to sit high on your chest, almost gravity defying in their ability to generate cleavage.  You pinch a nipple with one hand while the other toys with the juicy entrance of your folds.  Mmmm, it, like, feels too good not to touch yourself, and after your worries, you just need to relax. Sex is so easy and natural!  You lean back and start grunting as you plunge four fingers inside yourself, plowing your wet vagina like no tomorrow. Your clit is throbbing, and you give it an experimental caress that makes your " + this.player.legs() + " give out as you cum, splattering female fluids as you convulse nervelessly on the ground. You wake up in sweat, you hand on your groins.");
            }
            this.getGame().dynStats("lus", rand(10) + rand(this.player.sens / 4));
        }

        this.bimboWetDreamOrgasm(dreamType);

    }

    private bimboWetDreamOrgasm(dreamType: number = 4): void {
        ...
        switch (dreamType) {
            ...
            case 3:
                ...
                break;
            default:
                this.player.orgasm('VaginalAnal', false);
        }
