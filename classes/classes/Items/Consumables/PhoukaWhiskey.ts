...
export class PhoukaWhiskey extends Consumable {
    ...
    public phoukaWhiskeyAddStatus(player: Player): void {
        ...
        if (player.hasStatusEffect(StatusEffects.PhoukaWhiskeyAffect)) {
            const drinksSoFar: number = player.statusEffectv2(StatusEffects.PhoukaWhiskeyAffect);
            if (drinksSoFar < 4)
    ...
    public phoukaWhiskeyExpires(player: Player): void {
        const numDrunk: number = player.statusEffectv2(StatusEffects.PhoukaWhiskeyAffect);
        const libidoSensCombined: number = player.statusEffectv3(StatusEffects.PhoukaWhiskeyAffect);
        const intSpeedCombined: number = player.statusEffectv4(StatusEffects.PhoukaWhiskeyAffect);
