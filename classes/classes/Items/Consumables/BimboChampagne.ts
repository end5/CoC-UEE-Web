...
export class BimboChampagne extends Consumable {
    ...
    public applyEffect(player: Player, clearScreen: boolean = true, intro: boolean = true): boolean {
        ...
        if (intro) {
            if ((player.findPerk(PerkLib.FutaFaculties) >= 0 && player.findPerk(PerkLib.FutaForm) >= 0) || (player.findPerk(PerkLib.BimboBody) >= 0 && player.findPerk(PerkLib.BimboBrains) >= 0)) {
                this.outputText("You could've swore the stuff worked when you saw Niamh do it to others,"
                    + " but for some reason, it had, like, no effect on you. How weird!");
            } else if (!player.hasStatusEffect(StatusEffects.BimboChampagne)) {
                this.outputText("You uncork the bottle and breathe in the fizzy, spicy aroma of the sparkling liquor."
                ...
                this.outputText("You find yourself falling even further into the dense bimbo mindset."
        ...
        let champagneEffect: StatusEffect = player.statusEffectByType(StatusEffects.BimboChampagne);
        if (champagneEffect !== undefined) {
            ...
            if (!player.hasVagina()) {
                ...
                this.outputText(" start to tingle and squirm... mmm... that feels nice! There's a sensation you, like, can't describe,"
    ...
    public removeBimboChampagne(): void {
        const champagneEffect: StatusEffect = this.player.statusEffectByType(StatusEffects.BimboChampagne);
        ...
        if (champagneEffect.value2 > 0) {
            this.player.breastRows[0].breastRating -= champagneEffect.value2;
            this.outputText("  As the trecherous brew fades, your [chest] loses some of its... bimboliciousness."
        ...
        if (champagneEffect.value4 > 0) {
            this.player.butt.rating -= champagneEffect.value4;
        ...
        if (champagneEffect.dataStore !== undefined && champagneEffect.dataStore.hasOwnProperty('hipRatingChange') && champagneEffect.dataStore.hipRatingChange > 0) {
            this.player.hips.rating -= champagneEffect.dataStore.hipRatingChange;
        }
        this.player.removeStatusEffect(StatusEffects.BimboChampagne);
        this.outputText("\n");
    }
