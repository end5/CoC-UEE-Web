...
export class NagaSilkDress extends Armor implements TimeAwareInterface {
    ...
    public setColor(color: string = "purple"): void {
        this._name = "desert naga " + color + " and black silk dress";
        this._longName = "a " + color + " desert naga silk dress";
        this._description = "A very seductive dress made for naga or females without a human set of legs. It has a black collar, bikini top, sleeves with golden bangles and a waistcloth, all decorated with a golden trim. The bottom has a " + color + " silk veil that runs down to what would be the human knee while the center of the bikini is also covered by a small strand of silk. It helps accentuate your curves and increase your natural charm. The dress obviously is so minimalist that you could as well say you are naked yet it looks quite classy on a tauric or naga body giving you the air of a master seducer.";
    }
    public get name(): string { return this._name; }
    public useText(): void { // Produces any text seen when equipping the armor normally
        this.outputText("You wear the dress and hiss like a snake. \n\n Where did that come from?.\n");
    }

    public timeChangeLarge(): boolean { return false; }

    public timeChange(): boolean {
        if (this.player.armor instanceof NagaSilkDress && kGAMECLASS.time.hours == 5) { // only call function once per day as time change is called every hour
            this.Progression();
            return true;
        }
    ...
    public Progression(): void {
        ...
        const dreams = ["That night you have a strange dream. You are in the desert basking in the sun.   You look down and notice that there are not legs attached to your lower body. There is a large" +
        ...
        this.outputText("Lucky Day!   You find a vial of snake oil in your dress and quicky quaff it down.\n\n");
